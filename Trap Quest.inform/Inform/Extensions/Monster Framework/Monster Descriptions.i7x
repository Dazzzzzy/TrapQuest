...
To say monster-title-after of (M - kitsune):
	if the difficulty of target-disguise of M > 1, say "[if debuginfo > 0 or M is threatening] (lvl [difficulty of target-disguise of M])[end if][run paragraph on]";
	otherwise say "[if debuginfo > 0 or M is threatening] (lvl [starting difficulty of target-disguise of M])[end if][run paragraph on]";
