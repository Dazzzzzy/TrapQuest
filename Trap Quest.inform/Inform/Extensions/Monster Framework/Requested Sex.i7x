...
This is the default monster friendly convinced rule:
	if current-monster is bride-consort and key garter is worn:
		say "[speech style of current-monster]'Oh thank you thank you thank you thank you, yes please, you won't regret it, I promise[if presented-orifice is a fuckhole]! I'm going to make you feel so good[end if]...'[roman type][line break]";
	otherwise if the refractory-period of current-monster <= 0 and presented-orifice is a reasonable target:
...
To say PresentFriendlyAcceptanceFlav of (M - a monster):
	if M is intelligent:
		if presented-orifice is belly, say "[speech style of M]'You've caught me at a good moment, I do need [if slimeshooter fetish is 1]to drain my slime[otherwise]a [slime][end if], actually!'[roman type][line break]";
...
To say PresentFriendlyRejectionFlav of (M - a monster):
	if M is intelligent, say "[speech style of M]'[if presented-orifice is belly and belly is a potential target]I would, but I just don't need to [slime] right now.'[otherwise if M is male and presented-orifice is a reasonable target and the refractory-period of M > 0]I need some time to recover from my last orgasm!'[otherwise if presented-orifice is a reasonable target and the refractory-period of M > 0]I need some time to recover my energy!'[otherwise if presented-orifice is a potential target]Not today, honey. I don't feel like it.'[otherwise]Not interested.'[end if][roman type][line break]";
...
To say FriendlySexResistFlav of (M - a monster):
	...
	if M is penetrating an orifice:
		if M is not-getting-licked:
		...
		otherwise:
			say "You push back with your head, trying to pull your mouth away from [NameDesc of M][if M is getting-asslicked][']s asshole[end if].";
	otherwise:
		say "You squirm but can't get away!".
