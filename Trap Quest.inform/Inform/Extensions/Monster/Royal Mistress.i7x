Royal Mistress by Monster begins here.

mistress is a diamond-tutor. mistress is guarding. mistress is female. mistress can be puddle-investigating. The leftover-type of mistress is 130.

mistress is in Facility08. The text-shortcut of mistress is "rmst".

To decide which object is the default-local-room of (M - mistress):
	decide on Facility08.

Definition: mistress is a generic-unlocker: decide yes.

Figure of mistress is the file "NPCs/Facility/mistress1.png".
Figure of enslaved diapered mistress is the file "NPCs/Facility/mistress3.png".

To decide which figure-name is the pacified-image of (M - mistress):
	if M is sex-enslaved, decide on figure of enslaved mistress;
	if M is diaper-enslaved, decide on figure of enslaved diapered mistress;
	decide on figure of Missing NPC.

To decide which figure-name is the monster-image of (M - mistress):
...
To say NameDesc of (C - mistress):
	say "the [input-style][MediumDesc of C][roman type]".
To say BigNameDesc of (C - mistress):
	say "The [input-style][MediumDesc of C][roman type]".
To say FuckerDesc of (C - mistress):
	say "the [MediumDesc of C]".
To say BigFuckerDesc of (C - mistress):
	say "The [MediumDesc of C]".
...
To compute (M - a monster) stomping (N - mistress):
...
To say LeftoverDesc (N - 130):
	say "[BigNameDesc of mistress] is on the ground here, unconscious. [big he of mistress] has a [if pregnancy fetish is 1]pregnant looking[otherwise]fully stretched[end if] belly and [semen] slowly flowing out of [his of mistress] [asshole].".

To set up (M - mistress):
	reset M;
	now the monstersetup of M is 1;
	now M is in Facility08;
	now M is guarding;
	now the raw difficulty of M is the starting difficulty of M;
	now the health of M is the maxhealth of M.

To decide which number is the starting difficulty of (M - mistress):
	decide on 12.

To say speech style of (M - mistress):
	say first custom style.

Definition: mistress is dismissable: decide no.

Part - Motion

To compute unique boredom of (M - mistress):
	if playerRegion is not facility and M is undefeated, now M is in Facility08;
	now M is not puddle-investigating.
Definition: mistress is closest-toilet-preferring: decide yes.

To compute monstermotion of (M - mistress):
	if M is puddle-investigating or the bladder of M >= 1000:
		compute toilet seeking of M;
	otherwise if M is not in Facility08 or M is unfriendly:
		compute room leaving of M.

To compute guarding action of (M - mistress):
	...
	otherwise if the bladder of M >= 1000:
		compute toilet seeking of M;
	otherwise if M is in an academic room and M is not in Facility08 and M is not interested and M is friendly: [Sometimes the mistress gets moved around and it would be good to send her slowly back to her office]
		if M is in Facility04, try M going south;
		otherwise compute room leaving of M.

An all later time based rule (this is the facility recruiters rule):
	if armband is worn:
		if class-time < (trainingFrequency * -5) and the rank of the player < 6: [staff members stalk the player to get them to come back to class]
			if mistress is alive and mistress is undefeated and mistress is nonregional, regionally place mistress;
		...
		otherwise if playerRegion is facility: [staff members don't get stuck out in the field while the player is in the training facility!]
			repeat with M running through alive undefeated staff members:
				if M is not in an academic room:
					...
					otherwise if M is mistress:
						now M is in Facility08;
					otherwise:
						regionally place M.

To regionally place (M - mistress):
...
To say EnslavedDominationFlav of (M - mistress):
	say "You grab [NameDesc of M][']s ponytail and force [him of M] to [his of M] knees. ";
	if the number of worn pussy covering actually unavoidable clothing is 0:
...
To decide which number is the EnslavedDominationThreshold of (M - mistress):
	decide on a random number between 1000 and 6000.

Part - Perception

To compute nontrainee perception of (M - a mistress):
	say "[speech style of M]'You! [if M is in the facility]Where is your armband[otherwise]You think you can enrol in my institution and then just decide to drop out halfway through[end if]?!'[roman type][line break][big he of M] takes an aggressive stance.";
	anger M.
...
To compute trainee perception of (M - a mistress):
	if the armband-print of armband is "new recruit":
		...
		otherwise if the class of the player is alchemist or the class of the player is cheerleader:
			say "Heh, I appreciate your eagerness with that outfit, but [if diaper quest is 0]it's not that kind of institution. Your tutors will be choosing your uniforms[otherwise]depending on your tutors, you may not get to stay like that for long[end if].'";
...
To say WhoQuestion of (M - a mistress):
	say "[variable custom style]'Who are you?'[roman type][line break]".

To say WhoAnswer of (M - a mistress):
	say "[speech style of M]'[one of]It is rather obvious that I am in charge of this training facility. [or][stopping]While it is not the focus of this institution, it would behoove you to learn not to ask stupid questions.'[roman type][line break]".

To say WhereAnswer of (M - a mistress):
	say "[speech style of M]'[if the rank of the player <= 1]You are in the mistress['] office, young miss. You can find your classes at the end of the hall on the right.'[otherwise]Ranks are earned, and not without effort. Do not forget that.'[end if][roman type][line break]".

To say EscapeAnswer of (M - a mistress):
...
To say StoryAnswer of (M - a mistress):
	say "[speech style of M]'If you have time to ask questions, you have time to focus on your classwork.'[roman type][line break]".

To say AdviceAnswer of (M - a mistress):
	say "[speech style of M]'Skipping classes will earn you a reconditioning... or worse.'[roman type][line break]".

To compute teaching offer of (M - a mistress):
	say "[speech style of M]'I have my [ShortDesc of M] duties to take care of. Ask a tutor if you would like more training.'[roman type][line break]".

talk-mistress-toilet is a talk-object.
mistress-toilet-ban is initially false.


Definition: mistress is fluff-question-appropriate:
	if locked-toilets is true and facility-toilet-key is not held, decide no;
	decide yes.
...
To execute (T - talk-mistress-toilet) for (M - a monster):
	...
	otherwise:
		...
		otherwise if facility-toilet-key is not carried by M:
			...
			otherwise:
				say "[BigNameDesc of M] shrugs.[line break][speech style of M]'The key has not yet been returned to me. One of your fellow trainees must have it. You'll have to wait, or go find them.'[roman type][line break]";
		...
		otherwise:
			...
			otherwise:
				say "[BigNameDesc of M] smiles.[line break][speech style of M]'Sure. Remember to get a tutor to supervise you.'[roman type][line break]";
				compute key drop of M.
