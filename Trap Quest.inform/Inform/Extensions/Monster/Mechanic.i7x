...
Definition: mechanic is infernal:[he only counts as infernal if he is able to transform]
	if the xavier-power of mechanic > 3 or mechanic is carrying mystical amulet, decide yes;
	decide no.
...
To compute monstermotion of (M - mechanic):
	...
	if W is 1 and the wrench-timer of M <= 0:[he can't repair anything without his wrench]
		...
		if N is off-stage monster:
			...
			now the wrench-timer of M is 150;[temporarily loses the wrench]
		destroy O;
	otherwise:
...
To compute perception of (M - mechanic):
	...
	otherwise if M is babifying the player:
		...
		anger M;
	otherwise if the player is prone and M is not carrying mystical amulet and M is not acquaintance:
	...
	otherwise if diaper quest is 0 and M is carrying mystical amulet and M is carrying pink-spraybottle:
		now pink-spraybottle is spray;
		say "[speech style of M]'Hey there. I guess it's time I returned this to you.'[roman type][line break][BigNameDesc of M] hands you another [ShortDesc of pink-spraybottle].";
		now pink-spraybottle is carried by the player;
...
To compute kneeling reaction of (M - mechanic):
	say "[BigNameDesc of M] cracks a wide smile.[line break][speech style of M]'I'm glad we could come to an agreement.'[roman type][line break][if the player is not shameless]You shiver with shame.[end if]";
	strongHumiliate.
...
To compute (M - mechanic) protecting against (X - a monster):
	...
	otherwise if X is robot and M is not guardian:
		if the health of X < the maxhealth of X:
			say "[speech style of M]'HEY! Careful with that!'[roman type][line break][BigNameDesc of M] moves to defend [NameDesc of X]!";
			anger M;
...
To compute xavier reward of (M - mechanic):
	...
	otherwise if R is 3 and J is actually summonable and the player is possessing a penis:
		...
		bless J;
		DelicateDown 2;
	otherwise if R is 4 and demon tail plug is actually summonable:
		say "[BigNameDesc of M] points at you with [his of M] hands, and lightning fires from [his of M] fingertips, enveloping your body. It hurts, but somehow, you feel even more confident because of it! You feel your [asshole] being forced open as a [MediumDesc of demon tail plug] materialises inside you.";
		summon demon tail plug;
...
This is the mechanic applies collar rule:
	...
	if C is actually summonable and C is not in the location of the player and the class of the player is not living sex doll and the bimbo of the player <= 15 and the xavier-power of current-monster < 2 and the player is souled and the power-stolen of current-monster is 0 and bride-consort is not current-monster:
...
To compute (M - mechanic) entering mouth:
	if watersports fetish is 1 and the bladder of M >= 300 and the latex-transformation of the player <= 4 and the charisma of the player < a random number between 0 and (the favour of M / 2) and M is not wrapped and M is unfriendly:
		say "[big he of M] cruelly grabs you by the hair, yanking you forward so your face is oriented straight at [his of M] crotch. [if the bimbo of the player < 13]You look on in [horror][otherwise]Your heart pumps with excitement[end if] as [he of M] begins [SlimeDraining] in your face.";
		FaceDrain from M;
...
To say StrikingSuccessFlav of (M - mechanic) on (B - a body part):
	if the wrench-timer of M <= 0, say "[BigNameDesc of M] [one of]strikes[or]whacks[or]smacks[at random] you [TargetName of B] with [his of M] wrench! Ouch!!";
...
To compute (M - mechanic) hurting (B - a body part):
	if the wrench-timer of M <= 0, BodyRuin a random number between 1 and 2;[he does extra damage when he has the wrench]
	otherwise BodyRuin 1.

To compute (M - mechanic) hurting (B - breasts):
	if the wrench-timer of M <= 0, BodyRuin a random number between 2 and 3;
	otherwise BodyRuin 2.

To compute (M - mechanic) hurting (B - face):
	if the wrench-timer of M <= 0, BodyRuin a random number between 2 and 3;
	otherwise BodyRuin 2.
...
To decide if (M - mechanic) is losing wrapper in (F - a fuckhole):
	...
	if the reaction of the player is 0:
		unless the player is getting lucky, decide yes;
	otherwise:
		if the player is getting very unlucky, decide yes;
	decide no.

To say CondomFailFlav of (M - mechanic) in (F - a fuckhole):
	say "[BigNameDesc of M] suddenly pulls out. [one of]You think for a moment that it might be over, only to[or]you know better than to believe it's over, and a moment later you[stopping] hear a sharp *SNAP* as [he of M] tears off the condom and slams [his of M] [DickDesc of M] back in, [one of]completely bareback[or]fully bareback[or]fully raw[in random order].[line break][speech style of M]'[one of]I'm going to jizz inside you, bitch!'[or][if F is vagina and the player is able to get pregnant]Hope you're on the pill, bitch,[otherwise]Hope you like creampies, bitch,[end if] this load is going to be big!'[or]You're gonna take it all, bitch![or]That's right bitch, enjoy all my cum!'[at random][roman type][line break]";
	if the reaction of the player is 0, say "[if the semen addiction of the player < 7]You struggle as much as you can without setting [him of M] off immediately[otherwise if the semen addiction of the player < 12]You half-heartedly struggle to get away[otherwise]You playfully struggle to get away[end if], but [he of M] holds you down effortlessly, [one of][his of M] [DickDesc of M] continuing to slam in and out as wave after wave of [semen] rushes into your now unprotected [variable F][or]continuing to thrust as [his of M] [DickDesc of M] plants [his of M] [semen] nice and deep inside your now unprotected [variable F][or][his of M] [DickDesc of M] throbbing powerfully as it floods your unprotected [variable F] with [his of M] load, pushing it deeper with every thrust[at random].";
	otherwise say "[if the semen addiction of the player < 7]You shudder[otherwise if the semen addiction of the player < 12]You glance over your shoulder[otherwise]You look over your shoulder and lick your lips[end if] as [he of M] deposits [his of M] load deep inside your newly unprotected [variable F].";
	if the reaction of the player > 0 and the class of the player is not princess, say GotUnluckyFlav;

To say CondomPieFlav of (M - mechanic) in (F - a fuckhole):
	say "[one of][BigNameDesc of M] tightens [his of M] grip, pistoning fervently as the condom fills with warmth. [big he of M] slaps your ass as [he of M] pulls out.[or][BigNameDesc of M] groans, tightening [his of M] grip as [his of M] [DickDesc of M] shoots jet after jet of fresh [semen] into the condom. [big he of M] gives your ass a hearty spank as [he of M] pulls out.[at random]";
	if the reaction of the player is 0, say GotLuckyFlav;

To decide if (M - mechanic) is willing to creampie (F - a fuckhole):
	if the class of the player is princess, decide yes;[always creampies princess]
	if F is vagina and the player is a pussy slut, decide yes;
	if F is asshole and the player is a butt slut, decide yes;
	if the reaction of the player is 2:[You can beg him not to cum in you. Charisma dependent]
		let C be the charisma of the player;
		if a random number between 0 and C > 0, decide no;
	decide yes.

To say CreampieFlav of (M - mechanic) in (F - a fuckhole):
	say "[one of][BigNameDesc of M] slams [his of M] [DickDesc of M] home, groaning with pleasure as [he of M] floods your [variable F] with [semen]. [big he of M] gives you a healthy smack across the ass as [he of M] pulls out, climbing to [his of M] feet and wiping a bit of sweat from [his of M] brow.[or][BigNameDesc of M] jams [his of M] [DickDesc of M] in as far as it will go, reaching underneath you to play with your [ShortDesc of breasts] [he of M] fills your [variable F] with [his of M] load. [big he of M] roughly shoves you off [his of M] [DickDesc of M], sighing in satisfaction as [he of M] gets to [his of M] feet.[or][BigNameDesc of M] groans, tightening [his of M] grip as [his of M] [DickDesc of M] shoots jet after jet of fresh [semen] directly into your [variable F]. [big he of M] finishes [himself of M] off with a few short thrusts before pulling out and climbing to [his of M] feet.[at random]".

To compute (M - mechanic) pulling out of (F - a fuckhole):
	if a2m fetish > 0 and face is not actually occupied and (M is infernal or the player is getting unlucky), compute M messily pulling out of F;
...
To say PullOutFlav of (M - mechanic) in (F - a fuckhole):
	say "[BigNameDesc of M] pulls out, grunting as [he of M] blows [his of M] load all over the floor.[line break][speech style of M]'[one of]Next time, I'm going to nut inside you.'[or]Don't say I never did anything nice for you.'[or]It's your lucky day, bitch.'[in random order][roman type][line break]";
...
To say ConfiscationDeclarationFlav of (M - mechanic) on (C - a clothing):
	say "[speech style of M]'[if C is plentiful accessory]This might be worth something to m... I mean BABIES like you shouldn't have this!'[otherwise if C is equippable]Hmph, you're WAY too old to be using this... thing. Into the toolbox it goes.'[otherwise]It's so considerate that you're helping me childproof this hotel by bringing me things like this!'[end if][roman type][line break]".
...
To compute damage reaction of (M - mechanic):
	if the health of M > 0 or M is carrying mystical amulet:
		...
		otherwise:
			if M is friendly:
				...
				if the class of the player is succubus, say "[speech style of M]'A betrayal eh? I will show you your place, slave'[roman type][line break]";
			otherwise:
				let FLAV be 0;
				if the woman-status of woman-player is 91 and the mechanic-scene of woman-player <= 4 and woman-player is in the location of the player:
					say "[BigNameDesc of M] roars with frustration.[line break][speech style of M]'Fine, have it your way!'[roman type][line break][big he of M] pulls [his of M] [if diaper quest is 1]wand vibrator away[otherwise][DickDesc of M] out of [NameDesc of woman-player][']s throat[end if] and turns to face you.";
					if the woman-bimbo of woman-player > 4 + diaper quest:
						say "[BigNameDesc of woman-player] coos softly.[line break][speech style of woman-player]Aww, [please] put it back, [literalMistress of M]!'[roman type][line break]";
					otherwise:
...
To say DamageReactSubmissive of (M - mechanic):
	if M is carrying mystical amulet or the wrench-timer of M <= 0, say "[BigNameDesc of M] recoils in pain, seeming to be looking for somewhere to escape to.";
...
To say OfferFriendshipFlav of (M - mechanic):
	say "[speech style of M][if M is guardian]'You and I could conquer the world[otherwise if M is ally]'You have friends in high places now[otherwise if M is buddy]'Make yourself at home[otherwise if M is acquaintance]'I'm very glad you chose to visit my hotel[otherwise]'Perhaps you won't be the first one thrown into the Pit of Unending Molestation when the demons take over this realm. ...Forget I said anything[end if].'[roman type][line break]".

To decide which number is the bartering value of (T - a submissive collar) for (M - a mechanic):
	decide on the unworn outrage of T / 3.

To say MonsterOfferAcceptFlav of (M - a mechanic) to (T - a submissive collar):
	say "[BigNameDesc of M] smirks.[line break][speech style of M]'I do have a weak spot for collars like this. I just love to collar a bitch and make her my own, you know?'[roman type][line break]".
...
If the player drops the item, they have a chance to grab it once the sex is over. They can degrade themselves to distract him, but then they'll have to degrade themselves to distract him. Otherwise, the player has a simple dexterity check to be able to grab the item first.
...
To suggestFucker (F - a body part) for (M - mechanic):
	...
	otherwise if F is a fuckhole:
		say "Do you want to try [if M is carrying a mystical amulet]stealing the amulet. You can't think of any way to distract [him of M], except your [variable F][otherwise if the wrench-timer of M <= 0]stealing [his of M] wrench? You might be able to use your [variable F] to distract [him of M][otherwise]riding [him of M]? [big he of M] might turn the tables on you[end if]...";
		if the player is consenting:
			now player-fucker is F;
...
To decide which number is the dominationtype of (M - mechanic) using (F - penis):
...
To decide which number is the submissiveness base of (M - mechanic):
	...
	if player-fuckchoice >= 0:
		decrease D by the times-submitted of M;[If you're having trouble dominating the mechanic, fuck him a couple times]
		if M is carrying a mystical amulet, increase D by 3;[It's much harder to steal the amulet]
...
To say PowerBottomComment of (M - mechanic):
	if the player is not able to speak or the player is wrist bound:
		say "You gesture for [him of M] to lie down.";
	otherwise if the bimbo of the player < 6:
		if the player is gendered male:
			say "[first custom style]'[one of]Lie down. This isn't gay, I just really need you in my ass right now.'[or]Lie down. And NO, there's nothing gay about this. Except the anal, I guess.'[or]It's your lucky day, you get to fuck me. Lie down.'[at random][roman type][line break]";
		otherwise:
			say "[first custom style]'[one of]You understand then. Good. Lie down.'[or]Lie down. Now.'[or]Lie down and don't move a muscle.'[at random][roman type][line break]";
	otherwise if the bimbo of the player < 12:
		say "[variable custom style][one of]Lie down and get nice and hard for me.'[or]Lie down. We're going to have sex.'[or]Lie down for me, alright?'[at random][roman type][line break]";
	otherwise:
...
To compute failed dominance punishment of (M - mechanic):
	...
	otherwise if B is off-stage and B is actually summonable:
		...
		summon B cursed;
		Satisfy M;
	otherwise if E is clothing and E is actually summonable:
		say "[big he of M] forces a [ShortDesc of E] onto your feet!";
		summon E cursed;
		Satisfy M;
	otherwise:
		say "[line break][speech style of M]'It's true you're a little violent, but I like that bold streak. Consider your interview underway.'[roman type][line break]";
...
To say DominanceFailure of (M - a mechanic):
	say "You grab [NameDesc of M] by the wrists and attempt to force [him of M] to [his of M] knees, but [he of M][']s stronger than [he of M] looks, and [he of M] forces you to the ground before you can find a weakness to take advantage of.".

To blowjob dominate (M - mechanic):
	if sexual-penis-length > 8:
		say "You force [NameDesc of M] to [his of M] knees, shoving your [SexDesc of penis] in [his of M] face.[line break][speech style of M]'N-no.'[roman type][line break][big he of M] stammers, unable to take [his of M] eyes off your huge [sexual-player-penis]. You alternate cock-slapping either side of [his of M] face until [his of M] cheeks are an angry red.[line break][speech style of M]'...Yes.'[roman type][line break][big he of M] looks up at you, shivering as [he of M] takes you into [his of M] mouth. To [his of M] credit, [he of M]'s a fairly obedient cocksucker, but [if the times-submitted of M > 0]you haven't forgotten how [he of M] treated you when the tables were turned[otherwise][he of M] still needs to be taken down a couple pegs[end if]. You force [him of M] to gag on your [sexual-player-penis] over and over, until you finally [if bukkake fetish is 1]shove [him of M] away and blow your [load] all over [his of M] face[otherwise]shove [him of M] down and unleash your [load] right down [his of M] throat[end if].";
		BlowGet;
		obsceneDignify;
		orgasm;
	otherwise if sexual-penis-length > 3:
		say "You force [NameDesc of M] to [his of M] knees, looking down expectantly as you present [him of M] with your [SexDesc of penis].[line break][speech style of M]'No way.'[roman type][line break]You glare at [him of M].[line break][speech style of M]'N-no.'[roman type][line break]You intensify your glare.[line break][speech style of M]'N-....Yes.'[roman type][line break][big he of M] looks up at you with a defeated look in [his of M] eyes as [he of M] takes you into [his of M] mouth. To [his of M] credit, [he of M]'s a fairly obedient cocksucker, and [if the times-submitted of M > 0]as much as [he of M] deserves some roughness[otherwise]as much as you want to rough [him of M] up[end if], you simply let [him of M] do the work. After a little while, you [if bukkake fetish is 1]pull out and pump out your [load] on [his of M] face[otherwise]fill [his of M] mouth with your [load][end if].";
		BlowGet;
		severeDignify;
		orgasm;
	otherwise:
		let P be a random off-stage generic-appropriate patron;
		say "You force [NameDesc of M] to [his of M] knees, looking down hopefully as you present [him of M] with your [SexDesc of penis].[line break][speech style of M]'No.'[roman type][line break][big his of M] outright refusal to comply with your demand is pretty damaging to your confidence, but before you give up, a [ShortDesc of P] walks into the room.[line break][speech style of M]'Oh, am I interrupting?'[roman type][line break]You grin, kneeling down behind [FuckerDesc of M] and pinning [his of M] arms behind [his of M] back. [if the player is able to speak][line break][variable custom style]'My friend here lost a bet and [he of M] doesn't want to pay up. Can you help us out?'[roman type][line break][FuckerDesc of P] doesn't even hesitate[otherwise][FuckerDesc of P] seems to understand, immediately crossing the room and shoving [his of P] [DickDesc of P] into [NameDesc of M][']s mouth. [NameDesc of M] shoots you an indignant glare, but to [his of M] credit, [he of M]'s an obedient cocksucker, submissively polishing the [ShortDesc of P]'s [DickDesc of P] until [he of P] jizzes straight down [his of M] throat. You release [NameDesc of M] as [ShortDesc of P] walks away.";
		Bore P;
		strongDignify;
...
To ride dominate (M - mechanic):
	let J be mystical amulet;
	unless J is carried by M, now J is wrench;
	unless the wrench-timer of M <= 0, now J is throne;
	say "You try to [NameDesc of M] force [him of M] to [his of M] knees, but [if the player is wrist bound]it's easier said than done with your wrists bound, [otherwise][he of M][']s stronger than [he of M] looks, [end if]and as [he of M] begins to overpower you, you frantically search around for a weakness. [run paragraph on]";
	if J is mystical amulet:
		say "Your eyes find [his of M] amulet, and you desperately grab it [if the player is wrist bound]with your teeth [end if]and yank it off [his of M] neck. [big he of M] grinds [his of M] teeth as [he of M] drops to [his of M] knees with [his of M] hands up. [PowerBottomComment of M] They stay up as [he of M] lies on [his of M] back.";
	otherwise if J is wrench:
		say "Your eyes find [his of M] wrench, and you desperately grab it [if the player is wrist bound]with your teeth [end if]and wrest it from [his of M] hand. That seems to have the desired effect, although [he of M] gives you a confused look as [he of M] gets on [his of M] knees with [his of M] hands up. [PowerBottomComment of M] They stay up as [he of M] lies on [his of M] back.";
	otherwise:
		say "Your eyes find a dangerous looking doo-hickey hanging from [his of M] belt, and you make a desperate lunge to yank it off [his of M] belt and point it at [him of M]. [big his of M] eyes widen as [he of M] drops down with [his of M] hands up. [PowerBottomComment of M] They stay up as [he of M] lies on [his of M] back.";
	now refractoryperiod is 1;[Ensures the player does not cum just from being penetrated]
...
To vaginal ride dominate (M - mechanic):
	let J be mystical amulet;
	unless J is carried by M, now J is wrench;
	if the wrench-timer of M > 0, now J is the throne;
	otherwise now J is carried by the player;
	now M is penetrating vagina;
	let R be the semi-dominance roll for M;[check if the mechanic turns the tables]
	let N be 0;
	...
	unless player-fucking is DOMINANT-SHAMEFUL:
		if N is 3, now player-fucking is DOMINANT-SHAMEFUL;[happens when the player loses focus]
		check vaginal dominate focus of M for J;
	FuckCount;
...
To check vaginal dominate entrance of (M - a mechanic) for (J - a thing):
	...
	if player-fucking is DOMINANT-SHAMEFUL:[immediate loss]
		if debugmode > 0, say "[bold type]FAILED![roman type][line break]";
		say "You [if the player is not able to speak or the player is wrist bound]swallow[otherwise]lick your lips[end if] in anticipation of how good it's going to feel to have [him of M] inside you, hungrily eyeing [his of M] [DickDesc of M] as you lower yourself into [his of M] lap. [BigNameDesc of M] immediately grabs [if the player is wrist bound]your bound[otherwise]you by the[end if] wrists, [if J is clothing and the player is wrist bound]knocking the [ShortDesc of J] out of your mouth as [he of M][otherwise if J is clothing]knocking the [ShortDesc of J] out of your hand as [he of M][otherwise] and[end if] easily wrestles you onto your back.[line break][speech style of M]'Don't worry, bitch, I'll get you some dick. Hang on...'[roman type][line break]";
		...
		otherwise if M is wrapped:
			if J is clothing and the player is getting unlucky:
				say "[big he of M] peels the condom off [his of M] [DickDesc of M] and slides it into your [vagina] completely bareback.[line break][speech style of M]'Oh, can't forget this!'[roman type][line break][BigNameDesc of M] snatches the [ShortDesc of J] off the ground, smirking down at you as [he of M] begins to thrust. [GotUnluckyFlav]";
				now M is carrying J;
			otherwise:
		...
		otherwise:
			if J is clothing and the player is getting unlucky:
				say "[big he of M] snatches the [ShortDesc of J] off the floor and stows it in the pocket of [his of M] discarded pants.[line break][speech style of M]'Now, let's get started.'[roman type][line break][BigNameDesc of M] smirks, grabbing your waist as [he of M] begins to thrust. [GotUnluckyFlav]";
				now M is carrying J;
			otherwise:
				...
				slightHumiliate;
		Ruin vagina times 2;
		say "Your thoughts are divided between [if the delicateness of the player > 5]the rush[otherwise]the humiliation[end if] of not being in control, and how stupid you were to try and take control in the first place. It's obvious now that there's only one natural position for you, and [NameDesc of M] emphasises where that is with every thrust. You [if G < -1]feel yourself being stretched out more with every moment[otherwise if G < 2]enjoy yourself more with every moment[end if], and you begin to moan as the pleasure builds and builds. [big he of M] doesn't show any signs of stopping, and you clench your fists as your lips form into a big, submissive 'O.'";
		now refractoryperiod is 0;
...
To check vaginal dominate vigilance of (M - mechanic) for (J - a thing):
	if player-fucking is DOMINANT-SHAMEFUL:[player gets too excited and the mechanic grabs J or makes player drop it]
		if debugmode > 0, say "[bold type]FAILED![roman type][line break]";
		let G be the openness of vagina - the girth of M;
		say "It feels so good that you don't even notice [him of M] grabbing your [if the player is wrist bound]bound [end if]wrists until it's too late, and after an embarrassingly brief struggle, [he of M] rolls on top of you and pins you to the ground.[line break][speech style of M]'Don't say I never let you have a turn.'[roman type][line break][BigNameDesc of M] barely [if J is clothing]gives you time to realise you dropped the [ShortDesc of J][otherwise]leaves you time to gasp[end if] as [he of M] immediately begins to thrust at twice your original pace. [if G < -1][big he of M]'s painfully big, but it's[otherwise]It's[end if] impossible to not enjoy the feeling of [his of M] [DickDesc of M] persistently rubbing up against your G-spot, and pleasure rapidly builds as you [if the player is ashamed]begin to moan. You close your eyes in anticipation of your climax, unable to ignore how much better it feels now that [he of M]'s[otherwise]desperately try not to moan. You shut your eyes in reluctant anticipation of your climax, silently trying to convince yourself that it felt just as good when you were[end if] doing the fucking.";
		unless J is throne, now J is in the location of the player;
		moderateHumiliate;
		if M is wrapped:
			let N be the semen load of M;
			passively stimulate vagina from M;
			say "But instead of finishing you off, [he of M] pulls out.[line break][speech style of M]'Lose this.'[roman type][line break][big he of M] gestures at the condom covering [his of M] [DickDesc of M]. Pull it off? It might be your only chance to cum...";
			...
			otherwise:
				...
				if J is clothing, now M is carrying J;
		otherwise:
			if J is clothing and a random number between 0 and the appearance of the player < 10, now M is carrying J;
			say "But instead of finishing you off, [he of M] pulls out.[line break][variable custom style]'W-why did you stop?'[roman type][line break]You blurt, eliciting a nasty grin from your fucktoy turned stud.[line break][speech style of M]'Oh, just making sure you don't cum.'[roman type][line break][big he of M] [if M is carrying J]grabs the [ShortDesc of J] off the ground[otherwise]smirks[end if], jerking off to stay hard as [he of M] waits for you to cool off. You can only look up at [him of M] in dejectedly as [he of M] slams [his of M] [DickDesc of M] back in and ejaculates within only a few thrusts. Equal parts embarrassment and frustration swirl inside you as [he of M] floods your [vagina] with [semen].";
			passively stimulate vagina from M;
			slightHumiliate;
			PussyFill the semen load of M;
	otherwise:
		if debugmode > 0, say "[bold type]PASSED![roman type][line break]";
		say "You [if the player is wrist bound]dodge [his of M] hand[otherwise]smack [his of M] hand away[end if] as [he of M] tries to [if J is clothing]sneak the [ShortDesc of J] out of your grip[otherwise]grab your wrists[end if], and you open your eyes just long enough to stare [him of M] down until [he of M] looks away. Putting [him of M] in [his of M] place eggs you on, and you ride [him of M] harder and harder, [if the relevant sex addiction of the player < 7]panting quietly[otherwise]moaning to yourself[end if] as [his of M] tip strokes your G-spot. [run paragraph on]".

To check vaginal dominate focus of (M - mechanic) for (J - a thing):
	if player-fucking is DOMINANT-SHAMEFUL:[player is distracted by how it feels and drops J]
		if debugmode > 0, say "[bold type]FAILED![roman type][line break]";
		say "Just as soon as you begin to really enjoy yourself, [he of M] grabs your wrists, making you struggle to shake [him of M] off and [if J is clothing]hold onto the [ShortDesc of J][otherwise]stay on top[end if] at the same time. [if the player is wrist bound]After a short scuffle, you fail at both, since your wrists are already bound.[otherwise]After a short scuffle, you fail at both, and [NameDesc of M] pins your arms firmly behind your back.[end if][line break][speech style of M]'Looks like I WIN, bitch.'[roman type][line break]";
		unless J is throne, now J is in the location of the player;
		if M is wrapped:
	...
	otherwise:
		if debugmode > 0, say "[bold type]PASSED![roman type][line break]";
		say "[big he of M] compensates with enthusiasm of [his of M] own, panting in an off rhythm with your [if the sex addiction of the player < 7]growing moans[otherwise]increasingly throaty moans[end if] as the pleasure comes to a peak. [run paragraph on]";
		if M is wrapped:
			if J is amulet:
				now player-fucking is DOMINANT-NEUTRAL;
				say "[BigNameDesc of M] grabs your wrists just before you go over the edge, both of you gasping with pleasure as the struggle pushes [him of M] into you at a sudden angle. You lose your grip on the [ShortDesc of J] as you cum, and [he of M] roughly pins your arms behind your back as orgasm crashes through your body. [big he of M] isn't far behind, and [he of M] emits a primal groan as the condom floods with warmth";
				now J is in the location of the player;
			otherwise:
				say "[BigNameDesc of M] grabs your wrists just before you go over the edge, both of you gasping with pleasure as the struggle pushes [him of M] into you at a sudden angle. [big he of M] groans, loosening [his of M] grip as [his of M] [DickDesc of M] spasms, and you finish at the same time. Orgasm crashes heavily through your body as the condom fills with [his of M] load.";
			orgasm;
		otherwise:
			if J is amulet:
				now player-fucking is DOMINANT-NEUTRAL;
				say "[BigNameDesc of M] grabs your wrists just before you go over the edge, both of you gasping with pleasure as the struggle pushes [him of M] into you at a sudden angle. You go over the edge as [he of M] forces your arms together behind your back, and you drop the [ShortDesc of J] as orgasm rocks your body. [big he of M] stays right on your tail, groaning primally as [he of M] floods your [vagina] with [his of M] [semen].";
				now J is in the location of the player;
			otherwise:
				say "[BigNameDesc of M] grabs your wrists just before you go over the edge, both of you gasping with pleasure as the struggle pushes [him of M] into you at a sudden angle. [big he of M] groans, loosening [his of M] grip as [he of M] floods your [vagina] with [semen]. You finish at the same time, little fireworks going off behind your eyes as orgasm crashes heavily through your body.";
			orgasm;
			PussyFill the semen load of M;
		orgasm M.

To anal ride dominate (M - mechanic):
	let J be mystical amulet;
	unless J is carried by M, now J is wrench;
	if the wrench-timer of M > 0, now J is the throne;
	...
	unless player-fucking is DOMINANT-SHAMEFUL:
		if N is 2, now player-fucking is DOMINANT-SHAMEFUL;[happens when the player isn't paying attention]
		check anal dominate vigilance of M for J;
	unless player-fucking is DOMINANT-SHAMEFUL:
		if N is 3, now player-fucking is DOMINANT-SHAMEFUL;[happens when the player loses focus]
		check anal dominate focus of M for J;
	AnalCount;
	compute domination aftermath of M for J;
...
To check anal dominate entrance of (M - a mechanic) for (J - a thing):
	...
	if player-fucking is DOMINANT-SHAMEFUL:[immediate loss]
		if debugmode > 0, say "[bold type]FAILED![roman type][line break]";
		say "[if C is worn or the player is not possessing a penis]You giggle[otherwise]You're already hard[end if] in anticipation of how good it's going to feel to have [him of M] inside you, hungrily eyeing [his of M] [DickDesc of M] as you lower yourself into [his of M] lap. [BigNameDesc of M] immediately grabs [if the player is wrist bound]your bound[otherwise]you by the[end if] wrists, [if J is clothing and the player is wrist bound]knocking the [ShortDesc of J] out of your mouth as [he of M][otherwise if J is clothing]knocking the [ShortDesc of J] out of your hand as [he of M][otherwise] and[end if] easily wrestles you onto your back.[line break][speech style of M]'Don't worry, [if the player is gendered male][sissy slut], I can handle this by myself. Hang on...'[otherwise]bitch, the man is in charge now. Hang on...[end if]'[roman type][line break]";
		unless J is throne, now J is in the location of the player;
		if J is amulet:[if you want to steal the amulet, you actually need to dominate him properly]
		...
		say "[if the player is not sexed male][big his of M] tip feels incredible sliding in and out of you[otherwise if G < -1]It's way too big, but the pain is overshadowed by the feeling of [his of M] tip ramming against your prostate[otherwise][big his of M] tip feels incredible ramming up against your prostate[end if], and [if C is worn]clear precum dribbles out of your cage as it jiggles[otherwise if the size of penis > 6]clear precum forms a squiggly line on your belly as your [ShortDesc of penis] jiggles[otherwise if the player is possessing a penis]clear precum pools on your belly as your [ShortDesc of Penis] bobs[otherwise]waves of pleasure wash over you as you pant[end if] in time with [NameDesc of M][']s powerful thrusts. You [if the player is ashamed]eagerly moan[otherwise]moan involuntarily[end if] as [he of M] pounds away, shame rushing through you as you realise that not only did you instantly mess everything up, but it never would have felt this good if you were still in control.[line break][second custom style]'[if face is actually occupied]MMMGG MMMHHH!'[otherwise]OOOH, FUCK ME!'[end if][roman type][line break]";
		now refractoryperiod is 0;
		anally orgasm shamefully;
		if previous penis length is 0 or refractoryperiod is 0 or the player is not possessing a penis:
			say "You don't ejaculate, but the look on [NameDesc of M][']s face tells you [he of M] definitely knows you [if refractoryperiod > 0]came[otherwise]almost did[end if]. [run paragraph on]";
		otherwise if the size of penis < 4 or C is worn:
			say "You moan uncontrollably as [semen] dribbles out of your [sissy-penis], forming a pitiful white pool on your belly. [run paragraph on]";
			AnnouncedSquirt semen on belly by the semen load of the player;
		otherwise if the size of penis < 8:
			...
			AnnouncedSquirt semen on breasts by the semen load of the player;
		otherwise:
			...
			AnnouncedSquirt semen on face by the semen load of the player;
...
To check anal dominate vigilance of (M - mechanic) for (J - a thing):
	...
	if player-fucking is DOMINANT-SHAMEFUL:[player gets too excited and the mechanic grabs J or makes player drop it]
		...
		if M is wrapped:
			...
			if the player is bimbo consenting:
				...
				now M is unwrapped;
				if previous penis length is 0 or refractoryperiod is 0 or the player is not possessing a penis:
					say "You [if refractoryperiod > 0]cum[otherwise if the player is possessing a penis]don't ejaculate[otherwise]don't cum[end if], but [NameDesc of M] doesn't seem to notice, groaning primally as as [he of M] floods your [asshole] with [his of M] thick, creamy [semen].";
				otherwise if the size of penis < 4 or C is worn:
		...
		otherwise:
			if J is clothing and a random number between 0 and the appearance of the player < 10, now M is carrying J;
			say "But instead of finishing you off, [he of M] pulls out.[line break][variable custom style]'W-why did you stop?'[roman type][line break]You blurt, eliciting a nasty grin from your fucktoy turned stud.[line break][speech style of M]'Oh, just making sure you don't cum.'[roman type][line break][big he of M] [if M is carrying J]grabs the [ShortDesc of J] off the ground[otherwise]smirks[end if], jerking off to stay hard while [he of M] waits for you to cool off. You look up at [him of M] dejectedly as [he of M] slams [his of M] [DickDesc of M] back in and ejaculates within only a few thrusts. Equal parts embarrassment and frustration swirl inside you as [he of M] floods your [asshole] with [semen].";
			slightHumiliate;
			AssFill the semen load of M;
			orgasm M;
			passively stimulate asshole from M;
	otherwise:
		if debugmode > 0, say "[bold type]PASSED![roman type][line break]";
		say "You [if the player is wrist bound]dodge [his of M] hand[otherwise]smack [his of M] hand away[end if] as [he of M] tries to [if J is clothing]sneak the [ShortDesc of J] out of your grip[otherwise]grab one of your wrists[end if], and you open your eyes just long enough to stare [him of M] down until [he of M] looks away. Putting [him of M] in [his of M] place eggs you on, and you ride [him of M] harder and harder, [if the relevant sex addiction of the player < 7]panting quietly[otherwise]moaning to yourself[end if] as [his of M] tip repeatedly [if the player is sexed male]hits your prostate[otherwise]strokes your inner walls[end if]. [run paragraph on]".

To check anal dominate focus of (M - mechanic) for (J - a thing):
	if player-fucking is DOMINANT-SHAMEFUL:[player is distracted by how it feels and drops J]
		if debugmode > 0, say "[bold type]FAILED![roman type][line break]";
		say "Just as soon as you begin to really enjoy yourself, [big he of M] grabs your wrists, making you struggle to shake [him of M] off and [if J is clothing]hold onto the [ShortDesc of J][otherwise]stay on top[end if] at the same time. [if the player is wrist bound]After a short scuffle, you fail at both, since your wrists are already bound.[otherwise]After a short scuffle, you fail at both, and [NameDesc of M] pins your arms firmly behind your back.[end if][line break][speech style of M]'[if the player is gendered male]REAL men always come out on top, [sissy slut].'[otherwise]Men always come out on top, bitch.'[end if][roman type][line break]";
		unless J is throne, now J is in the location of the player;
		if M is wrapped:
	...
	otherwise:
		...
		otherwise:
			if J is amulet:
				now player-fucking is DOMINANT-NEUTRAL;
				say "[BigNameDesc of M] grabs your wrists just before you go over the edge, both of you gasping with pleasure as the struggle pushes [him of M] into you at a sudden angle. You go over the edge as [he of M] forces your arms together behind your back, and you drop the [ShortDesc of J] as [if the player is possessing a penis]your load dribbles out on [his of M] belly[otherwise]orgasm rocks your body[end if]. [big he of M] stays right on your tail, groaning primally as [he of M] floods your [asshole] with [semen].";
				now J is in the location of the player;
			otherwise:
				say "[BigNameDesc of M] grabs your wrists just before you go over the edge, both of you gasping with pleasure as the struggle pushes [him of M] into you at a sudden angle. [big he of M] groans, loosening [his of M] grip as [he of M] floods your [asshole] with [semen]. You finish just after [him of M], little fireworks going off behind your eyes as [if the player is possessing a penis]your [semen] dribbles out on [his of M] belly[otherwise]orgasm crashes heavily through your body[end if].";
			orgasm;
			AssFill the semen load of M;
		orgasm M.
...
To compute domination aftermath of (M - mechanic) for (J - a clothing):
	...
	if M is carrying J:
		if face is not actually occupied and a2m fetish > 0:
	...
	otherwise if J is not held:
		if a2m fetish > 0 and the player is not wrist bound, say "[line break]Your eyes meet as you both pant with exhaustion, and you turn toward the [ShortDesc of J] at the same time. [big he of M]'s still hard. Do you try to distract [him of M]?";
		if a2m fetish > 0 and the player is not wrist bound and the player is bimbo consenting:
...
To compute domination aftermath of (M - mechanic) for (J - throne):
	...
	otherwise:
		...
		ultraDignify;[getting to this point is difficult, so the reward is big]
		say AfterDominationComment 7 of M.
