...
To decide which figure-name is the monster-image of (M - demon lord):
	if gyno fetish is 1, decide on figure of demon lady;
...
To compute labour to (M - demon lord):
	if M is regional and M is alive:
		...
		if watersports fetish is 1 or diaper lover >= 1:
			say "SOIL YOURSELF AS YOU GIVE BIRTH TO [one of]THE FUTURE RULER[or]A FUTURE PRINCE[stopping] OF THIS WORLD!'[roman type][line break]You realise [he of M] is completely right, you are [slime]ing on yourself.";
...
To say CondomForceSuccessFlav of (M - demon lord):
	if chosen-orifice of M is face:
		say "You take out a condom, and before [NameDesc of M] can react, [if the bimbo of the player > 10 and face is not actually occupied]use your mouth to roll it down [his of M] length[otherwise]quickly roll it down [his of M] length[end if], making [one of]sure the rubber is in intimate contact with every inch of the[or]every bump and vein glisten as you stretch and smooth the rubber over [his of M][at random] tool.[line break][speech style of M]'[one of]A CONDOM?! NO! [or]I FELL FOR IT AGAIN?! [stopping]NOOOOOO!'[roman type][line break]";
	otherwise:
...
To compute angry punishment of (M - demon lord):
	if the player is diapered:
		...
		otherwise:
			say "[SlimeContainer] swelling rapidly, quickly to breaking point, and then past it, as you begin [one of]to [slimeonself] more than you ever have before, and ever thought possible[or]another impossibly long [if slimeshooter fetish is 1]slime draining[otherwise]urination[end if][stopping]! ";
...
To compute punishment of (P - xavier-nightmare-gag):
	...
	clean K;
	SlimeSoakUp K by the soak-limit of K;
	...
	say "As you drink, something weird happens. You feel the liquid travelling down your throat, and then it just hits your [SlimeContainer] instantly. [if the player is bladder incontinent]Due to your incontinence you are powerless to prevent yourself from peeing immediately[otherwise]It's not long at all until you are bursting and so you are soon forced to [slimedrain] in your diaper[end if]. Thankfully your diaper is so giant that there's still a lot of dry padding left. For now. You reflect on what just happened - it seems like your stomach is now directly connected to your [SlimeContainer], in some kind of magic fashion. Everything you drink - and there's a lot - is going straight into your [SlimeContainer]. And since you're being forced to drink such a lot, it's all coming out pretty immediately too. You're essentially stuck in a permanent state of both drinking and peeing, which means that your diaper is gradually getting heavier and heavier. No matter how big it is, it's got to get full at some point, right?![line break]But no matter how much you pee, it doesn't seem to leak. And you keep on peeing. The demon plane's sun moves across the sky and you have nothing to do but drink and wee, drink and wee.[line break][italic type]Glug glug glug[line break]Hsssssss[roman type][line break]Eventually it gets to the point where you can't really remember what it feels like to not be peeing. It just feels normal to be permanently [SlimeDraining]. It's like it's your natural state. And still time moves forward. Day turns to night, and night to day. It must be nearly 24 hours since you were imprisoned here. With nothing to stimulate your attention, time slowly begins to lose meaning. All you know is drinking and [slime]ing. Your diaper feels so heavy that it's like it's made of lead. Another day passes, and then another night. Xavier's daughter is nowhere to be seen. Will nobody come to release you? Why do you not feel hungry, or tired? Are those just weird quirks of this infernal dimension, or is it some magic effect that has been specifically applied to you? Does it matter? But you have nothing better to occupy your mind than questions like this.";
...
To compute punishment of (P - xavier-nightmare-belt):
	...
	say "The matter inside you doubles, triples, quadruples in volume, and keeps going! Your belly bulges and swells and expands, making you look like you're eight months pregnant. The pressure on your [SlimeContainer][if diaper messing >= 3] and bowels[end if] is now immediately too much - you instantly wet[if diaper messing >= 3] and mess[end if] yourself, beginning to fill your [if D is worn]new [end if]diaper with your shame. By the time your belly has visibly deflated a bit and you are able to regain control of your body, your diaper is already feeling rather soggy and full.";
	...
	say "The effects of the next spoonful is just as bad as the first. As soon as the soup hits your stomach, you find yourself immediately [SlimeDraining][if diaper messing >= 3] and pooping[end if], filling the room with more hissing and crinkling and driving you wild with [if the diaper addiction of the player > 13]arousal[otherwise]shame[end if]. This time you find yourself allowing your body to continue excavating itself, emptying your entire [if diaper messing >= 3]belly[otherwise][SlimeContainer][end if] load into your now overly full nappy. The demon prince gives a childish snigger before snapping his fingers, causing your diaper to become completely dry once again.";
...
Check pulling ex-princess when ex-princess is guarding and current-final-battle-object is final-battle-wand-curse:
	...
	if E is clothing:
		say "Your [E] drops to the ground, and your hand clasps around [NameDesc of royal sceptre]!";
		now E is in the location of the player;
	summon royal sceptre uncursed;
	now royal sceptre is cursed;
	now the quest of royal sceptre is cursed-orgasm-quest;
...
Check masturbating when royal sceptre is worn cursed clothing:
	if the quest of royal sceptre is cursed-orgasm-quest:
		allocate 6 seconds;
		say "You can't stop yourself from bringing [NameDesc of royal sceptre] to your [genitals]! The powerful magic buzzing sends you straight into a powerful climax.";
...
To execute (F - final-battle-imp-piss):
	...
	otherwise:
		...
		otherwise:
			...
			set numerical response 1 to "suck [slime] through the straw, spitting as you go (slower)";
			set numerical response 2 to "suck [slime] through the straw, swallowing as you go (faster)";
			set numerical response 0 to "do something else";
			compute multiple choice question;
		if player-numerical-response is 1:
			say "You take the straw and suck up a big mouthful of [slime]. Pulling back, you spit it out onto the floor.";
			SlimePuddleUp 4;
			if a random number between 1 and 2 is 1, decrease the uses of F by 1;
			if a random number between 1 and 20 > the raw slime taste addiction of the player, compute slightly addictive tasting of slime;
			now another-turn is 1;
		otherwise if player-numerical-response is 2:
			say "You take the straw and suck up two big mouthfuls of [slime], gulping down each as soon as your mouth is full.";
			StomachSlimeUp 6;
			decrease the uses of F by 1;
			if the uses of F > a random number between 1 and 5, decrease the uses of F by 1;
			if a random number between 1 and 20 > the raw slime taste addiction of the player, compute addictive tasting of slime;
			now another-turn is 1;
	if the uses of F < 3:
	...
	if the health of ex-princess <= 0 or a random number between 1 and 5 is 1:
		say "[bold type]The portal shimmers, and then instantly vanishes! [roman type]The box and chains disappear, and the remaining [slime] cascades downwards over [NameDesc of ex-princess][']s body. [if the health of ex-princess < 0][big he of ex-princess] collapses to the ground[otherwise]coughs and splutters and spits up [slime]. [BigNameDesc of demon lord] snarls.[line break][speech style of demon lord]'YOU SURVIVED?!'[roman type][line break][end if]";
		now current-final-battle-object is final-battle-magic-clash. [this object has concluded]
...
A time based rule (this is the final battle diaper facehugger engulf rule):
	if the player is in DiaperFacehuggerBlindfolded:
		if ex-princess is guarding and the uses of final-battle-facehugger < a random number between 3 and 5:
			say "The wet padding soaked with [NameDesc of ex-princess][']s [slime] presses tightly against your face[one of]. You can barely breathe at all through the saturated padding[or]. Despite your best attempts to resist, you can't pull it off[or][stopping].";
			SlowGrossOut messyDiaperFacesitGrossnessLevel; [the face is on the inside of the slime soaked padding - approximately as bad as being on the outside of messed padding]
...
To compute final battle victory cleanup:
	now royal sceptre is carried by ex-princess;
...
Definition: demon lord is slime-blast-fuckable:[Too rare for me to want to deal with.]
	decide no.
...
To blowjob dominate (M - demon lord):
	...
	say "You put one hand on [NameDesc of M]'s [if M is carrying mystical amulet]amulet[otherwise]chest[end if], using your own magic to temporarily stabilise [his of M] energy as your other hand [if S < 4]eagerly tugs[otherwise if S < 9]pumps[otherwise]strokes[end if] your [if penis is not penis-erect]hardening [end if][SexDesc of penis]. [big he of M] doesn't seem to notice anything has changed, and falls to the ground while screaming at the top of [his of M] lungs.";
	if S > 8:
