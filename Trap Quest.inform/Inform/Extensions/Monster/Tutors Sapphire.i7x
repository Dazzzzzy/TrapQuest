Tutors Sapphire by Monster begins here.

To say MediumDesc of (M - a sapphire-tutor):
	say "sapphire tutor [tutor-name of M]".

Part - Serenity

tutor-serenity is a sapphire-tutor.

The text-shortcut of tutor-serenity is "tese".

Figure of serenity is the file "NPCs/Facility/Tutor/tutor1.png".
Figure of enslaved diapered serenity is the file "NPCs/Facility/Tutor/tutor1c.png".
Figure of serenity heel training session is the file "NPCs/Facility/Tutor/tutor1d.jpg".

To decide which figure-name is the monster-image of (M - tutor-serenity):
	decide on figure of serenity.

To decide which figure-name is the pacified-image of (M - tutor-serenity):
...
The tutor-name of tutor-serenity is "Serenity".

To say EnslavedDominationFlav of (M - tutor-serenity):
...
To decide which number is the EnslavedDominationThreshold of (M - tutor-serenity):
	decide on 12000.

heel-training is a training session. The training-tutor of heel-training is tutor-serenity.

To decide which number is the min-trainees of (L - heel-training):
	decide on 2.
To say TrainingSessionTitle of (L - heel-training):
	say "High Heels".

To compute teaching of (L - heel-training):
	allocate 65 seconds;
	let M be the training-tutor of L;
	let H be a random worn shoes;
	let PH be a random pvc court heels;
	say "[speech style of M]'Today's training session will be on wearing high heels.'[roman type][line break][BigNameDesc of M] speaks quickly and efficiently as if [he of M] is under a strict time limit. As [he of M] speaks, [he of M] clicks [his of M] fingers. In a flash, everyone is wearing an identical pair of 4 inch pink pvc court heels!";
	if PH is not worn:
	...
	say "[speech style of M]'Okay, now follow my lead: put your hands out to the side like a delicate lady and step, then step, then step, always putting one foot directly in front of the other.'[roman type][line break]You join the other trainees in doing your best to mimic [his of M] perfect walk.";
	...
	repeat with ST running through trainees in the location of the player:
		...
		say "[BigNameDesc of ST] [if R < -1]manages to walk perfectly, [one of]and even adds in a cheeky skip! [or]adding a cheeky curtsey at the end. [purely at random][otherwise if R <= 0][one of]struts around almost as well as miss [tutor-name of M][or]seems to have no problems learning and perfecting the walk[purely at random]. [otherwise if R is 1]wobbles [one of]as [he of ST] tries to practise the steps[or]once or twice[or]only a little as [he of ST] tries [his of ST] best to copy [tutor-name of M][in random order]. [otherwise if R is 2][one of]wobbles dangerously and very nearly falls over[or]has to frantically wave [his of ST] arms to steady [himself of ST] as [he of ST] almost topples over[in random order]! [otherwise if R is 3][one of]slips and falls onto [his of ST] bum! Ouch[or]suddenly stumbles and has to grab a desk to avoid falling over[in random order]! [otherwise]can hardly move! [big he of ST] tries to take a single step forward and immediately slips up, clattering to the ground. [end if]";
	say "[if X <= 0 and the heel skill of the player > 5]You show up the entire room by not only perfectly strutting around but also adding in some 360 degree twirls and ballet-like prances into the air. The other faces in the room are a mix of awe, envy and fury. [otherwise if X <= 0]You manage to do very well yourself. You make it around the room with an almost faultless performance, placing one foot precisely in front of the other each time, with no noticeable wobbles. You feel proud! [otherwise if X is 1]You manage to do pretty well yourself. You make it around the room with only one or two minor wobbles. [otherwise if X is 2]You make it around the room successfully but not without several definitely noticeable wobbles as you do. During your worst wobble you could swear you hear one of the other trainees snicker from behind you. [otherwise if X is 3]It's a very wobbly journey but it does seem like you might make it all the way around the room without falling over. But then tragedy strikes! It's like a rug is pulled out from underneath you as you suddenly go toppling to the ground and land on your [buttcheeks] with a thud. An array of chuckles from several of the other trainees greets your ears. [otherwise if X is 4]You only make it forward a few steps before you trip over yourself and clatter to the ground, landing on your [buttcheeks] with a thud. Nearly all the other trainees giggle as they witness your pathetic attempt.[otherwise]You try to put one foot exactly in front of the other just like miss [tutor-name of M] but as soon as you do you instantly lose your balance and end up on your [buttcheeks]! The whole class points and laughs at your abject failure. [end if]";
	HeelUp 1;
	say "[BigNameDesc of M] calls for quiet.[line break][speech style of M]'Okay, that's enough practice for today. Now we will have a little competition between two of you, to see who's surest on their heels. The winner will get promoted, and the loser will have to wear their shoes for a little longer. So, I need two daring volunteers from among you all. Who's up for it?'[roman type][line break]";
	...
	otherwise if the number of trainees in the location of the player > 1:
		maybe-map-display Figure of serenity heel training session;
		let ST2 be a random trainee in the location of the player;
		while ST is ST2:
			now ST2 is a random trainee in the location of the player;
...
To compute teaching of (L - roulette-training):
	...
	otherwise:
		while ST is entry 1 in LST and the number of trainees in the location of the player > 1:
			let ST be a random trainee in the location of the player;
	...
	if the number of entries in LST is 2:
			...
			if P is yourself:
				...
				if the player is bimbo consenting:
					...
					if CD is 0:
						...
						otherwise:
							TasteGrossOut 5;
						repeat with TST running through innocent trainees in the location of the player:
							if TST is not listed in LST, FavourDown TST; [innocent trainees that are watching are disgusted]
					otherwise if entry CD in LD matches the text "alcohol":
						...
						if alcohol-level <= 3, increase alcohol level; [Can't allow it to compute drunken adventure in the middle of a training session]
						StomachUp 1;
				...
				otherwise:
					...
					repeat with TST running through trainees in the location of the player:
						if TST is listed in LST:
							HappinessUp TST;
						otherwise if TST is tryhard trainee:
							FavourDown TST; [tryhard trainees that are watching are unimpressed with your lack of dedication]
			otherwise:
				...
				if STD > CDRisk:
					...
					if CD is 0:
						...
						if yourself is listed in LST:
							...
							repeat with TST running through unfriendly trainees in the location of the player:
								if TST is not P:
									HappinessDown TST;
		...
		repeat with P running through LST:
			if P is yourself:
				...
				update trainees; [an important line which makes boring old trainees disappear and new cool ones appear]
			otherwise:
				say "[speech style of M]'Congratulations, [trainee-name of P].'[roman type][line break]";
				promote P;
	otherwise:
...
tutor-angela is a sapphire-tutor.

The text-shortcut of tutor-angela is "tean".

Figure of angela is the file "NPCs/Facility/Tutor/tutor2.png".
Figure of enslaved diapered angela is the file "NPCs/Facility/Tutor/tutor2c.png".
Figure of kissing angela is the file "NPCs/Facility/Tutor/tutor2d.jpg".

To decide which figure-name is the monster-image of (M - tutor-angela):
	decide on figure of angela.

To decide which figure-name is the pacified-image of (M - tutor-angela):
...
To say WhoAnswer of (M - tutor-angela):
...
To say StoryAnswer of (M - tutor-angela):
...
To decide which number is the EnslavedDominationThreshold of (M - tutor-angela):
	decide on 20000.

make-up-training is a training session. The training-tutor of make-up-training is tutor-angela.

To decide which number is the min-trainees of (L - make-up-training):
	decide on 0.
To say TrainingSessionTitle of (L - make-up-training):
	say "Make Up".

To compute teaching of (L - make-up-training):
	let M be the training-tutor of L;
	...
	if P is nothing:
		...
		say "[speech style of M]'You are my only trainee at the moment, hmm?'[roman type][line break][BigNameDesc of M] looks at you.[line break]";
		if P is the player:
	...
	if P is the player:
		...
		say ClothingDesc of armband;
		repeat with STTH running through tryhard trainees in the location of the player:
			HappinessDown STTH;
		...
		update trainees; [an important line which makes boring old trainees disappear and new cool ones appear]
	otherwise if P is trainee:
		allocate 40 seconds;
		promote P;
		say "You watch as [his of P] sapphire ID card turns into an emerald one, and [he of P] is now '[trainee-name of P] the [trainee-print of P]'. [big he of P] grins widely and then prances out of the room.";
	...
	if P is not the player:
		...
		if MP is a thing and the number of held make up kit is 0:
			say "[BigNameDesc of M] hands [if P is trainee]each of [end if]you a small make up kit.[line break][speech style of M]'For you to practize viz at home.'[roman type][line break]";
			now MP is carried by the player;
			now focused-thing is MP;
		say "[big he of M] [if P is trainee]turns to face the whole class and [end if]claps [his of M] hands loudly.[line break][speech style of M]'Enough! Clazz dizmizzed!'[roman type][line break]".

[To compute make-up-training-application of (M - a trainee):
...
To compute make-up-training-application of (M - a nasty trainee):
...
To compute make-up-training-application of (M - a tryhard trainee):
...
To compute make-up-training-application of (M - an innocent trainee):
...
patience-training is a training session. The training-tutor of patience-training is tutor-angela.

To decide which number is the min-trainees of (L - patience-training):
	decide on 2.
To say TrainingSessionTitle of (L - patience-training):
	say "Patience and Desperation".

Definition: patience-training is training-appropriate:
...
To compute teaching of (L - patience-training):
	...
	while TM > 0:
		say "[bold type]There are about [TM] minute[if TM is not 1]s[end if] left of the training session.[roman type][line break]";
		decrease TM by 1;
		repeat with ST running through trainees in the location of the player:
