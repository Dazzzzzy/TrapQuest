...
To decide which figure-name is the monster-image of (M - a royal guard):
	if gyno fetish is 1, decide on figure of female royal guard;
...
To say MonsterDesc of (M - a royal guard):
	if M is diaper-enslaved:
		say "[BigNameDesc of M][']s royal armour has been transformed into a dark blue guard-themed baby cosplay onesie, complete with a giant safety pin on the front and an inflatable floppy sword. [if gyno fetish is 1]A cute blue ribbon adorns [his of M] now pigtailed hair[otherwise if TG fetish > 0][BigNameDesc of M] has been transformed into a woman, complete with a cute blue ribbon that now adorns [his of M] now pigtailed hair[otherwise][big his of M] hair has been put into pigtails with a blue ribbon, making [him of M] look like a cute little [sissy] girl[end if]. A thick bulge below the waist makes it clear that [he of M] has been clothed in a big thick nappy.";
...
To say GroundSlimeReaction of (M - a royal guard):
...
To say DiaperReaction of (M - a royal guard):
	if there is a currently visible diaper or the player is shameless:
		...
		otherwise:
			say "[speech style of M]'Yuck.'[roman type][line break][BigNameDesc of M] looks genuinely disgusted.[line break][speech style of M]'Are you truly unable to go to the [SlimeTarget] like an adult? I'm not sure I've ever seen anything quite so disgraceful[if the favour of M < the bimbo of the player + 3]. Maybe I have made a mistake - it does not appear that you deserve to be treated with respect[end if].'[roman type][line break]";
...
To compute (M - a monster) stomping (N - a royal guard):[generic M here, but shouldn't matter.]
	...
	otherwise:
		...
		repeat with C running through the taxableItems of N:
			add C to the tradableItems of N, if absent;
...
Definition: a royal guard (called M) is uniquely distracted:
	...
	if the refractory-period of M < 0 and the class of the player is not princess and M is reactive:
		...
		otherwise if N is adult baby slave and (N is defeated or N is uninterested or N is friendly):
			say "[BigNameDesc of M][']s eyes are drawn [if M is interested]away from you [end if]to [NameDesc of N]. [big he of M] grabs [him of N] by the arm.[line break][speech style of M]'Good goddess! You need a change.'[line break][speech style of N]'Nu-uh, I don't need a change, I'm totally fine as I- HEY!'[roman type][line break]Forcing [NameDesc of N] onto [his of N] back, [NameDesc of M] sets about un-taping [NameDesc of N][']s diaper, wiping [him of N] down, and changing [him of N] out of [his of N] messy nappy, despite [his of N] protests.";
...
To compute unique final action of (M - prison guard):
	if M is undefeated and M is awake and M is in Dungeon06 and skeleton key is off-stage and M is not combative and skeleton key is not listed in the tradableItems of M:
		if the player is in Dungeon06, say "[BigNameDesc of M] picks up a [skeleton key] from a hidden spot under [his of M] mattress.";
		add skeleton key to the tradableItems of M.
...
To compute (M - a royal guard) npcBattling (N - an aeromancer):
	if N is ballooned:
		say "[BigNameDesc of M][']s eyes are drawn [if M is interested]away from you [end if]to [NameDesc of N].[line break][speech style of M]'[one of]I have had enough of your grating voice, wench.' [or]You there, wench. Such a lascivious display is against the law. Get on your knees and receive your punishment.'[or]You cannot walk these halls with such an appearance, wench. Get on your knees. Now.'[in random order][roman type][line break][big he of M] narrows [his of M] eyes at [FuckerDesc of N], who turns to look at [him of M] with a smirk.[line break][speech style of N]'Do you cirrusly think you can take me?'[roman type][line break][BigFuckerDesc of M] dives at [FuckerDesc of N], but a blast of wind catches [him of M] in mid-air and slams [him of M] into the ground. [BigFuckerDesc of N] floats away laughing.";
		now the refractory-period of M is 50;[so he doesn't try again immediately]
		bore M;
		DifficultyUp N by 1;
		bore N;
		compute mandatory room leaving of N;
	otherwise:
		say "[BigNameDesc of M][']s eyes go wide as they are drawn [if M is interested]away from you [end if]to [NameDesc of N].[line break][speech style of M]'[one of]Did you think I wouldn't recognise you, wench!'[or]You there, what are you doing out of your cell!'[or]You-! How did you escape those chains?'[in random order][roman type][line break][big he of M] grabs [FuckerDesc of N] by the wrist and pulls out a pair of handcuffs.[line break][speech style of N]'[one of]I'm sniffing out the real killer! Haven't you noticed my inno-scent?'[or]But I was acloud out for good behaviour!'[or]You've got it all wrong! That was only a dis-skies!'[in random order][roman type]";
		if the previously-ballooned of N > 0 and the balloon of N > a random number between 2 and 5:[titty explosion]
			if a random number between 1 and 4 is not 4:[guard loses]
				say "[line break][speech style of M]'PUNS ARE AGAINST THE L-UNGH!'[roman type][line break][BigNameDesc of M]'s sentence is cut off as [NameDesc of N]'s breasts explode outward, knocking [him of M] into a wall! [BigNameDesc of N] looks more confident!";
				now the current-balloon of N is 1;
				DifficultyUp N by 1;
				bore M;
				compute mandatory room leaving of M;
				now the refractory-period of M is 50;
			otherwise:
				say "[line break][speech style of M]'PUNS ARE AGAINST THE L-UNGH!'[roman type][line break][BigNameDesc of M]'s sentence is cut off as [NameDesc of N]'s breasts explode outward, smacking [him of M] in the face! Unfortunately for [NameDesc of N], that isn't enough to break [his of M] grip, and [his of N] [ChestDesc of N] jiggle obscenely as [he of N] is roughly handcuffed, gagged, and anklebound.[line break][speech style of M]'Worry not, citizen, the criminal has been apprehended! I'll get [him of N] out of your sight.'[roman type][line break][BigNameDesc of N] emits a muffled groan of frustration as [he of N] is dragged out of the room.";
				now the balloon of N is 0;
				DifficultyUp N by 1;
				DifficultyUp M by 1;
				bore M;
				compute mandatory room leaving of M;
		...
		otherwise:
			if a random number between 1 and 2 is 1:[guard loses]
				say "[line break][speech style of M]'PUNS ARE AGAINST THE LAW!'[roman type][line break][BigNameDesc of M] snaps a cuff around one of [NameDesc of N]'s wrists, and is about to grab the other when a blast of air hits [him of N] in the chest, sending [him of N] flying out of the room! [BigNameDesc of N] runs off in the other direction.";
				bore M;
				bore N;
				compute mandatory room leaving of M;
				compute mandatory room leaving of N;
				now the refractory-period of M is 50;
			otherwise:[guard wins]
				say "[line break][speech style of M]'PUNS ARE AGAINST THE LAW!'[roman type][line break][BigNameDesc of N] tries to shove [NameDesc of M] with a blast of air, but [NameDesc of M] jerks [his of N] arm at the perfect time, and [he of N] gets hit with [his of N] own attack! [BigNameDesc of N] is flattened against [NameDesc of M]'s chest, where [he of N] is easily handcuffed, gagged, and dragged to the ground. [NameDesc of M] turns back to you.[line break][speech style of M]'Worry not, citizen, the criminal has been apprehended! I'll get [him of N] out of your sight.'[roman type][line break][BigNameDesc of N] emits a muffled groan of frustration as [he of N] is dragged out of the room.";
				DifficultyUp M by 1;
				satisfy M;
				bore M;
				bore N;
...
To compute perception of (M - a royal guard):
	...
	if gyno fetish is 0, display interaction of M.
...
To compute DQ appearance assessment of (M - a royal guard):
	...
	otherwise if the saved cringe appearance of the player > the cringe tolerance of M:
		if M is buddy:
			say "[speech style of M]'Erm, I'm sure I've just caught you at a bad moment, but you really should find more dignified clothing that reflects your [maturity] if you wish for us to remain amicable. ";
		otherwise if the favour of M > the aggro limit of M + 2: [will stay friendly]
			say "[speech style of M]'[if the times-changed of M is 0]You don't look like a grown adventurer to me. Go away and find some more appropriate attire before I am forced to treat you as though you have the [maturity] that you outwardly represent! [otherwise]You still look like a little [boy of the player]. I suggest finding something more appropriate, and urgently. [end if]";
		otherwise: [will turn aggressive]
			say "[speech style of M]'[if the times-changed of M is 0]You don't look like a grown adventurer to me. And we have strict rules for those who present themselves in an infantile manner. It is my duty as a royal guard to enforce this law. Prepare yourself... [otherwise]You still look like a little [boy of the player]. So I must once again enforce the law of the land. Do not resist. [end if]";
		FavourDown M by 2;
	otherwise if the saved appearance of the player > the outrage tolerance of M:
		if M is buddy:
			say "[speech style of M]'I understand that you're trying to look like an adult, but your appearance is a disgrace. [big please] try and find something more sensible to wear before we meet again. ";
		otherwise if the favour of M > the aggro limit of M + 2: [will stay friendly]
			say "[speech style of M]'I am rather disgusted by your appearance. You look like a common whore. Find something more dignified to wear before crossing paths with me again, or I will be forced to treat you with the lack of respect that you deserve. ";
		otherwise: [will turn aggressive]
			say "[speech style of M]'I am rather disgusted by your appearance. You look like a common whore. Since you cannot dress yourself, the responsibility falls on me to dress you properly. Do not resist... ";
		FavourDown M by 2;
	otherwise if the player is top-wardrobe-malfunctioning:
...
To compute DQ perception of (M - a royal guard):
	...
	if gyno fetish is 0 and M is interested, display interaction of M.
...
To get orgasm image of (M - a royal guard) in (F - a fuckhole):
	if gyno fetish is 0, cutshow figure of guard cutscene 3 for M.
...
To get creampie image of (M - a royal guard) in (F - a fuckhole):
	if the reaction of the player is 0 or gyno fetish is 1, cutshow figure of guard cutscene 5 for M;
...
Definition: prison guard is actually-lick-unsatisfied:
	if bondage protection < 2 and the pregnancy of the player is 0 and there is an off-stage fetish appropriate chastity bond:
		compute chastity locking of prison guard after thighs;
		decide yes;
	otherwise if prison guard is default-actually-lick-unsatisfied:
		decide yes;
	decide no.
...
Definition: guard-incarceration (called P) is appropriate:
	if current-monster is royal guard and the number of barriers in the location of the player is 0 and Dungeon03 is placed and the class of the player is not princess and (current-monster is prison guard or (shopkeeper is off-stage and shopkeeper is unfriendly)), decide yes;
	decide no.
...
To compute gift dropping of (X - a thing) by (M - a royal guard):
	say "[BigNameDesc of M] reaches for [his of M] [ShortDesc of X][if X is royal sigil] and detaches it from [his of M] clothes[end if].".
...
To say IdentifiablePosterReactionFlav of (M - a royal guard):
...
To say UnidentifiablePosterReactionFlav of (M - a royal guard) with (P - a poster):
...
To compute teaching of (M - a royal guard):
	say "[speech style of M]'If you ever find yourself in dire straits, never underestimate the power of admitting your inferiority and pleading for mercy. You never know, it might just soften their heart.'[roman type][line break]";
	if gyno fetish is 0, cutshow figure of guard cutscene 1 for M;
...
To vaginal ride dominate (M - a royal guard):[with times-submitted we'll assume the sex you had was the "punishment" kind]
	...
	otherwise:
		say "[line break][speech style of M]'I can't hold it any longer!'[roman type][line break][if the player is wrist bound]With your wrists bound, there is nothing you can do[otherwise if the soreness of vagina > 5]Your recent ordeals have left you too wound up to react[otherwise]A flicker of guilt slows your reaction[end if] as [he of M] suddenly grabs your waist, groaning as [he of M] ejaculates deep inside your [vagina]. The [if M is wrapped]bulging condom adds just enough friction to push[otherwise]spasming of [his of M] [cockshaft of M] pushes[end if] you over the edge, and you emit a [if the player is able to speak]muffled[otherwise]shrieking[end if] moan as you clamp down and [if sexual-penis-length > 0]jizz[otherwise]squirt[end if] all over [his of M] belly.";
		moderateDignify;
		orgasm;
		if M is wrapped, PussyFill the semen load of M;
		orgasm M;
		say AfterDominationComment 1 of M;
...
To anal ride dominate (M - a royal guard):
	...
	if R >= 0:
		...
		otherwise:
			...
			say AfterDominationComment 2 of M;
	otherwise:
		if debugmode > 0 and M is unwrapped, say "[bold type]FAILED[roman type][line break]";
		say "[if the player is wrist bound]You know [he of M][']s gonna cum, but with your wrists bound you can't do much as[otherwise]You're so turned on that you don't realize [he of M][']s about to cum until a split second after[end if] [if M is wrapped]the condom[otherwise]your colon[end if] catches the first spurt of warmth. Your [if A is clothing][SexDesc of penis] jiggles pathetically in its [ShortDesc of A], [otherwise if sexual-penis-length < 1]legs shake, [otherwise if sexual-penis-length < 4][SexDesc of penis] bobs excitedly, [otherwise][SexDesc of penis] throbs, [end if][if A is clothing or sexual-penis-length < 4]dribbling your [load] onto[otherwise if sexual-penis-length >= 4]spitting your [load] in a line across[otherwise if the player is possessing a vagina]and you squirt on[otherwise]and you rock against[end if] [his of M] belly as [he of M] pumps [if M is wrapped]the latex[otherwise]your [asshole][end if] full of [semen].";
		now player-fucking is DOMINANT-NEUTRAL;
		slightDignify;
		orgasm;
		ruin asshole;
