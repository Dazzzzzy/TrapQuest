Tutors Pink Diamond by Monster begins here.

To say MediumDesc of (M - a pink-diamond-tutor):
	say "pink diamond tutor [tutor-name of M]".

Part - Pink Diamond Tutor Hyacinthe

tutor-hyacinthe is a pink-diamond-tutor.

The text-shortcut of tutor-hyacinthe is "tehy".

Figure of hyacinthe is the file "NPCs/Facility/Tutor/tutor9a.png".
Figure of sex enslaved hyacinthe is the file "NPCs/Facility/Tutor/tutor9b.png".
Figure of diaper enslaved hyacinthe is the file "NPCs/Facility/Tutor/tutor9c.png".

To decide which figure-name is the pacified-image of (M - tutor-hyacinthe):
	if M is diaper-enslaved, decide on figure of diaper enslaved hyacinthe;
	decide on figure of sex enslaved hyacinthe.

To decide which figure-name is the monster-image of (M - tutor-hyacinthe):
	decide on figure of hyacinthe.

To say MonsterDesc of (M - tutor-hyacinthe):
	if M is diaper-enslaved:
		say "[tutor-name of M][']s dark red bodycon dress has been swapped out for a bright red onesie patterned with love-hearts, complete with a thick diaper. Matching baby booties adorn [his of M] feet. A red collar around [his of M] neck, a red pacifier in [his of M] mouth, and a pair of cuffs keeping [his of M] arms bound behind [his of M] back completely remove any sense of threat or power. [big he of M] looks absolutely furious at [his of M] new assignment as baby slave and would no doubt go on a murderous rampage if [he of M] ever managed to escape.";
...
To say WhereAnswer of (M - tutor-hyacinthe):
...
To say EscapeAnswer of (M - tutor-hyacinthe):
	say "[speech style of M]'Oh honey, I would love to let you skip ahead and find out that sort of thing already, but it wouldn't be fair on the other trainees now, would it? But if you put your mind to it I just know that you'll be able to find out for yourself in no time.'[roman type][line break]".

To say StoryAnswer of (M - a tutor-hyacinthe):
	say "[speech style of M]'Our beloved mistress keeps this place going, as a safe haven from the outside world, where everyone is completely free to become exactly who they were born to be.'[roman type][line break]".

To say AdviceAnswer of (M - tutor-hyacinthe):
...
To compute teaching of (M - tutor-hyacinthe):
	say "[speech style of M]'[if the rank of the player < 4]Oh sweetie, one day soon I'm sure you'll be ready for my training sessions, but as of right now I'm afraid you haven't progressed enough to be able to handle what awaits you in the pink diamond classroom[otherwise if the rank of the player is 4]Yes dear, that's why I'm here[otherwise]I'm afraid I've taught you all that I can. You're ready for what lies ahead[end if].'[roman type][line break]".

traitor-training is a training session. The training-tutor of traitor-training is tutor-hyacinthe.
traitor-training has a number called traitor-h.

Definition: traitor-training (called L) is training-appropriate:
	if chess-training is appropriate, decide no; [Chess training session is way better at the moment]
	decide yes.
To decide which number is the min-trainees of (L - traitor-training):
	decide on 0.
To say TrainingSessionTitle of (L - traitor-training):
	say "Betray Your Friends Hypnosis".

To compute teaching of (L - traitor-training):
	allocate 6 seconds;
	let M be training-tutor of L;
	let STN be the number of trainees in the location of the player;
	...
	otherwise:
		...
		now the traitor-h of L is hypno-strength;
	...
	repeat with S running through trainees in the location of the player:
		...
		if S is promotable:
			say "[speech style of M]'You've done well today, [trainee-name of S].'[roman type][line break]";
			promote S;
		otherwise if S is demotable and armband is pure diamond:
			...
			demote S;
		otherwise:
			say "[speech style of M]'There's still too much [one of]hesitance[or]reluctance[or]rebellion[then at random] in you, [trainee-name of S]. Maybe next time.'[roman type][line break]";
	if armband is pure diamond, update trainees. [an important line which makes boring old trainees disappear and new cool ones appear]

enema-tag-training is a training session. The training-tutor of enema-tag-training is tutor-hyacinthe.
Definition: enema-tag-training is training-appropriate:
	if the player is not the donator or enema fetish is 0, decide no;
	repeat with C running through worn clothing:
		if C is penetrating asshole and (C is unremovable or C is glued or C is cursed or C is locked), decide no;
		if C is penetrating face and (C is unremovable or C is glued or C is cursed or C is locked), decide no;
	[if diaper quest is 0 and (a2m fetish < 2 or watersports fetish is 0), decide no;]
	if the number of alive training-appropriate trainees < 2, decide no;
	decide yes.
To say TrainingSessionTitle of (L - enema-tag-training):
	say "Enema Run 2021".

To compute teaching of (L - enema-tag-training):
	...
	repeat with C running through worn clothing:
		if C is penetrating asshole or C is penetrating face:
	...
	repeat with ST running through trainees in the location of the player:
		now the trainingInt1 of ST is 0; [distance travelled]
		now the trainingInt2 of ST is 0; [times humped]
		now the trainee-diaper-mess of ST is 0; [enema situation]
		...
		repeat with ST running through LST:
			...
			if ST is yourself: [display info]
				...
				repeat with RNR running through LST:
					if RNR is a trainee:
						let lapD be the remainder after dividing trainingInt1 of RNR by distancePerLap;
						say "[BigNameDesc of RNR] is [(lapD * 100) / distancePerLap]% of the way through lap [1 + (trainingInt1 of RNR / distancePerLap)]. [big he of RNR] [if the trainee-diaper-mess of RNR is 0]has a full belly[otherwise if diaper quest is 0]has an empty belly[otherwise if diaper messing >= 3]has a full messy [his of RNR] diaper[otherwise]has a full diaper[end if].[roman type][line break]";
					...
					if TGT is not latestZentaiTarget:
						if zentaiDistance >= -1 or zentaiDistance <= -999: [humping not started]
							...
							otherwise:
								...
								if latestZentaiTarget is not yourself, now pos1 is trainingInt1 of latestZentaiTarget;
								if TGT is not yourself, now pos2 is trainingInt1 of TGT;
							...
							if TGT is yourself and latestZentaiTarget is trainee, HappinessUp latestZentaiTarget by 1;
					...
					if zentaiDistance <= -1: [humping]
						if zentaiDistance is -1: [start of humping]
							...
							if TGT is listed in LSS:
								remove TGT from LSS;
								say "The enema nozzle is yanked out of [if TGT is yourself]your[otherwise][trainee-name of TGT][']s[end if] butthole. ";
								if diaper quest is 1:
							...
							if diaper quest is 0:
								say "[big his of male-m] erect [manly-penis] springs free from the crotch region of [his of male-m] zentai and within moments is pressing against [if TGT is yourself]your[otherwise][trainee-name of TGT][']s[end if] asshole... and then it's shoved inside![line break][speech style of TGT]'[one of]Aaahn!'[or]Fuuuuckkkk!'[or]Haaaaaah!'[or]Eeeeeee!'[in random order][roman type][line break]The [man of male-m] in black begins ass-fucking [YouDesc of TGT] for all [he of male-m][']s worth.";
							otherwise:
								say "[big he of male-m] starts humping away at the [if diaper messing >= 3]filth-filled[otherwise]soggy[end if] seat of [if TGT is yourself]your[otherwise][trainee-name of TGT][']s[end if] padding, making nasty squelchy sounds as [he of male-m] bashes repeatedly against [if TGT is yourself]your[otherwise][trainee-name of TGT][']s[end if] [if TGT is yourself][player-crotch][otherwise]crotch[end if]!";
						otherwise:
							if diaper quest is 1, say "[bold type]The [man of male-m] in black continues to hump and squelch against [if TGT is yourself]your[otherwise][trainee-name of TGT][']s[end if] soiled diaper![roman type][line break]";
							otherwise say "[bold type]The [man of male-m] in black continues to rapidly thrust in and out of [if TGT is yourself]your[otherwise][trainee-name of TGT][']s[end if] asshole![roman type][line break]";
						...
						otherwise:
							increase trainingInt2 of TGT by 1;
							if zentaiDistance < a random number between -2 and -3:
								say "[speech style of TGT]'Nooo! I'm cumming!'[roman type][line break][BigNameDesc of TGT] announces in shame as [he of TGT] shudders and screams in climax. After [he of male-m] has left [him of TGT] exhausted and panting, the [man of male-m] in black leaves [trainee-name of TGT] alone to recover.";
								remove TGT from LSZ;
								now zentaiDistance is 0;
			...
			otherwise if ST is listed in LSS:
				...
				if ST is trainee:
					now the trainee-diaper-mess of ST is 0;
			...
			otherwise:
				if ST is not listed in LSE:
					...
					if ST is trainee:
						if the trainee-diaper-mess of ST is 0, now bellyFill is 30;
						otherwise now bellyFill is 0;
					let previousD be playerDistance;
					if ST is trainee, now previousD is trainingInt1 of ST;
					...
					if zentaied is true:
						...
						otherwise:
							if the trainingInt2 of ST > the dedication of ST + a random number between 2 and 3, now raceAbandoning is true;
					otherwise:
						if ST is yourself:
						...
						otherwise if ST is trainee:
							...
							increase the trainingInt1 of ST by D1;
							now D2 is trainingInt1 of ST;
						...
						otherwise:
							...
							otherwise if ST is trainee:
								...
								otherwise if DED <= 0:
									add ST to LSE;
								otherwise:
									say "[BigNameDesc of ST] stays still[if trainingInt1 of ST > 0] and catches [his of ST] breath[end if].";
						...
						if D2 > nextLapDistance:
							if ST is yourself:
								...
								otherwise:
									...
									add yourself to LSZ;
							otherwise:
								if the dedication of ST > the trainingInt1 of ST - nextLapDistance:
									now the trainingInt1 of ST is nextLapDistance;
									now pitstopping is true;
						otherwise:
							if ST is yourself:
	...
	if the number of entries in LSP > 0:
		if (diaper quest is 0 and a2m fetish >= 2 and watersports fetish is 1 and punishmentTarget is not M) or (diaper quest is 1 and diaper messing >= 6): [TODO: non a2m punishment]
			say "[BigNameDesc of M] turns to [YouDesc of punishmentTarget].[line break][speech style of M]'And now, the punishment for the biggest loser.'[roman type][line break][BigNameDesc of M] makes [if punishmentTarget is yourself]you[otherwise][trainee-name of punishmentTarget][end if] lie down on the ground, and then gets ";
			let E be the number of entries in LSP;
			repeat with ST running through LSP:
				say "[if ST is trainee][trainee-name of ST][otherwise]you[end if][if E is 2] and[otherwise if E > 2],[end if] ";
				decrease E by 1;
			say "to squat over [his of punishmentTarget] face.";
			repeat with ST running through LSP:
				say "[speech style of M]'[if ST is trainee][trainee-name of ST][otherwise][NameBimbo][end if], you [if E is 0]first[otherwise]next[end if].'[roman type][line break]";
				if diaper quest is 1:
					say "[BigYouDesc of ST] [if ST is trainee]lowers [his of ST][otherwise]lower your[end if] filthy nappy onto [if punishmentTarget is trainee][NameDesc of ST][']s[otherwise]your[end if] face, engulfing it, making [if punishmentTarget is trainee][him of punishmentTarget][otherwise]you[end if] retch and gag [one of]as [if punishmentTarget is trainee][he of punishmentTarget] struggles[otherwise]you struggle[end if] to breathe[or]some more[stopping].";
					if punishmentTarget is yourself and the player is air breathing vulnerable, SmellGrossOut messyDiaperSmellGrossnessLevel;
				otherwise:
					say "[BigYouDesc of ST] [if ST is trainee]lowers [his of ST][otherwise]lower your[end if] asshole onto [if punishmentTarget is trainee][NameDesc of ST][']s[otherwise]your[end if] mouth and push, forcing [if punishmentTarget is trainee][him of punishmentTarget][otherwise]you[end if] to take the full blast into [his of ST] mouth.[line break][one of][variable custom style]Yuck![roman type][line break][or][speech style of punishmentTarget]'Blllrrrrg!'[roman type][line break][if punishmentTarget is trainee]It spews out of [his of punishmentTarget] nose as [his of punishmentTarget] mouth overflows.[end if][stopping]";
				...
				increase E by 1;
		if yourself is listed in LSP, update trainees; [an important line which makes boring old trainees disappear and new cool ones appear]
	DexUp 2;
...
pledge-training is a training session. pledge-training can be abandoned.
Definition: pledge-training is training-appropriate:
	if diaper quest is 1 or pledge-training is not abandoned, decide yes;
	decide no.
To decide which number is the min-trainees of (L - pledge-training):
	decide on 0.
To say TrainingSessionTitle of (L - pledge-training):
	say "Magic Pledges".

A pledge-training-object is a kind of object. A pledge-training-object can be unoffered or offered. A pledge-training-object is usually unoffered. A pledge-training-object can be implanted.
Definition: a pledge-training-object is eligible:
	decide yes.
To pledge-execute (E - a pledge-training-object):
	say "BUG - training session object with no function.".

Figure of pledge TrainingSession Toilet User Cutscene is the file "Special/Cutscene/cutscene-toilet-curse1.jpg".
Figure of pledge TrainingSession Toilet Player Cutscene is the file "Special/Cutscene/cutscene-toilet-curse2.jpg".

pledge-training-toilet is a pledge-training-object.
Definition: pledge-training-toilet is eligible:
...
An all later time based rule (this is the toilet punishment rule):
	if pledge-training-toilet is implanted and toiletJustUsed is true:
...
To compute toilet predicament punishment:
	if diaper swapping >= 4:
		...
		appropriate-cutscene-display Figure of pledge TrainingSession Toilet User Cutscene;
		...
		appropriate-cutscene-display Figure of pledge TrainingSession Toilet Player Cutscene;
...
A magic consequences rule (this is the pledge-training magic consequence rule):
	if pledge-training-magic is implanted:
		if diaper quest is 0:
...
pledge-training-hold-back is a pledge-training-object.
Definition: pledge-training-hold-back is eligible:
	if diaper quest is 0, decide yes;
	decide no.
To pledge-execute (E - pledge-training-hold-back):
	say "[second custom style][if legacy content is 1]Only a dumb useless slut resists the will of [his of the player] masters[otherwise]Only an fool would resist the pleasures of the flesh[end if]. Whenever I [bold type]hold back[second custom style] from orgasming from now on, I will become more and more brainless and stupid.[roman type][line break]A large circular white rune approaches your neck and begins to phase inside you...".

pledge-training-short-skirts is a pledge-training-object.
Definition: pledge-training-short-skirts is eligible:
	if short skirts permanent suggestion is 0, decide yes;
	decide no.
To pledge-execute (E - pledge-training-short-skirts):
...
To pledge-implant (E - pledge-training-short-skirts):
	now E is implanted;
	now short skirts permanent suggestion is 4.

pledge-training-shorter-skirts is a pledge-training-object.
Definition: pledge-training-shorter-skirts is eligible:
	if short skirts permanent suggestion > 2, decide yes;
	decide no.
To pledge-execute (E - pledge-training-shorter-skirts):
...
To pledge-implant (E - pledge-training-shorter-skirts):
	now E is implanted;
	now short skirts permanent suggestion is 2.

pledge-training-panties is a pledge-training-object.
Definition: pledge-training-panties is eligible:
	if skimpy underwear suggestion is -1, decide yes;
	decide no.
To pledge-execute (E - pledge-training-panties):
...
To pledge-implant (E - pledge-training-panties):
	now E is implanted;
	now skimpy underwear suggestion is 1.

pledge-training-no-panties is a pledge-training-object.
Definition: pledge-training-no-panties is eligible:
	if skimpy underwear suggestion > 0, decide yes;
	decide no.
To pledge-execute (E - pledge-training-no-panties):
...
To pledge-implant (E - pledge-training-no-panties):
	now E is implanted;
	now skimpy underwear suggestion is 0.

pledge-training-spit is a pledge-training-object.
Definition: pledge-training-spit is eligible:
	if diaper quest is 0, decide yes;
	decide no.
To pledge-execute (E - pledge-training-spit):
...
pledge-training-mess is a pledge-training-object.
Definition: pledge-training-mess is eligible:
	if diaper messing >= 3, decide yes;
	decide no.
To pledge-execute (E - pledge-training-mess):
...
pledge-training-yes is a pledge-training-object.
To pledge-execute (E - pledge-training-yes):
...
To compute teaching of (L - pledge-training):
	now pledge-training is abandoned;
	now bigGameLoop is 3; [tells the game not to refresh any windows]
	let M be the training-tutor of L;
	let LST be the list of trainees in the location of M;
	say "[BigNameDesc of M] guides you [if the number of entries in LST is 1]and [trainee-name of entry 1 of LST] [otherwise if the number of entries in LST > 1]all [end if]to kneel down[if the number of entries in LST > 0] in a circle[end if]. With a flourish of [his of M] arms, [NameDesc of M] fills the room with countless floating magic runes, in an unknown language, all glowing a brilliant white.[line break][speech style of M]'Try to think of this as less of a training session and more of an initiation into our inner circle. All you have to do is remain still and allow me to conduct the [']celebrations['][if the number of entries in LST > 0]. If any of you get up and try to leave, you ALL fail[end if][if diaper quest is 0]. Furthermore, [bold type]this is your only chance to partake in this training session[speech style of M] - if any of you interrupt it, then the alternate ceremony that you all will have to attend will be much, much more... physical[end if].'[roman type][line break]";
	let trainingResists be 1;
	let trainingImplants be 0;
	let maxTrainingSessionImplants be 3;
	if the player is getting unlucky:
		increase maxTrainingSessionImplants by 1;
		if the player is getting unlucky, increase maxTrainingSessionImplants by 1;
	otherwise if the player is getting lucky:
		decrease maxTrainingSessionImplants by 1;
	while trainingImplants < maxTrainingSessionImplants and there is an eligible pledge-training-object:
		...
		let E be a random unoffered eligible pledge-training-object;
		now E is offered;
		pledge-execute E;
		if trainingImplants is 0, say "You understand that [NameDesc of M] is laying a curse on you, and it's pretty clear what the consequences of that will be if [he of M] is successful. You could [bold type]stand up and leave[roman type], [bold type]resist the curse[roman type] or [bold type]allow it to happen[roman type]. If you spend your energy resisting this curse, you probably won't have the mental strength to resist the next ones.";
		otherwise say "[if trainingResists > 0]Since you haven't resisted a curse yet, you're still able to resist this one, if you choose[otherwise]Since you've already resisted a curse, you can't resist any more[end if].";
		reset multiple choice questions; [ALWAYS REMEMBER THIS WHEN MAKING A MULTIPLE CHOICE QUESTION]
		set numerical response 1 to "Stand up and leave";
		set numerical response 2 to "Accept the curse";
		if trainingResists > 0, set numerical response 3 to "Resist the curse (you can only do this once)";
		compute multiple choice question;
		increase trainingImplants by 1;
		if player-numerical-response is 1:
			...
			now maxTrainingSessionImplants is 0; [This is how we indicate that the player left]
		otherwise if player-numerical-response is 3:
			decrease trainingResists by 1;
			say "You scrunch up your eyes and reject the curse. You feel like you've successfully resisted it[if trainingImplants < maxTrainingSessionImplants]! But you mind tires and you realise that you won't be able to do that again any time soon[end if].";
		otherwise:
	...
	if maxTrainingSessionImplants > 0:
		...
		if maxTrainingSessionImplants is 2, say "[variable custom style]There were only two? I expected more.[roman type][line break][GotLuckyFlav]";
		if maxTrainingSessionImplants >= 4, say "[variable custom style]There were so many pledges I had to make... I didn't expect there to be that many...[roman type][line break][GotUnluckyFlav]";
	...
	now bigGameLoop is 0;
	conclude consenting.
To pledge-implant (E - a pledge-training-object):
	now E is implanted.
...
To compute gold ceremony completion of (M - a monster):
	...
	if the number of entries in LST > 0:
		if diaper quest is 1:
			...
			repeat with ST running through LST:
				now ST is unleashed;
				now the trainee-print of ST is the armband-print of armband;
...
To compute princess reveal of (M - a monster):
	if ex-princess is caged:
		say "[BigNameDesc of M] continues to speak.[line break][speech style of M]'Now that you've proven your loyalty to our cause, I've got something special to show you. Come with me!'[roman type][line break][BigNameDesc of M] leads you down to the other end of the training halls.";
		repeat with N running through undefeated monsters in the location of the player:
		...
		if diaper quest is 0, say "[BigNameDesc of M] leans against the cell's metal bars.[speech style of M]'This is my favourite toy. And now that you're [if there is a trainee in the location of the player]all gold-star trainees[otherwise]a gold-star trainee[end if], I'm giving you permission to play with [him of ex-princess] too. Just try not to break [him of ex-princess].'[roman type][line break]And with that, [NameDesc of M] leaves you behind.";
		otherwise say "[BigNameDesc of M] leans against the cell wall.[speech style of M]'The diaper pail you saw outside leads into this chamber. In there is the [ShortDesc of mistress][']s favourite toy. And now that you're [if there is a trainee in the location of the player]all gold-star trainees[otherwise]a gold-star trainee[end if], [he of mistress] has given you permission to play with [him of ex-princess] too. Just try not to break [him of ex-princess].'[roman type][line break]And with that, [NameDesc of M] leaves you behind.";
		compute mandatory room leaving of M.


Tutors Pink Diamond ends here.
