...
Figure of aeromancer dominated 1 is the file "NPCs/MultiFloor/Aeromancer/cutscene-aeromancer-dominated1.jpg".
Figure of aeromancer dominated 2 is the file "NPCs/MultiFloor/Aeromancer/cutscene-aeromancer-dominated2.png".
...
To decide which figure-name is the pacified-image of (M - an aeromancer):
	if M is fucked-silly:
		decide on figure of aeromancer 7;
	otherwise if M is sex-enslaved:
		decide on Figure of aeromancer dominated 1;
	otherwise:
...
To decide which figure-name is the vaginal-sex-monster-image of (M - an aeromancer):
	decide on figure of aeromancer cutscene 4.

To say MonsterDesc of (M - an aeromancer):
	if M is fucked-silly:
		say "This defeated naked [man of M] has an impossible bloated belly, apparently full of an unending steady flow of [ShortDesc of fountain-target of M] [semen], which appears to be infinitely sustained by [his of M] own malfunctioning magic.";
	otherwise if M is sex-enslaved:
		say "This defeated naked [man of M] is tied up with magical indestructible golden chains, and [his of M] magic wand is cursed and embedded inside [his of M] butthole. [big he of M] is furious but helpless.";
	otherwise:
		if M is confident aeromancer:
			say "The [man of M] is wearing blue hotpants, high knee boots, silver gauntlets and [if aeromancers wear breast forms]an inflatable harness[otherwise]a birthday suit[end if] in place of a shirt. You can see a solid gold wand resting in [his of M] hand, glowing faintly as [his of M] clothes subtly flow with the direction of the wind. [big he of M] seems very self assured of [his of M] goals, whatever they are.";
		otherwise if M is spectacled aeromancer:
			say "The [man of M] is wearing a pair of stylish spectacles, purple hotpants, and[if the previously-ballooned of M is 0]a sleeveless purple vest.[otherwise if aeromancers wear breast forms]an inflatable harness instead of a shirt.[otherwise]a birthday suit for a shirt.[end if] You can see a solid gold wand resting in one hand, and [if doomed > 0]an ominously glowing black[otherwise]a faintly glowing purple[end if] [aerobook of M] in the other. [big he of M] seems hyper focused on [his of M] goals, whatever they are.";
		otherwise:
			say "The [man of M] is wearing a pair of stretchy blue hotpants, boots, and[if the previously-ballooned of M is 0]a stylish blue vest.[otherwise if aeromancers wear breast forms]an inflatable harness instead of a shirt.[otherwise]a birthday suit for a shirt.[end if] You can see a solid gold wand resting in [his of M] hand, which seems to leave behind a trail as it moves. [big he of M] seems very upbeat and focused on [his of M] goals, whatever they are.";
		if M is ballooned:
			let D be the difficulty of M - the starting difficulty of M;
			say "Right now, [if aeromancers wear breast forms]the harness is[otherwise][his of M] boobs are[end if] inflated[if D < 1], and [he of M] is hovering a few inches off the ground. Air is whistling around [him of M], increasing the strength of [his of M] magic![otherwise if D < 2], and [he of M] is hovering a foot off the ground. Air is whistling around [him of M], increasing the strength of [his of M] magic![otherwise], and [he of M] is hovering a foot off the ground. The air around [him of M] is howling, drastically increasing the strength of [his of M] magic![end if]".

To decide if aeromancers wear breast forms:
	if andro fetish is 1 or inflation fetish is 0, decide yes;
	decide no.

To say ChestDesc of (M - an aeromancer):
	let D be the difficulty of M;
	if aeromancers wear breast forms:
...
To say MonsterComment of (M - aeromancer):
	if M is ballooned:
		say "[line break][variable custom style]Something tells me [he of M]'s a bit more powerful while [he of M]'s got those [if aeromancers wear breast forms]balloons[otherwise]boobs[end if]...[roman type][line break]";
	otherwise:
		say "[if the bimbo of the player <= 7 and the player is gendered female][line break][first custom style][one of][big he of M]'s not that out of the ordinary, I suppose.[or]Where did [he of M] get all those clothes?[purely at random][otherwise if the bimbo of the player <= 7][one of]Is that real gold?![or][big he of M] probably looks better naked.[at random][otherwise if the bimbo of the player <= 12][line break][variable custom style][one of]That's a cute pair of pants. I guess.[or]That wand is kind of boner-like. Now that I think about it.[purely at random][otherwise][line break][second custom style][one of]Who needs magic when you can suck dick![or][big he of M]'s wearing way too much clothing. Everybody knows sluts look better nude![purely at random][end if][roman type][line break]".
...
Definition: an aeromancer is dungeon dwelling:
	if inflation fetish is 1 or artificial enhancements fetish is 1, decide yes;
	decide no.

To decide which number is the girth of (M - an aeromancer):
	decide on 6.

Definition: an aeromancer is human: decide yes.

To say LeftoverDesc (N - 113):
	say "An unconscious aeromancer on the ground has a [if pregnancy fetish is 1]pregnant looking[otherwise]fully stretched[end if] belly and [semen] slowly flowing out of [his of a random aeromancer] asshole.[line break]";

To decide which number is the base semen load of (M - an aeromancer):
...
To say DiaperReaction of (M - an aeromancer):
	if there is a currently visible diaper or the player is shameless:
		say "[BigNameDesc of M] smirks and looks directly at your dampening crotch.[line break][speech style of M]'[one of][if the class of the player is princess]Princess[otherwise]Pathetic[end if] [big SlimeTargetP]pants Prefers Peers['] Perception of [big his of the player] Pitiful Panty Peeing. Haha, I'm a poet and I don't even know it! Seriously though, you're a disgrace.'[or]Do you get off on me watching you [slimeonself], or something? Gross.'[stopping][roman type][line break]";
	...
	otherwise:
		say "It seems that [he of M] hasn't sensed anything, so you've gotten away with it this time.".
...
To say BecomesAggressive of (M - an aeromancer):
	say "You watch [him of M] draw power to [his of M] wand![line break][speech style of M]'[one of]I think you need to learn what happens to greedy sluts who expose themselves to others...'[or]That's fine with me, because I've been BURSTING to play with you!'[in random order][roman type][line break]".
		...
		otherwise:
			...
			otherwise:
				...
				otherwise if the science requests of M is 1:
					alwayscutshow figure of aeromancer interact 7 for M;
				otherwise:
					alwayscutshow figure of aeromancer interact 8 for M;
...
To compute perception of (M - an aeromancer):
	...
	otherwise if the player is in danger or there is a live thing penetrating a body part:
		...
		distract M;
		alwayscutshow figure of aeromancer interact 13 for M;
	otherwise:
		say "[BigNameDesc of M] [if M is spectacled aeromancer]looks you up and down, searching through [his of M] [aerobook of M][otherwise]looks you up and down[end if][if the player is sluttily dressed].[otherwise]![end if][line break]";
		compute appearance assessment of M;
	if M is interested and M is friendly, display interaction of M.
...
To compute aeromancer science of (M - an aeromancer):
	...
	let T be the substituted form of "";
	say "[speech style of M]'[one of]Sweet! Let's do this.' [or]Okay, cool! Here we go...' [or]Nice! Right, stay still...' [at random][roman type][line break]Pointing [his of M] wand towards you, [NameDesc of M] concentrates. ";
	if R is 0 or (inflation fetish is 0 and R is 2):
		if artificial enhancements fetish > 0 and (the silicone volume of breasts > 0 or the silicone volume of hips > 0 or inflation fetish is 0):
		...
		otherwise:
			...
			now T is the substituted form of "a sketch of your body being inflated with air";
		if the player is flying:
			cutshow figure of aeromancer cutscene 2 for M;
		otherwise if the player is not a flatchested trap:
			cutshow figure of aeromancer cutscene 3 for M;
	otherwise if R is 1:
	...
	otherwise if R is 2:
		...
		otherwise if the bladder of the player > 0:
			if legacy watersports mechanics is 1, say "Air fills your [SlimeContainer], putting immense pressure on it! You can't help but start to [slimeonself].";
			otherwise say "Your slimeshooter bubbles as it is suddenly filled with air, and you are unable to stop it as it suddenly ejects its contents!";
			now delayed urination is 1;
			try urinating;
		otherwise if C is transformable clothing:
			say "Your [ShortDesc of C] is suddenly hit by a beam of light from [his of M] wand!";
		...
		otherwise:
			if inflation fetish is 1:
				...
				AssFill 5 air;
			otherwise:
				say "Air suddenly forces its way inside your [asshole], stretching out your sphincter!";
	...
	otherwise: [Selkie: it'd be sweet if she could add a step or two to a latex-doll transformation that was underway]
		...
		OralSexAddictUp 1;
		SexAddictUp 1;
	if M is spectacled aeromancer:
		say "[speech style of M]'[one of]Incredible. Simply Incredible!'[or]Yes, EXCELLENT!'[or]Interesting...'[or]Well that was unexpected...'[or]Curiouser and curiouser.'[in random order][roman type][line break][BigNameDesc of M] notes something in [his of M] [aerobook of M], which glows ominously as he closes it.";
		if the number of blank rows in the Table of Published Disgraces > 0:
			choose a blank row in Table of Published Disgraces;
			now the content entry is the substituted form of "a [one of]detailed sketch[or]a flip animation[or]caricature[or]sketchy cartoon[in random order] of you[one of] being fucked by invisible tentacles[or]r breasts exploding out of a shirt[or] making a ridiculous O-face[in random order]";
	...
	otherwise:
		say "[speech style of M]'[one of]Whew, that was awesome!'[or]Thanks for your help.'[or]Interesting...'[or]Well that was unexpected...'[or]Curiouser and curiouser.'[in random order][roman type][line break][BigNameDesc of M] notes something in [his of M] [aerobook of M] and then closes it.".
...
To say SexSubmissionFlav of (M - an aeromancer): [this is customised because I rewrote the scene so the player is floating a couple feet off the ground.]
	...
	if the delicateness of the player < 13:
		say "[one of]Recognising how much it will hurt if you fall, you do your best not to resist and hope [NameDesc of M] will get bored soon.[or]As humiliating as it is to let [his of M] have [his of M] way, it's not like you can do much about it anyway.[or]You try your best not to break [NameDesc of M][']s concentration. [if the openness of F < 6]It hurts, but it would hurt more if [he of M] dropped you.[otherwise]It would hurt if [he of M] dropped you.[end if][or]You submit to being used as a plaything.[at random]";
	otherwise:
...
Definition: an aeromancer is ballooned:
...
To compute fuckhole sex of (M - an aeromancer): [separated these out so it's less confusing to do if statements]
	...
	if M is confident aeromancer:[The woods aeromancer is a great deal more powerful than the dungeon aeromancer, so not only is she strong enough to violate you with a giant air penis instead of several smaller tentacles, she's also skilled enough that she can do more things with it.]
		if the sex-length of M is 3:
			say "[BigNameDesc of M] jeers at you as the magical sex organ fucks you.";
		otherwise:
			say "[one of][BigNameDesc of M] badgers you with taunts as the magical sex organ continues to hammer away.[or]The magic [manly-penis] continues to fuck you. Somehow, [NameDesc of M] still has enough concentration left over to keep jeering at you.[or]Powerful gusts of wind smack your [BreastDesc] and pull your hair as the magical sex organ hammers away.[or]The magical [manly-penis] continues to drill your [if the soreness of F > 5]tender[end if][F], seemingly emboldened by [NameDesc of M][']s grating wordplay.[at random]";
	otherwise:
...
To compute unique climax of (M - an aeromancer) in (F - asshole):
	...
	if (N + 5 < the air volume of belly) or (HI + 2 < the total volume of hips):
		say "[big he of M] gleefully scribbles something in a small [aerobook of M] and turns away. Looks like [he of M][']s finished.";
	otherwise:
		say "[big he of M] writes something in a small [aerobook of M] and turns away. Looks like [he of M][']s finished.";
	orgasm dislodge M.
...
To say CreampieFlav of (M - an aeromancer) in (F - asshole):
	...
	otherwise say "You feel the tentacles throb in unison, groaning involuntarily as they take turns filling up your innards with gush after gush of compressed magic air. You feel the magic make its way to your [AssDesc] where it seems to [if the silicone volume of hips > 0]cause your buttcheek implants to expand in size[otherwise]take the form of silicone buttcheek implants[end if]!".

To compute (M - an aeromancer) finishing in (F - asshole):
	if inflation fetish is 1, AssFill (the semen load of M) air;
...
To say CreampieFlav of (M - an aeromancer) in (F - vagina):
	...
	cutshow figure of aeromancer cutscene 5 for M;
	say "[BigNameDesc of M] rather sloppily drops you on the ground, [one of]gleefully [or][stopping]entering something into [his of M] [aerobook of M] before losing interest.".

To compute (M - an aeromancer) finishing in (F - vagina):
	if inflation fetish is 1, BustInflate (the semen load of M / 2);
	otherwise BustImplantsUp the semen load of M / 3.
...
This is the aeromancer tries to curse the player rule:
	if there is a sex doll penetrating a body part and the latex-transformation of the player is 0 and inflation fetish is 1:
		...
		DifficultyUp current-monster by 2;
...
To compute the default taunting of (M - an aeromancer):
	if the inflation-choice of M <= 1:
		now the inflation-choice of M is a random number between 2 and 4;
		if inflation fetish is 0, now the inflation-choice of M is a random number between 2 and 3;
		if the inflation-choice of M is 2:
			say "[BigNameDesc of M] points at your [BreastDesc] in a fit of giggles.[line break][speech style of M]'[one of]It's all in the air as to whether this will help you or not!'[or]It won't be plain sailing, but I think you'll end up over the moon thanks to these enhancements!'[in random order][roman type][line break]You feel [if inflation fetish is 0 and the silicone volume of breasts is 0]silicone implants appear inside your breasts,[otherwise if inflation fetish is 0]your silicone breast implants[otherwise]them[end if] inflating rapidly.";
			if inflation fetish is 1, BustInflate 6;
			otherwise BustImplantsUp a random number between 1 and 2;
		otherwise if the inflation-choice of M is 3:
			say "[BigNameDesc of M] points at your [AssDesc] whilst uncontrollably chuckling.[line break][speech style of M]'[one of]Improving people's bodies like this gives me such a lift! Although I'm afraid you might find it a bit of a drag...'[or]Don't worry baby, I'm sure your fashion style will take off soon.'[or]You're going on the only flight where excess baggage is not only allowed, but encouraged!'[in random order][roman type][line break]You feel [if inflation fetish is 0 and the silicone volume of hips is 0]silicone implants appear inside your [AssDesc],[otherwise if inflation fetish is 0]your silicone ass implants[otherwise]them[end if] inflating rapidly.";
	...
	otherwise if the inflation-choice of M is 2:
		...
		otherwise BustImplantsUp a random number between 1 and 2;
	otherwise if the inflation-choice of M is 3:
		say "[BigNameDesc of M] is still making your [ShortDesc of hips] inflate!";
		if inflation fetish is 1, AssInflate 3;
		otherwise AssImplantsUp a random number between 1 and 2;
	otherwise if the inflation-choice of M is 4:
		say "[BigNameDesc of M] is still making your [BellyDesc] inflate!";
		AssFill 3 Air.
...
This is the aeromancer wind barrier rule:
	...
	if (M is wand-empowered or the science history of M > 6) and asshole is occupied and (the player is not possessing a vagina or vagina is occupied) and the number of barriers in the location of the player is 0:
		say "[BigNameDesc of M] creates a wall of air around the edges of the room.[line break][speech style of M]'This trick has a tiny margin of AIR-rer.'[roman type][line break]";
		now wind-wall is in the location of the player;
		rule succeeds.
The aeromancer wind barrier rule is listed in the aeromancer unique punishment rules.

This is the aeromancer settles for hip inflation rule:
	let M be current-monster;
	now the inflation-choice of M is 1; [Just in case]
	if asshole is occupied and (the player is not possessing a vagina or vagina is occupied):
		say "[BigNameDesc of M] seems to settle for inflating your body. [big he of M] points at your [AssDesc] whilst uncontrollably chuckling.[line break][speech style of M]'[one of]Improving people's bodies like this gives me such a lift! Although I'm afraid you might find it a bit of a drag...'[or]Don't worry baby, I'm sure your fashion style will take off soon.'[or]You're going on the only flight where excess baggage is not only allowed, but encouraged!'[in random order][roman type][line break]You feel [if inflation fetish is 0 and the silicone volume of hips is 0]silicone implants appear inside your [AssDesc],[otherwise if inflation fetish is 0]your silicone ass implants[otherwise]them[end if] inflating rapidly.";
		if inflation fetish is 1, AssInflate 4;
		otherwise AssImplantsUp 2;
		satisfy M;
		rule succeeds.
The aeromancer settles for hip inflation rule is listed in the aeromancer unique punishment rules.

To compute (M - an aeromancer) attacking (C - a diaper):
	if asshole is actually occupied or C is perceived messed:
		say "[BigNameDesc of M] senses your situation with your [if C is perceived messed]messed[otherwise]already occupied [asshole] and[end if] diaper and decides against penetrating you. Instead [he of M] simply summons [if inflation fetish is 0 and the silicone volume of hips is 0]silicone implants inside your [AssDesc][otherwise if inflation fetish is 0]more silicone into your ass[otherwise]more air directly inside your belly[end if]!";
		if inflation fetish is 1, AssFill 5 Air;
		otherwise AssImplantsUp 2;
		say "[BigNameDesc of M] seems satisfied and leaves you alone.";
		distract M;
	otherwise:
		say "[BigNameDesc of M] stretches out [his of M] arms and hands towards you. You feel invisible tentacles made of compressed air shoot up your [asshole] with incredible speed. Somehow the magic sex organs are simply phasing through your [printed name of C][if asshole is actually occupied] and the [printed name of random thing filling asshole][end if]!";
...
To compute (M - an aeromancer) destroying (K - a clothing):
	say "With a flick of two of [NameDesc of M][']s fingers, sharp air slashes into your [printed name of K], severely damaging it.";
	say "Your [K] have been completely destroyed!";
	destroy K.

To compute (M - an aeromancer) displacing (C - a clothing):
	say "Using [his of M] magic manipulation of air, [NameDesc of M] pulls the [if C is skirted]skirt up, exposing your crotch[otherwise if C is trousers]waistband down to your knees[otherwise]crotch section to one side[end if]!";
	displace C.
...
To say PresentAcceptanceFlav of (M - an aeromancer):
	say "[BigNameDesc of M] laughs at your request.[line break][speech style of M]'Fine by me. Let's do this!'[roman type][line break]".
...
To say ThreesomePrep of (M - a monster) with (N - an aeromancer) in (F - asshole):
	say "A strong gust of wind spins you around, leaving you facing [NameDesc of N] as [NameDesc of M] pulls apart your [AssDesc].".
To say ThreesomePrep of (M - a monster) with (N - an aeromancer) in (F - vagina):
	say "A strong gust of wind spins you around, leaving you facing [NameDesc of N] as [NameDesc of M] spreads your legs.".
...
To say ThreesomePrep of (M - an aeromancer) with (N - a monster) in (F - vagina):
	say "[BigNameDesc of M] stretches one [his of M] arms and hands towards you, giggling uncontrollably as [his of M] wand begins to glow.".
...
To say TwosomePrep of (M - an aeromancer) in (F - asshole):
	say "[BigNameDesc of M] stretches out [his of M] arms and hands towards you. You yelp as you are suddenly forced several feet off the ground by a powerful gust of wind. Cackling with glee, the aeromancer uses [his of M] mastery of air to buffet you from one end of the [if M is in the Woods]clearing[otherwise]room[end if] to the other, eventually leaving you suspended in the centre, at just above waist height. [big he of M] takes a couple steps forward, barely containing [his of M] mirth as you [if the relevant sex addiction of M < 7]desperately try and fail to get purchase on the ground below[otherwise]look up at [him of M] in wide-eyed anticipation[end if].".
...
To say PenetrationFlav of (M - an aeromancer) in (F - vagina):
	say "The air around you compresses itself into sparkling tentacles, which immediately shoot into your [vagina]. Wind lashes at your skin and clothes as [his of M] invisible tentacles begin to ruthlessly fuck you.".
...
To compute (S - a spike bra) damaging (M - an aeromancer):
	say "You see [NameDesc of M] flinch back in pain as [his of M] air whip strikes your [printed name of a random spike bra worn by the player]. The attack was somehow reflected!";
	decrease the health of M by 3;

To say StrikingFailureFlav of (M - an aeromancer) on (B - a body part):
	say "[BigNameDesc of M] tries to whip you with an invisible strand of air, but by listening to the sound you manage to jump to one side and dodge the strike!".

To say MonsterTripAnnounceFlav of (M - an aeromancer):
...
To compute SelectionFailure of (M - an aeromancer):
	say "[BigNameDesc of M] finds [himself of M] unable to use you how [he of M] planned, and seems to settle for casting a simple spell. ";
	now the inflation-choice of M is 1;
	compute the default taunting of M;
	bore M.

Section 2 - Damage

To say CombatProvokedReaction of (M - an aeromancer):
		say "[BigNameDesc of M] [if M is uninterested]notices you and [end if]takes an aggressive stance![if M is friendly][line break][speech style of M]'Fuck you, bitch, I was going to leave you alone! Maybe you are looking for a fuck after all.'[roman type][line break][end if]".

To say DamageReactHealthy of (M - an aeromancer):
	say "[BigNameDesc of M] takes the hit, giggling!".
...
To say DamageReactTired of (M - an aeromancer):
	say "[BigNameDesc of M] takes the hit, yelping!".

To say DamageReactWeak of (M - an aeromancer):
	say "[BigNameDesc of M] takes the hit, [if M is confident aeromancer]angrily fighting to[otherwise]tears streaming down [his of M] face [he of M] fights to[end if] maintain [his of M] balance!".
...
To say WeaknessDesc of (M - an aeromancer):
	if the noun is undefeated:
		if the health of the noun < the maxhealth of the noun / 4:
			if the noun is wenchy and saved-flat-intelligence > 6:
				let A be a random worn research airhancer;
				if A is clothing:
					say "[BigNameDesc of M] looks near defeat, but [he of M] doesn't seem to think of you as a threat. Could it have something to do with your [ShortDesc of A]?";
				if saved-flat-intelligence < 2 * (the difficulty of M / 3):
					say "[BigNameDesc of M] looks near defeat, but [he of M] seems like [he of M]'s daring you to reach for [his of M] wand. Could there be something about it that makes it dangerous?";
				otherwise:
...
To decide which number is the mental dominance roll for (M - an aeromancer):
	let chaste be 0;
	if there is a worn research airhancer, now chaste is 1;
	if debugmode > 0, say "[input style]Aeromancer chastity check...[if chaste is 1]AUTOMATIC FAILURE[end if][line break]";
	if chaste is 1, decide on -99;
	let N be the intelligence of the player;
	decrease N by 2 * (the difficulty of M / 3);
...
To decide which number is the dominationtype of (M - an aeromancer) using (F - penis):
...
Definition: an aeromancer is uniquely-fuckable:
	if the total magic power of the player >= 15, decide yes.

To say UniqueFuckDesc of (M - an aeromancer):
	say "Use [his of M] wand against [him of M] (this will permanently defeat [him of M]).".

To unique dominate (M - an aeromancer):
	say "[BigNameDesc of M] raises [his of M] wand as you force [him of M] onto [his of M] back, but it flies out of [his of M] hand and into yours as if attracted there by, well, magic. It howls with energy as sheets of air shoot from the tip and spin around [his of M] limbs, binding [him of M] up like a Thanksgiving turkey. You grin.[line break][if the player is able to speak][variable custom style]'How about a taste of your own medicine?'[roman type][line break][end if][big he of M] frantically shakes [his of M] head, but since you don't hear a safeword, you immediately jam the fat end of the wand up [his of M] ass. You punish [him of M] mercilessly, revelling in [his of M] groans of discomfort as air rushes inside of [him of M], visibly distending [his of M] belly. [big he of M] glares at you as you get back to your feet, leaving the wand sticking out of [his of M] butt.";
	now M is sex-enslaved;
	strongDignify;
	now player-fucking is DOMINANT-SUPER;
	say "[speech style of M]'Wait! No! You can't leave me like this!'[roman type][line break]You tug at the chains and the wand in [his of M] butt, but they seem very happy in their new locations. You shrug.[line break][if the player is able to speak][variable custom style]'Sorry hun, I don't even know how to free you if I wanted to.'[roman type][line break][otherwise]You don't know how to free [him of M] even if you wanted to.[end if]";
...
To watersports dominate (M - an aeromancer):
	...
	say "[BigNameDesc of M] raises [his of M] wand as you force [him of M] to [his of M] knees, but it flies out of [his of M] hand and into yours as if attracted there by, well, magic. It howls with energy as sheet of air erupts from the tip and spins into [his of M] mouth, forcing it wide open. You grin[if G is clothing] around your own gag [end if]and pull out your [SlimePistolDesc]. [if the player is able to speak][line break][first custom style]'If you swallow every drop, I'll let you have the wand back.'[roman type][line break][otherwise][run paragraph on][end if]";
	if sexual-penis-length >= 9:
		say "[big his of M] eyes widen at the sheer size of your [SlimeJunk], and [he of M] [if the previously-ballooned of M > 0]pushes up [his of M] [ChestDesc of M][otherwise]unzips [his of M] jacket[end if] without any further prompting. You let out an exaggerated sigh as you direct a [SlimeShade] stream into [his of M] mouth, moving closer and closer to [him of M] as the stream slowly peters out.";
	otherwise:
		say "[if the player is able to speak][big he of M] slowly nods, [otherwise]Your intent seems to dawn on [him of M], and [he of M] nods, [end if][if the previously-ballooned of M > 0]pushing up [his of M] [ChestDesc of M][otherwise]unzipping [his of M] jacket[end if] for effect. You sigh as you direct a [SlimeShade] stream into [his of M] mouth, moving closer and closer to [him of M] as the stream slowly peters out.";
	if R > 0:
		if debugmode > 0, say "[bold type]PASSED[roman type][line break]";
		say "Unfortunately for [him of M], having your [SlimeJunk] looming over [him of M] is way too intimidating, and [he of M] suddenly jerks away, spilling [his of M] entire mouthful of [slime] down [his of M] chest.";
		strongDignify;
		now player-fucking is DOMINANT-SUPER;
		say AfterDominationComment 1 of M;
	otherwise:
		if debugmode > 0, say "[bold type]FAILED[roman type][line break]";
		say "[big he of M]'s clearly more than a little intimidated to have your [SlimeJunk] looming over [him of M], but [if the player is wrist bound]with your wrists bound there isn't much you can do to hold onto the wand anyway. [big he of M][otherwise][he of M][end if] doesn't jerk away until [he of M] has swallowed every last drop. Satisfied, you drop the wand at [his of M] feet.";
		say AfterDominationComment 2 of M;
	now the bladder of the player is 0;
	SportsGet.

To blowjob dominate (M - an aeromancer):
	let C be a random bottom level protection clothing;
	let G be 0;
	if the player is able to speak, now G is 1;
	say "[BigNameDesc of M] raises [his of M] wand as you force [him of M] to [his of M] knees, but it flies out of [his of M] hand and into yours as if attracted there by, well, magic. It howls with energy as sheets of air shoot from the tip and spin around [his of M] wrists, yanking them above [his of M] head.[line break][speech style of M]'Fuck.'[roman type][line break][big he of M] looks from [his of M] bonds to [if C is not strapon-panties and C is clothing]your [ShortDesc of C]. [otherwise if the player is penis-erect]your [SexDesc of penis]. [otherwise]your swelling [sexual-player-penis]. [end if]";
	if the player is able to speak, say "[line break][variable custom style]'If you want it back you have 30 seconds to make me cum.'[roman type][line break]";
	otherwise say "[line break][speech style of M]'Fine. I'll finish this in 30 seconds or less.'[roman type][line break]";
	if sexual-penis-length >= 9:
		let R be the semi-dominance roll for M;
		if R >= 0:
			if debugmode > 0, say "[bold type]PASSED[roman type][line break]";
			say "The sheer size of your [SexShaft] causes [him of M] to hesitate despite the [unless G is 1]self imposed [end if]deadline, and [he of M] trembles a bit as you helpfully [unless the player is wrist bound]grab the back of [his of M] head and [end if]push your [SexTip] through [his of M] lips. In the next moment, your hand is nearly knocked away as [he of M] begins to bob [his of M] head with supernatural agility. You grit your teeth as an insanely powerful suction force attacks your [SexShaft], grabbing [his of M] head with BOTH hands and jamming your [sexual-player-penis] down [his of M] throat. [big he of M] gags and tries to pull up, but you hold [him of M] like that until the time runs out, relishing the indignant look on [his of M] face when you finally shoot your [load] down [his of M] throat.";
			now player-fucking is DOMINANT-SUPER;
			strongDignify;
			say AfterDominationComment 3 of M;
		otherwise:
			if debugmode > 0, say "[bold type]FAILED[roman type][line break]";
			say "The sheer size of your [SexShaft] causes [him of M] to hesitate despite the [unless G is 1]self imposed [end if]deadline, and [he of M] trembles a bit as you helpfully [unless the player is wrist bound]grab the back of [his of M] head and [end if]push your [SexTip] through [his of M] lips. In the next moment, [if the player is wrist bound]you are nearly[otherwise]your hand is[end if] knocked away, an insanely powerful suction force attacking your [SexShaft] as [his of M] bobs [his of M] head with supernatural agility. [if the player is wrist bound]Wrist bound, your only choice is[otherwise]Yelping, you try to take back control or at least concentrate enough[end if] to endure, but you don't even last 10 seconds, groaning in frustration as you shoot your [load] straight down [his of M] throat. As if satisfied, the wand snaps back into [his of M] hand.";
			moderateDignify;
	...
	otherwise if sexual-penis-length >= 4:
		...
		if R >= 0:
			if debugmode > 0, say "[bold type]PASSED[roman type][line break]";
			say "[big he of M] dives forward without hesitation, sucking your [sexual-player-penis] into [his of M] mouth and bobbing [his of M] head on it with supernatural speed. Insane suction force attacks your [SexShaft], and you clench every muscle in your body trying to hold on. The seconds tick by like molasses, but you somehow hold on until the very end, shooting your [load] in [his of M] mouth the very moment time is up. [big he of M] scowls as [he of M] spits it out.";
			moderateDignify;
			now player-fucking is DOMINANT-SUPER;
		otherwise:
			if debugmode > 0, say "[bold type]FAILED[roman type][line break]";
			say "[big he of M] dives forward without hesitation, sucking your [sexual-player-penis] into [his of M] mouth and bobbing [his of M] head on it with supernatural speed. Insane suction force attacks your [SexShaft], and you moan in surprise as you desperately try to slow [him of M] down or at least endure. [if the player is wrist bound]It's useless with your hands bound, though[otherwise][big he of M]'s just too fast, [end if]and within seconds you are shooting your [load] [if bukkake fetish is 1]all over [his of M] face. [otherwise]in [his of M] waiting mouth. [end if]As if satisfied, the wand snaps back into [his of M] hand.";
			slightDignify;
			now player-fucking is DOMINANT-NEUTRAL;
			say AfterDominationComment 4 of M;
	otherwise:
		say "Sucking in one enormous breath, [he of M] leans forward and shoots a focused blast of air right at your [SexTip]. A moan escapes your lips as you instantly ejaculate, your [load] splattering [if bukkake fetish > 0]across your belly and thighs. [otherwise]everywhere. [end if]You stand there dumbfounded as [he of M] yanks the wand from your hands.";
		if bukkake fetish > 0:
...
To penetration dominate (M - an aeromancer):
	if M is presenting as male, anal penetration dominate M;
	otherwise vaginal penetration dominate M.

To vaginal penetration dominate (M - an aeromancer):
	let C be a random bottom level protection clothing;
	say "[BigNameDesc of M] raises [his of M] wand as you force [him of M] to [his of M] knees, but it flies out of [his of M] hand and into yours as if attracted there by, well, magic. It howls with energy as sheets of air shoot from the tip and spin around [his of M] limbs, binding [him of M] up like a Thanksgiving turkey.";
	cutshow Figure of aeromancer dominated 2 for M;
	if sexual-penis-length >= 9:
		say "[line break][speech style of M]'W-what are you going to do with me?'[roman type][line break][big he of M] asks, voice trembling as [he of M] [if C is not strapon-panties and C is clothing]watches you free your [sexual-player-penis] from your [ShortDesc of C]. [otherwise if the player is penis-erect]stares at your [SexDesc of penis]. [otherwise]watches your [sexual-player-penis] *swell* to hardness. [end if] You silently line it up with [his of M] slit.[line break][speech style of M]'[one of]But that's too big! That's way t-too... [or]Aren't you satisfied after last time?! [stopping] Um, c-couldn't you just use my ass? I'll even let you keep my wand!'[roman type][line break]Go for [his of M] ass instead?";
		if the player is consenting:
			anal penetration dominate M;
		otherwise:[optimisation: replace all the he of M, etc. with just normal pronouns, since this is female presenting aeromancer only]
			say "Nope. You drop the wand, revelling in [his of M] long, low groan as your [sexual-player-penis] sinks into [his of M] incredibly tight pussy. [big he of M] treats you to a sweet song of dejected moans, squirming helplessly as you slowly begin to rock your hips. Without using [his of M] safeword, there isn't much [he of M] can do with [his of M] bound body, and it isn't long before [he of M] stops struggling completely, [his of M] [ChestDesc of M] heaving as [his of M] 'song' slowly heads toward a crescendo. [big he of M] throws [his of M] head back as [he of M] comes, [his of M] back arching as [his of M] inner walls desperately clench around your [SexDesc of penis]. The increase in sensation almost sets off your own 'crescendo,' but you purposely hold back and slow down before you lose control. You milk orgasm after orgasm out of [him of M], thoroughly enjoying yourself in the process, lasting as long as possible before finally filling [him of M] with your [load]. [big his of M] bonds disappear as you pull out.";
			MagicPowerUp 2;
	...
	otherwise if sexual-penis-length >= 7:
		say "[line break][speech style of M]'What are you going to do with me?'[roman type][line break][big he of M] asks cautiously, [if C is not strapon-panties and C is clothing]watching you free your [sexual-player-penis] from your [ShortDesc of C]. [otherwise if the player is penis-erect]staring at your [SexDesc of penis]. [otherwise]watching your [sexual-player-penis] swell to hardness. [end if] You silently line it up with [his of M] slit.[line break][speech style of M]'Do you have any idea how hard it is to clean-! Ah, u-um, my ass is better, so why not try it. [big please], I'll even let you keep my wand!'[roman type][line break]Go for [his of M] ass instead?";
		if the player is consenting:
			anal penetration dominate M;
		otherwise:
			say "Nope. You drop the wand, smirking at the look on [his of M] face as your [sexual-player-penis] sinks into [his of M] tight little pussy. [big he of M] treats you to a sweet song of dejected moans, squirming helplessly as you begin to fuck [him of M]. Without using [his of M] safeword, there isn't much [he of M] can do with [his of M] bound body aside from squirm, increasing the sensation around your [SexShaft]. It doesn't take too long for [him of M] to give up, [ChestDesc of M] heaving as [his of M] 'song' slowly heads toward a crescendo. However, it turns out [his of M] efforts were more 'effective' than [he of M] realised, and you try to slow down without letting [him of M] know you're even closer than [he of M] is. [run paragraph on]";
			let R be the semi-dominance roll for M;
			if R >= 0:
				if debugmode > 0, say "[bold type]PASSED[roman type][line break]";
				say "[big he of M] doesn't suspect a thing, and a few moments later [he of M] throws [his of M] head back, [his of M] inner walls clenching around your [SexDesc of penis] as [he of M] shudders in orgasm. With no more reason to hold back, you blow your load, groaning with pleasure as you plant every drop of your [load] deep inside [his of M] pussy.";
				orgasm M;
				strongDignify;
				FavourDown M by 2;
				say AfterDominationComment 5 of M;
			otherwise:
				if debugmode > 0, say "[bold type]FAILED[roman type][line break]";
				say "It seems like [he of M] doesn't suspect a thing, but [he of M] struggles fiercely just before you go over the edge, causing your [sexual-player-penis] to slip out of [him of M] at the last moment. [big he of M] sighs in relief as your [load] shoots out on [if bukkake fetish is 1][his of M] belly. [otherwise]the ground. [end if]Your [sexual-player-penis] softens as [his of M] bonds slowly disappear.";
				now player-fucking is DOMINANT-NEUTRAL;
				moderateDignify;
			...
			FuckGet;
	otherwise if sexual-penis-length >= 4:
		say "[line break][speech style of M]'What are you going to do with me?'[roman type][line break][big he of M] asks, frowning as [he of M] [if C is not strapon-panties and C is clothing]watches you free your [sexual-player-penis] from your [ShortDesc of C]. [otherwise if the player is penis-erect]stares at your [SexDesc of penis]. [otherwise]watches your [sexual-player-penis] swell to hardness. [end if] You silently line it up with [his of M] slit.[line break][speech style of M]'Oh. W-well, my ass is way better. How about it? Y-you can even have my wand!'[roman type][line break]Go for [his of M] ass instead?";
		if the player is consenting:
			anal penetration dominate M;
		otherwise:
			say "Nope. You throw the wand over your shoulder, smirking at the look on [his of M] face as your [sexual-player-penis] sinks into [his of M] warm pussy. [big he of M] groans dejectedly as you begin to fuck [him of M], and [ChestDesc of M] shakes as [he of M] struggles ineffectively. Without using [his of M] safeword, there isn't much [he of M] can do aside from increase the sensation around your [SexShaft] with [his of M] squirms, but that turns out to be enough. It isn't long at all before you're struggling to hold back your orgasm, hoping [FuckerDesc of M] won't notice and take advantage of it somehow. At first it seems like [he of M] hasn't noticed, but it turns out [he of M]'s only waiting for the right moment, and starts struggling fiercely just before you go over the edge. [run paragraph on]";
			let R be the semi-dominance roll for M;
			if R >= 0:
				if debugmode > 0, say "[bold type]PASSED[roman type][line break]";
				say "You act quickly and grab [his of M] hips before [he of M] accomplishes anything, making sure every drop of your [load] is planted deep inside [his of M] pussy.";
				orgasm;
				moderateDignify;
				FavourDown M by 2;
				say AfterDominationComment 2 of M;
			otherwise:
				if debugmode > 0, say "[bold type]FAILED[roman type][line break]";
				say "Your short [sexual-player-penis] slips out of [him of M] at the last moment, ruining your orgasm and causing your [load] to shoot out on [if bukkake fetish is 1][his of M] belly. [otherwise]the ground. [end if]Your [sexual-player-penis] softens as [his of M] bonds slowly disappear.";
				now player-fucking is DOMINANT-NEUTRAL;
				orgasm;
				Arouse 1000;
				slightDignify;
			...
			FuckGet;
	otherwise:
		say "[line break][speech style of M]'So, uh... what happens now?'[roman type][line break][big he of M] asks, a noticeable hint of mirth in [his of M] voice as [he of M] [if C is not strapon-panties and C is clothing]watches you [']free['] your [sexual-player-penis] from your [ShortDesc of C]. [otherwise if the player is penis-erect]stares at your [SexDesc of penis]. [otherwise]stares at your swelling [sexual-player-penis]. [end if] You silently line it up with [his of M] slit.[line break][speech style of M]'Haha, and you think you can do that AND steal my wand? Why don't you just give it back, then I'll let you fuck my ass. Sound good?'[roman type][line break]Go for [his of M] ass instead?";
		if the player is consenting:
			anal penetration dominate M;
		otherwise:
			say "[line break][speech style of M]'Woooow... OK, don't cum inside-and don't touch my [if aeromancers wear breast forms]harness[otherwise]boobs[end if].'[roman type][line break]You decide that's pretty fair, emitting a sigh as you push forward, allowing your [SexTip] to slightly part [his of M] folds. [big he of M] sighs and moans here and there, just tight enough to feel you, but in the same way that [he of M] would probably feel a finger or the end of a pen. It's much more intense for you though, and before long you pull out and coat [if bukkake fetish is 1][his of M] stomach[otherwise]the ground[end if] with your [load]. [big he of M] sits up when the chains disappear, yanking the wand from your hand.";
			now player-fucking is DOMINANT-NEUTRAL;
			say AfterDominationComment 6 of M;
			moderateDignify;
			FuckGet;
...
To anal penetration dominate (M - an aeromancer):
	let C be a random bottom level protection clothing;
	if andro fetish is 1, cutshow Figure of aeromancer dominated 2 for M;
	if sexual-penis-length >= 9:
		if andro fetish is 1, say "[BigNameDesc of M] raises [his of M] wand as you force [him of M] to [his of M] knees, but it flies out of [his of M] hand and into yours as if attracted there by, well, magic. It howls with energy as sheets of air shoot from the tip and spin around [his of M] limbs, binding [him of M] up like a Thanksgiving turkey.[line break][speech style of M]'W-what?! You're betraying me?! But I was only looking!'[roman type][line break]";
		otherwise say "[line break][speech style of M]'Phew...'[roman type][line break]";
		say "[big he of M] takes a deep breath as you flip [him of M] onto [his of M] stomach and spit on your [SexDesc of penis], throwing a complicated look over [his of M] shoulder as your [SexShaft] slides into [his of M] ass. You're a little rough with [him of M], but [if andro fetish is 1]you took the time to lube yourself up first[otherwise]you're doing [him of M] a favour here[end if], so it's no surprise to you that [he of M] doesn't complain. In fact, [he of M] seems to love the treatment, arching [his of M] back and moaning with pleasure as you furiously drill [his of M] ass. [big his of M] enthusiasm is pleasurable all by itself, and before you realise it you've lost control, your [sexual-player-penis] is throbbing as it fills [his of M] belly with your [load].";
	...
	otherwise if sexual-penis-length >= 4:
		...
		say "[big he of M] does [his of M] best to wiggle [his of M] ass enticingly as you roll [him of M] onto [his of M] stomach, palming [his of M] ass as you slide your [SexDesc of penis] into [his of M] asshole. You have no intention of being rough at first, but [his of M] enthusiasm catches you off guard, and you're speeding up before you've even realised it. [big he of M] spends the entire time looking over [his of M] shoulder, egging you on with hungry moans as you furiously drill [his of M] ass. There's no way you could keep it up for long, and you groan with pleasure as you pull out and cover [if bukkake fetish is 1][his of M] back[otherwise]the ground[end if] with your [load].";
		say AfterDominationComment 7 of M;
		slightDignify;
		now player-fucking is DOMINANT-SUPER;
	otherwise:
		if andro fetish is 1, say "[BigNameDesc of M] raises [his of M] wand as you force [NameDesc of M] onto [his of M] back, but it jumps out [his of M] hands and into yours! Howling with energy, a sheet of compressed air shoots out of the tip and passes over [his of M] body, making [his of M] clothes disappear like smoke. [big he of M] puts [his of M] hand on your shoulder as you try to line up your [SexDesc of penis] with [his of M] asshole.[line break][speech style of M]'Look, I know you're trying to dominate me and all, but I'm really going to need my wand back. Now.'[roman type][line break][big he of M] grins as you hand it back to [him of M].[line break][speech style of M]'Thanks a lot. OK, go ahead.'[roman type][line break]";
		otherwise say "[big he of M] grins as you hand the wand back to [him of M].[line break][speech style of M]'Thanks a lot. Enjoy!'[roman type][line break]";
		say "[big he of M] wiggles [his of M] ass enticingly as you roll [him of M] onto [his of M] stomach, moaning seductively as you push your [SexDesc of penis] into [his of M] asshole. You originally intended to savour it a little, but [his of M] enthusiasm turns you on way too much, and it only takes seconds for you to lose control. [big he of M] sighs as you fill [his of M] ass with your [load].";[no dignity bonus]
		now player-fucking is DOMINANT-NEUTRAL;
...
To say AfterDominationComment (N - a number) of (M - an aeromancer):
	...
	if N is 0:
		say "'[one of]I'll get you back for this, jerk!'[or]You're going to pay for this, jerk!'[in random order]";
	otherwise if N is 1:
		say "'[one of]S-so I guess the wand is yours then...'[or]D-did you really have to [slime] on me?'[in random order]";
	otherwise if N is 2:
		say "'[one of]UGH, I'll get you back for this.'[or]GROSS! You're SO going to pay for that.'[in random order]";
	otherwise if N is 3:
		say "'[one of]I'll get my wand back, you know. S-someday...'[or]I c-can't believe I lost my wand...'[in random order]";
	otherwise if N is 4:
	...
	otherwise if N is 5:
		say "'[one of]E-even if you cum inside me... t-this isn't over.'[or]I-it all went inside me... I w-won't forget this.'[or]Fuck. There's so m-much in me... SO gross...'[in random order]";
	otherwise if N is 6:
		say "'[one of]See you later, idiot.'[or]Later, idiot.'[in random order]";
	otherwise if N is 7:
		say "'[one of]When I get my wand back, watch out.'[or]Enjoy my wand while you can.'[in random order]";
	otherwise:
		say "'[one of]I have experiments to plan. See you.'[or]Later.'[in random order]";
	say "[roman type][line break]";

To say DominationEscapeFlav of (M - an aeromancer):
	if player-fucking is not DOMINANT-SUPER:
		say "A gust of wind picks up [NameDesc of M] and whisks [him of M] away!";
	otherwise:
		say "[BigNameDesc of M] crawls away as soon as [he of M] has the strength. You've got a feeling you won't be seeing [him of M] again for a while.";

To replace (M - an aeromancer) after domination:
	if player-fucking is not DOMINANT-SUPER:
...
To say DominanceFailed of (M - an aeromancer):
	say "[line break]A gust of wind picks up [NameDesc of M] and carries [him of M] away, leaving you to stew in your own embarrassment as your chains dissolve into thin air.".

To compute unique dominance reward of (M - an aeromancer):
...
To say DominanceFailure of (M - an aeromancer):
	let A be a random worn research airhancer;
	if A is clothing:
		say "You grab [NameDesc of M] and try to shove [him of M] to the ground, but the moment you touch her, golden chains explode out of your [Shortdesc of A]! They twist in the air before coiling tightly around your body, leaving you at [his of M] mercy.";
	otherwise:
		say "You shove [NameDesc of M] to the ground and wrench the wand from [his of M] hand. It heats up in your gasp, emitting blue sparks as golden chains explode from the tip, twisting rapidly in the air before coiling tightly around your body. [BigNameDesc of M] smirks as the wand floats back to [him of M], forcing you to your knees with [if M is confident aeromancer]a tiny flick[otherwise]an exaggerated flourish[end if] of [his of M] wrist.".

To compute failed dominance punishment of (M - an aeromancer):
	let A be a random worn research airhancer;
	if A is clothing:
		if M is the parent-mage of A, say "[line break][speech style of M]'Haha, wow, I can't be-breeze you thought that would work. Did you forget you were wearing it?[roman type][line break][big he of M] flicks your flaccid penis.[line break][speech style of M]'[if player-fucker is penis]That's not going inside me. Sorry~!'[otherwise]I'm not dumb enough to leave a loophole. Sorry~!'[end if][roman type][line break][BigNameDesc of M] loses interest.";
		otherwise say "[line break][speech style of M]'Haha, wow, I don't know who put that on you, but you've got hot air between your ears if you thought you'd get anywhere wearing that.'[roman type][line break][big he of M] flicks your flaccid penis.[line break][speech style of M]'Bad luck for you~!'[roman type][line break][BigNameDesc of M] loses interest.";
		moderateHumiliate;
		calm M;
	if the player is not possessing a penis:
		say "[speech style of M]'This next experiment is really going to [if belly liquid types > 0]suck[otherwise]blow[end if]!'[roman type][line break][BigNameDesc of M] presses the tip of [his of M] wand to your belly button, laughing uncontrollably as your belly expands audibly[if inflation fetish is 1] with a rush of air[end if].[if belly liquid types > 0] You hear a gurgle as the liquid contents of your belly are shunted out the other end![end if]";
		SemenPuddleUp the semen volume of belly;
		MilkPuddleUp the milk volume of belly;
		SlimePuddleUp the slime volume of belly;
		empty belly liquids;
		if inflation fetish is 1, AssFill 10 Air;
		strongHumiliate;
	otherwise if the player is getting unlucky:
		let T be a random off-stage research airhancer;
		if T is actually summonable:
			say "[speech style of M]'I think you'll find my methods to be airtight!'[roman type][line break][BigNameDesc of M] touches [his of M] wand to your [ShortDesc of penis], whistling as air swirls around your length. You watch as a semi-transparent sheet of compressed air layers itself over your shaft, causing your [if the player is a bit horny]erection[otherwise]half-erection[end if] to slowly go away as a golden watch materialises around your wrist.[line break][speech style of M]I call this my research airhancer. Get it? AIR-hancer?[roman type] [big he of M] pauses to laugh uncontrollably.[line break][speech style of M]So, this thing monitors the air pressure in your belly, and if it dips too low, it should keep your dick from getting hard. No more sex for you unless you're a good little test subject and help out with my experiments! I'm pretty sure I'm the only one who can take it off, but hey, I guess you can't BLOW IT until you try! Hahaha!'[roman type][line break]";
			summon T;
			now the parent-mage of T is M;
			calm M;
		otherwise:
...
To compute tornado experiment of (M - an aeromancer):
	say "[speech style of M]I'm sorry about this experiment. It really blows loads![roman type][line break]A tiny tornado flows out of the tip of [his of M] wand, grabbing your [ShortDesc of penis] and locking it in a prison of powerful suction. [run paragraph on]";
	if the size of penis >= 9 or the health of M < 3:
		say "It feels incredible for a moment, but the cone dissipates after only a couple seconds.[line break][speech style of M]'Wow, looks like I'm gonna have to turn up the juice!'[roman type][line break][BigNameDesc of M] furrows [his of M] brow as [he of M] raises [his of M] wand again, whistling forcefully as a second wind tunnel twice as large as the original slowly flows from the tip of [his of M] wand, sucking in air from all around you as it assaults your penis with a rapidly intensifying vortex of pressure. The aeromancer struggles to maintain the spell, clothes ruffling in the wind and eventually tearing off as the suction builds and builds, torturing you with an ever mounting onslaught of pleasure.";
		...
		otherwise:
			...
			orgasm;
	otherwise:
		say "It feels incredible! The aeromancer makes sure not to let you enjoy it though, deviously manipulating the width of the tunnel according to the intensity of your moans. [if the size of penis >= 7][big he of M] holds your dick hostage for what feels like an eternity, cruelly releasing you from the spell just before you're able to cum[otherwise][big he of M] holds your dick hostage for what feels like an eternity, cruelly releasing you from [his of M] spell just after you go over the edge, ensuring that your orgasm is as short and unsatisfying as possible[end if]. You can't help your [if the delicateness of the player < 7]infuriated[otherwise]pleading[end if] stare as [he of M] slowly climbs to [his of M] feet.";
...
To say FirstResponse of (M - an aeromancer):
	...
	alwayscutshow figure of aeromancer interact 15 for M.
...
To say RepeatResponse of (M - an aeromancer):
	...
	alwayscutshow figure of aeromancer interact 16 for M.

To say SubmissiveResponse of (M - an aeromancer):
	if the class of the player is faerie:
		say "[speech style of M]'[one of]Hmm, most fairies are obsessed with getting pregnant, but it seems like all you care about is sex!'[or]I knew you'd love my experiments, lucky for you I'm close to a blow through! Hahaha!'[then at random][roman type][line break]";
	...
	otherwise:
		say "[speech style of M]'[one of]Hmm, strange decision to show your face here again.'[or]Ready for some more science?'[or]Hahaha... your eagerness to be a test subject [please]s me greatly!'[at random][roman type][line break]";
	alwayscutshow figure of aeromancer interact 2 for M.
...
To say AnnoyedResponse of (M - an aeromancer):
...
To say TauntAccepted of (M - an aeromancer):
	if M is wand-empowered:
		say "[speech style of M]'There's no way you're stealing my wand again! I'm out of here!'[roman type][line break]A gust of wind blows through the room, whisking [NameDesc of M] away.";
		regionally place M;
	otherwise:
		say "[speech style of M]'Oh yeah, w-well...What if I just leave then?'[roman type][line break][BigNameDesc of M] blushes slightly and backs away.";

To say TauntRejected of (M - an aeromancer):
...
To say WhereAnswer of (M - an aeromancer):
	...
	alwayscutshow figure of aeromancer interact 18 for M.
...
To say AdviceAnswer of (M - an aeromancer):
	say "[speech style of M]'[one of]If there's a room you might need to spend a lot of time on your knees, for example a room with furniture, maybe it would make sense to not risk opening the container in that room, in case it's trapped and releases pink smoke everywhere.'[or]Pretty much every living thing in this world seems to like being given shiny jewellery. Even the mindless ones like the sex dolls.'[or]I hear there's a machine somewhere that gives you magic tattoos depending on what you donate it. I bet that giving it a potion you've crafted might get you a tattoo that makes you smarter!'[in random order][roman type][line break]";
	alwayscutshow figure of aeromancer interact 16 for M.
...
Definition: confident aeromancer is woods dwelling:
	if inflation fetish is 1, decide yes;
	decide no.

The previously-ballooned of confident aeromancer is usually 1.
...
To compute aeromancer science of (M - confident aeromancer):
	...
	otherwise if R is 2:
		...
		otherwise if the bladder of the player > 0:
			...
			try urinating;
		otherwise if C is transformable clothing:
			say "Your [C] is suddenly hit by a beam of light from [his of M] wand!";
			potentially transform C;
		otherwise:
			say "Air suddenly forces its way inside your [asshole], stretching your sphincter as it makes its way in!";
			gape asshole times 1;
	...
	otherwise: [Selkie: it'd be sweet if she could add a step or two to a latex-doll transformation that was underway]
		...
		OralSexAddictUp 1;
		SexAddictUp 1;
	say "[speech style of M]'[one of]Whew, that was awesome!'[or]Thanks for your help.'[or]Interesting...'[or]Well that was unexpected...'[or]Curiouser and curiouser.'[in random order][roman type][line break][BigNameDesc of M] [one of]adds short note[or]smirks at you and enters just a word or two[or]makes a quick note[or]adds a few words[at random] into [his of M] [aerobook of M] and then closes it.".
...
To say CreampieFlav of (M - confident aeromancer) in (F - vagina):
	say "[BigNameDesc of M] fucks you harder and harder, screaming with delight as [his of M] magical phallus floods your [vagina] with warm, glittery [semen]. [big he of M] releases [his of M] hold on your body as the gooey fluid soaks into your skin, causing a strange tingling feeling to travel up your torso and into your chest. You watch with [horror] as your breasts suddenly balloon with air!";
	cutshow figure of aeromancer cutscene 5 for M;
	say "[BigNameDesc of M] drops you on your back, taking a brief moment to smirk at [his of M] handiwork before losing interest.".

To say TwosomePrep of (M - confident aeromancer) in (F - asshole):
	say "[BigNameDesc of M] stretches out [his of M] arms and hands towards you. You yelp as you are suddenly forced several feet off the ground by a powerful gust of wind. Cackling with glee, the aeromancer uses [his of M] mastery of air to buffet you from one end of the [if M is in the Woods]clearing[otherwise]room[end if] to the other, eventually leaving you suspended in the centre, at just above waist height. [big he of M] takes a couple steps forward, barely containing [his of M] mirth as you [if the relevant sex addiction of M < 7]desperately try and fail to get purchase on the ground below[otherwise]look up at [him of M] in wide-eyed anticipation[end if].".
...
To say PenetrationFlav of (M - confident aeromancer) in (F - vagina):
...
To say FriendlyPenetrationFlav of (M - confident aeromancer) in (F - asshole):
	say PenetrationFlav of M in F.

To say FriendlyPenetrationFlav of (M - confident aeromancer) in (F - vagina):
	say PenetrationFlav of M in F.

Section 2 - Spectacled Aeromancer

spectacled aeromancer is an aeromancer. The text-shortcut of spectacled aeromancer is "iae".
To say MediumDesc of (M - spectacled aeromancer):
	say "spectacled [BalloonDesc of M]aeromancer".

Definition: spectacled aeromancer is dungeon dwelling: decide no.
Definition: spectacled aeromancer is hotel dwelling:
	if inflation fetish is 1 and (infamy fetish is 1 or doomed > 1), decide yes;
	decide no.

To say aerobook of (M - an aeromancer):
	if infamy fetish is 1, say "tablet";
	otherwise say "book";

To compute appearance assessment of (M - spectacled aeromancer):
	if the player is top-wardrobe-malfunctioning:
		compute tq nip slip reaction of M;
	otherwise:
		FavourDown M by the science requests of M * 2;
		let A be the air volume of belly + the air volume of breasts + the air volume of hips;
		if A >= 15:
			say "[speech style of M]'[one of]Wow, a real-life bimbius bubblius! [or]Is this real? The [aerobook of M] said the bimbius bubblius was extinct! [or]Incredible. So the bimbius bubblius truly does exist! [in random order]You, I need you for my experiment! How about it?'[roman type][line break]";
		otherwise if the player is exposed:
			say "[speech style of M]'[one of]A bimbius nudies... how majestic! [or]So its true what they say about the bimbius nudius... [or]Intriguing. The bimbius nudies IS different from the bimbius sluttius. [in random order]You, I need you for my experiment! How about it?'[roman type][line break]";
		otherwise:
			say "[speech style of M]'[one of]Hmm, so it's true what they say, the bimbius sluttius can really be found anywhere! [or]Right, the bimbius sluttius, also known as the common bimbo... [or]Is that a bimbius sluttius? Perfect! [in random order]You, I need you for my experiment! How about it?'[roman type][line break]";
		if M is interested:
			if the player is bimbo consenting:
				FavourUp M;
				compute aeromancer science of M;
				...
				if M is unfriendly, satisfy M;
			otherwise if A < 15 or the science requests of M > 2:
				say "[speech style of M]'[one of]Uneducated swine...'[or]Ignorant fool...'[or]Stultus!'[at random][roman type][line break]";
				increase the science requests of M by 1;
				bore M.
