...
Definition: dominatrix is willing to do licking:
	if grossness fetish > 0, decide yes;
	decide no.

Definition: dominatrix is willing to do anilingus: decide no.
...
To say MonsterComment of (M - dominatrix):
	if diaper quest is 1:
		...
		otherwise:
			say "[variable custom style]Why does that strap-on look so appetising?";
	otherwise if the strap-on girth of M < 4:
		...
		otherwise if the player is gendered female and the bimbo of the player < 14:
			say "[variable custom style][one of]Why does that strap-on look so appetising?[or]Why do I feel like I should do anything [he of M] says?[or]I feel safe when I'm near [him of M], but scared at the same time.[in random order]";
		otherwise if the player is gendered female:
			say "[second custom style][one of]I want to give [his of M] strap-on a big sloppy blowjob![or]I'll do anything [he of M] says! I hope [he of M] wants something naughty![or]I can tell [he of M] knows all the best ways of having a good time![in random order]";
		otherwise if the bimbo of the player < 7:
			say "[first custom style][one of]Is that a dildo? Haha, it's pointing the wrong way. Right?[or][big he of M][']s bad news. I'm not sure I want to risk messing with [him of M].[or]Why do I feel like [he of M] expects something from me?[or][if the size of penis < 1]It's not fair [he of M] gets to have a dick that big and I don't even get one![otherwise if the size of penis < 8]That dildo is actually bigger than my dick. Heh...[otherwise]That strap-on is actually the size of a real dick... on [him of M] it seems a little bit excessive.[end if][or]That strap-on probably isn't for [him of M].[in random order]";
		otherwise if the player is gendered male and the bimbo of the player < 14:
			say "[variable custom style][one of][if the size of penis < 1]Maybe if I get on [his of M] good side I can use [his of M] strap-on as a replacement penis.[otherwise if the size of penis < 8]That dildo isn't even that big, but it's still bigger than me. Can I really call myself a man if [he of M] can out-compete me with a hunk of plastic?[otherwise]It takes more than a hunk of plastic to out-compete me! Even if I don't really deserve a penis this big...[end if][or]All I can think about when I look at [him of M] is how much more of a man [he of M] is than me.[or]I really want to know what [his of M] strap-on would feel like inside me. It wouldn't be gay, [he of M]'s a [man of M].[in random order]";
		otherwise:
			say "[second custom style][one of]I'll give anyone a blowjob, even a girl![or]I prefer guys, but that strap-on looks almost as good as the real thing![or]All [manly-penis]s make me [if the size of penis < 1]horny[otherwise]hard[end if], even fake ones![if the size of penis < 1]Between us, there's only one penis, but [he of M] probably deserves it way more![otherwise if the size of penis < 8]So that's what a real penis looks like. Well, it's plastic, but it's certainly realer than mine. I'm the girl and [he of M][']s the man! Tee-hee![otherwise]My penis is bigger than [his of M] strap-on, but [hers of M] matters more, since it feels better when I cum with my asshole.[end if][in random order]";
	otherwise if the strap-on girth of M < 6:
		...
		otherwise:
			say "[second custom style][one of]That is going to feel so good inside me! I'm glad one of us knows how to use a dick![or]I love big cocks, even if they're plastic and attached to a [man of M] instead of a sexy man.[or]Before I started this game, I wanted to fuck girls, now I want to give blowjobs and get fucked. With [him of M] around, I can do both! Tee-hee![or][big literalMistress of M] knows I've been good, so I deserve to get fucked HARD![or][if the size of penis < 1][big literalMistress of M] is my lesbian lover![otherwise if the size of penis < 5]My penis is useless, but [literalMistress of M] has more than enough for the both of us![otherwise if the size of penis < 9][big his of M] dildo is bigger than my penis! Just the way things should be.[otherwise if the player is sexed male][big his of M] strap-on is still smaller than my cock. Oh well, that just means I'll cum more when [he of M][']s done pounding my prostate![end if][or]If [literalMistress of M] says not to cum, I shouldn't. But [he of M] must have no idea what it's like to have a big hard cock ram your [if the player is sexed male]prostate[otherwise]hole[end if]![or]If I want a bigger toy to play with, I need to control my orgasms. But since I'm such a dirty slut, it will not be easy![in random order]";
	otherwise:
		if the player is gendered female and the bimbo of the player < 7:
...
Definition: dominatrix (called M) is objectifying the player:
	...
	if the slave-status of M < -3:
		now the objectification of M is 0;
		decide no;
	if the times-submitted of M > 3 and the frustration of M < 1:
		now the objectification of M is 1;
		decide yes;
	...
	if the player is feeling submissive:
		...
		decide yes;
	if M is in the location of the player:
		if the player is prone or there is a wrestler in the location of M:
			now the objectification of M is 1;
			decide yes;
	now the objectification of M is 0;
	decide no.
...
To compute monstermotion of (M - dominatrix):
	...
	otherwise if M is not in Hotel37 and M is unleashed:
		if the player is in Hotel37 or M is in the location of the player or M is nearby, compute room leaving of M;
		otherwise now M is in Hotel37.
...
To say LickInitiationFlav of (M - dominatrix):
...
To say NearingClimaxLicking of (M - dominatrix):
...
To say LickSubmissionResponse of (M - dominatrix):
	if the sex-length of M > 0:
...
To say LickResistingResponse of (M - dominatrix):
...
To compute unique banishment of (M - dominatrix):
	...
	if the strap-on girth of M >= 6 and equine-strapon is off-stage, now X is equine-strapon;
...
To watersports dominate (M - dominatrix):
	say "You successfully force [NameDesc of M] to [his of M] knees, holding [him of M] down as you rip off [his of M] strap-on, leaving [his of M] crotch bare. [big he of M] [if the slave-status of M > -3]stares at you silently for a moment before cocking an eyebrow and spreading [his of M] legs.[line break][speech style of M]'Alright. Here's your chance.'[otherwise]waits a moment before submissively spreading [his of M] legs.[line break][speech style of M]'Alright.'[end if][roman type][line break]";
...
To say EmergencyArm of (M - dominatrix):
	let R be the room east from Hotel37;
	say "[line break][BigNameDesc of M] pulls out a small remote, pressing a button as a robotic arm shoots out of the floor and drags you into the [R]. A metal shutter noisily clatters down behind you.".
...
To compute fleeing of (M - dominatrix):
	if M is in Hotel37 and the player is in Hotel37:
		let R be the room east from Hotel37;
		say EmergencyArm of M;
		now the player is in R;
	otherwise:
