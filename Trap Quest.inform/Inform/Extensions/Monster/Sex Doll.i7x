...
To compute perception of (M - a sex doll):
	...
	otherwise if a random number between 6 and 15 < highest body part sex addiction and the player is not at least partially immobile and the player is not flying and the player is not dildo stuck:
		anger M;
		say "[BigNameDesc of M][']s fake penis wobbles as it walks around. In a moment of weakness, you find yourself dropping to your knees to satisfy the cravings of your [variable most desirable body part]!";
		allocate 1 seconds;
		try kneeling;
...
This is the sex doll compression rule:
	...
	if M is doll-charged:
		if the difficulty of M >= the starting difficulty of M + 2 and the player is upright:[At max power, it gains an additional attack.]
		...
		rule succeeds;
	otherwise if the difficulty of M > the starting difficulty of M and a random number between 1 and 5 is 1 and the player is upright:
...
To compute FluidFill of (M - a sex doll):
	if the player is a flatchested trap:
		say "[BigNameDesc of M] suddenly stops, its [manly-penis] bulging slightly as several spurts of cool fluid flow inside of [if M is wrapped]the condom, which almost immediately bursts, filling [end if]you. You feel your hips growing in size!";
		HipUp 1;
	otherwise:
		let R be a random number between 1 and 3;
		if M is wrapped, say "[if R is 3][BigNameDesc of M] suddenly stops, its [manly-penis] bulging slightly as several spurts of cool fluid flow inside the condom, which almost immediately bursts, filling you. You feel your breasts and hips both growing in size![otherwise][one of]After one final thrust, you feel [NameDesc of M][']s [manly-penis] inflate slightly, as if to create an airtight seal. You gasp as several spurts cool fluid flow inside of the condom, which almost immediately bursts, filling you. Your breasts jiggle as they quickly grow in size.[or]You feel [NameDesc of M][']s [manly-penis] inflate slightly, as if to create an airtight seal. Several spurts of cool fluid flow inside of the condom, which almost immediately bursts, filling you. A strange sensation spreads through your body, dissipating as your breasts suddenly burgeon with size.[or][BigNameDesc of M] suddenly stops, its [manly-penis] bulging slightly as several spurts of cool fluid flow inside of the condom, which almost immediately bursts, filling you. Your breasts suddenly grow![at random][end if][if the bimbo of the player < 7][line break][first custom style][one of] What the fuck is going on?![or]Not again...[stopping][roman type][line break]The sex doll disappears into nothingness.";
...
To compute unique climax of (M - a sex doll) in (F - vagina):
	...
	if M is airfilled-sex-doll:
		compute FluidFill of M;
	otherwise:
		say "[one of]After one final thrust, you feel [NameDesc of M] begin to deflate, its warm, sticky contents shooting out inside of [if M is wrapped]the condom, which almost immediately bursts, filling [end if]you.[or]With one final thrust, [NameDesc of M] begins to deflate, hissing quietly as it evacuates its warm, sticky contents directly into [if M is wrapped]the condom, which almost immediately bursts, filling [end if]your [vagina].[or][BigNameDesc of M] begins to deflate, hissing quietly as its warm sticky contents shoot up inside of [if M is wrapped]the condom, which almost immediately bursts, filling [end if]you.[or][BigNameDesc of M] stops after one final thrust, deflating as it floods [if M is wrapped]the condom, which almost immediately bursts, filling [end if]your [vagina] with its sticky white contents.[at random] [one of][if the bimbo of the player < 7][line break][first custom style]Oh God, I think that was cum. All of it went inside me...[otherwise if the bimbo of the player < 12][line break][variable custom style]I think that was cum. There's so much of it inside me...[otherwise][line break][second custom style]That was cum! Mmm, there so much of it inside me...[end if][or][if the bimbo of the player < 7][first custom style]Ugh... I'm so full...[otherwise if the bimbo of the player < 12][variable custom style]I'm so full...[otherwise][second custom style]There's so much.~[end if][or][if the bimbo of the player < 7][first custom style]Not again...[otherwise if the bimbo of the player < 12][variable custom style]There's always a lot...[otherwise][second custom style]I think there's even more than last time![end if][stopping][roman type][line break]The sex doll disappears into nothingness.";
		cutshow figure of sexdoll cutscene 1 for M;
		PussyFill 12;
	destroy M.
