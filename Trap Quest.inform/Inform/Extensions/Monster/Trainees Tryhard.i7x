Trainees Tryhard by Monster begins here.

A tryhard trainee is a kind of trainee. Understand "dedicated" as a tryhard trainee.

To say MediumDesc of (M - a tryhard trainee):
	say "dedicated [rank-colour of M] trainee [trainee-name of M]".
To say NameDesc of (M - a tryhard trainee):
	say "[input-style]dedicated [trainee-name of M][roman type]".
To say BigNameDesc of (M - a tryhard trainee):
	say "[input-style]Dedicated [trainee-name of M][roman type]".

To decide which number is the dedication of (M - a tryhard trainee): [We can use this for various calculations to check which trainees do what]
	decide on a random number between 2 and 5.

To compute trainee perception of (M - a tryhard trainee):
...
To say LewdAppearanceAssessment of (M - a tryhard trainee):
	say "[BigNameDesc of M] [one of]clenches [his of M] fists[or]grinds [his of M] teeth[in random order].[line break][speech style of M]'[one of]You're making the rest of us look bad!'[or]If you look like that, the tutors won't pay all of their attention to me!'[in random order][roman type][line break]".
...
trainee-laci is a tryhard trainee.

Definition: trainee-laci is dark skinned: decide yes.

The text-shortcut of trainee-laci is "trla".

Figure of rank 1 laci is the file "NPCs/Facility/Trainee/trainee3a.png".

To decide which figure-name is the monster-image of (M - trainee-laci):
...
To say ClitLeadDesc of (M - trainee-laci):
...
To update name of (M - trainee-laci):
	...
	otherwise:
		...
		now the trainee-print of M is "spankable alchemist".

To say FirstResponse of (M - trainee-laci):
...
To say RepeatResponse of (M - trainee-laci):
...
To say StoryAnswer of (M - trainee-laci):
	say "[speech style of M]'[if diaper quest is 1]I've been sent here to prove my [maturity]. And as you can see, I am wearing sexy ADULT panties! You can see them, right??? No nappies here!'[otherwise if the current-rank of M is 1]My kingdom's culture says that the most desirable part of a [prince] to look upon is [his of M] panties, so I'm here to train to be less shy in short skirts and always bend over at the waist. Am I doing it right?'[otherwise]My citizens are going to go nuts when they see me waltzing around in tiny skirts like this! I'm going go down in history as our best princess ever!'[end if][roman type][line break]".

To say WhereAnswer of (M - trainee-laci):
	say "[speech style of M]'[if the rank of the player <= 1]Um, dumb-head alert! Quit talking to me before the tutors think I'm an idiot too!'[otherwise]Practise makes perfect. Oops, I dropped my pen, tee-hee!'[end if][roman type][line break]".

Book - Misty

trainee-misty is a tryhard trainee.

The text-shortcut of trainee-misty is "trmi".

Figure of rank 1 misty is the file "NPCs/Facility/Trainee/trainee4a.png".

To decide which figure-name is the monster-image of (M - trainee-misty):
...
A diaper quest fix rule:
	if watersports fetish is 0, now the max-rank of trainee-misty is 4.

To update name of (M - trainee-misty):
	if the current-rank of M is 1:
		now the trainee-name of M is "Missy";
		if diaper quest is 0, now the trainee-print of M is "tight topped trollop";
		otherwise now the trainee-print of M is "thirsty trollop";
	otherwise if the current-rank of M is 2:
		now the trainee-name of M is "Misty";
		if diaper quest is 0, now the trainee-print of M is "technically-not-topless tart";
		otherwise now the trainee-print of M is "thirsty tinkler";
	otherwise if the current-rank of M is 3:
		now the trainee-name of M is "Minxie";
		now the trainee-print of M is "wet wench";
	otherwise if diaper quest is 0:
		now the trainee-name of M is "Minx";
		if the current-rank of M is 4, now the trainee-print of M is "sheer shirted slut";
		otherwise now the trainee-print of M is "cum-covered cunt";
...
trainee-candy is a tryhard trainee.

The text-shortcut of trainee-candy is "trca".

Figure of rank 1 candy is the file "NPCs/Facility/Trainee/trainee5a.png".

To decide which figure-name is the monster-image of (M - trainee-candy):
...
To say MonsterDesc of (M - trainee-candy):
...
To say ClitLeadDesc of (M - trainee-candy):
	say "[big his of M] pierced nipples are permanently connected to [his of M] pierced clitoris by two long dangling gold chains. ".

To update name of (M - trainee-candy):
	if the current-rank of M is 1:
		now the trainee-name of M is "Candace";
		now the trainee-print of M is "fancy feline";
		now the text-shortcut of M is "stca";
	otherwise if the current-rank of M is 2:
		now the trainee-name of M is "Candy";
		now the trainee-print of M is "pretty kitty";
		now the text-shortcut of M is "stca";
	otherwise:
		now the trainee-name of M is "Kandi";
		now the trainee-print of M is "prancing pussy";
		now the text-shortcut of M is "stka".

To say FirstResponse of (M - trainee-candy):
...
To say RepeatResponse of (M - trainee-candy):
...
To say WhoAnswer of (M - a tryhard trainee):
	say "[speech style of M]'My tutor says it is [trainee-name of M], so it's [trainee-name of M].'[roman type][line break]".

To say WhereAnswer of (M - a tryhard trainee):
	say "[speech style of M]'[if the rank of the player <= 1]God, how can you not even know where we are! Unless, wait, is that something the tutors will mark you up for?!'[otherwise]By FOCUSING ON YOUR STUDIES!'[end if][roman type][line break]".

To say EscapeAnswer of (M - a tryhard trainee):
...
To say AdviceAnswer of (M - a tryhard trainee):
	say "[speech style of M]'[if diaper quest is 0][one of]You'll make a lot of friends if you're willing to suck a lot of [manly-penis][or]If you want to get ahead in life, you just need to suck a [manly-penis] or five[at random][otherwise]Sometimes the mature thing to do is admit that the seniors know best[end if].'[roman type][line break]".

To compute teaching offer of (M - a tryhard trainee):
	say "[speech style of M]'I totally could, because I'm way better at this than you. But I'm technically not allowed, since I'm not a qualified tutor here.'[roman type][line break]".

To say reconditioning chair tease of (M - a tryhard trainee):
