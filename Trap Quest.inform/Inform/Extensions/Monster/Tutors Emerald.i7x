Tutors Emerald by Monster begins here.

To say MediumDesc of (M - an emerald-tutor):
	say "emerald tutor [tutor-name of M]".

Part - Hannah


Definition: tutor-hannah is motionless-when-defeated: decide yes.

The text-shortcut of tutor-hannah is "teha".

Figure of babslave hannah is the file "NPCs/Facility/Tutor/tutor5c.png".
Figure of Tutor Hannah Cutscene 1 is the file "Special/Cutscene/cutscene-hannah-bondage1.jpg".

To decide which figure-name is the monster-image of (M - tutor-hannah):
	decide on figure of Tutor Hannah.

To decide which figure-name is the pacified-image of (M - tutor-hannah):
	if M is diaper-enslaved, decide on figure of babslave hannah;
	decide on figure of slave hannah.

To say MonsterDesc of (M - tutor-hannah):
	if M is pacified:
...
The tutor-name of tutor-hannah is "Hannah".

To say WhoAnswer of (M - tutor-hannah):
	say "[speech style of M]'Compliance.'[roman type][line break]".

To say WhereAnswer of (M - tutor-hannah):
...
To say AdviceAnswer of (M - tutor-hannah):
...
To say DefeatFlav of (M - tutor-hannah):
...
To say EnslavedDominationFlav of (M - tutor-hannah):
...
To compute teaching of (L - pain-training):
	...
	otherwise if diaper quest is 0 and clitoris piercing is eligible and (a random number between 1 and 3 > 1 or portal gag is not actually summonable) and andro fetish is 0:
		...
		if the player is bimbo consenting:
			...
			let ST2 be a random demotable trainee in the location of the player;
			say "You [if the player is feeling dominant]confidently[otherwise]hesitantly[end if] rise to your feet and take a step forward. Then [NameDesc of ST] finds the courage to follow your example and follow you towards the front. One by one, all the other trainees join you until all of you have decided to do it. ";
			if ST2 is a trainee and ST2 is not ST:
				say "All of you, that is, except [NameDesc of ST2], who remains seated, teeth gritted, probably too scared of the prospect of imminent pain. [NameDesc of M][']s face sours.[line break][speech style of M]'Really, [trainee-name of ST2]? You can't even bring yourself to do it when every single one of your peers has taken the plunge? Maybe you're just not cut out for this rank at all.'[roman type][line break]";
				demote ST2;
	...
	otherwise if there is a worn clitoris piercing and clitoris lead is not worn and the number of cursed pussy covering clothing <= the number of cursed displacable pussy covering clothing and andro fetish is 0:
		...
		if the player is bimbo consenting:
			...
			unless the player is consenting:
				...
				if ST is innocent trainee or a random number between 1 and 3 is 1:
				...
				otherwise:
					...
					unless the player is consenting:
						...
						if ST is not tryhard trainee and a random number between 1 and 2 is 1:
							say "[ClitPiercingConcede of ST]";
							now competition-success is 1;
						otherwise:
							PainUp 40;
							...
							unless the player is consenting:
								...
								say "[ClitPiercingConcede of ST]";
								now competition-success is 1;
			appropriate-cutscene-display figure of tutor hannah cutscene 2 with priority 3; [draw it straight away]
			if competition-success is 1:
				...
				update trainees; [an important line which makes boring old trainees disappear and new cool ones appear]
			otherwise:
				say "As you take the step back and the pain disappears, you feel relief wash over you. Sweat drips down your face and you are breathing heavily. [BigNameDesc of M] claps [his of M] hands.[line break][speech style of M]'Well done [trainee-name of ST]. Yes, you've definitely earned a promotion.'[roman type][line break]";
				promote ST;
				HappinessUp ST;
			...
			allocate 60 seconds;
		otherwise:
			say "[if the player is feeling dominant]There's no way you're going to risk any more pain right now! You fold you arms and audibly huff. The other trainees seem to be influenced by you and in the end, nobody volunteers at all[otherwise]You look around but nobody is making any move to go forward, and you don't have the bravery to go through it alone[end if]. [BigNameDesc of M] frowns.[line break][speech style of M]'You do realise, by all being complete wusses that you've essentially proven my point? Bah. Well, we'll try again next time, and see if any of you have grown some kahunas. Now get out of my sight.'[roman type][line break]";
			allocate 12 seconds;
	otherwise if wrist collar bar is actually summonable or portal gag is actually summonable:
		...
		if wrist collar bar is actually summonable and (portal gag is not actually summonable or a random number between 1 and 2 is 1):
			...
			if diaper lover > 0, appropriate-cutscene-display figure of tutor hannah cutscene 1;
		...
		otherwise:
			...
			otherwise:
				...
				say "[if N is M][BigNameDesc of M] pockets the device.[line break][speech style of M]'I think I'll keep it for myself.'[otherwise][BigNameDesc of M] twirls the device in [his of M] hand.[line break][speech style of M]'I think I'll give it to Tutor [tutor-name of N]. Something tells me [he of N] will be able to find some good uses for your mouth! If you haven't met [him of M] yet, [he of M][']s the proud owner of a rather juicy [DickDesc of N], which pretty soon your mouth and tongue are likely to be well acquainted with, I should think!'[end if][roman type][line break]";
...
To say ClitPiercingConcede of (M - a trainee):
...
To say ClitPiercingConcede of (M - a tryhard trainee):
...
To say ClitPiercingConcede of (M - an innocent trainee):
...
toy-training is a training session. The training-tutor of toy-training is tutor-hannah.

To say TrainingSessionTitle of (L - toy-training):
	say "Butt Plugs".

Definition: toy-training is training-appropriate:
	let CM be current-monster;
	now current-monster is the training-tutor of toy-training;
			...
			if ST is yourself:
				...
				otherwise:
					...
					if choseBiggest is 1:
						repeat with STX running through trainees in the location of the player:
							FavourUp STX;
					...
					otherwise:
						...
						unless the player is broken:
							...
							obsceneDignify;
			otherwise:
				say "[speech style of M]'[trainee-name of ST], [one of]you're up[or]you're next[or]go ahead[then at random].'[roman type][line break]";
				...
				if ST is not promotable or (a random number from 1 to 7) + (a random number from 1 to 7) <= the size of STP:
					...
					if the size of STP < 5 and ST is demotable:
						...
						demote ST;
						say "[speech style of M]'Now get the fuck out of my classroom.'[roman type][line break][BigNameDesc of ST] lets out a quiet [if ST is nasty trainee]growl[otherwise]sob[end if] before leaving.";
						compute mandatory room leaving of ST;
					otherwise:
						say "[speech style of M]'[one of]Aww, that's too bad[or]Tough luck, [trainee-name of ST][in random order]. That means you're definitely not getting promoted. [one of]Make sure to put it back on the table before you go back, there's a good [boy of ST][or]Remember to put it back on the table now[stopping].'[roman type][line break]";
				otherwise:
					say "[one of]With an audible slurp, the plug quickly slips inside [his of ST] greedy hole! [big he of ST] [if ST is ditzy trainee]grins widely[otherwise]blushes furiously and hides [his of ST] face[end if] as [he of ST] returns to [his of ST] desk[or][BigNameDesc of ST] makes several attempts to push the plug in but can't seem to get it all of the way. Then with one big gulp [he of ST] pushes really hard, and it finally squeezes its way inside. But then [NameDesc of ST] whimpers, clearly unable to handle the intense sensation! [he of ST] tries to pull it out again but now the curse has taken hold and it won't budge. [big he of ST] groans with desperation, still pulling in vain at the base of the [if the size of STP > 5]oversized [end if]toy as [he of ST] slowly waddles back to [his of ST] desk[or][big he of ST] takes it slow and carefully, and the plug slowly but surely takes its place inside its new home. [BigNameDesc of ST] walks proudly back to [his of ST] desk[or][big he of ST] goes with the 'ripping off a band-aid' approach and shoves the plug in with one quick motion. It works, but you can see the pain and regret on [his of ST] face as [he of ST] gingerly walks away[or][big he of ST] manages to get it in without much difficulty, and happily walks back to [his of ST] place[in random order].";
					say "[speech style of M]'[one of]Well done[or]Good job[or]Great job[or]Nice one[or][if the size of STP > 4]Impressive[otherwise]Fair enough[end if][in random order], [trainee-name of ST]. You've earned [size of STP] [if size of STP is 1]copy[otherwise]copies[end if] of your name in the raffle.'[roman type][line break]";
					remove STP from LP;
		...
		if delayed fainting < 1:
			...
			while winnersAnnounced < raffleWinners:
				...
				if TN > 0:
					...
					if armband is ABC or WST is trainee:
						say "[BigNameDesc of M] shuffles the pieces of paper in [his of M] raffle box and then picks one. [big he of M] looks up with a smile.[line break][speech style of M]'Congratulations [if WST is yourself][NameBimbo][otherwise][trainee-name of WST][end if]! [one of]You're a winner today[or]Lucky you[or]Your name's on this one[at random].'[roman type][line break]";
						if WST is yourself or WST is promotable trainee, say "[big he of M] snaps [his of M] fingers as [he of M] makes gesture towards [if WST is yourself]you[otherwise][him of WST][end if].";
						if WST is trainee:
							promote WST;
						otherwise:
							if armband is ruby, now armband is pink diamond;
...
Definition: a text (called T) is toy-training-responsive:
...
Definition: a text (called T) is toy-training-examine-responsive:
...
tutor-bianca is an emerald-tutor.

The text-shortcut of tutor-bianca is "tebi".

Figure of bianca is the file "NPCs/Facility/Tutor/tutor3a.png".
Figure of bianca goblet is the file "NPCs/Facility/Tutor/tutor3d.jpg".

To decide which figure-name is the monster-image of (M - tutor-bianca):
	if M is defeated, decide on figure of bianca defeated;
	decide on figure of bianca.

To say MonsterDesc of (M - tutor-bianca):
...
To say WhoAnswer of (M - tutor-bianca):
	say "[speech style of M]'Fashion!'[roman type][line break]".

To say WhereAnswer of (M - tutor-bianca):
...
To decide which number is the EnslavedDominationThreshold of (M - tutor-bianca):
	decide on 12000.

dress-training is a training session. The training-tutor of dress-training is tutor-bianca.

To say TrainingSessionTitle of (L - dress-training):
	say "Upskirts".

To decide which number is the min-trainees of (L - dress-training):
	decide on 0.

Definition: dress-training (called L) is training-appropriate:
...
To compute teaching of (L - dress-training):
	let M be the training-tutor of L;
	say "[speech style of M]'Today's training session is on the proper purpose of skirts.'[roman type][line break][NameDesc of M] stands at the front of the room and motions at [his of M] own dress with [his of M] hands as [he of M] speaks.";
		...
		if the number of trainees in the location of the player is 0 or the player is bimbo consenting:
			...
			otherwise:
				...
				if a random number between 3 and 12 > the dexterity of the player:
					say "You think you're going to manage it, but suddenly you feel unsteady and begin to topple. You try to react to this in time by putting some of your weight on your hands, but with your locked knees this goes badly and just sends you further off balance. You stumble and then fall to the ground, landing on your face with your butt high in the air, still exposed to all the other trainees in your cohort behind you. They all burst out laughing and you turn redder than ever!";
...
drink-training is a training session. The training-tutor of drink-training is tutor-bianca.

To say TrainingSessionTitle of (L - drink-training):
	say "Unique Tastes".

To decide which number is the min-trainees of (L - drink-training):
	decide on 0.

Definition: drink-training (called L) is training-appropriate:
	if face is actually occupied or the total volume of face > 0, decide no;
	if there is a nonstalking evil-wisp, decide yes;
	decide no.

To compute teaching of (L - drink-training):
	let M be the training-tutor of L;
	...
	repeat with ST running through LST:
		...
		if ST is a trainee:
			now the trainingInt1 of ST is the dedication of ST + a random number between -1 and 1;
			if the trainingInt1 of ST <= 0:
				...
				remove ST from LSL;
			otherwise if the trainingInt1 of ST is 1:
			...
			otherwise:
				say "[BigNameDesc of ST] takes a long sip, and then holds it in [his of ST] mouth. [big he of ST] [if the trainingInt1 of ST is 2]looks like [he of ST] is struggling to resist spitting it back out[otherwise if the trainingInt1 of ST is 3]uncomfortable, but willing to hold onto it[otherwise]unaffected by having to hold that stuff in [his of ST] mouth[end if]. [BigNameDesc of M] nods with approval.";
				promote ST;
		otherwise:
	...
	if the number of entries in LSL > 0 or the total volume of face > 0:
		...
		repeat with ST running through LSL:
			if ST is trainee:
				say "A curse wisp suddenly appears behind [NameDesc of ST][if the trainingInt1 of ST is 1]! And then it suddenly disappears again. Presumably because [he of ST] has got an empty mouth.[otherwise]![end if]";
