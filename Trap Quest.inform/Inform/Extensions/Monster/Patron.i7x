...
A patron is a kind of monster. A patron is intelligent. A patron is male. The raw difficulty of a patron is 3. A patron has an object called wanking-target. A patron can be trickdone or not trickdone. A patron is usually not trickdone.
...
To say PatronDesc of (M - an inexperienced patron):
	say "This particular [if gyno fetish is 1]girl[otherwise]guy[end if] looks like [he of M]'s in [his of M] early twenties, and is very over-eager.[line break]".
...
chubby patron is a gross patron. hairy patron is a gross patron. The text-shortcut of chubby patron is "cpa". The text-shortcut of hairy patron is "hpa". Understand "chunky" as hairy patron when gyno fetish is 1.
To decide which number is the girth of (M - gross patron):
	decide on 2.
To say LongDickDesc of (M - a gross patron):
	if super-gyno fetish is 1, say "ribbed strap-on dildo";
...
To say MediumDesc of (M - hairy patron):
	if gyno fetish is 1, say "chunky patron";
...
To set up (M - a patron):
	...
	now M is unleashed;
	now M is composed; [In case the patron was angry during his last session with the player. He's calmed down since then.]
...
A time based rule (this is the patron encountering rule):
	if playerRegion is Hotel and the player is not in a nonstandard room and the player is not in an elevator-room:
		...
		if R > 15 and the player is not in danger:
			...
			otherwise if diaper quest is 0 and player-currently-resting is 0 and the number of on-stage patrons is 0 and the player is in an unbossed room:
				...
				otherwise if P is a monster and (M is a thing or a random number between 1 and 2 is 1):
					now P is in the location of the player;
					[set up P;]
					if M is a thing:
...
To compute patronMeeting of (M - dominatrix-cage) with (P - a patron):
	...
	say "[he of P] whips out [his of P] [if slimeshooter fetish is R]slimeshooter[otherwise][DickDesc of P][end if] and pushes it through the bars of the cage, pointing at your face.";
	if R is watersports fetish:
		say "[big he of P] doesn't even bother to talk to you at all. [big he of P] just lets loose, [SlimeShoot]ing on your face as if it was the most normal thing in the world.";
		FaceDrain from P;
...
To compute patronMeeting of (M - toilet) with (P - a patron):
	...
	if human-toilet-key is in Holding Pen and the human-toilet-scene of woman-player > 1 and the player is able to speak:
		say "[BigNameDesc of P] wanders into the room! You see [him of P] toying with the key for the [SlimeTarget] bondage in one of [his of P] hands. Do you beg [him of P] for the key? ";
		if the player is consenting:
			...
			if watersports fetish is 1:
				...
				otherwise:
					say "[BigNameDesc of P] smiles widely.[line break][speech style of P]'Wow, calm down slut, you didn't have to gobble it down quite so greedily, you know! Drinking [slime] is nothing to be proud of. What a perfect pair of pathetic [SlimeTarget] sluts you two make. Anyway, as promised, here's the key[if freedom tattoo is drawable]... but here's a little secret. If you put this in the chute in the inspiration room, it'll give you a magic tattoo that will make you much faster. And if you choose to do that, instead of rescuing [him of woman-player]... nobody except me and the [SlimeTarget] over there will ever know. I'll leave the decision up to you[end if].'[roman type][line break]";
			...
			otherwise:
				...
				if the player is bimbo consenting:
					...
					say "[speech style of P]'Hahaha, what a show! As promised, here's the key[if freedom tattoo is drawable]... but here's a little secret. If you put this in the chute in the inspiration room, it'll give you a magic tattoo that will make you much faster. And if you choose to do that, instead of rescuing [him of woman-player]... nobody except me and [big SlimeTargetP]-Face over there will ever know. I'll leave the decision up to you[end if].'[roman type][line break]";
					now human-toilet-key is in the location of the player;
					compute autotaking human-toilet-key;
				otherwise:
	...
	if patronBarter is 0:
		if watersports fetish is 1:
			if slimeshooter fetish is 1, say "[he of P] whips out [his of P] slimeshooter, which wobbles as [he of M] drains it into [if the human-toilet-scene of woman-player > 1][NameDesc of woman-player][']s[otherwise]the poor human [SlimeTarget]'s[end if] mouth. [big he of woman-player] gargles weakly as [he of woman-player] is forced to swallow gulp after gulp after gulp of the hot [slime]. [big his of woman-player] belly audibly groans as it is forced to stretch even further.";
			otherwise say "[he of P] whips out [his of P] [DickDesc of P] and [one of]takes an impressively long[or]takes a quick[or]sighs as he takes a[purely at random] [slime] into [if the human-toilet-scene of woman-player > 1][NameDesc of woman-player][']s[otherwise]the poor human [SlimeTarget]'s[end if] mouth. [big he of woman-player] gargles weakly as [he of woman-player] is forced to swallow gulp after gulp after gulp of the hot [slime]. [big his of woman-player] belly audibly groans as it is forced to stretch even further.";
		otherwise:
...
To compute player urinal use of (M - a monster):
	...
	if the player is upright:
		say "[line break][speech style of M]'On your knees. I need to [SlimeShoot].'[roman type][line break]Do you get on your knees? ";
		if the player is bimbo consenting:
			now auto is 1;
			try kneeling;
			now auto is 0;
	if the player is prone:
		say "Without saying a word, [he of M] begins [SlimeShoot]ing through your gag!";
		FaceDrain from M;
	otherwise:
		say "[speech style of M]'This [SlimeTarget] is malfunctioning. I'll be sure to tell the mechanic about this, so [he of mechanic] can [']fix['] it.'[roman type][line break]";
...
To compute patronMeeting of (M - a wrestler) with (P - a patron):
	...
	if M is in the location of the player, say "A [MediumDesc of P] walks into the room and makes a beeline for [NameDesc of M]. [big he of M] immediately drops to [his of M] knees, pushing up [his of M] [ChestDesc of M] and eagerly lolling out [his of M] tongue as [NameDesc of P] [if R is 1]showers [him of M] with [slime][otherwise]ejaculates all over [NameDesc of M][']s face[end if].";
	if R is 1, SlimePuddleUp 2 in the location of M;
...
To compute patronEncounter of (M - a gross patron):
	...
	if R is 1 and watersports fetish is 1:
		if M is in the location of the player, say "A [MediumDesc of M] walks into the room, stumbling a bit as [he of M] [if slimeshooter fetish is 1]empties [his of M] slimeshooter[otherwise][SlimeShoot]es[end if] in the corner.";
		if debuginfo > 1, say "[input-style]Patron is [SlimeShoot]ing on the floor in [location of M].[roman type][line break]";
		SlimePuddleUp (the semen load of M) in (the location of M);
	otherwise:
...
To compute patronEncounter of (M - a dickhead patron):
	if M is in the location of the player:
		if the player is prone:
			...
			if R is 1 and watersports fetish is 1:
				say "A [MediumDesc of M] walks into the room and makes a beeline for you as soon as [he of M] notices you.[line break][speech style of M]'You could use a shower, whore.'[roman type][line break][BigNameDesc of M] immediately starts [SlimeShoot]ing in your face.";
				FaceDrain from M;
				destroy M;
		...
		otherwise:
			...
			if the player is hotel employed:
				...
				if O >= the favour of M - 6:
					say "[speech style of M]'[if C is tattoo and the number of worn tattoos is 1]Your tattoo is[otherwise if C is tattoo]Your tattoos are[otherwise]That [ShortDesc of C] you're [one of]wearing[or]sporting[or]showing yourself off in[as decreasingly likely outcomes] is[end if] pretty [if O > a random number between 13 and 16]fucking outrageous[otherwise]damn sexy[end if]. I might pay you even more next time.'[roman type][line break]";
					FavourUp M;
...
To compute SelectionWaiting of (M - a patron):
	...
	otherwise:
		...
		if R is 0, now M is aggravated; [This will never happen if the reason the player couldn't accommodate the patron was because there were other patrons using the player]
...
To compute facial climax of (M - a patron):
	...
	otherwise:
		...
		if T is worn:
			say "Taking out [his of M] permanent marker, [NameDesc of M] [if M is aggravated]growls while adding[otherwise]adds[end if] another strike to the tally on your cheek.";
			increase the tallies of T by 1;
		otherwise if there is a worn[ ink-me] tattoo:
			say "Taking out a permanent marker, [NameDesc of M] [if M is aggravated]growls while drawing[otherwise]draws[end if] a permanent tally on your cheek.";
			summon T.
...
To compute disgusting spit reaction of (M - gross patron):
	say "[BigNameDesc of M] [one of]chuckles[or]grins[in random order].[line break][speech style of M]'Was that [if the slime volume of face > 0 and the semen volume of face > 0][one of][slime][or]cum[purely at random][otherwise if the slime volume of face > 0][slime][otherwise]cum[end if]? [one of]What a messy girl![or]That's nasty, baby![or]Nasty, just how I like it[in random order]!'[roman type][line break][if the slime volume of face > 0][strongHumiliateReflect][otherwise][moderateHumiliateReflect][end if]".
...
To compute sexBegging of (M - a patron) in (F - a fuckhole):
	let C be 0;
	if M is not dickhead patron and M is composed:
...
To compute post climax effect of (M - a patron) in (F - asshole):
	...
	otherwise:
		...
		otherwise if there is a worn[ ink-me] tattoo:
			...
			summon T;
		if watersports fetish is 1 and grossness fetish > 0 and M is willing to urinate and M is gross patron:
			now the bladder of M is a random number between 600 and 1000;
			compute anal slimation of M.
...
To decide if (M - a patron) is willing to creampie (F - a fuckhole):
	...
	if (M is composed and M is not dickhead patron):
...
Definition: a patron is eager to warn angrily:
...
Definition: an experienced patron (called M) is willing to masturbate:
	if the health of M is the maxhealth of M and M is composed and a random number between 0 and 5000 < the arousal of the player, decide yes;
	decide no.
...
To compute diaper urinal resistance punishment check of (M - a patron):
	if M is aggravated or (diaper-invasion-waistband is false and diaper-invasion-hold is false):
		aggravate M;
		bore M;
	otherwise:
		if the player is getting lucky:
			say "[speech style of M]'I guess I've had worse experiences with you babies. I'll pay you... THIS time.'[roman type][line break][GotLuckyFlav]";
			satisfy M;
		otherwise:
			aggravate M;
			bore M.

To compute diaper cumrag resistance punishment check of (M - a patron):
...
To say SpankingDeclarationFlav of (M - a patron):
	say "[speech style of M]'[if M is aggravated]Well well well, it looks like the tables have turned. Do you have any regrets?'[otherwise if the number of worn diapers is 0]I can't believe you're not even wearing a diaper. What sort of baby-for-hire are you?! Well, it's your problem, really. It just means that this spanking is going to hurt twice as much...'[otherwise if M is experienced patron]I trust you've been trained in how to properly receive a spanking.'[otherwise]I'm gonna make you scream, little [boy of the player].'[end if][roman type][line break]".

To say SpankingMercyRejectionFlav of (M - a patron):
	say "[speech style of M]'[if M is aggravated]There's no chance of that, you little brat!'[otherwise if M is dickhead patron]Do I look like the sort of [man of M] who goes easy on whiny little bitches like you?'[otherwise]No no baby, I'm determined to get my money's worth today. You'll take what you're given.'[end if][roman type][line break][BigNameDesc of M] has an evil smile on [his of M] face.".
...
To say LickResisting of (M - a patron):
	say BallResisting of M.

To say LickResistingResponse of (M - a patron):
	say BallResistingresponse of M.

To say LickSubmissionResponse of (M - a patron):
	say BallSubmissionResponse of M.

To say LickInitiationFlav of (M - a patron):
	say BallInitiationFlav of M.

To say LickEndFlav of (M - a patron):
...
To compute failed dominance punishment of (M - a patron):
	...
	if F is a fuckhole and diaper quest is 0 and the player is reverse bimbo consenting:[The player stalls]
		if the player is wrist bound, say "You clench your thighs [if the player is modest]force yourself to push[otherwise if the player is shameless]enthusiastically push[otherwise]swallow your hesitation and push[end if] out your chest. [if face is actually occupied]You try your best to give [him of M] a look that conveys how much you want [him of M] to feel you up first.[otherwise if the player is not shameless][line break][variable custom style]'A-aren't you going to touch me?'[roman type][line break][otherwise][line break][variable custom style]'Touch me, baby! I fucking need it!'[roman type][line break][end if][big his of M] hands immediately begin to rove across your body, but from the way [his of M] hips edge further and further toward your own, it's clear [he of M]'s still aiming for a home run. You [if the sex addiction of the player < 6]pretend to love[otherwise if the sex addiction of the player < 12]exaggerate your enjoyment of[otherwise]slightly exaggerate your love of[end if] having your [AssDesc] slapped and [if face is not actually occupied]fingers thrust in your mouth. [otherwise][BreastDesc] squeezed. [end if]Although your efforts seem to be working, that only makes [him of M] grow more bold, and you struggle to keep your legs closed as [he of M] starts trying to push [his of M] [DickDesc of M] into your [variable F]. ";
		...
		if R < 0:
			say "[big he of M] comes very close to entering you a few times, but luckily you're able to divert [him of M] just long enough to make [him of M] blow [his of M] load harmlessly on the bed.";
			say AfterDominationComment 7 of M;
			Bore M;
		otherwise:
			say "[line break][speech style of M]'[if M is experienced patron]That's enough. I'll be taking over.'[otherwise if M is dickhead patron]Stop stalling, bitch!'[otherwise if M is gross patron]Come on, foreplay is stupid! Let me make you feel good, baby!'[otherwise]I'm getting close, so let's finish this!'[end if][roman type][line break][if the player is wrist bound]With your wrists bound, [otherwise][BigNameDesc of M] grabs both of your wrists and pins them above your head. With your one method of influencing [him of M] finally sealed away, [end if]you can only look on [if the sex addiction of the player < 6]dejectedly[otherwise if the sex addiction of the player < 12]'dejectedly'[otherwise]in anticipation[end if] as [if F is vagina][his of M] [cockhead of M] glides back and forth between your labial folds, [otherwise][his of M] [cockhead of M] begins to prod your now defenceless sphincter, [end if]coming just a bit closer to penetrating you each time. Finally, [his of M] [DickDesc of M] slides in, and [he of M] puts both hands on your hips as [he of M] begins to fuck you.";
	...
	otherwise:
		if F is a fuckhole, say "You give up on resisting, lying there like the cheap whore you now are as you[if P is 2]and [his of M] friend[otherwise if P > 1]all [his of M] friends[end if] wait to see what [FuckerDesc of M] does next.";
...
To blowjob dominate (M - a gross patron):
	...
	if R >= 0:
		...
		if sexual-penis-length > 3:
			say "You push [NameDesc of M] onto the bed and straddle [his of M] torso.[line break][speech style of M]'This is just like my Japanese animes!'[roman type][line break]You rip open [his of M] shirt as [he of M] [if C is clothing]pulls out your[otherwise]strokes your[end if] hardening [sexual-player-penis], [if sexual-penis-length > 8]obviously trying not to look intimidated[otherwise]glancing up at you seductively[end if] as [he of M] wraps [his of M] [if gyno fetish is 1]breasts[otherwise]lips[end if] around your [SexShaft]. [big he of M][one of]'s clearly never done it before[or] clearly still doesn't know what [he of M]'s doing[stopping], but [he of M] makes up for it with raw enthusiasm, [if gyno fetish is 1 and sexual-penis-length > 8]noisily suckling the tip and pumping your length with [his of M] tits until you fill [his of M] mouth with [semen][otherwise if gyno fetish is 1 and bukkake fetish is 1]licking [his of M] lips and massaging your length with [his of M] tits until you cum all over [his of M] face[otherwise if gyno fetish is 1]licking [his of M] lips and massaging your length with [his of M] tits until you cum[otherwise]slurping noisily until you finally fill [his of M] mouth with [semen][end if]";
			moderateDignify;
			orgasm;
	...
	otherwise:
		...
		say "You try to push [NameDesc of M] onto the bed, but [if the player is wrist bound]it's much too difficult with your wrists bound![otherwise][he of M] won't budge![end if][line break][speech style of M]'No way! Just let me fuck you already!'[roman type][line break]";
...
To blowjob dominate (M - an experienced patron):
	...
	otherwise:
		if debugmode > 0, say "[bold type]FAILED[roman type][line break]";
		say "You try to gesture for [NameDesc of M] to get on [his of M] knees, but [if the player is wrist bound]it's not possible with your wrists bound![otherwise][he of M] refuses![end if][line break][speech style of M]'You're in no position to ask things of me, whore. [if M is aggravated]You're already not getting paid[otherwise]Get on the bed, [please][end if].'[roman type][line break]";
...
To penetration dominate (M - an inexperienced patron):
	...
	otherwise:
		if debugmode > 0, say "[bold type]FAILED[roman type][line break]";
		say "You try to push [NameDesc of M] onto the bed, but [if the player is wrist bound]it's not possible with your wrists bound![otherwise][he of M] isn't interested[end if][line break][speech style of M]'I don't think I'm ready for a dominatrix yet. I'd rather just fuck you.'[roman type][line break]";
...
To penetration dominate (M - a patron):[dickhead patron]
	...
	otherwise:
		if debugmode > 0, say "[bold type]FAILED[roman type][line break]";
		say "You try to push [NameDesc of M] onto the bed, but [if the player is wrist bound]it's not possible with your wrists bound![otherwise][he of M] won't have it![end if][line break][speech style of M]'If anyone's about to get fucked like a slut, it's YOU, bitch!'[roman type][line break]".
...
To watersports dominate (M - a patron):[no chance of backfiring]
	...
	if slimeshooter fetish is 1, say "You shove [NameDesc of M] to the ground, take aim with your slimeshooter and start spraying right in [his of M] face. [run paragraph on]";
	otherwise say "You shove [NameDesc of M] to the ground, [if C is clothing and the player is possessing a penis]pull out your [player-penis][otherwise if C is clothing]pull down your pants[otherwise if the player is possessing a penis]take aim with your [player-penis][otherwise]spread your lower lips[end if] and start [SlimeShoot]ing right in [his of M] face. [run paragraph on]";
	if M is experienced patron:
		say "The humiliation of being treated like a human [SlimeTarget] seems to hit [him of M] really hard, and [he of M]'s actually shivering by the time you stop.";
		severeDignify;
	otherwise if M is dickhead patron:
		say "[big he of M] starts to complain, so you make sure to direct your stream into [his of M] mouth, forcing [him of M] to go silent and contemplate [his of M] new place as a human [SlimeTarget].";
		severeDignify;
	otherwise if M is inexperienced patron:
		...
		moderateDignify;
	otherwise:
		say "[big he of M] catches most of your [slime] in [his of M] mouth, spilling most of it as [he of M] swallows it in several gulps.";
		moderateDignify;
...
To ride dominate (M - an experienced patron):
	...
	otherwise:
		if debugmode > 0, say "[bold type]FAILED[roman type][line break]";
		say "[big he of M] shoves you down when you try to sit up, and you let out a [if the player is able to speak]confused yelp[otherwise]muffled yelp[end if] as [he of M] [if the player is wrist bound]takes full advantage of the status of your wrists. [otherwise]immediately pins your wrists against the headboard. [end if]You struggle, but it's no use against an experienced [man of M] like [him of M], and you have no choice but to lie there as [his of M] [cockhead of M] relentlessly [if F is asshole and the player is sexed male]punches your prostate[otherwise]strokes your G-spot[end if]. Maybe you'd be able to cum from that if you were still in charge, but now it's all about [him of M], and [he of M]'s clearly planning to finish it as soon as possible. Sure enough, [his of M] [DickDesc of M] begins to throb a few moments later, and [he of M] emits a low groan as [if M is wrapped]the condom floods with [his of M] load[otherwise][he of M] shoots several spurts of [semen] into your [variable F][end if]";
		ruin F;
		stimulate F times 1;
		orgasm M;
		if F is vagina, PussyFill the semen load of M;
...
To ride dominate (M - a dickhead patron):[dickhead patron]
	...
	otherwise:
		...
		say "[if the player is wrist bound]Your wrists are bound and [his of M] aren't, and in the end [he of M] ends up [otherwise]Eventually [he of M] manages to pin both of your hands behind your back, [end if]grinding your face into the pillows as [he of M] brutally drills away at your [variable F]. Your little wrestling match apparently left [him of M] close to the edge, and a few moments later, [he of M] slams [his of M] [DickDesc of M] home, roaring with pleasure as [he of M] fills [if M is wrapped]the condom with [his of M] load[otherwise]you with [semen][end if].";
		ruin F times 2;
		orgasm M;
		if F is vagina:
			PussyFill the semen load of M;
...
To say PatronDesc of (M - subby patron):
	say "This young [man of M] looks like [he of M][']s [']escorted['] a couple people [him of M]self, and is very eager to flip the script.[line break]".
...
Definition: subby patron is presenting as male:
...
To say PatronDesc of (M - furry patron):
	...
	otherwise:
		say "This [man of M] in particular is wearing a huge foam dog head. [big his of M] [LongDickDesc of M] is proportionally big, 14 inches long, and accompanied by a pair of big, fat[if super-gyno fetish is 0], hairy[end if] nuts.[line break]".
