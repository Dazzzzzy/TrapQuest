...
Definition: yourself is feeling full:
	if the player is full and ((the player is not rectum incontinent and failed potty training tattoo is not worn) or suppository > 0 or there is worn desperation clothing), decide yes;
	decide no.
...
An all later time based rule (this is the fullness causes intelligence loss rule):
	...
	otherwise:
		...
		if F > 0 and the player is diaper aware, say "Now that your bowels are [if rectum > 1]less full[otherwise]empty[end if], your ability to concentrate improves and your intelligence returns.".
...
Definition: yourself is rectum diaper aware: [Do they always know the state of their diaper after messing?]
	if chess table is grabbing the player or the player is in a predicament room, decide yes;
	if the player is not potentially diaper aware or the rectum-incontinence of the player >= 10, decide no;
	decide yes.

To decide which number is rectum-urgent-level:
	decide on 11.

To decide which number is rectum-risky-level:
	let R be rectum-urgent-level - (rectum-incontinence of the player + suppository + the number of worn desperation clothing);
	if R < 2, decide on 2;
	decide on R.
...
Definition: yourself is able to mess:
	if rectum > 1 and there is a worn total protection soilable knickers and asshole is not actually occupied and the number of live things penetrating vagina is 0 and (the number of things grabbing the player is 0 or diaper quest is 1), decide yes;
	decide no.
...
To check real messing with reason (T - a text):
	if rectum > 0 and earnings < starting-earnings - 6:
		...
		if the player is feeling full:
			...
			if the player is in a predicament room and the player is not rectum incontinent, now messAware is 2; [won't accidentally mess in predicament zone]
		if the player is able to mess, now canMessNow is 1;
		...
		if shouldMessNow is 1:
			if canMessNow is 1 and messAware > 0:
						...
						if T is "":
							...
							if the raw delicateness of the player < 20 and rectum-incontinence of the player < the max-rectum-incontinence of the player:
								...
								if the player is reverse bimbo consenting:
									...
									let FOW1 be (suppository * 2) - 20;
									let FOW2 be (suppository * 4) + the rectum-incontinence of the player;
									let RNG be a random number between FOW1 and FOW2;
									if debuginfo > 0, say "[input-style]Poop anyway check: (laxative level [suppository] * 2) - 20 = [FOW1]; (laxative level [suppository] * 4) + rectum incontinence ([rectum-incontinence of the player]) = [FOW2]; RNG([FOW1]~[FOW2]) = [RNG] | 0.5 force of will[roman type][line break]";
									if RNG <= 0:
				...
				otherwise:
					...
					if messAware <= 1, say "[if the intelligence of the player < 4]Your mind is so foggy[otherwise]You are so distracted thinking about other things[end if] that by the time you realise that you have instinctively reacted to your desperate need to use the [SlimeTarget] by squatting and pushing, it's already happening.";
					compute partial messing;
			if hasMessedNow is false:
				if willMessNow is 1:
					let D be a random worn diaper;
					if the player is rectum diaper aware or D is not diaper:
						say "All of a sudden, you feel your rectal muscles spasming and [if the rectum-incontinence of the player > 2]due to your rectal incontinence, [end if]you have absolutely no control as it begins to empty itself of its contents! ";
						compute partial messing;
					otherwise:
				...
				otherwise if messAware > 0 and shouldMessNow is 1:
					...
					if T is "" and the number of worn soilable knickers is 0 and diaper focus is 1 and D is diaper and asshole is not actually occupied and the location of the player is not toilets and the location of the player is not urinals and the player is not in a predicament room:
						...
						say "a [ShortDesc of D]![one of][line break][variable custom style][if the delicateness of the player < 14]Oh my god, the game is trying to make me mess myself...[otherwise]At least now I have a [SlimeTarget] to go in. Thank you game![end if][roman type][line break][or][stopping]";
...
reactions-suppressed is initially false.
...
To compute messing:
	...
	otherwise:
		...
		if the total squirtable fill of belly > 0:
			now reactions-suppressed is true;
			AssSquirt;
		...
		now reactions-suppressed is false;
		if mess-phase > 0:
			...
			otherwise:
				...
				if suppository > 1, decrease suppository by 1; [suppository effects are slightly relieved]
...
To compute diaper mess effects:
	compute diaper mess reflections;
	compute diaper mess reactions.

To compute diaper mess reflections:
	let D be random worn knickers;
...
To compute diaper mess reactions:
	update appearance level;
	if reactions-suppressed is false: [If it's set to true then we already computed enema reactions and we don't want the NPCs to have two separate reactions to the same event.]
		let D be a random worn knickers;
		let PC be the at least partial concealer of D;
		if the messskill of the player is 1 and PC is actually dense clothing:
			repeat with N running through reactive monsters:
				say "Thanks to your ability to mess stealthily, [NameDesc of N] doesn't notice you messing your [ShortDesc of D] underneath your [ShortDesc of PC].";
		otherwise:
			if mess-phase < 2:
			...
			repeat with N running through reactive monsters:
				if the current-errand of N is not mess-yourself-errand and N is not urinal-rival of urinal-competition-punishment:
					if N is diaperMessReacted:
						compute continued diaper mess reaction of N;
					otherwise:
						compute diaper mess reaction of N;
						now N is diaperMessReacted;
		if the player is in a predicament room:
			if current-predicament is bystanders-always-mess-aware or there is a currently visible diaper:
				repeat with N running through reactive bystanders:
					compute diaper mess reaction of N;
			otherwise:
				repeat with N running through reactive bystanders:
					say "[BigNameDesc of N] doesn't seem to realise that you're messing yourself.";
				if there is a reactive bystander, say "[variable custom style][one of]Phew[or]I've gotten away with it[or]Thank goodness[at random]...".
...
To compute continued diaper mess reaction of (M - a person):
	if diaper quest is 1 and M is monster:
		say "[BigNameDesc of M] continues to watch you with [one of]a smug expression[or]a judging expression[or]a caring expression[or]a mildly disgusted sneer[or]a smirk[in random order].";
...
Check messing:
	...
	if the player is not feeling full and the player is rectum incontinent, say "[if temporary-rectum-incontinence > 0]You're temporarily under an effect that's making you temporarily fecally incontinent, so you currently cannot feel or control this[otherwise]You've become so incontinent that you cannot feel or control this anymore[end if]! No matter how much you push, you feel absolutely nothing!" instead;
	if auto is 0:
		if the player is able to use a toilet and the location of the player is toilets:
			...
			otherwise:
				say "You sit on the [SlimeTarget] and push, but nothing comes out." instead;
		if the location of the player is toilets and the player is flying and the diaper addiction of the player < 15:
			say "[variable custom style]I'll need to wait until I land to use that [SlimeTarget].[roman type][line break]" instead;
		otherwise if the location of the player is toilets and there is a worn total protection diaper:
			...
			otherwise say "[variable custom style]I can't use the [SlimeTarget] like that right now, I need to remove some clothing first.[roman type][line break]" instead;
		otherwise if the location of the player is toilets:
			say "[variable custom style]I can't use the [SlimeTarget] right now.[roman type][line break]" instead;
...
Carry out messing:
	...
	if the player is bursting and wetting-valued <= 0 and the bladder-incontinence of the player < 5 and the rectum-incontinence of the player < 5: [at higher incontinence of either kind, automatic messing doesn't cause urinating]
		...
		say "The act of purposefully messing yourself also makes you [slimeonself].";
		try urinating;
	if berri-scene is 1:
		DelicateDown 2;
