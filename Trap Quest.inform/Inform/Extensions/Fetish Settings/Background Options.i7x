...
To decide which number is background-selected:
	if choice in row 52 of the Table of Player Options > 0, decide on 1;
	decide on 0.
...
"Background (only one): Nursing Trainee (you can sometimes scrounge up some bandages that slightly heal you, base intelligence slightly higher) - 5 points ([if background-nurse is 0]not [otherwise][bold type][end if]chosen[roman type])"	--	--	first aid skill toggle rule
...
"Background (only one): [if diaper quest is 0]Pure (you start out really caring about your virginity: virgin warrior quest is mandatory unless you lose your virginity early, but gives you positive stats over time once started[otherwise]Teen Bedwetter (you have horrible memories of [slime]ing the bed in your teens, and now really care about your continence: you start with +1 strength & dexterity and this number grows over time but if you ever accidentally [slimeonself] you suffer serious penalties[end if]) - 5 points ([if background-pure is 0]not [otherwise][bold type][end if]chosen[roman type])"	--	--	pure background toggle rule
