...
Definition: the throne is egg-fathering: decide yes.
...
To say ExamineDesc of (C - throne):
	...
	otherwise say "A pink plastic chair cheaply fashioned into a princess theme through a crown decal on the backrest. A hole in the middle of the seat contains a training [SlimeTargetP].".
...
Check entering the throne:
	...
	if the charge of the throne <= 0:
		...
		if debuginfo > 0:
			...
			otherwise say "[if R is not R-old] = [R] [end if]| 1) Tiara; 2-3) [big SlimeTargetP]; 4) Intelligence + 1; 5) Strength + 1; 6) Dexterity + 1; 7) Humiliation reduction[roman type][line break]";
		if R is 1:
		...
		otherwise if R is not 4 and R is not 5 and R is not 6 and (there is a worn diaper or (diaper quest is 1 and R <= 3) or (diaper lover >= 1 and the noun is not transformed and ((the class of the player is princess and diaper quest is 1) or the player is an adult baby or the player is a sissy))):
			if debuginfo > 0 and diaper quest is 0, say "[input-style]Player is a bab - [big SlimeTargetP] override![roman type][line break]";
			if the noun is transformed:
				let K be a random worn knickers;
				if rectum > 3:
					say "All of a sudden, you suddenly completely lose control over your sphincter! You try to stand up but your [if K is clothing][K][otherwise]butt[end if] is somehow stuck in the [SlimeTargetP]! You shudder as your butt begins to empty itself, forcing the mushy contents of your guts into your [if K is diaper]nappy[otherwise if K is clothing][K][otherwise]new [SlimeTarget][end if]. ";
					if K is knickers, say "[if the diaper addiction of the player < 10]You are completely powerless to stop it, so you end up just sitting there awkwardly whilst your poop chute gets to work. There's so much! By the time you finish, you are very confident that you are now empty.[otherwise if the diaper addiction of the player < 15][one of]You can't help but almost enjoy messing yourself - the warm feeling spreading over your butt and thighs actually feels kind of nice.[or]You give into the pleasurable feelings and sigh gently as you feel the warmth spreading beneath you.[stopping][otherwise]You giggle like a naughty alchemist and rub your butt into the mush as you continue to fill your [K]. It feels so good - you wish it could last for ever! Once you've finished pooping yourself, you sit there with a massive grin on your face, wiggling your bum in the seat to keep feeling the weight of it sagging beneath you in the [SlimeTargetP].[end if]";
					now rectum is 0;
					now the slime-soak of K is 1; [to make sure it's cleaned directly below]
				if K is clothing and (the slime-soak of K > 0 or K is messed):
					if K is perceived soiled, say "Suddenly, a wave of magic flows through the [SlimeTargetP] to your [K] and you feel that it has been magically cleaned. It's like it was brand new!";
		...
		otherwise if R is not 4 and R is not 5 and R is not 6 and the noun is transformed and (the bladder of the player > 0 or rectum > 1):
			...
			say "You let go of the hold on your [SlimeContainer][if rectum > 1] and sphincter[end if], using the [SlimeTargetP] for its intended purpose. After a few [if rectum > 1]embarrassing noises coming from your rear end[otherwise]awkward moments listening to yourself tinkle[end if], you're done. The [SlimeTargetP] makes a weird gurgling sound, and then is suddenly completely empty and clean again!";
			reset rectum;
