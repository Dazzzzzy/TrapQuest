...
facility portal is a warp portal. facility portal is in Facility01.
...
[increase the chance of a team predicament by sending a trainee down if available from the hallway]
Report going south when the player is in Facility01:
	let ST be a random trainee in Facility02;
	if the number of trainees in Facility01 is 0 and ST is trainee:
		try ST going south.
...
Check an actor going (this is the can't go that way except for warp portals rule):
	if the room gone to is nothing or the room gone to is Solid Rock:
		...
		if the door gone through is nothing:
			...
			if WP is warp portal and the covered-direction of WP is the noun and (WP is not regionally in the destination of WP or the destination of WP is facility):
...
To decide which direction is the covered-direction of (C - facility portal):
	decide on south.

predicamentJustDone is initially true. [We only want one 'extra credit' predicament between each training session.]
predicamentSavedMakeUp is a number that varies.
predicamentSavedBladder is a number that varies.
predicamentSavedRectum is a number that varies.
...
To teleport via (W - a warp portal):
	...
	if W is facility portal:
		if receptionist is in the location of the player:
			let ST be a random trainee in the location of the player;
			if ST is trainee, now team-predicament-partner is ST;
		now predicamentsAvailable is the number of eligible appropriate predicaments;
		now team-predicament-partner is nothing;
		now the recently-used of facility-fuckhole is 0;
		repeat with M running through monsters in the facility:
			deinterest M;
			if M is trainee, TrainingSessionReset M; [When the player leaves the facility, all trainees reset messiness, cum covered, etc.]
	reset multiple choice questions; [ALWAYS REMEMBER THIS WHEN MAKING A MULTIPLE CHOICE QUESTION]
	if W is not in the Dungeon:
		set next numerical response to "go to the Dungeon";
		increase NOptions by 1;
	if W is not in the facility and the class of the player is not berri and (armband is worn or armband is solid gold or ex-princess is unconcerned or receptionist is in the location of the player):
		set next numerical response to "go to the Royal Training Halls";
		increase NOptions by 1;
	...
	if W is not in the Mansion and location of mansion portal is discovered:
		...
		increase NOptions by 1;
	if W is facility portal and the player is the donator and predicamentJustDone is false and ex-princess is not unconcerned and predicamentsAvailable > 0 and the player is predicament-ready:
	...
	otherwise:
		...
		otherwise if T matches the text "Training" or T matches the text "Extra":
			now the destination of W is the facility;
	...
	otherwise if forced-portal is not a warp portal and there is worn spookiness clothing and the player is not getting lucky:
		...
		say GotUnluckyFlav;
	otherwise:
		if W is in the facility:
			if the destination of W is not facility:
				if receptionist is in the location of the player:
					let EG be nothing;
					...
					otherwise if predicamentJustDone is false:
						let M be receptionist;
						repeat with ST running through alive unfriendly trainees: [find a really angry trainee]
						...
						if M is trainee:
							...
							if there is an eligible appropriate predicament and the player is predicament-ready:
								...
								satisfy M;
								now the destination of W is facility;
		...
		zero focus stuff;
		if W is D: [facility portal leads to extra credit zone. Anything added here might also need to be added to the reconditioning code in Staff Framework.i7x]
			now the destination of W is the Dungeon;
			display entire map;
			...
			if (P is nothing or team-predicament-partner is not trainee) and receptionist is in the location of the player:
				let ST be a random trainee in the location of the player;
				if ST is trainee:
		...
		otherwise if D is facility portal:
			if receptionist is alive and receptionist is not in Facility01, now receptionist is in Facility01;
			if armband is worn and armband is not solid gold and there is an alive undefeated correctly-ranked tutor and the armband-print of armband is not "new recruit":
				if class-time < 0, now trainingJustDone is false;
				...
				if A is assembly:
					...
					compute start of A;
				otherwise:
					say "As you go through the portal, you appear back in the Royal Training Halls again, just in front of the training room. After you've gone through, the portal closes behind you. There's a training room right in front of you. A bell rings - it's time for a training session right now!";
					if armband is sapphire:
						now the player is in Facility07;
					otherwise if armband is emerald:
						now the player is in Facility03;
					otherwise if armband is ruby:
						now the player is in Facility26;
					otherwise if armband is pink diamond:
						now the player is in Facility25;
					otherwise:
						now the player is in Facility24;
			...
			otherwise:
				...
				compute bladder cleanup;
				display entire map;
				say "As you go through the portal, you find yourself [one of]in a large room with expensive marble walls and floor[or]back in the training halls reception[stopping].";
				if class-time is 0, now class-time is -1; [extra credit zone resets at -1 and lower]
				if class-time > 0, now class-time is 0; [training session required at 0 and lower. in this way, extra credit zone is still forbidden but player can't leave until they've had a training session]
		otherwise if D is warp portal:
			...
			if receptionistChasing is true:
				...
				say "[BigNameDesc of receptionist] follows you through the portal![line break][speech style of receptionist]'It's a one way ticket to the Royal Dungeon for you, [NameBimbo]!'[roman type][line break]";
	if playerRegion is not facility, compute clothing cleanup. [otherwise extra credit zone causes jewellery to get cleaned up]
