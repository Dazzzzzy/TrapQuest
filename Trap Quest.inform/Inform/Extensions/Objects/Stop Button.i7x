...
To say ExamineDesc of (C - stop-button):
	say "The capsule has a giant red button on the top with STOP in big letters on it. Above it, a large mechanical counter reads '$[endgame wealth of the player]'. Maybe it ends the game[if endgame wealth of the player > 0], and that's your prize money[end if]! Are you ready for the game to end?".

Check pushing stop-button:
	repeat with M running through combative monsters:
		unless M is robomatron, say "[BigNameDesc of M] blocks you from reaching the button! You're going to have to deal with [him of M] first!" instead;
	...
	say "This is it! The game will end if you push the button. Remember only [bold type]worn[roman type] jewellery will count towards your prize winnings, along with any other bonus prize money earned (currently $[endgame wealth of the player]). Are you sure you want to hit the button?";
