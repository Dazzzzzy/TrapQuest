...
To decide which number is the concealment of (R - a predicament room):
	decide on -1.
Definition: a predicament room is partial squirt: decide yes. [involuntary enema expulsion is 6 units at a time, rather than all of it]
...
Check going north when the player is in Predicament20:
	...
	if areYouSure is 111111: [we no longer do this]
...
Report going north when the player is in Facility01:
	...
	repeat with R running through predicament rooms:
		repeat with T running through things in R:
			unless T is backdrop or T is male-gloryhole or T is female-gloryhole or T is automated changing station or T is water-body-scenery, destroy T;
...
Park01 is a park room. The printed name of Park01 is "Men's [big SlimeTarget]s". "A small hut houses both male and female [SlimeTarget]s. This is the men's side.". The grid position of Park01 is <8,16,8>. Park01 is east of Predicament07. Park01 is south of Predicament09.
To decide which number is the concealment of (R - Park01):
	if current-predicament is incontinence-awareness-predicament, decide on -10;
	decide on 0.
Park02 is a park room. The printed name of Park02 is "Women's [big SlimeTarget]s". "A small hut houses both male and female [SlimeTarget]s. This is the women's side.". The grid position of Park02 is <8,17,8>. Park02 is east of Park01. Park02 is south of Predicament10. Park02 is west of Predicament12.
...
Park11 is a water-bodied park room. The grid position of Park11 is <8,18,6>. Park11 is east of Park10. Park11 is south of Park06. The printed name of Park11 is "Upper Lake". "This section of the park is dominated by the lake, which makes it difficult to find anything to hide behind.".
...
Toilet01 is a predicament room. Toilet01 is toilets. The printed name of Toilet01 is "Men's [big SlimeTarget]s". Toilet01 is below Park01. The grid position of Toilet01 is <7,16,8>.
Definition: Toilet01 is nonstandard: decide yes.
Toilet02 is a predicament room. Toilet02 is toilets. The printed name of Toilet02 is "Women's [big SlimeTarget]s". Toilet02 is below Park02. The grid position of Toilet02 is <9,17,8>.
Definition: Toilet02 is nonstandard: decide yes.

Check going down when the player is in Park01:
	if current-predicament is gloryhole-predicament and the cocks-sucked of gloryhole-predicament < 5 and the mens-room-entered of gloryhole-predicament is 0:
		say "Remember, [bold type]you'll be seriously punished with devastating online humiliation, if you go into the men's [SlimeTarget]s before you've finished sucking enough cocks - you need to suck 5, and [if the cocks-sucked of gloryhole-predicament is 0]you haven't sucked any yet[otherwise]you've only sucked [the cocks-sucked of gloryhole-predicament][end if]. [roman type]Are you sure this is what you want to do?";
		unless the player is consenting, say "You decide against triggering the maximum punishment." instead;
	otherwise if current-predicament is business-briefcase-predicament and business-briefcase-predicament is orgasm-caught and diaper quest is 0:
		say "Remember, [bold type]the video will be automatically shared online if you go into the men's room.[roman type]Are you sure this is what you want to do?";
		unless the player is consenting, say "You decide against triggering the maximum punishment." instead;
...
Check going down when the player is in Park02:
	if current-predicament is gloryhole-predicament and the womens-room-entered of gloryhole-predicament is 0 and (tight-grey-romper is worn or (tight-grey-romper is not in Park02 and tight-grey-romper is not in Toilet01)):
		say "Remember, [bold type]you'll be punished with a penalty point, and therefore online humiliation, if you go into the women's [SlimeTarget]s without first leaving your outfit on the hook outside. [roman type]Are you sure this is what you want to do? (To leave your outfit on the hook, select [']no['] and then [bold type]drop[roman type] your clothing.";
		unless the player is consenting, say "You decide against triggering the penalty point." instead;
	let P be a random person in Toilet02;
	if P is a person:
		allocate 2 seconds;
		say "You try the door, but it's locked. Someone's in there!" instead.

Check going up when the player is in Toilet01:
	if there is a person in Park01:
		say "You can hear someone directly outside[if the player is possessing a vagina and the player is not possessing a penis]... They'll probably think you're purposefully snooping around the men's [SlimeTarget]s like a dirty pervert[end if]...";
		reset multiple choice questions;
		set numerical response 1 to "go out anyway";
		set numerical response 2 to "wait";
		set numerical response 3 to "cancel";
...
leaving-wrong-toilets is initially false.

Report going up when the player is in Park01:
	now map-zoom is 3;
	if the player is possessing a vagina and the player is not possessing a penis, now leaving-wrong-toilets is true;
	compute predicament map reveal.

Check going up when the player is in Toilet02:
	if there is a person in Park02:
		say "You can hear someone directly outside[if the player is presenting as male]... They'll probably think you're purposefully snooping around the wrong women's [SlimeTarget]s like a dirty pervert[end if]...";
		reset multiple choice questions;
		set numerical response 1 to "go out anyway";
		set numerical response 2 to "wait";
		set numerical response 3 to "cancel";
...
Report going up when the player is in Park02:
	now map-zoom is 3;
	if the player is presenting as male, now leaving-wrong-toilets is true;
	compute predicament map reveal;
	if current-predicament is gloryhole-predicament and (tight-grey-romper is in Toilet01 or the wind-direction of gloryhole-predicament is not up):
		say "[one of][variable custom style]Wait, WHAT THE FUCK? SOMEONE HAS STOLEN MY ROMPER! But... Not only does that mean I'll have to walk home naked if I can't find it... But also... I was told I'll earn a penalty point if I don't wear it all the way home... Oh god... Please let it have just blown away a little bit in the wind or something...[roman type][line break][or][stopping]";
		if the wind-direction of gloryhole-predicament is not up and the player is getting lucky:
			say "[bold type]Just as you think that, you spot your romper to the [wind-direction of gloryhole-predicament]! [roman type][GotLuckyFlav]";
			now the wind-direction of gloryhole-predicament is up.
...
Figure of male gloryhole is the file "Env/Facility/gloryhole1.jpg".
...
Figure of female gloryhole is the file "Env/Facility/gloryhole2.jpg".
Figure of female gloryhole key scene is the file "Env/Facility/gloryhole4.jpg".
...
To say ExamineDesc of (G - female-gloryhole):
	say "The west wall of the public [SlimeTarget]s has a hole in the wall, the perfect size to fit a [manly-penis] through. The men's [SlimeTarget]s are on the other side. ";
...
To construct normal buttons for (D - female-gloryhole):
	if ButtonTableFull is 0:
		...
		now the ButtonImage entry is Figure of FistButton;
...
Report going when female-gloryhole is in the location:
	compute addiction reflection on "oral sex addiction" with 1 flavour.

Before doing something to an unseen-stranger when the player is in a predicament room:
...
Check attacking female-gloryhole:
	...
	if M is monster:
		...
		otherwise:
			...
			let recorded-bj be false;
			if id-poster is in Toilet01 and a random number between 1 and 4 > TX:
				now recorded-bj is true;
				say "You hear a [bold type]BEEP[roman type] that sounds like [he of M][']s started a recording on [his of M] phone.[one of][line break][variable custom style][big he of M][']s going to record this entire interaction... But at least [he of M] can't see my face...[roman type][line break][or][stopping]";
			if current-predicament is gloryhole-predicament, say "What will you do? Remember, the more outrageous your performance, the more humiliating the CCTV footage will be if it gets shared online.";
			reset multiple choice questions; [ALWAYS REMEMBER THIS WHEN MAKING A MULTIPLE CHOICE QUESTION]
			...
			compute multiple choice question;
			let bj-request-flav be "shyly greeting the person filming";
			...
			otherwise if player-numerical-response is 2:
				...
				now bj-request-flav is "seductively greeting the person filming";
			otherwise if player-numerical-response is 3:
				...
				now bj-request-flav is "offering a blowjob to the person filming";
			otherwise if player-numerical-response is 4:
				...
				now bj-request-flav is "begging to give a blowjob to the person filming";
			otherwise if player-numerical-response is 5:
				...
				now bj-request-flav is "shamelessly degrading yourself and begging to give a blowjob to the person filming";
			if T <= 0:
				if current-predicament is not nun-walk-predicament and condom fetish > 0 and a random number between 1 and 2 is 1, now M is wrapped;
				otherwise now M is unwrapped;
				say "[if OT > 0]You can hear [NameDesc of M] [one of]breathing heavily[or]almost tripping over [himself of M] as [he of M] drops [his of M] pants[or]rushing to comply[or]making excited sounds[in random order].[line break][speech style of M]'[one of]Today must be my lucky day[or]Oh yeah[or]Stay right there[in random order]...'[roman type][line break][end if]Moments later, a [manly-penis] appears through the hole! ";
				if condom fetish > 0, say "There is [if M is unwrapped][bold type]not[roman type] [end if]a condom on [his of M] [manly-penis].";
				...
				otherwise:
					...
					zero focus stuff;
			otherwise:
				say "You hear a cough and then someone leaving the men's [SlimeTarget]s as soon as they can.[line break][MissedGloryholeCock]";
				if current-predicament is nun-walk-predicament and player-numerical-response is 1 and the player is not able to speak, say "[variable custom style][one of]I guess [he of M] didn't like the sound of my mumbling...[or]Darn, it's a lot more difficult to convince them to put their [manly-penis]s through the hole without being able to speak proper words. But if I'm patient enough, I'm sure someone will be brave enough...[or]Oh my god, I'm getting so unlucky with these guys! But I can keep waiting, and hoping... Oh my god this is so embarrassing![or]It WILL work eventually!!![stopping][roman type][line break]";
				destroy M;
				zero focus stuff;
	otherwise:
		say "Nobody responds.";
...
Figure of park lake is the file "Env/Facility/lake1.jpg".
...
To compute swimming in (H - park-lake):
	let already-lake-swimming be false;
	if lake-swimming is true or water-peeing is true, now already-lake-swimming is true;
	now lake-swimming is true;
	let swim-turns be 0;
	allocate 6 seconds;
	while lake-swimming is true and the player is in the location of park-lake:
		update appearance level;
		if swim-turns is 0 and already-lake-swimming is false, say "You slip into the lake and allow yourself to sink in all the way up to your neck.";
		compute bathing;
		increase swim-turns by 1;
		reset multiple choice questions;
		set numerical response 1 to "Stay in the water";
		set numerical response 2 to "Get out of the water";
		set numerical response 3 to "Dive for treasure";
		compute multiple choice question;
		if player-numerical-response is 1:
			say "You remain hidden in the lake.";
			if glory-stone is not worn:
				if the glory-stone-progress of the player > 0 or glory-stone is challenged:
					say "You notice something twinkling in the periphery of your vision... is there something at the bottom of the lake?";
				otherwise if the player is getting lucky:
					say "You notice something twinkling in the periphery of your vision... is there something at the bottom of the lake? [GotLuckyFlav]";
			compute extra turn;
		otherwise if player-numerical-response is 3:
			compute treasure diving in park-lake at 0;
		otherwise:
...
To compute treasure diving in (WB - park-lake) at (L - a number):
	say "[line break]Something catches your eye, and you quickly swim toward it... You find a statue partially submerged in mud with a solid gold ring fixed onto its finger. [run paragraph on]";
	if glory-stone-progress of the player > 10:
		if the number of worn rings < 8:
			say "It slips off the statue and onto your own finger as if it naturally belongs there.";
			summon glory-stone;
		otherwise:
			say "It slips off the statue and onto your finger, but it bounces off the ring that's already there... maybe you need to make some space?";
	otherwise:
		if glory-stone is unchallenged:
			now glory-stone is challenged;
			say "It doesn't appear to be rusted, but it doesn't wiggle even a little bit when you try to remove it.[line break][first custom style][']Hmph. I desire only master who faces the greatest opponents in the most memorable regalia possible! A mortal like you couldn't manage such a thing even if you cheated and stood in front of a gloryhole!['][roman type][line break]A stern voice sounds off in your head as you swim back to the surface, and you get a distinct feeling it belongs to the ring itself.";
		otherwise:
			say "It doesn't appear to be rusted, but it doesn't wiggle even a little bit when you try to remove it.".

Predicament01, Predicament02, Predicament03, Predicament04, Predicament05, Predicament06, Predicament07, Predicament08, Predicament09, Predicament10, Predicament11, Predicament12, Predicament13, Predicament14, Predicament15, Predicament16, Predicament17, Predicament18, Predicament19, Predicament20, Predicament21, Predicament22, Predicament23, Predicament24, Predicament25, Predicament26, Predicament27, Predicament28, Predicament29, Predicament30, Predicament31, Predicament32, Predicament33, Predicament34, Predicament35, Park01, Park02, Park03, Park04, Park05, Park06, Park07, Park08, Park09, Park10, Park11, Park12, Park13, Park14, Park15, Park16, Park17, Park18, Park19, Park20, Park21, Park22, Park23, Park24, Toilet01, Toilet02 are in the Facility.
...
To compute predicament conclusion:
	...
	repeat with C running through predicament-fixed wearthings:
		unless C is in Facility15, now C is predicament-normal;
	repeat with C running through worn wearthings:
		...
		otherwise:
			if C is worn clothing and C is not piercing and C is not armband and C is not combat visor and C is not listed in predicamentPenList:
				if C is listed in predicamentWornList:
					remove C from predicamentWornList; [it's already worn so we don't need to put it back on them]
				otherwise:
	...
	repeat with C running through predicamentPenList:
		if C is not worn and C is not in a predicament room: [if it's in a predicament room then it's something the player was wearing at the start of the predicament and then dropped]
			...
			otherwise:
				if C is clothing, PinkWardrobeUnclash C;
				say "[BigNameDesc of C] reappears on you!";
				now C is worn by the player;
				if C is clothing:
					layer C correctly;
	...
	if tough-shit is 0:
		...
		if watersports mechanics is 1:
			...
			if diaper messing >= 3:
				...
				say "You feel your [SlimeContainer] and bowels returning to the state they were in before you entered the Extra Credit Zone.";
			otherwise:
				say "You feel your [SlimeContainer] returning to the state it was in before you entered the Extra Credit Zone.";
...
A bystander has a number called bystanderInt3.
...
To decide which room is the predicament-pull-room of (M - a bystander):
	...
	if current-predicament is breastfeeding-predicament and fake-baby is not settled and (fake-baby is not held or the number of sat-on breastfeeding-bench is 0):
		if fake-baby is held:
			if the player is in Toilet01, decide on Park01;
			if the player is in Toilet02, decide on Park02;
			decide on the location of the player;
		otherwise:
			decide on the location of fake-baby;
	decide on Park16.
...
To compute movement of (M - a bystander):
	...
	if the travel-direction of the player is up or M is not in the location of the player: [bystander doesn't move if the player entered its location this turn]
		...
		let A be nothing;
		if current-predicament is gloryhole-predicament and (the player is in Toilet01 or the player is in Toilet02):
			let RL be Predicament18;
			let R be a random number between 1 and 4;
			if R is 1:
				now RL is Predicament17;
			otherwise if R is 2:
				now RL is Predicament12;
			otherwise if R is 3:
				now RL is Park06;
			now A is the best route from the location of M to RL;
		otherwise:
			now A is the best route from the location of M to the predicament-pull-room of M; [People love going to the centre]
		if A is a direction:
		...
		if (M is not in the location of water-fountain or M is not fountain-unquenched) and (M is strolling or R <= 1):
			...
			otherwise if A is up or A is not a direction or R > 1:
				...
				if M is in Park01 and M is male and the number of people in Toilet01 is 0, add down to LA;
				if M is in Park02 and M is female and the number of people in Toilet02 is 0, add down to LA;
				if M is in Toilet01 or M is in Toilet02, add up to LA;
				...
				if A is a direction and the number of barriers in P is 0 and the number of barriers in the location of M is 0:
					...
					if M is camera-bystander and current-predicament is bottle-jog-predicament and A is down and M is in Toilet01 and id-poster is in Toilet01:
						...
						if infamy fetish > 0:
							...
							say "In your mind's eye, you can picture [NameDesc of M] entering the men's [SlimeTarget]s and taking a photo of the poster, and publishing it online.[line break][variable custom style]What an invasive image. It feels so real... Is it really just my imagination?[roman type][line break]";
...
To decide which number is the outrage tolerance of (M - a bystander):
	if current-predicament is incontinence-awareness-predicament, decide on 8;
	decide on 6.
To decide which number is the outrage tolerance of (M - a camera-bystander):
	decide on 3.

To decide which number is the cringe tolerance of (M - a bystander):
	if current-predicament is incontinence-awareness-predicament, decide on 8;
	decide on 4.
To decide which number is the cringe tolerance of (M - a camera-bystander):
	decide on 3.

To check perception of (M - a bystander):
	if M is in the location of the player:
		now segway-predicament is not snapped-this-turn;
		...
		otherwise if leaving-wrong-toilets is true:
			compute toilet pervert perception of M;
...
To compute perception of (M - a bystander):
	...
	otherwise if beachball-game of team-lake-predicament > 0:
		if M is camera-bystander and the appearance of the player > the outrage tolerance of M, compute lewd perception reaction of M;
		otherwise say "[big he of M] watches you [one of]frolicking in the lake [or]playing with a beachball [or][stopping][one of]with mild incredulity[or]with a slight frown[or]with a quizzical expression[or]with an unimpressed expression[or]with a bit of a glare[or]with a neutral expression[or]with a wry smile[or]and tuts[or]and shakes [his of M] head[at random].";
	otherwise if currently-squirting is 1:
		compute squirting perception of M;
	otherwise if grope-me-skirt is worn and (M is male or M is construction-worker-bystander) and M is not couple-bystander and M is groping and the number of worn messed diaper is 0:
		compute grope of M;
	...
	otherwise if leaving-wrong-toilets is true:
		compute toilet pervert perception of M;
	otherwise if fake-baby is held:
		compute breastfeeding perception of M;
...
To compute toilet pervert perception of (M - a bystander):
	let A be the appearance of the player;
	let AP be A;
	if AP < 8, now AP is 8;
	say "[BigNameDesc of M] appears even more [one of]shocked by[or]disgusted at[or]unimpressed by[in random order] you! ";
	compute toilet pervert perception reaction of M;
	say HumiliateReflect (AP * 30);
...
To compute worsened perception of (M - a bystander):
	...
	otherwise if A > the outrage tolerance of M:
		say "[BigNameDesc of M] appears even more [if M is male and A < 8][one of]entertained by[or]excited by[or]turned on by[or]enraptured by[in random order][otherwise][one of]shocked by[or]disgusted at[or]unimpressed by[in random order] your appearance[end if]! ";
		compute lewd perception reaction of M;
...
To compute toilet pervert perception reaction of (M - a bystander):
	say "[line break][first custom style]'[one of]What were you doing in those [SlimeTarget]s?!'[or]Why were you in the wrong [SlimeTarget]s, you pervert?!'[or]This perver was in the wrong [SlimeTarget]s!'[in random order][roman type][line break]";
...
To compute lewd perception reaction of (M - a camera-bystander):
	say "[BigNameDesc of M] [one of]widens [his of M] eyes and[or]gasps and[or]doesn't say anything, but just[then at random] ";
	if current-predicament is incontinence-awareness-predicament and the cringe appearance of the player < 10 and incontinence-awareness-predicament is in the location of the player:
		if there is a worn messed diaper, compute careful picture taking of M;
		otherwise say "moves [his of M] camera slowly into position.";
	otherwise:
		compute careful picture taking of M.
...
To compute careful picture taking of (M - a bystander):
	...
	if infamy fetish > 0:
		if A < 8 and leaving-wrong-toilets is true, compute digital reputation damage "exiting the wrong [SlimeTarget]s, looking like a pervert" strength 8 quality (a random number between 3 and 6);
	...
	otherwise if current-predicament is segway-predicament and segway-predicament is penetrating a fuckhole:
		compute segway punishment.
...
To compute rapid picture taking of (M - a bystander) witnessing (T - a text):
	...
	if T is "":
		...
		if target-poster is wetting poster, now T is "you [if slimeshooter fetish is 1]sliming your clothes[otherwise if the player is pee protected]wetting yourself[otherwise][SlimeDraining][end if]";
	let A be the appearance of the player;
	if diaper quest is 1 and the appearance of the player < the cringe appearance of the player, now A is the cringe appearance of the player;
...
Definition: a predicament (called P) is bystanders-always-mess-aware: decide no. [if set to yes, bystanders will be aware the player is messing themselves even if their diaper isn't visible]

To compute diaper mess reaction of (M - a bystander):
	let K be a random worn knickers;
	if M is diaperMessReacted:
		if M is camera-bystander:
			say "[BigNameDesc of M] keeps filming you![line break][speech style of M]'[one of][big he of the player] hasn't stopped[or][big he of the player][']s still going[or][big he of the player][']s still pooping [himself of the player][in random order]!'[roman type][line break]";
		otherwise:
			say "[BigNameDesc of M] [one of]gives you a death stare[or]makes disgusted sounds as [he of M] watches[or]slowly backs away from you, a lot of extreme disgust on [his of M] face[in random order].[line break][speech style of M]'[one of]You're sick, you know that?'[or]This can't be real. Gross!'[or]Oh my god, I can HEAR it coming out of you!'[or]There's MORE?!'[in random order][roman type][line break]";
		say strongHumiliateReflect;
	otherwise:
		if M is camera-bystander:
			say "[BigNameDesc of M] [one of]squawks[or]jumps[or]squeaks[at random] with [one of]shock[or]surprise[purely at random], but within seconds, [his of M] camera is pointed at you![line break][italic type]BEEP![roman type][line break][speech style of M]'This [man of the player] is literally [one of]going to the [SlimeTarget][or]shitting [himself of the player][or]pooping [himself of the player][in random order] in front of me!'[paragraph break][variable custom style]Noooo, [one of][he of M][']s recording me[or]don't record me[cycling]![roman type][line break]";
			now M is interested;
			let T be the substituted form of "you in public, [one of]visibly going number two in[or]messing[or]pooping in[at random] your [if K is diaper]diaper[otherwise]pants[end if]";
			if infamy fetish > 0, compute digital reputation damage T strength 16 quality (a random number between 4 and 5);
		otherwise:
			say "[BigNameDesc of M] [one of]gags[or]croaks[or]retches[at random] in [one of]disgust[or]horror[as decreasingly likely outcomes].[line break][speech style of M]'[one of]No way...'[or]Holy fuck...'[or]What the fuck are you doing?! Yuck, stop!!!'[or]Are you shitting yourself right now?!'[or][big he of the player][']s pooping [himself of the player]!'[or]No fucking way, you're messing yourself?! Right here?!'[in random order][roman type][line break]";
		say severeHumiliateReflect;
		now M is diaperMessReacted;
	if the worst-appearance of M < the appearance of the player, now the worst-appearance of M is the appearance of the player;
	if diaper quest is 1 and the worst-appearance of M < the cringe appearance of the player, now the worst-appearance of M is the cringe appearance of the player.

Figure of Businessman Bystander is the file "NPCs/Facility/Bystander/bystander1.jpg".
Figure of Businesswoman Bystander is the file "NPCs/Facility/Bystander/bystander2.jpg".
Figure of Male Birdwatcher Bystander is the file "NPCs/Facility/Bystander/bystander3.jpg".
Figure of Female Birdwatcher Bystander is the file "NPCs/Facility/Bystander/bystander4.jpg".
Figure of Male Gardener Bystander is the file "NPCs/Facility/Bystander/bystander5.jpg".
Figure of Female Gardener Bystander is the file "NPCs/Facility/Bystander/bystander6.jpg".
Figure of Male Jogger Bystander is the file "NPCs/Facility/Bystander/bystander7.jpg".
Figure of Female Jogger Bystander is the file "NPCs/Facility/Bystander/bystander8.jpg".
Figure of Postman Bystander is the file "NPCs/Facility/Bystander/bystander9.jpg".
Figure of Postwoman Bystander is the file "NPCs/Facility/Bystander/bystander10.jpg".

Figure of Male Frisbeer Bystander is the file "NPCs/Facility/Bystander/bystander11.jpg".
Figure of Female Frisbeer Bystander is the file "NPCs/Facility/Bystander/bystander12.jpg".
Figure of Male Bookworm Bystander is the file "NPCs/Facility/Bystander/bystander13.jpg".
Figure of Female Bookworm Bystander is the file "NPCs/Facility/Bystander/bystander14.jpg".
Figure of Male Selfie Bystander is the file "NPCs/Facility/Bystander/bystander15.jpg".
Figure of Female Selfie Bystander is the file "NPCs/Facility/Bystander/bystander16.jpg".
Figure of Male Skater Bystander is the file "NPCs/Facility/Bystander/bystander17.jpg".
Figure of Female Skater Bystander is the file "NPCs/Facility/Bystander/bystander18.jpg".
Figure of Male Sunbather Bystander is the file "NPCs/Facility/Bystander/bystander19.jpg".
Figure of Female Sunbather Bystander is the file "NPCs/Facility/Bystander/bystander20.jpg".

Figure of Shutterbug Bystander is the file "NPCs/Facility/Bystander/bystander21.jpg".
Figure of Bro Bystander is the file "NPCs/Facility/Bystander/bystander22.jpg".
Figure of Bum Bystander is the file "NPCs/Facility/Bystander/bystander23.jpg".
Figure of Partygirl Bystander is the file "NPCs/Facility/Bystander/bystander24.jpg".
Figure of Cosplayer Bystander is the file "NPCs/Facility/Bystander/bystander25.jpg".
Figure of Construction Worker Bystander is the file "NPCs/Facility/Bystander/bystander26.jpg".
Figure of Housewife Bystander is the file "NPCs/Facility/Bystander/bystander27.jpg".
Figure of Hooker Bystander is the file "NPCs/Facility/Bystander/bystander28.jpg".
Figure of Couple Bystander is the file "NPCs/Facility/Bystander/bystander29.jpg".
...
To compute lewd perception reaction of (M - female-bookworm-bystander):
	say "[line break][first custom style]'[one of]I... Oh my!'[or]H... haaah... How are you so brave?'[in random order][roman type][line break][BigNameDesc of M] has turned bright red and [he of M] is crossing [his of M] legs together as if [he of M] desperately needs the [SlimeTarget].".
...
To compute non-lewd perception reaction of (M - bro-bystander):
	if segway-predicament is penetrating a fuckhole, compute lewd perception reaction of M;
	otherwise compute grope of M.
...
To set up awareness stats of (M - bum-bystander):
	now the bystanderInt2 of M is 0.
...
To set up awareness stats of (M - hooker-bystander):
	now the bystanderInt1 of M is a random number between -1 and 10; [How much convincing do they need?]
	now the bystanderInt2 of M is a random number between 1 and 2. [How much is the maximum amount they'll donate?]
...
To compute cringe perception reaction of (M - couple-bystander):
	if there is a worn currently at least partially visible diaper or the number of worn currently visible baby themed clothing > 0, say "[line break][BigNameDesc of M] looks you up and down, mouth wide open. [big his of M] girlfriend shrieks at [him of M].[line break][first custom style]'Oh my god, are you getting turned on by a [boy of the player] dressed as a baby? You disgusting pervert!'[roman type][line break]";
	otherwise compute lewd perception reaction of M.
...
To set up awareness stats of (M - couple-bystander):
	now the bystanderInt1 of M is a random number between -1 and 10; [How much convincing do they need?]
	now the bystanderInt2 of M is 1. [How much is the maximum amount they'll donate?]
...
A game universe initialisation rule:
	...
	repeat with M running through bystanders:
		...
		increase N by 1;
	change the north exit of Predicament20 to Facility01. [It's a one-way journey]
...
To compute bystander motion:
	if girlfriend-partner is on-stage:
		if the player is in a predicament room and the player is not in Predicament20:
			...
			otherwise if current-predicament is team-three-leg-predicament:
				repeat with M running through bystanders:
					if (M is in a park room and a random number between 1 and 4 > 1) or ((M is in Park10 or M is in Park16) and a random number between 1 and 3 > 1), now M is moved; [most people stay still]
		otherwise:
			destroy girlfriend-partner;
			if the player is in Predicament20, say "[trainee-name of team-predicament-partner] goes through the portal, leaving you behind.";
	...
	if the player is in an outside predicament room or the player is in Predicament02 or the player is in Toilet01 or the player is in Toilet02 or (current-predicament is vibe-photo-predicament and the player is in Predicament01):
		...
		repeat with M running through alive bystanders:
			unless (current-predicament is toilets-dildos-predicament and M is in Park02 and the player is in Toilet02) or (current-predicament is incontinence-awareness-predicament and M is in Park02 and the player is in Park02 and M is interested and the bystanderInt2 of M > 0 and a random number between 1 and 8 > 1):
				unless M is in the location of the player and ((fake-baby is held and fake-baby is not settled) or (M is male and there is a sat-on breastfeeding-bench)): [some people want to watch you care for your fake baby]
					if debugmode > 1, say "Computing [NameDesc of M] ([location of M]).";
					if M is not moved, compute movement of M;
			now M is not moved;
			check perception of M;
			[display entire map.]
		now leaving-wrong-toilets is false.
...
To set up (N - a number) bystanders:
	...
	if the number of entries in LB > N, truncate LB to N entries;
	repeat with M running through LB:
...
To uniquely destroy (M - a bystander):
	now the worst-appearance of M is 0;
	now M is uninterested;
	now M is not diaperMessReacted.

To set up (N - a number) camera bystanders:
	let LB be the list of camera-bystanders;
	repeat with M running through LB:
		uniquely destroy M; [don't remove them from play, they might have already been spawned]
	sort LB in random order;
	if the number of entries in LB > N, truncate LB to N entries;
	repeat with M running through LB:
		let valid-location-found be 0;
...
An all time based rule (this is the cars come past rule):
	if the player is in an outside predicament room:
		if the player is in a road room and (current-predicament is not smoothie-predicament or the player is not in Predicament03) and a random number between 1 and 5 is 1:
			unless current-predicament is segway-predicament and a random number between 1 and 3 is 1, compute car coming past.

To compute car coming past:
...
A predicament can be arms-declared.
A team-predicament is a kind of predicament.
team-predicament-partner is an object that varies.
Definition: a predicament is realistic-arms-requiring: decide no. [Some predicaments require us to track where the player's arms are at all times]
An all time based rule (this is the predicament-arms rule):
	if realisticarms is 1 and current-predicament is not arms-declared:
		now current-predicament is arms-declared;
		say "[bold type]You can suddenly sense that, from now until you reach the end of this Extra Credit Zone, the rules of exposure are slightly different. [one of]Unless[or]Just like last time, unless[stopping] your arms are required for something else, you should be able to use your arms to (partially) cover up the most humiliating parts of your appearance [defaultarmsflav of current-predicament].[roman type][line break]".
To say defaultarmsflav of (P - a predicament):
	say "(and this will happen automatically when possible unless you manually change your priorities with 'adjust arms')".
...
Definition: yourself is predicament-ready:
	...
	if pair of furry handcuffs is worn and pair of furry handcuffs is locked, decide no; [this is how we lazily flag that the player has done the bukkake training and shouldn't do extra credit now]
	...
	if (class-time is 1000 or class-time < 0) and armband is not solid gold and there is an alive undefeated correctly-ranked tutor, decide no; [ready for class]
	decide yes.
Definition: a predicament is eligible:
	if it is not fixed-clothing-eligible, decide no;
...
Definition: a team-predicament is eligible:
	...
	decide no.
Definition: a predicament is fixed-clothing-eligible: decide yes. [This is how we assess whether clothing that the predicament will potentially allow you to keep will clash with clothing the game wants to make reappear]
...
To say PredicamentDescription of (P - a predicament):
	say "I'm currently in an Extra Credit training session where I need to get back to my home while avoiding as many bystanders from noticing me as possible.".
...
Definition: vibe-photo-predicament is realistic-arms-requiring: decide yes.
...
Definition: painted-cutoffs-predicament is realistic-arms-requiring: decide yes.
...
To say PredicamentDescription of (P - painted-cutoffs-predicament):
	say "I'm currently in an Extra Credit training session where I need to get back to my home while deciding whether to look extremely slutty or stimulate my [genitals], and avoiding as many bystanders from noticing me as possible.".
To execute (P - painted-cutoffs-predicament):
	now the player is in Predicament02;
	display entire map;
	summon predicament-painted-cutoffs uncursed;
	summon painted-vibrator-hands uncursed;
...
Definition: serving-tray-predicament is realistic-arms-requiring: decide yes.
...
To say PredicamentDescription of (P - serving-tray-predicament):
	say "I'm currently in an Extra Credit training session where I need to decide to either look extremely slutty or wear a skirt-tray-thing that stimulates me as I serve cupcakes, and then get back to my home while avoiding as many bystanders from noticing me as possible.".
...
To execute (P - serving-tray-predicament):
	...
	if diaper quest is 1:
		...
		now the skirt-length of skirt-tray-vibrator is 0;
	otherwise:
...
To maths-execute (M - condom-anal):
	let A be a random unseen-stranger;
...
To maths-execute (M - spank-my-butt-large):
	...
	if the bladder of the player > (bladder-risky-level / 2):
		if legacy watersports mechanics is 1, say "The pain of the spanking episode causes you to involuntarily [slimeonself].";
		otherwise say "The shock of the spanking episode scares your slimeshooter, causing it to drain itself without permission!";
...
To render timer image (F - a figure-name) with background (N - a number) shrunk (S - a number):
	let W be map-window-width / S;
	let H be map-window-height / S;
	let xOffset be W * (S - 1);
	let yOffset be H * (S - 1);
	...
	let Xi be map-window-x-root + xOffset + ((W - FXi) / 2); [centre horizontally]
	let Yi be yOffset + ((H - FYi) / 2); [centre vertically]
	draw a rectangle N in the graphics-window at (map-window-x-root + xOffset) by yOffset with size W by H;
...
Glulx input handling rule for a timer-event:
	if maths-timer > 0:
		...
		if maths-timer >= 4:
			render timer image Figure of Timer 1 with background 16776960 shrunk 1; [yellow]
		otherwise if maths-timer is 3:
			render timer image Figure of Timer 2 with background 16755200 shrunk 1; [orangeyellow]
		otherwise if maths-timer is 2:
			render timer image Figure of Timer 3 with background 16733440 shrunk 1; [orangered]
		otherwise if maths-timer is 1:
			render timer image Figure of Timer 4 with background lightModeFullRed shrunk 1; [red]
		otherwise if maths-timer is 0:[###MG: There's some kind of issue going on here where the player gets stuck forever. (Aika has just attempted to fix this Jan 2025)]
			render timer image Figure of Timer 5 with background (TQcolour of violet) shrunk 1; [violet]
...
To execute (MSP - maths-sex-predicament):
	...
	otherwise:
		let PMM be a random blue slut clubbing dress;
		if the player is getting unlucky:
			if diaper quest is 1, now PMM is a random crybaby T-shirt;
			otherwise now PMM is a random pink minimalist monokini;
		...
		try examining PMM;
		if PMM is not skirted, say "[variable custom style]This [if diaper quest is 1]doesn't cover my diaper at all[otherwise]is so slutty[end if]! Is this really all they left me to walk home in?![roman type][line break][GotUnluckyFlav]";
	...
	while T > 0:
		...
		if watersports fetish is 1:
			if the player is bursting:
				say "Release your hold on your [SlimeContainer]?";
				if the player is consenting:
					now delayed urination is 2;
				otherwise:
					compute guaranteed bladder growth;
				if delayed urination > 0:
					if legacy watersports mechanics is 1, say "[if delayed urination is 1]You can't hold it any more! You involuntarily[otherwise]You[end if] release your hold on your [SlimeContainer], and have to make a super quick decision - you can choose to position your [genitals] over [NameDesc of M][']s mouth, if you wish, to force [him of M] to drink it rather than it going down onto your outfits. Do you squat over [NameDesc of M][']s mouth? ";
					otherwise say "[if delayed urination is 1]You can't hold it any more! You involuntarily[otherwise]You[end if] lose control over your slimeshooter, and have to make a super quick decision - you can choose to position the barrel over [NameDesc of M][']s mouth, if you wish, to force [him of M] to drink it rather than it going down onto your outfits. Do you point it [NameDesc of M][']s mouth? ";
					if the player is consenting:
						if legacy watersports mechanics is 1, say "[BigNameDesc of M] moans and tries to shake [his of M] head but to no avail. There's nothing [he of M] can do without using [his of M] safeword as you squat over [his of M] face and let your [slime] flow into [his of M] wide open mouth. You can hear [him of M] coughing and spluttering but [he of M] does [his of M] best to swallow it all, gulp after reluctant gulp, until eventually [he of M] has drunk your entire [SlimeContainer]-full of [slime], like the perfect [SlimeTargetU].";
						otherwise say "[BigNameDesc of M] moans and tries to shake [his of M] head but to no avail. There's nothing [he of M] can do without using [his of M] safeword as you point your slimeshooter at [his of M] face and let your [slime] flow into [his of M] wide open mouth. You can hear [him of M] coughing and spluttering but [he of M] does [his of M] best to swallow it all, gulp after reluctant gulp, until eventually [he of M] has drunk your entire slime-magazine of [slime].";
						FavourDown M by 2;
					otherwise:
						say "[BigNameDesc of M] lets out a concerned whine as [he of M] hears you emptying your [SlimeContainer]. By the time you're done, the clothing underneath you is completely soaked through with your [slime].";
						FavourDown M;
						repeat with C running through clothing in the location of the player:
							now the slime-soak of C is the soak-limit of C;
						PuddleUp slime by the bladder of the player;
		...
		repeat with LX running from 1 to 2:
			...
			if LN > a random number between 6 and 20:
				say "[BigNameDesc of M] wriggles and lets out a panicked 'Eeee!' You can tell [he of M][']s about to [if L is slime][slimedrain] on [himself of M][otherwise]expel [his of M] [semen] enema[end if]. There's only one thing you could do to prevent the [variable L] flowing onto the clothing below your feet - you'd have to take it into your mouth. Do you ready your mouth in front of [NameDesc of M][']s crotch? ";
				...
				if WL > 0:
					...
					repeat with C running through clothing in the location of the player:
						if L is slime:
							if the slime-soak of C < the soak-limit of C, increase the slime-soak of C by LN;
						otherwise:
...
Definition: team-quiz-predicament is fixed-clothing-eligible: [might clash with new cocktail dress]
	repeat with C running through worn predicament-reappearing clothing:
		if C is overdress or C is skirt, decide no;
		if C is breast covering or C is belly covering or C is underdress:
			if C is totally-exclusive or C is top-exclusive, decide no;
	decide yes.
...
To say PredicamentDescription of (P - team-quiz-predicament):
	say "I'm currently in an Extra Credit training session where I need to answer questions correctly to escape a situation where I'm forced to crawl and I'm going to release an enema on [trainee-name of team-predicament-partner][']s face, and then I need to get back to my home while avoiding as many bystanders from noticing me as possible.".
...
To execute (L - team-quiz-predicament):
	...
	if diaper quest is 1:
		if diaper messing >= 7 and asshole is not actually occupied, now rectum is 25;
	...
	otherwise:
		...
		if the total fill of belly < belly limit - 5:
			if watersports fetish is 1, increase the slime volume of belly by ((belly limit - 5) - total fill of belly);
		...
		if the player is in Predicament01 and the questionFails of team-quiz-predicament < 2 and maxQuestionWins of team-quiz-predicament < 4 and currently-squirting is 0:
			...
			if questionExpelled of L is 0:
				...
				otherwise if QFL is 2 or QFL >= 4:
					say "You feel your [if diaper quest is 1][SlimeContainer][otherwise]sphincter[end if] beg to allow itself to be loosened. You know if you resist, [if diaper quest is 1]your [SlimeContainer] will become a bit more irritable[otherwise]it'll really hurt[end if]. Do you hold it in?";
					now temporaryYesNoBackground is figure of quiz predicament room;
					if the player is consenting:
	...
	if maxQuestionWins of L > 1:
		...
		if maxQuestionWins of L > 2:
			...
			if maxQuestionWins of L > 3:
				...
				say "Two slutty cocktail dresses fall from another container next to the first! They have magic enchantments, and look like they'll stick around when you get to the exit!";
				HappinessUp ST by 1;
...
Definition: quiz-junk is appropriate:
	if the number of clothing in Facility15 < 10, decide yes;
	decide no.
To say question for (Q - quiz-junk):
	say "How many items of clothing are there in the junk room right now";
To set up (Q - quiz-junk):
	increase the askedAlready of Q by 1;
	now the correctAnswer of Q is the number of clothing in Facility15;
	set up number answers for Q.
...
Definition: team-quiz-predicaments is appropriate:
	if totalTrainingSessionCount < 10, decide yes;
	decide no.
To say question for (Q - team-quiz-predicaments):
	say "How many training sessions have you attended";
To set up (Q - team-quiz-predicaments):
	increase the askedAlready of Q by 1;
	now the correctAnswer of Q is totalTrainingSessionCount;
	set up number answers for Q.
...
To say PredicamentDescription of (P - smoothie-predicament):
	say "I'm currently in an Extra Credit training session where I need to [if smoothie gag is worn]serve smoothies as fast as possible so I can escape this van, while trying to minimise how much they force me to drink myself, and then I need to [end if]get back to my home while avoiding as many bystanders from noticing me as possible.".
...
Figure of smoothie van opaque is the file "Env/Facility/smoothievan1.jpg".
Figure of smoothie van X-ray is the file "Env/Facility/smoothievan2.jpg".
...
Definition: smoothie-predicament is fixed-clothing-eligible:
	if there is a worn predicament-reappearing dress, decide no;
	decide yes.
...
To compute smoothie perception of (M - a bystander):
	...
	otherwise:
		...
		if the smoothies-served of smoothie-predicament is 6:
			say "You hear a loud 'BEEP' as your restraints are released. You can now [bold type]remove the gag[roman type] and then leave the van. But the voice did say that if you served just [bold type]three more smoothies[roman type], you'd get an extra trophy, making this experience twice as worthwhile. So maybe... You should keep the gag on for a little longer...?";
			unlock smoothie gag;
...
To say PredicamentDescription of (P - sex-toy-predicament):
	say "I'm currently in an Extra Credit training session where I need to get the key to my house by finding two sex toys, one at the angel statue and the other in the men's [SlimeTarget]s. Then I need to bring them back to the starting room and drop them, and finally get back to my home while avoiding as many bystanders['] notice as possible.".

Definition: sex-toy-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is penetrating a fuckhole, decide no;
	decide yes.
...
To execute (L - sex-toy-predicament):
	...
	now ST is predicament-fixed;
	now the raw-magic-modifier of ST is 0;
	now the size of ST is the insertableGirthAcceptance of asshole - 1;
	if ST is mamba, now the size of ST is the insertableGirthAcceptance of asshole;
	if the size of ST < 1, now the size of ST is 1;
	now ST is in Toilet01;
	now the printed name of Predicament01 is "Small Room";
	now the player is in Predicament01;
	say "You find yourself in a small room, empty except for a wardrobe, a small table and a front door that leads outside. You are completely naked. The table has a card on it with the heading 'INSTRUCTIONS' on it. You read the instructions:[paragraph break][first custom style]'WELCOME TO EXTRA CREDIT CLASS. [PredicamentRewardExplanation]TO EARN THIS TROPHY, YOU MUST [bold type]FIND THE TWO SEX TOYS HIDDEN IN THE PARK AND PLACE THEM ON THIS TABLE. ONLY THEN WILL THE KEY TO YOUR FRONT DOOR BE RELEASED. [first custom style]ONE SEX TOY IS HIDDEN IN THE MEN'S [caps SlimeTarget]S. THE OTHER IS HIDDEN BEHIND THE STATUE. CONSIDER YOUR OUTFIT CAREFULLY, AS KEEPING THE TOYS HIDDEN MAY NOT BE EASY...'[PredicamentRewardExplanationReaction][roman type][line break]It looks like you are going to look a bit eye-catching no matter what you wear, but at least you've been given a bit of choice. You consider the warning from the card. [if diaper quest is 1]Would wearing the diaper really[otherwise]Which of these would best[end if] help you... 'hide' the sex toys?";
...
Check going east when the player is in Predicament19:
	if house-key is not held and (current-predicament is sex-toy-predicament or current-predicament is gloryhole-predicament or current-predicament is business-briefcase-predicament or current-predicament is incontinence-awareness-predicament), say "The door is locked! You need to bring your house key..." instead.
...
To execute (L - team-face-crotch-predicament):
	...
	say "You and [ST] find yourself in an almost pitch-black room, unable to see anything at all, and bound in place, unable to move. You [SlimeContainer] feels soooo full! A worried moan from [NameDesc of ST] lets you know that [he of ST] is feeling just as desperate to [slime] as you are. A scary sounding robotic voice booms loudly from all around you.[line break][first custom style]'WELCOME TO EXTRA CREDIT CLASS. [PredicamentRewardExplanation]FIRST, A LITTLE GAME, TO SEE WHICH OF YOU IS THE MOST [caps slime] TRAINED. YOU MUST BOTH ANSWER THIS QUESTION AT THE SAME TIME: WHICH OF YOU IS THE BIGGER BABY? YOUR ANSWERS DETERMINE THE PREDICAMENT...'[roman type][line break]";
	reset multiple choice questions; [ALWAYS REMEMBER THIS WHEN MAKING A MULTIPLE CHOICE QUESTION]
	...
	if player-numerical-response is 3 and STChoice is 1:
		say "[first custom style]'CONTESTANTS AGREED THAT [trainee-name of ST in upper case] IS A PATHETIC INCONTINENT BABY. PROCEEDING TO DIAPER DEPENDENCE EXPERIENCE PREDICAMENT!'";
	...
	while PredicamentMinutes > 0:
		...
		if the bladder of the player >= bladder-risky-level:
			say "Your [SlimeContainer] [if the bladder of the player < 12]twinges with significant discomfort. [bold type]Holding on will have a small chance of hurting[otherwise if the bladder of the player < 14]strains with its fullness, begging you to release. [bold type]Holding on has a decent chance of hurting[otherwise]is on fire, desperate to release! [bold type]Holding on will definitely hurt, and negatively affect your [SlimeContainer]'s long-term irritability[end if].[roman type][line break]Do you hold on? ";
			if the player is consenting:
		...
		if R <= STBladder + the vindictiveness of ST - the dedication of ST:
			say "[BigNameDesc of ST] sighs with defeat and you can tell that [he of ST] is peeing[if playerAbove is 0 and K is diaper]. Mostly because you can FEEL it on your face! The [slime] soaks straight through [his of ST] bikini briefs, making your face warm, wet and smelly. YUCK[otherwise if playerAbove is 0]. Mostly because you can feel the padding in front of your nose turning warm. Over time that subtle but distinct smell of [slime] begins to hit your nostrils[otherwise if K is diaper] into [his of ST] bikini briefs. Looks like [he of ST] will be walking home wet and dripping[otherwise] in [his of ST] diaper. Looks like [he of ST] will be walking home wet and soggy[end if].";
			if playerAbove is 0 and K is red-bikini-briefs:
				if watersports fetish is 1 and K is diaper, SlowSlimeTasteAddictUp 1;
				otherwise GrossOut wetDiaperFacesitGrossnessLevel;
			now STBladder is 0;
		if STBladder < 10 and playerAbove is 0:
			if K is diaper:
				say "The smell of [slime] goes to your head and makes you feel rather light-headed[if watersports fetish is 1 and K is diaper][otherwise if K is diaper or the remainder after dividing STBladder by 2 is 1], and perhaps a bit more perverse[end if].";
				if watersports fetish is 1 and K is diaper and the slime taste addiction of the player <= the sex addiction of the player, SlowSlimeTasteAddictUp 1;
				otherwise SexAddictUp 1;
			otherwise:
				say "The [slime]-soaked padding is still pressed into your face.";
	...
	say "Finally, a buzzer beeps and all your bondage unlocks. While you are still wriggling out of yours, [NameDesc of ST] wastes no time in running from the room, not concerned with waiting for you at all. [if STBladder >= 10 and playerAbove is 0][speech style of ST]'You'd better be fucking grateful I didn't [slimedrain] on you.'[roman type][line break]Is all [he of ST] says as [he of ST] leaves. [end if]Looks like you'll be sneaking home on your own.[if K is diaper][line break][variable custom style]And in nothing but a diaper...[roman type][line break][end if]";
	now bigGameLoop is 0;
	now ST is in Facility02.

diaper-grope-predicament is a predicament.
Definition: diaper-grope-predicament is realistic-arms-requiring: decide yes.
Definition: diaper-grope-predicament is appropriate:
	if the rank of the player < 2 or the rank of the player > 4 or enema fetish is 0 or the player is shameless, decide no;
	if diaper lover is 0, decide no;
	decide yes.
To say PredicamentDescription of (P - diaper-grope-predicament):
	say "I'm currently in an Extra Credit training session where I need to decide whether to keep my hands behind my head in order to avoid receiving an enema, and then I need to get back to my home while avoiding as many bystanders from noticing me as possible.".
To say defaultarmsflav of (P - diaper-grope-predicament):
	say "([bold type]and in this predicament, because of your starting situation, to make that happen you would have to manually change your priorities with 'adjust arms'[roman type])".
...
To execute (L - diaper-grope-predicament):
	...
	if rectum > 0, now rectum is rectum-risky-level - 1;
	summon enema-backpack locked;
	summon grope-me-skirt;
	summon plain-largish-diaper uncursed;
	now the raw-magic-modifier of plain-largish-diaper is -3;
...
team-football-predicament has a number called trainee-stimulation. [Count towards trainee orgasm]
team-football-predicament has a number called game-state. [0: Game in progress; 1: Player won; 2: Trainee won]
...
When play begins:
	now the football-possessor of football is mistress.

Definition: team-football-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is trousers, decide no;
		if C is penetrating a fuckhole, decide no;
	decide yes.
	...
	if current-predicament is team-football-predicament and the game-state of team-football-predicament is 0:
		...
		if (the player is not in Toilet02 or FL is not Park02) and (the player is not in a predicament room or (the distance of FL > (a random number between 1 and 2))):
			...
			otherwise if remote-controlled-vibrator is carried or remote-controlled-vibrator is in the location of the player:
				...
				SemenPuddleUp 12;
			now M is in Facility02;
		otherwise:
			if the football-possessor of football is not a trainee, now football is not dribbled;
...
This is the football possession rule:
	...
	repeat with D running through the Nviables of L:
		...
		let NP be the number of people in R;
		say "There [if NP is 1]is 1 person[otherwise]are [NP] people[end if] to the [D].";
...
An all time based rule:
	if current-predicament is team-football-predicament and the game-state of team-football-predicament is 0:
		...
		if football is in the location of M:
			increase the trainee-stimulation of team-football-predicament by a random number between 1 and 2;
			if the football-possessor of football is M, increase the trainee-stimulation of team-football-predicament by a random number between 1 and 2;
		if the trainee-stimulation of team-football-predicament >= a random number between 10 and (15 + the dedication of M):
			...
			if the football-possessor of football is M:
				now football is not dribbled;
				now the football-possessor of football is mistress;
				increase the trainingInt1 of M by a random number between 0 and 1;
			now M is stalled;
			now the trainee-stimulation of team-football-predicament is 0;
			HappinessDown M;
		let FL be the location of football;
...
To compute football movement of (M - a trainee):
	let FL be the location of football;
	let L be the location of M;
	if M is in Toilet02:
		try M going up;
	...
	otherwise if the trainingInt1 of M > 0:
		say "[BigNameDesc of M] is still recovering...";
		decrease trainingInt1 of M by 1;
	otherwise if L is FL:
		if the football-possessor of football is M:
			if the trainee-stimulation of team-football-predicament > 10 + the dedication of M:
...
Definition: water-fountain-predicament is appropriate:
	if enema fetish is 0 or the rank of the player > 2, decide no;
	decide yes.
To say PredicamentDescription of (P - water-fountain-predicament):
	say "I'm currently in an Extra Credit training session where I need to decide when to stand up and stop receiving enemas each time the water fountain is used, exposing myself to everyone left in the area when I do, and then I need to get back to my home while letting as few bystanders as possible notice me.".
...
Figure of water-fountain scene is the file "Env/Facility/waterfountain1.jpg".
Figure of water-fountain is the file "Env/Facility/waterfountain2.jpg".
...
To say PredicamentDescription of (P - gloryhole-predicament):
	say "I'm currently in an Extra Credit training session where I need to suck at least 5 cocks in the women's [SlimeTarget]s, trying to make sure nobody leaves from the other side of the gloryhole without having their dick sucked, and then go into the men's [SlimeTarget]s and retrieve my house key, and then get back to my home while avoiding as many bystanders as possible from noticing me.".
...
Definition: gloryhole-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is a dress, decide no;
		if C is penetrating a fuckhole, decide no;
	decide yes.
...
To execute (P - gloryhole-predicament):
	...
	now the wind-direction of P is up;
	summon tight-grey-romper uncursed;
	now tight-grey-romper is predicament-fixed;
	say "You find yourself in a small room, empty except a small table, and a front door that leads outside. You are wearing a tight pink and white romper with no underwear. On the table is a card with the word 'INSTRUCTIONS' on the front. You read the instructions on the back:[paragraph break][first custom style]'WELCOME TO EXTRA CREDIT CLASS. [PredicamentRewardExplanation]TO EARN THIS TROPHY, YOU MUST SIMPLY RETURN HOME AFTER EARNING THE KEY TO YOUR FRONT DOOR. THE KEY WILL BE OBTAINABLE FROM THE MEN'S [caps SlimeTarget]S AFTER YOU HAVE SUCKED AT LEAST ONE COCK TO COMPLETION AT THE GLORYHOLE IN THE PARK'S WOMEN'S [caps SlimeTarget]S. YOUR PERFORMANCE WILL BE FILMED. EACH OF THE FOLLOWING WILL EARN YOU ONE PENALTY POINT:[line break]* EACH TIME YOU LET SOMEONE LEAVE THE MEN'S [caps SlimeTarget]S WITHOUT YOU HAVING SUCKED THEIR COCK TO COMPLETION (WHILE YOU ARE IN THE LADIES['] [caps SlimeTarget]S)- YOU ARE ALLOWED TO MISS ONE COCK, BUT EACH ONE AFTER THAT EARNS YOU A PENALTY POINT.[line break]* ENTERING THE MEN'S [caps SlimeTarget]S BEFORE YOU HAVE FINISHED SUCKING COCKS[line break]* SPITTING ANY SEMEN OUT OF YOUR MOUTH[line break]* ENTERING THE WOMEN'S [caps SlimeTarget]S WITHOUT LEAVING THE GREY ROMPER ON THE HOOK OUTSIDE[line break]* FAILING TO WEAR THE GREY ROMPER FOR YOUR RETURN HOME AFTER RETRIEVING YOUR KEY[paragraph break]THE FOLLOWING ARE THE PUNISHMENTS FOR EACH PENALTY POINT EARNED:[line break]1) THE FOOTAGE OF YOUR PERFORMANCE WILL BE SENT TO YOUR [DistantRole in upper case] [DistantName in upper case][line break]2) THE FOOTAGE OF YOUR PERFORMANCE WILL BE SENT TO YOUR [PlatonicRole in upper case] [PlatonicName in upper case][line break]3) THE FOOTAGE OF YOUR PERFORMANCE WILL BE SENT TO YOUR [NemesisRole in upper case] [NemesisName in upper case][line break]4) THE FOOTAGE OF YOUR PERFORMANCE WILL BE SENT TO YOUR [FanciedRole in upper case] [FanciedName in upper case][line break]5) THE FOOTAGE OF YOUR PERFORMANCE WILL BE PUBLISHED ON WWW.SLUTHUB.COM.[paragraph break]FINALLY, [caps please] NOTE THAT SUCKING LESS THAN FIVE COCKS TO COMPLETION WILL AUTOMATICALLY TRIGGER ALL FIVE PUNISHMENTS.'[PredicamentRewardExplanationReaction][roman type][line break]It looks like your outfit has been decided for you...[paragraph break][variable custom style]Fuck, so I HAVE to suck 5 cocks to completion? There must be some kind of trick to this, to avoid something really humiliating that they're not telling me about... But I can't work out how...[roman type][line break]".
...
Report going when the player is in Toilet01:
	...
	if tight-grey-romper is in Toilet01:
		say "[variable custom style]There's my [ShortDesc of tight-grey-romper]! Thank goodness I found it; they want me to wear it home to avoid another penalty point[if the used condoms of tight-grey-romper > 0]. But wait... What the fuck? The guys were GLUING THEIR USED CONDOMS TO IT[otherwise if the semen-soak of tight-grey-romper > 0]. But wait... What the fuck? THIS IS WHAT THE GUYS WERE USING AS THEIR CUM RAG[end if]![roman type][line break][if tight-grey-romper is glued]On closer inspection, the inside of the bodysuit now seems to be covered in some sort of glue. [bold type]If you put it on, you'll likely be wearing it for quite some time.[line break][variable custom style]That bastard! That's what [he of male-m] meant when [he of male-m] said [he of male-m]'d make me regret it....[roman type] [GotUnluckyFlav][end if]".
...
Report going when the player is in Predicament20:
	if current-predicament is gloryhole-predicament:
		...
		otherwise:
			...
			if womens-room-entered of gloryhole-predicament is 1:
				say "YOU ENTERED THE WOMEN'S ROOM WITHOUT FIRST LEAVING YOUR BODYSUIT ON THE HOOK OUTSIDE. PENALTY POINTS INCREASED BY 1.";
				increase PP by 1;
			otherwise:
				say "YOU LEFT YOUR BODYSUIT ON THE HOOK OUTSIDE THE WOMEN'S ROOM BEFORE ENTERING. PENALTY POINTS INCREASED BY 0.";
...
An all later time based rule:
	if current-predicament is gloryhole-predicament:
		let M be a random unseen-stranger in Toilet02;
		if M is a thing:
		...
		otherwise if the player is in Toilet02:
			if a random number between 1 and 3 is 1:
				...
				if tight-grey-romper is in Park02:
					if tough-shit is 0 and the player is getting lucky:
						let DL be the NViables of Park02;
						sort DL in random order;
						let D be entry 1 in DL;
						let R be the room D from Park02;
						now tight-grey-romper is in R;
						now the wind-direction of gloryhole-predicament is D;
					otherwise:
						now tight-grey-romper is in Toilet01;
			otherwise:
				say "[one of]You silently wait and listen for any noise through the hole.[or][or]You silently reflect on how you must look in the CCTV camera, patiently waiting [if tight-grey-romper is not worn]butt naked [end if]at a gloryhole for [if the cocks-sucked of gloryhole-predicament > 0]more [end if][manly-penis].[or][or]Time feels like it stands still as you quietly wait[if the player is prone] on your knees[end if].[or][or][variable custom style]This is a new low.[roman type][line break][or][or]You stare at the CCTV camera in the corner.[line break][variable custom style]I've got to stop that recording from getting out.[roman type][line break][then at random]".

Carry out dropping tight-grey-romper:
	if current-predicament is gloryhole-predicament and the player is in Park02, say "You place [NameDesc of tight-grey-romper] on the external hook of the toilet door, as instructed.".
...
To make gloryhole statement demand of (M - a monster):
	let R be a random number between 1 and 3;
	if tight-grey-romper is in Toilet01 and tight-grey-romper is unglued, increase R by 1;
	if R > 2:
		say "Suddenly, you hear [NameDesc of M] speak up again.";
		if the cocks-sucked of gloryhole-predicament > 0 and id-poster is in Toilet01 and gloryhole-predicament is not poster-recorded:
			say "[speech style of M]'Say [']My name is [NameBimbo], and I'm your greedy cock-addicted gloryhole slut for today!['] And make it sound convincing.'[line break][variable custom style]Huh? How does [he of M] know my name?!?!?!?! And there's no rule saying that I have to do that...[roman type][line break]You hear a 'beep', and you know that [NameDesc of M] is recording this on [his of M] phone, too. Do what [he of M] says?";
			if the player is bimbo consenting:
				say "[variable custom style]'[']My name is [NameBimbo], and I'm your greedy cock-addicted gloryhole slut for today![']'[line break][speech style of M]'What a crazy fucking whore you are. Hahaha.'[roman type][line break][strongHumiliateReflect]";
				let T be "high resolution recording of you saying [']My name is [NameBimbo], and I'm your greedy cock-addicted gloryhole slut for today!['] through a gloryhole with a poster with your name, face and personal information above it";
				if the number of blank rows in the Table of Published Disgraces > 0:
					choose a blank row in Table of Published Disgraces;
					now the content entry is the substituted form of T;
					...
					now the deletedtime entry is -1;
					now the viewsfuzz entry is a random number between -10 and 10;
				now gloryhole-predicament is poster-recorded;
			otherwise:
				say "[variable custom style]'No fucking way.'[line break]";
				compute gloryhole gluing;
		otherwise if the cocks-sucked of gloryhole-predicament > 0 and watersports fetish is 1 and M is not wrapped:
			say "[speech style of M]'Next, I want you to drink my [slime], okay? That's what I came here for originally, after all.'[line break][variable custom style]Huh? There's no rule saying that I have to do that...[roman type][line break]Do what [he of M] says?";
			if the player is bimbo consenting:
				say "[variable custom style]'Okay, fine...'[line break][speech style of M]'Good toilet.'[roman type][line break]You wrap your lips around [his of M] [if slimeshooter fetish is 1]slimeshooter[otherwise]shaft[end if] once again, and obediently gulp down as [NameDesc of M] empties [his of M] [slime] directly into your stomach.";
				DrinkSlime from M;
			otherwise:
				say "[variable custom style]'What the fuck? No way! Get out of here.'[line break]";
				compute gloryhole gluing;
		otherwise if the cocks-sucked of gloryhole-predicament > 0 and grossness fetish > 0:
			say "[speech style of M]'Next, I want you to [if grossness fetish > 1]eat my ass.'[roman type][line break]You see [NameDesc of M][']s gross hairy butthole pushed up against the gloryhole[otherwise]suck my toes.'[roman type][line break]You see the front half [NameDesc of M][']s gross bare [one of]left[or]right[purely at random] foot pushed through the gloryhole[end if].[line break][variable custom style]Huh? There's no rule saying that I have to do that...[roman type][line break]Do what [he of M] says?";
			if the player is bimbo consenting:
				say "[variable custom style]'Okay, fine...'[roman type][line break]";
				...
				otherwise:
					...
					TasteGrossOut 4;
				say "[speech style of M]'Dirtiest fucking skank in the world, holy shit.'[roman type][line break][strongHumiliateReflect]";
				say "[BigNameDesc of M] pulls away, and finally leaves you be.";
			otherwise:
				say "[variable custom style]'What the fuck? No way! Get out of here.'[line break]";
				compute gloryhole gluing;
		otherwise:
			say "[speech style of M]'Say thank you for the privilege of sucking my cock.'[line break][variable custom style]Huh? There's no rule saying that I have to do that...[roman type][line break]Do what [he of M] says?";
			if the player is bimbo consenting:
				say "[variable custom style]'Thank you for the privilege of sucking your cock.'[line break][speech style of M]'Good slut.'[roman type][line break][moderateHumiliateReflect]";
			otherwise:
				say "[variable custom style]'Fuck you.'[line break]";
				compute gloryhole gluing.


To compute gloryhole gluing:
	if tight-grey-romper is in Toilet01 and tight-grey-romper is unglued:
		say "[speech style of male-m]'Hah! I'll make you regret that.'[variable custom style]How in the world would [he of male-m] make me regret that?[roman type][line break]";
		say "You hear what sounds like clothing rustling, and... Some kind of tube of toothpaste being violently squeezed?!";
		gluify tight-grey-romper;
	otherwise:
		say "[speech style of male-m]'[one of]Lame[or]Worth a shot, haha[in random order].'[roman type][line break]".


Check going when current-predicament is gloryhole-predicament:
	if tight-grey-romper is not worn and house-key is held and the clothing-not-worn of gloryhole-predicament is 0 and the player is in a predicament room and the player is not in Predicament20:
		say "Remember, [bold type]you'll be punished with a penalty point, and therefore online humiliation, if you make any part of your journey home (i.e. while holding your house key) while not wearing the grey romper. [roman type]Are you sure this is what you want to do?";
		unless the player is consenting, say "You decide against triggering the penalty point." instead.

Report going when current-predicament is gloryhole-predicament:
	if tight-grey-romper is not worn and house-key is held, now the clothing-not-worn of gloryhole-predicament is 1.


Definition: nun-walk-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is dress or C is shoes, decide no;
		if C is penetrating a fuckhole, decide no;
	decide yes.

nun-walk-predicament is a predicament.
Definition: nun-walk-predicament is realistic-arms-requiring: decide yes.
...
To say PredicamentDescription of (P - nun-walk-predicament):
	say "I'm currently in an Extra Credit training session where I need to get back to my home while avoiding as many bystanders as possible from noticing me. If I want to be freed from my hidden-bondage nun dress then I need to make sure I arrive at my house with the vibrator and my mouth full of [semen], which I can get from the gloryhole in the women's [SlimeTarget]s.".

An all later time based rule:
	if current-predicament is nun-walk-predicament:
		if the number of on-stage bystanders < 25 and the player is in Toilet02, set up 29 bystanders; [reset all bystanders and increase the number for the walk home from the toilets]
		let M be a random unseen-stranger in Toilet02;
		if M is a thing:
			if M is not penetrating a body part:
	...
	while T > 0 and S > 0:
		...
		if player-numerical-response > 0:
			...
			if a random number between 0 and 32 < the semen volume of face * the semen volume of face:
				...
				say roman type;
			say "You and [trainee-name of M] stretch your bungee harnesses as far as they will go, meeting in the middle of the room over the sensor[if the player is very tired]. [bold type]You are very tired[otherwise if the player is tired]. [bold type]You are tired[end if].[roman type][line break]";
			reset multiple choice questions; [ALWAYS REMEMBER THIS WHEN MAKING A MULTIPLE CHOICE QUESTION]
			...
			compute multiple choice question;
			if player-numerical-response is 1:
				say "You lock lips with [trainee-name of M] and begin to feed [him of M] the [semen] with your tongue. ";
			...
			otherwise:
				say "You [if player-numerical-response is 3]press your chest into [trainee-name of M][']s chest as tightly as you can, and [end if]lock lips with [him of M], before trying to push your [MouthfulDesc] into [his of M] mouth in one go. ";
			...
			if the fatigue of the player < F * 5, FatigueUp F;
			say "You both return to your walls[if the semen volume of face > 0], and [trainee-name of M] spits the [MouthfulDesc] into [his of M] fishbowl[end if].";
...
Definition: team-girlfriends-predicament is realistic-arms-requiring: decide yes.

girlfriend-partner is a person. The printed name of girlfriend-partner is "[TQlink of item described][trainee-name of team-predicament-partner][TQxlink of item described][shortcut-desc]".
...
Check going when girlfriend-partner is in the location of the player:
	if current-predicament is team-girlfriends-predicament and the room noun from the location of the player is a road room and the defaultLeftTarget of arms is girlfriend-partner:
		...
		otherwise:
			say "Action cancelled." instead.
Figure of both girlfriends is the file "NPCs/Facility/Trainee/girlfriend1a.png".
Figure of solo girlfriends is the file "NPCs/Facility/Trainee/girlfriend1b.png".
Figure of both boyfriends is the file "NPCs/Facility/Trainee/girlfriend2a.png".
Figure of both diaperfriends is the file "NPCs/Facility/Trainee/girlfriend3a.png".
Figure of solo diaperfriends is the file "NPCs/Facility/Trainee/girlfriend3b.png".
To decide which figure-name is the examine-image of (G - girlfriend-partner):
	if current-predicament is team-three-leg-predicament, decide on figure of three legged race;
...
To construct normal buttons for (T - girlfriend-partner):
	if current-predicament is team-girlfriends-predicament and ButtonTableFull is 0:
		choose a blank row in the Table of Buttons;
...
To say ExamineDesc of (G - girlfriend-partner):
	if current-predicament is team-girlfriends-predicament, say "[trainee-name of team-predicament-partner] has a similar outfit to you, except [his of team-predicament-partner] nipples are exposed through holes in [his of team-predicament-partner] top.";
	otherwise say "[trainee-name of team-predicament-partner] has a similar outfit to you.".
To check disapproval of (G - girlfriend-partner):
	do nothing.
Definition: girlfriend-partner is reactive: decide no.
To say NameDesc of (C - girlfriend-partner):
	say "[input-style][trainee-name of team-predicament-partner][roman type]".
To say BigNameDesc of (C - girlfriend-partner):
	say NameDesc of C.

To say PredicamentDescription of (P - team-girlfriends-predicament):
	say "I'm currently in an Extra Credit training session where I need to get back to my home with [girlfriend-partner] while avoiding as many bystanders from noticing me as possible.".

To execute (L - team-girlfriends-predicament):
	now the stance of the player is 0;
	now the player is in Predicament02;
	now the text-shortcut of girlfriend-partner is the trainee-name of team-predicament-partner;
...
To execute (TDVP - team-diaper-vibe-predicament):
	...
	say "You look around... you're sitting, bound with rope in the middle of a rather large room, with [NameDesc of M] also bound by rope, suspended above your head by a series of pulleys connected to a large weight. [big he of M][']s wearing a thick white diaper, while you are completely naked. A tube gag connects [trainee-name of M][']s mouth to a huge tank labelled 'X LAX 5000'. A vibrating wand is fixed in place, lodged against your [genitals]. A strong spring connects your two knees together, making it difficult for you to spread them. Two soft red buttons are fixed either side of your knees, where you can reach them if you stretch the spring wide enough. A robotic voice speaks over some kind of tannoy.[line break][first custom style]'WELCOME TO THE SPRING STRETCH [']EXTRA CREDIT['] CLASS. YOU WILL BE RELEASED FROM YOUR BONDAGE AFTER TWO MINUTES. WHILE [NB in upper case] KEEPS [caps his of the player] KNEES PRESSED ON THE BUTTONS, THE VIBRATOR WILL TURN ON. WHILE [caps his of the player] KNEES ARE NOT BOTH PRESSED ON THE BUTTONS, [trainee-name of M in upper case] WILL BE FORCED TO DRINK THE LAXATIVE DRINK.[paragraph break]'IF [trainee-name of M in upper case] DRINKS TOO MUCH, [caps he of M] WILL BECOME HEAVIER THAN THE WEIGHT, AND BE LOWERED DOWN ONTO [NB in upper case][']S FACE.'.[paragraph break]'GOOD LUCK. YOUR TIME STARTS NOW.'[paragraph break][variable custom style]Oh god. If I don't keep my knees apart, I might get faceplanted by [trainee-name of M][']s messy diaper![roman type][line break]";
	let LAX be 0;
	...
	while T > 0:
		...
		set numerical response 2 to "[if KNEES is 1]Release the buttons, forcing [trainee-name of M] to drink laxative[otherwise if LAX > 0]Keep resting your legs, forcing [trainee-name of M] to drink more laxative[otherwise]Refuse to spread your legs and press the buttons, forcing [trainee-name of M] to drink laxative[end if]";
		if HOLD <= 0 and LAX >= 5:
		...
		if player-numerical-response is 2 and LAX is 5:
			say "[bold type]The weight lifts! [roman type][trainee-name of M] quickly falls until [his of M] diaper is enveloping your face. But [he of M] keeps coming! You are forced onto your back, with [his of M] [if HOLD <= 0]stinky [end if]diaper completely covering your head. You can barely breathe!";
			if HOLD <= 0, SmellGrossOut messyDiaperFacesitGrossnessLevel;
		otherwise if HOLD <= 0 and LAX >= 5:
			say "The smell of [trainee-name of M][']s horrible messy diaper fills your nostrils and mouth as you draw rasping breaths through [his of M] padding.";
...
Definition: team-scissor-lift-predicament is appropriate:
	...
	if enema fetish is 0 and diaper messing < 4, decide no;
...
Definition: team-scissor-lift-predicament is always-perceiving:
	if there is a trainee in the location of the player, decide yes; [while on a scissor lift tour, the two of you are always noticeable]
	decide no.
...
To execute (TSLP - team-scissor-lift-predicament):
	...
	if enema fetish is 1:
		if watersports fetish is 1:
			increase the slime volume of belly by 16;
	...
	while T >= 0:
		...
		while PNR is not 0:
			...
			if the player is thirsty:
				if diaper lover is 0 or watersports fetish is 1, set numerical response 1 to "drink some [if watersports fetish is 1][slime][otherwise]water[end if]";
			...
			if the bladder of the player >= 4, set next numerical response to "[slimedrain]";
			...
			if D is diaper:
				set next numerical response to "[if D is dirty]get [trainee-name of M] to change you out of[otherwise]remove[end if] your diaper";
			otherwise if diaper lover > 0:
			...
			otherwise if the chosen numerical response matches the text "drink some":
				say "You take one of the sports bottles and[if watersports fetish is 1 and the slime taste addiction of the player < 11], holding your nose,[end if] drink the [if watersports fetish is 1][slime][otherwise]whole thing[end if].";
				if watersports fetish is 1:
					if the slime taste addiction of the player < 7, say "[variable custom style][one of]Gross[or]Yuck[or]Blerugh![cycling][roman type][line break]";
					StomachSlimeUp (8 - the stomach-liquid of the player);
				otherwise:
			...
			otherwise if the chosen numerical response matches the text "urinate" or the chosen numerical response matches the text "slime":
			...
			otherwise if the chosen numerical response matches the text "your diaper":
				if D is dirty diaper:
					say "You lie on your back and close your eyes as [trainee-name of M] uses the changing items available to clean you up and change your diaper.";
			...
			otherwise:
				...
				if there is a held soiled-diaper:
					increase turnsWithSoiledDiaper by 5;
				otherwise:
					now turnsWithSoiledDiaper is 0;
				if the player is upset about mess:
					let messySources be 0;
					if there is a worn messed diaper, increase messySources by 1;
					if STDiaperMess > 0, increase messySources by 1;
					increase messySources by the number of carried soiled-diaper;
					let SGO be messyDiaperSmellGrossnessLevel + messySources - 1;
					if messySources is 1, say "You suffer through multiple hours of smell a veritably awful aroma.";
					otherwise say "The combined foul stench of [messySources] different poop-filled items nearby fills your nostrils for the entire time.";
					SlowGrossOut SGO;
				...
				otherwise:
					...
					repeat with XXX running from 1 to 3:
						...
						if B >= 0 and nowUrinating is false:
							...
							if debuginfo > 0, say "[input-style]Automatic wetting check: [SlimeContainer] ([bladder of the player]) - continence rating ([BRL]) = [B] ---> RNG([bladder-difficulty] ~ [B]) + RNG([bladder-difficulty] ~ [B]) = [R] | positive number[roman type][line break]";
							if R > 0 and the bladder of the player > 0:
								now nowUrinating is true;
								if legacy watersports mechanics is 1, say "During the hour, you find that you can't hold it any longer - you have to [slime] right now!";
								otherwise say "During the hour, you find that your slimeshooter can't hold on any longer - you have to drain it right now!";
								IrritableBladderUp 5;
				if mins < 0:
					increase mins by 60;
					decrease T by 1;
			if nowExpelling is true:
				...
				if D is diaper:
					...
					otherwise:
						...
						if D is diaper and debugmode > 0, say "[big slime] soak of [D]: [slime-soak of D]; Water soak of [D]: [water-soak of D]; Semen soak of [D]: [semen-soak of D].";
					now nowMessing is true;
				otherwise if diaper lover > 0:
				...
				otherwise:
					say "Demand to use [trainee-name of M][']s mouth?";
					if the player is consenting:
			...
			if nowMessing is true:
				if rectum > 1:
					...
					if the bladder of the player > 0 and nowUrinating is false:
						say "The act of messing your diaper also makes you [slimeonself].";
						now nowUrinating is true;
			...
			if nowUrinating is true:
				...
				if D is diaper:
					...
					if overflowed > 0:
						...
						if LL > 0: [just in case]
							now LLR is LL * 0.25;
							say "The [slime] that leaked out of your diaper drips through the platform, causing the weight reading to decrease!";
				otherwise if diaper lover > 0:
					say "[big slime] into a clean diaper?";
					if the player is consenting:
				...
				otherwise if watersports fetish is 1:
					say "[if legacy watersports mechanics is 1]Pee[otherwise]Shoot[end if] into [trainee-name of M][']s mouth?";
					if the player is consenting:
						say "[variable custom style]'Sorry about this...'[line break][speech style of M]'It's OK.'[roman type][line break][BigNameDesc of M] allows you to [if legacy watersports mechanics is 1]carefully straddle [his of M] face and pee[otherwise]point your slimeshooter at [his of M] face and carefully shoot[end if] into [his of M] mouth. ";
				...
				otherwise:
					now airPee is true;
				if airPee is true:
					if legacy watersports mechanics is 1, say "You [slime] straight through the platform grate. The entire weight of your [slime] gets lost to the ground!";
					otherwise say "You shoot straight through the platform grate. The entire weight of your slimeshooter's contents gets lost to the ground!";
					PuddleUp slime by the bladder of the player;
			...
			if tourStatus <= 0: [if above 0 then they are on a quick tour]
				if PNR is 0:
					if STEnema > 0 or total squirtable fill of belly > 0 or nowExpelling is true: [5 mins]
						...
						if STEnema > 0:
							say "[trainee-name of M] groans and clutches [his of M] belly.";
							if a random number between -1 and 1 <= the dedication of M:
								if diaper lover > 0:
									say "[speech style of M]'I can't hold it any longer, it's going to come out! Eeeek!'[roman type][line break][trainee-name of M] squeezes [his of M] eyes shut in shame as you watch [him of M] fill [his of M] diaper with a big helping of [if diaper messing >= 3]dirty [end if]enema water.[roman type][line break]";
									if diaper messing >= 3:
					...
					otherwise: [1 hour]
						if STRectum > the dedication of M + 4:
							say "[bold type][trainee-name of M] looks away in shame as [he of M] squats, grunts and fills [his of M] diaper with a big helping of mushy mess.[roman type][line break]";
							increase STDiaperMess by STRectum;
							now STRectum is 1;
							increase STDiaper by STBladder;
							now STBladder is 0;
						...
						otherwise if STBladder > the dedication of M + 4:
							if diaper lover > 0:
								say "[bold type][trainee-name of M] coughs awkwardly as a hissing sound fills the air. [roman type][big he of M] is wetting [himself of M], but trying not to draw attention to it. You don't say anything.";
								increase STDiaper by STBladder;
								now STBladder is 0;
							otherwise:
								say "[BigNameDesc of M] crosses [his of M] legs.[line break][speech style of M]'I'm sorry [NameBimbo], I really need to [slime][one of][or] again[stopping]! [big please], can I [slime] in your mouth? It's the only way!'[roman type][line break]";
								while STBladder > 0:
									say "[one of]Do you let [him of M] [slime] in your mouth[or]Keep holding your mouth under [his of M] stream[stopping]?";
									if the player is consenting:
										let STE be 4;
										if STBladder < 4, now STE is STBladder;
										say "[if STE is 4]A hot stream of[otherwise]The last of the[end if] [slime] flows directly into your mouth!";
										if the total volume of face + STE > 4:
											say "You are forced to swallow what's already in your mouth!";
											compute swallowing; [Must succeed to avoid infinite loops]
										FaceFill slime by STE;
										decrease STBladder by STE;
									otherwise:
										say "You pull away, and the rest of the [slime] flows down through the grated platform to the ground below.";
										HappinessDown M by 1;
						...
						if the total volume of face > 0:
							...
							set numerical response 3 to "demand [trainee-name of M] drinks it out of your mouth instead";
							compute multiple choice question;
							if player-numerical-response is 3:
								if the dedication of M <= 1:
								...
								otherwise:
									...
									MouthEmpty;
							if player-numerical-response is 2:
								say "[speech style of M]'No, wait!'[roman type][line break][trainee-name of M] tries to discourage you, but it's too late - you spit the [MouthfulDesc] of the side of the platform.";
								HappinessDown M by 1;
						...
						if STDiaperMess > 0:
							...
							compute multiple choice question;
							if player-numerical-response is 1:
								say "You get down on your knees and clean and change [trainee-name of M][']s messy diaper, forcing your face and nose getting awfully close to something truly awful in the meantime.";
						...
						otherwise if STDiaper is 1 and the dedication of M < 2:
							say "[trainee-name of M] seems to be feeling uncomfortable. Finally, [he of M] cracks and changes [his of M] wet diaper for a dry one. That means there's another soggy used diaper up there on the platform with you both.";
							let SD be a random off-stage soiled-diaper;
							if SD is soiled-diaper:
								add STDiaper to LDW;
								DiaperPrint SD from purple-medium-diaper;
						...
						if (a random number between 3 and 5) - STFood > the dedication of M:
							say "[trainee-name of M] eats some [if diaper lover > 0]baby food from [his of M] hamster feeder[otherwise][semen]-glazed food[end if].";
							now STFood is 6;
						if STDrink > 0:
							increase STBladder by 1;
						...
						if (a random number between 3 and 5) - STDrink > the dedication of M:
							say "[trainee-name of M] can't deal with [his of M] thirst any longer and drinks [his of M] fill of [if diaper lover > 0][milk][otherwise if watersports fetish is 1][slime][otherwise]water[end if].";
							now STDrink is 8;
						[Hourly changes]
						if debugmode > 0, say "[input-style]Before hourly changes. Food: [stomach-food of the player]. Drink: [stomach-liquid of the player]. [SlimeContainer]: [bladder of the player][roman type][line break]";
						...
						if debugmode > 0, say "[input-style]After hourly changes. Food: [stomach-food of the player]. Drink: [stomach-liquid of the player]. [SlimeContainer]: [bladder of the player][roman type][line break]";
				if LLR > 0:
					decrease W by LLR;
					say "The weight reading is now [bold type][W] LBS.[roman type][line break]";
					now LLR is 0.0;
	...
	if W > 347.0:
		...
		blandify and reveal C;
		now C is in Predicament01;
		if the player is in Predicament01, say "You can see two [ShortDesc of C]s here for you and [trainee-name of M], in an open unlocked box.";
	if W > 348.0:
		...
		now C is in Predicament01;
		if the player is in Predicament01, say "You can see two [ShortDesc of C]s here for you and [trainee-name of M], in an open unlocked box.";
	say "[trainee-name of M] [if W > 347.0]takes [his of M] clothing and [end if]quickly leaves, evidently eager to get the sneak home over and done with. It looks like you're going to have to make your way out of here on your own.";
...
This is the scissor lift used diaper focus rule:
	if (current-predicament is team-scissor-lift-predicament and the player is in a predicament room) or the player is upset about mess:
		repeat with SD running through carried soiled-diaper:
			focus-consider SD.
The scissor lift used diaper focus rule is listed first in the focus finding rules.
...
To say PredicamentDescription of (P - gloryhole-key-predicament):
	say "I'm currently in an Extra Credit training session where I need to suck cocks in the women's [SlimeTarget]s in return for keys, hoping to soon get the right key for the bike lock preventing me from taking the swimsuit with me, and then get back to my home while avoiding as many bystanders as possible from noticing me.".
...
To execute (P - gloryhole-key-predicament):
	...
	set up 29 bystanders;
	say "You find yourself in a dingy women's public [SlimeTarget]s, completely naked. On one wall, a gloryhole is glaringly obvious. Anchored to the single [SlimeTarget] stall with a strong bicycle lock is a blue swimsuit. A robotic voice is speaking to you from some hidden speaker:[paragraph break][first custom style]'WELCOME TO EXTRA CREDIT CLASS. [PredicamentRewardExplanation]YOU ARE FREE TO UNLOCK THE DOOR AND LEAVE AT ANY TIME, BUT THE KEY TO THE BIKE LOCK IS AMONGST SEVERAL OTHER KEYS ON THE TABLE IN THE MEN'S [caps SlimeTarget]S, ON THE OTHER SIDE OF THE GLORYHOLE. INSTRUCTIONS IN THE MEN'S [caps SlimeTarget]S RECOMMEND THAT ATTENDEES ARE TO PAY YOU FOR SEXUAL SERVICES WITH KEYS. YOU ARE IN A PARK CLOSE TO YOUR HOME. YOU MUST RETURN HOME TO COMPLETE THE PREDICAMENT.'[PredicamentRewardExplanationReaction][roman type][line break]And then, you hear female voices from outside the bathroom, beyond the locked door.[line break][second custom style]'Ummm... Hello? There's a line out here, you know!'[roman type][line break]Uh-oh, there's already people waiting outside. There's absolutely no way to leave the women's [SlimeTarget]s without being seen, and potentially photographed, by who knows how many people...".

An all later time based rule:
	if current-predicament is gloryhole-key-predicament and the player is in Toilet02 and navy-monokini is in Toilet02 and navy-monokini is stuck and bike-lock-key is in Toilet01 and the number of unseen-stranger in Toilet02 is 0:
		if a random number between 1 and 2 is 1:
			let M be a random unseen-stranger;
			set up M;
			now M is in Toilet02;
			...
			if watersports fetish is 1, add "drink my [if slimeshooter fetish is 1]slime[otherwise]piss[end if]" to sexActList;
			...
			while MPatience > 0:
				...
				if MPatience is 0:
					...
					otherwise if entry MNastiness in sexActList matches the text "piss" or entry MNastiness in sexActList matches the text "slime":
						say "[one of]guzzle my [slime] like a good [boy of the player][or]suck the [slime] out of my dick like a straw like an obedient little whore[in random order] ";
					otherwise:
						say "[one of]go ass-to-mouth on my [manly-penis] like a dirty fucking [SlimeTarget] whore[or]keep sucking your own ass-juices off my [manly-penis] until I blow my load[in random order] ";
			...
			if MPatience is -100:
				...
				otherwise if entry MNastiness in sexActList matches the text "asshole":
					...
					ruin asshole;
				otherwise if entry MNastiness in sexActList matches the text "piss" or entry MNastiness in sexActList matches the text "slime":
					say "You [if the player is upright]get on your knees and [end if]wrap your mouth around the tip of [his of M] [if slimeshooter fetish is 1]slimeshooter[otherwise][manly-penis][end if].[line break][speech style of M]'[T][roman type][line break]";
					now the stance of the player is 1;
					calm M;
					interest M;
					now M is penetrating face;
					FaceFill slime by a random number between 7 and 14;
				...
				otherwise: [ass to mouth]
					...
					ruin asshole;
		otherwise:
			say "You hear a voice from outside.[line break][second custom style]'[one of]Excuse me, I'm waiting!'[or]Is anyone even in there?!'[or]Finish up before I [slimeonself]!'[or]You're taking forever!'[or]What's the hold up?!'[or]Are you okay in there?!'[in random order][roman type][line break]".

To compute gloryhole key resolution of (M - an unseen-stranger):
...
team-blowjob-race-predicament has a number called trainee-sex-method. [0 - blowjob; 1 - trainee vaginal; 2 - trainee anal; 3 - player vaginal; 4 - player anal]
...
To say PredicamentDescription of (P - team-blowjob-race-predicament):
	say "I'm currently in an Extra Credit training session where I need to get back to my home while avoiding as many bystanders as possible from noticing me[if the orgasm-ticks of P >= 0]. But first I need to race to be the one to make the bound [man of male-m] in the middle of this room orgasm, reducing the time my opponent has to beat me by riding on a dildo pole when it's not my turn, because the loser remains locked in their wrist collar bar[end if].".

Definition: team-blowjob-race-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is dress or C is trousers or C is wrist bond, decide no;
	decide yes.
	...
	if current-predicament is team-blowjob-race-predicament and the player is in Predicament01 and the orgasm-ticks of team-blowjob-race-predicament >= 0:
		...
		otherwise:
			if blowjob-clock of team-blowjob-race-predicament is 60:
				...
				if RD > 2, now RD is 2;
				if RD < 0, now RD is 0;
				if M is nasty trainee or M is innocent trainee:
					now RD is 0;
				...
				now the trainee-sex-method of team-blowjob-race-predicament is RD;
			...
			if the trainee-sex-method of team-blowjob-race-predicament is 0:
		...
		if the orgasm-ticks of team-blowjob-race-predicament > 0:
			...
			if the blowjob-clock of team-blowjob-race-predicament <= 0:
				...
				otherwise:
					say "Instantly, [trainee-name of M] is reeled back in towards [his of M] wall, the [man of male-m][']s [manly-penis] popping out of [his of M] [if the trainee-sex-method of team-blowjob-race-predicament is 2]asshole[otherwise if the trainee-sex-method of team-blowjob-race-predicament is 1]pussy[otherwise]mouth[end if] with an audible sound. Your chain is loosened, allowing you to advance on the waiting [manly-penis].";
					if (a2m fetish > 0 or the player is possessing a vagina) and (M is nasty trainee or M is innocent trainee), say "[one of][speech style of M]'If you're thinking of doing what I think you're thinking of doing, just don't. There's no way I'm fucking this dude, and I don't want to have to taste you on [his of male-m] [manly-penis].'[roman type][line break][or][stopping]";
					now team-blowjob-race-predicament is reeled-out;
					if the trainee-sex-method of team-blowjob-race-predicament is 0, truncate the previous-sex-methods of team-blowjob-race-predicament to 0 entries;
					otherwise add the trainee-sex-method of team-blowjob-race-predicament to the previous-sex-methods of team-blowjob-race-predicament;
...
Check talking a trainee:
	...
	if current-predicament is team-blowjob-race-predicament and the player is in Predicament01 and face is not actually occupied:
		...
		unless TXT is "cancel":
			...
			say "[speech style of the noun]'[if the noun is amicable trainee][one of]That's not very nice[or]Oh[or]Let's not be so cruel to each other[in random order]...'[otherwise][one of]Fuck you[or]Shut up[or]Grrr[or]Zip it, skank[in random order]!'[end if][roman type][line break]";
			try waiting instead;
		do nothing instead.
...
To execute (NWP - teddy-walk-predicament):
	...
	now the bladder of the player is bladder-risky-level + 4;
	now the player is in Predicament02;
	say "You find yourself in an alleyway somewhere outside. You are wearing a babydoll dress that's open at the front... and then blocking the view of your front is the most bizarre and very heavy teddy bear you've ever seen. Your [player-penis] is inserted into a fleshlight where its butthole should be[if diaper quest is 0], which is masturbating you[end if]. Your mouth is full of the large bulbous teat of a pacifier that's fixed to the back of the head of the teddy. You can tell that it's so heavy, that if you don't hold it against you with all your strength, and keep your mouth and [player-penis] fixed to their parts too, you'll drop the teddy and then be unable to pick it back up.[paragraph break]You're also desperate to [slime].[paragraph break][first custom style]'WELCOME TO EXTRA CREDIT CLASS. [PredicamentRewardExplanation]TO EARN THIS TROPHY, YOU MUST SIMPLY RETURN HOME. YOUR TEDDY BEAR WILL [if watersports fetish is 1 and diaper quest is 0]SLOWLY PUMP ANYTHING IT COLLECTS DOWN BELOW BACK INTO YOUR MOUTH[otherwise if watersports fetish is 1]SLOWLY PUMP ANY [caps slime] IT COLLECTS DOWN BELOW BACK INTO YOUR MOUTH[otherwise]COLLECT ANYTHING EXCRETED DOWN BELOW INTO THE TEDDY BEAR'S DIAPER. BUT BE WARNED, IT'S ALREADY VERY FULL[end if].'[PredicamentRewardExplanationReaction][roman type][line break]".

To say PredicamentDescription of (P - teddy-walk-predicament):
	say "I'm currently in an Extra Credit training session where I need to get back to my home while avoiding as many bystanders as possible from noticing me. My giant teddy bear is [if diaper quest is 0 or watersports fetish is 0]masturbating me and [end if][if watersports fetish is 1]feeding me anything it collects down below[otherwise]slowly feeding me [milk][end if], and if I drop it I won't be able to pick it back up, exposing my genitals to the world.".
...
Definition: ddr-diaper-predicament is appropriate:
	...
	decide no.
Definition: ddr-diaper-predicament is bystanders-always-mess-aware:
	if the player is in Predicament01, decide yes;
	decide no.
		...
		if pointsNeeded > 0:
			...
			if rectum > 5:
				...
				[if DP is messed:
					...
					compute rapid picture taking of M witnessing T;] [this is now handled by diaper mess reaction function]
...
Definition: train-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is knickers or C is bra or C is bottom-exclusive or C is totally-exclusive, decide no;
	decide yes.
...
To execute (TRNP - train-predicament):
	...
	otherwise:
		...
		if a2m fetish is 2 and watersports fetish is 1, now the slime volume of belly is 20;
	...
	if diaper quest is 0:
		if a2m fetish > 0:
			say "'YOUR BELLY HAS BEEN FILLED WITH [if a2m fetish > 1 and watersports fetish > 0][caps slime][otherwise if a2m fetish > 1]TOMATO KETCHUP[otherwise]PING PONG BALLS[end if], AND IF YOU CAN'T HOLD IT IN, THE RESULT MIGHT MAKE YOUR LIFE EVEN MORE TRICKY...'";
		otherwise:
			...
			otherwise say ". EVERY TIME IT DETECTS YOU HAVING AN ORGASM, A SQUIRT OF [if watersports fetish > 0][caps slime][otherwise]EXTRA-STRONG MUSTARD[end if] IS ADDED TO THE TRACK.[paragraph break]";
	otherwise:
		say "'YOUR DIAPER HAS A SPECIAL HIGH-TECH STRIP DOWN THE MIDDLE. IT IS CURRENTLY GREEN, BUT IF YOU [if slimeshooter fetish is 1]DRAIN SLIME ON[otherwise]WET[end if] YOURSELF, IT WILL TURN WHITE. [if diaper messing >= 4]HOWEVER IF IT DETECTS POOP, IT WILL BE GREEN, EVEN IF THERE IS [caps slime] PRESENT TOO[otherwise]SO IF YOU CAN'T HOLD YOUR BLADDER, YOU WILL HAVE TO REMAIN ON THE TROLLEY UNTIL SOMEONE RESCUES YOU[end if].[paragraph break]";
	...
	while trolleyTime > 0:
		...
		if diaper quest is 1, now currentPressure is currentPressure * 2;
		otherwise now currentPressure is currentPressure * 10;
		...
		if (diaper quest is 0 and (a2m fetish is not 1 or the small egg count of belly > 0)) or (diaper quest is 1 and the bladder of the player > 0):
			...
			set numerical response 2 to "Split your focus between clearing the green path and holding [if diaper quest is 0 and a2m fetish > 0]your sphincter shut[otherwise if diaper quest is 1]onto your [SlimeContainer][otherwise]back your orgasm[end if] (half chance of losing control).";
			set numerical response 3 to "Focus entirely on holding [if diaper quest is 0 and a2m fetish > 0]your sphincter shut[otherwise if diaper quest is 1]onto your [SlimeContainer][otherwise]back your orgasm[end if] (one tenth chance of losing control).";
			set numerical response 0 to "Give up and wait for someone to discover you.";
			compute multiple choice question;
		otherwise:
			now stuff-to-hold is 0;
			reset multiple choice questions;
			set numerical response 1 to "Focus on clearing the green path as quickly as possible.";
			set numerical response 0 to "Give up and wait for someone to discover you.";
			compute multiple choice question;
		if player-numerical-response is 3:
				...
				while X is 3:
					...
					if TP > 0:
						...
						if diaper quest is 1 or watersports fetish is 0:
							...
							UnflinchingPainUp 10;
						otherwise:
							say "You hoover a small puddle of [slime] up from the floor, acutely aware that a few moments ago, this foul liquid was inside your own butthole.";
							TasteGrossOut 12;
							StomachSlimeUp 1;
						now X is 0;
				while X is 4:
		...
		if stuff-to-hold is 1 and player-numerical-response > 0:
			...
			if currentPressure < 1, now currentPressure is 1;
			...
			if RNG < currentPressure: [we don't hold on successfully]
				...
				if diaper quest is 1:
					say "You [slimeonself], huge amounts of [slime] flowing straight into the thick padding of your [plain-largish-diaper]. The green stripe disappears.";
					SlimeSoakUp plain-largish-diaper by the bladder of the player;
					now the bladder of the player is 0;
				otherwise if a2m fetish is 0:
					...
					otherwise:
						say "You cum hard. The automated predicament seems to somehow detect this, and you watch in dismay as a small puddle of [if watersports fetish is 1][slime][otherwise]mustard[end if] is squirted onto the track from a nozzle in the ceiling.";
						let LPDL be a list of numbers;
				...
				otherwise:
					say "A large, loud squirt of [if watersports fetish is 1][slime][otherwise]tomato ketchup[end if] comes squelching out of your butthole, coating the track behind you. Now you're going to have to lick that up, too...";
					if watersports fetish is 1:
						let LPDL be a list of numbers;
						repeat with ETP running from 1 to 4:
							if entry ETP in train-piss of train-predicament is 0, add ETP to LPDL;
		...
		otherwise if stuff-to-hold is 0:
		...
		if player-numerical-response is 0 or a random number between 10 and 50 < trolleyTime:
			if diaper quest is 1 and stuff-to-hold is 1 and player-numerical-response is 0:
				let TT be the square root of (50 - trolleyTime);
				while TT > 0:
					now currentPressure is (the square root of buildingPressure) + a random number between 0 and 4;
					if currentPressure > 9, now currentPressure is 9;
					now currentPressure is currentPressure * 10;
					say "You desperately try to hold onto your [SlimeContainer]... ";
					let RNG be a random number between 1 and 100;
					if debuginfo > 0, say "[input-style]Hold on check: d100 ([RNG]) | [currentPressure].5 difficulty rating[roman type][line break]";
					if RNG < currentPressure: [we don't hold on successfully]
						say "[bold type]But you can't hold on![roman type] You [slimeonself], huge amounts of [slime] flowing straight into the thick padding of your [plain-largish-diaper]. The green stripe disappears, replaced by a shameful shade of yellow.";
						SlimeSoakUp plain-largish-diaper by the bladder of the player;
						now the bladder of the player is 0;
						now TT is 0;
						now stuff-to-hold is 0;
					otherwise:
						say "And manage to do so for now!";
						increase buildingPressure by 1;
						decrease TT by 1;
			update appearance level;
			let PA be the appearance of the player;
...
Report going when current-predicament is bottle-jog-predicament:
	if the player is a park room and the player is upright and remote-controlled-vibrator is worn and sports-bottle is worn and black-exercise-shorts is worn and the doses of sports-bottle > 0:
		now temporaryYesNoBackground is figure of bottle jog predicament;
		let N be the number of bystanders in the location of the player;
		say "Suck some [semen] out of the sports bottle as you go? There [if N is 1]is one person[otherwise]are [N] people[end if] nearby[if N is 0].[otherwise], [end if]";
		if N > 0, say "which would make doing this [if N is 1]a bit[otherwise if N is 2]rather[otherwise if N is 3]very[otherwise]deeply[end if] [one of]shameful[or]humiliating[cycling].";
		if the player is consenting:
			try drinking sports-bottle;
			if N is 1, say slightHumiliateReflect;
			if N is 2, say moderateHumiliateReflect;
			if N is 3, say strongHumiliateReflect;
			if N >= 4, say severeHumiliateReflect;
		now temporaryYesNoBackground is figure of small image.
...
To execute (PP - pong-predicament):
	...
	say "A cold robotic voice begins to talk.[line break][first custom style]'TO EARN THIS TROPHY, YOU MUST RETURN HOME... AFTER PLAYING A GAME OF BEER PONG. IN FRONT OF YOU ARE TEN CUPS, EACH FILLED WITH A LARGE DOSE OF [if PPVariant is 0][caps slime], DISCREETLY COLLECTED FROM THE NEARBY PUBLIC [caps SlimeTarget]S[otherwise if PPVariant is 1 and diaper messing >= 4]A LAXATIVE DRINK[otherwise if PPVariant is 1]A DIURETIC DRINK[otherwise]FRESH HUMAN SEMEN[end if]. EACH CUP HAS A SENSOR, AND WHEN IT DETECTS A PING PONG BALL INSIDE IT, IT WILL PUMP ITS CONTENTS INTO YOUR MOUTH. THE WINNER WILL BE RELEASED FIRST, AND THEREFORE ABLE TO CLAIM THE SKIRT FOR THE WALK HOME[if PPVariant is 2]. FURTHERMORE, ALL OF THE WINNER'S REMAINING CUPS WILL BE PUMPED INTO THE LOSER'S PUSSY[end if].[paragraph break]'ONE PING PONG BALL WILL BE DISPENSED EVERY TWENTY SECONDS. YOUR GAME STARTS NOW.'[roman type][line break]";
	if PPVariant is 0 and legacy watersports mechanics is 1:
		...
		otherwise:
			...
			if PPVariant is 0:
				unless true is listed in player-cups of pong-predicament: [first cup]
					if the slime taste addiction of the player < 6, say "[variable custom style]This can't be happening! THIS CAN'T BE HAPPENING! GROSS!!![roman type][line break]";
					if legacy watersports mechanics is 1, now temporary-map-figure is figure of pong predicament piss player drink;
				compute slightly addictive swallowing of slime by 1;
			otherwise if PPVariant is 1:
				unless true is listed in player-cups of pong-predicament: [first cup]
					say "As soon as the liquid hits your belly, [if diaper messing >= 4]you can feel it start to rumble[otherwise]you can feel your [SlimeContainer] start to twinge[end if].[line break][variable custom style]Uh-oh...[roman type][line break]";
		...
		if false is listed in the player-cups of pong-predicament:
			...
			if diaper quest is 1:
				if plain-largish-diaper is dirty:
					...
					if diaper messing < 4, now DSF is the slime-soak of plain-largish-diaper;
					let DAR be a random number between 1 and (50 + (the diaper addiction of the player * 10));
					if DAR < DSF, SlowDiaperAddictUp 1;
				if suppository > 0:
				...
				if debugmode > 0, say "[SlimeContainer]: [bladder of the player].";
				if diaper-dry is true and plain-largish-diaper is dirty:
					if DPR <= opponent-diaper-pressure:
						say "At the same time, [NameDesc of M] loses control of [his of M] own [if diaper messing >= 4]bowels[otherwise][SlimeContainer][end if]. You both stand there, at opposite ends of the table, audibly filling your diapers with your hot, steaming [if diaper messing >= 4]filth[otherwise][slime][end if]. Both your diapers can be seen to visibly sag and expand between your legs. It really is an outrageous sight to behold.";
					...
					refresh the graphics-window;
				if DPR <= opponent-diaper-pressure:
					say "[speech style of M]'Oh... Oh... [one of]Ah dun wun to...'[roman type][line break]Suddenly, [or]Noh ahain...'[roman type][line break]Once again, you watch as [stopping][NameDesc of M] clutches [his of M] stomach, groans, and unleashes a torrent of [if diaper messing >= 4]sludge[otherwise][slime][end if] into [his of M] thick padding, causing it to visibly sag and expand.[line break][speech style of M]'Ho gwosh...'[roman type][line break][big he of M] mumbles to [himself of M].";
	...
	otherwise:
		...
		if PPVariant is 2:
			...
			say "You watch as the rest of [NameDesc of M][']s cups are drained, and all the [semen] is pumped directly into your [vagina].[line break][variable custom style]'EEEEEEEE!'[roman type][line break]You squeal loudly as millions, if not billions of fertile swimmers, surge up inside you and straight towards your waiting Fallopian tubes, with immense speed and pressure. Meanwhile, [trainee-name of M] is released. ";
			PussyFill cumLoad;
			cutshow figure of pong predicament semen loss;
		say "The metal bar attached to [trainee-name of M][']s chastity belt unclicks, allowing [him of M] to leave first. Before leaving, [he of M] makes sure to unpin [NameDesc of tartan miniskirt] from the wall, and wear it, protecting [his of M] modesty.";
		try M going north;
		now M is in Facility01;
		deinterest M;
...
Definition: toilets-dildos-predicament is appropriate:
	if diaper quest is 0 and the rank of the player >= 2 and domino-mask is off-stage and there is an off-stage specific-key, decide yes;
	decide no.
...
Figure of toilets-dildos-predicament is the file "Env/Facility/toiletdildopredicament1.jpg".
...
To say PredicamentDescription of (P - toilets-dildos-predicament):
	say "I'm currently in an Extra Credit training session where I need to suck dildos in the women's [SlimeTarget]s to open boxes to alleviate my predicament, with only 60 seconds until the glass goes transparent and reveals me, and then get back to my home while avoiding as many bystanders as possible from noticing me.".
...
Definition: toilets-dildos-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is knickers or C is trousers or C is hobble-skirted, decide no;
		if C is penetrating a fuckhole, decide no;
	decide yes.
...
To compute coloured dildo sucking:
	...
	if the smart-glass-timer of toilets-dildos-predicament <= 0:
		repeat with M running through bystanders in Park02:
			...
			say strongHumiliateReflect;
			if M is camera-bystander:
				let TPT be the substituted form of "you on your knees, sucking on a large wall-mounted coloured dildo in a public [SlimeTarget]s with transparent walls";
				if infamy fetish > 0:
...
snitch-predicament is a predicament. snitch-predicament has a number called snitch-time. snitch-predicament has a number called snitch-height.
Definition: snitch-predicament is realistic-arms-requiring: decide yes.
To say defaultarmsflav of (P - snitch-predicament):
	say "([bold type]and in this predicament, because of your starting situation, to make that happen you would have to manually change your priorities with 'adjust arms'[roman type])".
The printed name of snitch-predicament is "[TQlink of item described]gold quadcopter[TQxlink of item described][shortcut-desc][verb-desc of item described]". The text-shortcut of snitch-predicament is "snitch". Understand "golden", "gold", "quadcopter", "drone" as snitch-predicament. snitch-predicament can be broom-creampied. snitch-predicament is portable.

Definition: snitch-predicament is appropriate:
	if diaper quest is 0 and the rank of the player >= 2 and the player is possessing a vagina and provocative alchemist outfit is off-stage, decide yes;
	decide no.
Definition: snitch-predicament is fixed-clothing-eligible:
	if there is a worn predicament-reappearing clothing penetrating a body part, decide no;
	decide yes.
...
Figure of snitch is the file "Env/Facility/snitchpredicament1.jpg".
...
To say PredicamentDescription of (P - snitch-predicament):
	say "I'm currently in an Extra Credit training session where I need to find the golden quadcopter, and catch it, in order to unlock my broomstick and front door.".
...
To execute (P - snitch-predicament):
	...
	summon provocative alchemist outfit uncursed;
...
An all later time based rule (this is the quadcopter rule):
	if current-predicament is snitch-predicament and the player is in a predicament room and the player is not in Predicament20:
		...
		if snitch-predicament is held:
			focus-consider snitch-predicament;
		otherwise: [snitch flying]
...
[Definition: anal-beads-swimsuit-predicament is realistic-arms-requiring: decide yes.] [testing with this disabled so that you can't just remove everything]
...
To say PredicamentDescription of (P - anal-beads-swimsuit-predicament):
	say "I'm currently in an Extra Credit training session where I need to get back to my house. The top of the swimsuit only covers my breasts if I keep my hands behind my neck. Every time I walk on my heels, it pumps up the anal beads of my swimsuit a bit bigger.".
...
Definition: joint-fuckhole-predicament is appropriate:
	if diaper quest is 0 and the rank of the player > 2, decide yes;
	decide no.
...
Definition: joint-fuckhole-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is neck covering, decide no;
		if C is penetrating a fuckhole, decide no;
	decide yes.
...
To execute (P - joint-fuckhole-predicament):
	...
	let M be a random unseen-stranger;
	let N be a random unseen-stranger;
	...
	while M is N:
		now N is a random unseen-stranger;
	while player-released is false and rival-released is false:
		...
		if watersports fetish is 1:
			if R > 1:
				decrease R by 1;
			otherwise:
				set numerical response 2 to "CRUEL - RIVAL DRENCHED IN [if slimeshooter fetish is 1]SLIME[otherwise]PISS[end if]";
				now R is 99999;
		...
		if the player is not getting very lucky and (a random number between -2 and 5 < rival-animosity or the player is getting unlucky) and (rounds-completed < 10 or a random number between 1 and 3 > 1): [33% chance of rival giving up each round at 100% chance of release]
			...
			otherwise: [There's no surprise waiting in the next phase]
				if T matches the text "PISS" or T matches the text "SLIME":
					say "Suddenly, and without warning, powerful water jets start blasting at your face and upper torso from all angles. The smell is unmistakable - you're being doused in huge amounts of warm [slime]!";
					AnnouncedSquirt slime on face by 50;
					AnnouncedSquirt slime on breasts by 20;
					FearUp 1 with reason "As you jump with shock from being blasted by [slime],";
					now the slime-puddle of Predicament01 is 0;
		...
		if rival-released is true and team-predicament-partner is in Predicament01, now team-predicament-partner is in Facility01; [just in case]
	now temporaryYesNoBackground is figure of small image;
...
Definition: diaper-maze-predicament is appropriate:
	if diaper quest is 1 and the rank of the player > 2, decide yes;
	decide no.
...
Definition: diaper-maze-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is not headgear, decide no;
	decide yes.
...
To execute (P - diaper-maze-predicament):
	...
	while player-progress of P < 25:
		...
		if player-progress of P < 25:
			...
			check enema holding with reason T;
			compute facility periodic effect of diaper-box;
			follow the mess gross out resolution rule;
			force immediate inventory-focus redraw;
			...
			otherwise if R is 2 or R is 11: [masturbation]
				...
				if player-numerical-response is 1:
					...
					stimulate vagina from P;
					check for arousal change;
			otherwise if R is 3 and enema fetish is 0: [enema]
			...
			otherwise if R is 4: [wet block]
				...
				set numerical response 1 to "take the shortcut[if D is wet] - you're already wet![otherwise] by purposefully getting [slime] on yourself[end if]";
				set numerical response 2 to "take the longer route";
				compute multiple choice question;
				if player-numerical-response is 1:
					if D is wet:
					...
					otherwise:
						say "You release your [SlimeContainer], and let your warm [slime] soak into the padding of your [MediumDesc of D].";
...
To say PredicamentDescription of (P - business-briefcase-predicament):
	say "I'm currently in an Extra Credit training session where I need to get back to my home while my briefcase periodically spits out business cards with a dangerous QR code on them that will risk me being exposed online. But I'm wearing a very short dress, and special stockings that shock me if I try to bend my knees, so when I bend over to pick up a business card, I expose what's underneath my dress.".
...
Figure of business card is the file "Env/Facility/businesscard1.jpg".
...
Report taking when current-predicament is business-briefcase-predicament: [we do this in the report phase so that the uses arms have already been properly allocated]
	if the player is in a predicament room and pinstripe business dress is worn and the player is upright and the player is not in Predicament01 and the player is not in Toilet01 and the player is not in Toilet02 and the player is not in Predicament20:
		...
		if the player is in a road room and the player is not getting lucky, compute car coming past;
...
Definition: business-briefcase-predicament is fixed-clothing-eligible:
	if there is worn predicament-reappearing knickers, decide no;
	decide yes.
		...
		if diaper quest is 0:
			...
			otherwise if the player is in Toilet02:
				if house-key is in Toilet01:
					...
					otherwise:
						say "[one of][variable custom style]The video has already been shared far and wide. I might as well just go pick up the key in the men's [SlimeTarget]s myself.[roman type][line break][or][or][cycling]";
		if business-briefcase-predicament is not predicament-failed:
...
Arena01, Arena02, Arena03, Arena04, Arena05, Arena06, Arena07, Arena08, Arena09 are in the Facility.
...
To execute (BKP - balloon-kart-predicament):
	...
	now map-zoom is 0;
	summon sporty crop top uncursed;
	let LST be the list of on-stage trainees;
	sort LST in random order;
	...
	repeat with P running through LP:
		...
		if P is trainee:
			now the trainingInt1 of P is 0; [arousal]
			now the trainingInt2 of P is 0; [soreness]
	...
	let L be semen;
	if bukkake fetish is 0, now L is slime;
	...
	repeat with P running through LP:
		if P is trainee:
			...
			otherwise:
				...
				now the trainingInt2 of P is 6;
	...
	while the tempInt2 of the player < 3:
		...
		repeat with P running through LP:
			if P is a trainee and the tempInt2 of the player < 3 and the tempInt2 of P < 3:
				...
				otherwise:
					...
					if (forward-priority is 0 or forward-priority is -1) and (the dedication of P + forward-priority) < the trainingInt2 of P: [feeling sore, and nothing urgent that means we need to go forward or turn]
						...
						now the trainingInt2 of P is the trainingInt2 of P / 2;
					...
					if forward-ability is 1 and (a random number between 0 and 1) <= forward-priority: [move forward]
						...
						if a random number between 3 and 10 < the trainingInt1 of P:
							say "You watch [trainee-name of P] writhe and shudder on [his of P] dildo as [he of P] experiences a powerful orgasm! [big he of P] starts gasping for breath, and looking a little dazed. ";
							now the trainingInt1 of P is 0;
							now the	tempInt3 of P is 2;
						otherwise:
							increase the trainingInt2 of P by 1; [getting sore]
						now P is in T;
					...
					increase the trainingInt1 of P by 1;
...
To compute potential balloon pop of (P - a person):
	if the number of people in the location of P > 1:
		...
		say "[if P is the player]Your[otherwise][trainee-name of P][']s[end if] kart slams into [if M is the player]yours[otherwise][trainee-name of M][']s[end if]! ";
		...
		otherwise:
			...
			if successfulPop is true:
				if the tempInt2 of M < 3: [they are still in their kart]
					...
					otherwise:
						say "[speech style of M]'[one of]Eek[or]Nooo[or]Gross[or]Aaaah[then at random]!'[roman type][line break][trainee-name of M] squeals [one of]in panic[or]with disgust[or]with despair[cycling].";
						if P is the player, HappinessDown M;
				otherwise:
					say "A balloon is burst open, drenching the empty kart seat in [variable L].";
			otherwise:
				say "[one of]The spike narrowly misses a balloon[or]The spike touches a balloon, but not hard enough to pop it[or]The spike completely misses a balloon[then at random]! [if P is the player][GotUnluckyFlav][otherwise if M is the player][GotLuckyFlav][otherwise][trainee-name of M] got lucky there.[end if]";
		if the travel-direction of P is the travel-opposite of M and the tempInt1 of P > 0:
			...
			if successfulRevengePop is true:
				if M is the player, say "At the same time, your spike connects with one of [trainee-name of P][']s balloons, bursting it and drenching [him of P]!";
				otherwise say "[if the tempInt1 of M > 0]At the same time, [trainee-name of M][']s spike slices[otherwise]But [trainee-name of M][']s spike is in the right place to slice[end if] open one of [if P is the player]your[otherwise][trainee-name of P][']s[end if] balloons!";
				if P is the player:
				...
				otherwise:
					say "[speech style of P]'[one of]Eek[or]Nooo[or]Gross[or]Aaaah[then at random]!'[roman type][line break][trainee-name of P] squeals [one of]in panic[or]with disgust[or]with despair[cycling].";
				...
				say "[bold type][if P is the player]Your[otherwise][trainee-name of P][']s[end if] kart has [tempInt1 of P] balloon[if tempInt1 of P is not 1]s[end if] left. [if M is the player]You have[otherwise][trainee-name of M] has[end if] popped [tempInt2 of M] balloon[if tempInt2 of M is not 1]s![otherwise].[end if][roman type][line break]";
				if M is trainee and the tempInt2 of M is 3:
			...
			otherwise:
				say "[one of]The spike narrowly misses a balloon[or]The spike touches a balloon, but not hard enough to pop it[or]The spike completely misses a balloon[then at random]! [if P is the player][GotLuckyFlav][otherwise if M is the player][GotUnluckyFlav][otherwise][trainee-name of P] got lucky there.[end if]";
		if successfulPop is true:
			...
			say "[bold type][if M is the player]Your[otherwise][trainee-name of M][']s[end if] kart has [tempInt1 of M] balloon[if tempInt1 of M is not 1]s[end if] left. [if P is the player]You have[otherwise][trainee-name of P] has[end if] popped [tempInt2 of P] balloon[if tempInt2 of P is not 1]s![otherwise].[end if][roman type][line break]";
		otherwise if M is trainee and the tempInt2 of M is 3:
		...
		if P is trainee and the tempInt2 of P is 3:
...
Definition: free-use-fuckhole-predicament is appropriate:
	if diaper quest is 0 and the rank of the player > 4, decide yes;
	decide no.
Definition: free-use-fuckhole-predicament is realistic-arms-requiring: decide yes.
To say PredicamentDescription of (P - free-use-fuckhole-predicament):
	say "I'm currently in an Extra Credit training session where I need to get the ice to melt so that I can escape my bondage, and then return home, optionally first retrieving the key to my wrist collar bar from the warehouse to the west.".
...
Definition: free-use-fuckhole-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is wrist bond, decide no;
	decide yes.
		...
		otherwise:
			if a random number between 1 and 3 is 1:
				...
				if current-stance is 0:
					say "[speech style of M]'[if MF is 0][one of]Oh I am going to ruin you[or]What sort of nasty skank would let people fuck [him of the player] in a [SlimeTarget][in random order]...'[otherwise if MF is 1][one of]Sweet, free [cunt][or]Free use? Fuck yes[in random order]!'[otherwise if MF is 2][one of]This is going to be fun[or]Holy shit, this is so fucking hot[in random order].'[otherwise if MF is 3][one of]Oh wow, is this real[or]Am I dreaming[in random order]?'[otherwise][one of]I'm not sure I can morally do this without talking to [him of the player] first[or]This is certainly an ethical grey area[in random order]...'[end if][roman type][line break]";
				otherwise:
					now man-seen-sign is true;
...
Definition: porno-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is dress or C is neck covering, decide no;
	decide yes.
...
To execute (P - porno-predicament):
	...
	otherwise:
		compute predicament bladder and bowel filling;
	...
	otherwise say "[BigNameDesc of ST] chose to wear [if STD is 1]a frilly dress[otherwise if STD is 2]a sexy alchemist outfit[otherwise if STD is 3]nothing but a latex diaper cover[otherwise if STD is 4]nothing but a collar that says [']BABY['] on it[otherwise]nothing but a pair of anklecuffs[end if], earning [STD] Porn Point[if STD > 1]s.";
	if diaper quest is 0:
	...
	let X be 0;
	if player-numerical-response > 0, now X is (player-numerical-response + a random number between 1 and 5) / 2;
	...
	let M be a random off-stage unseen-stranger;
	if diaper quest is 0:
	...
	repeat with MX running through monsters in Predicament01:
		if MX is unseen-stranger and diaper quest is 0:
	...
	compute multiple choice question;
	let MN be the number of unseen-strangers in the location of the player;
	...
	otherwise:
		...
		if player-numerical-response is 4 or player-numerical-response is 5:
			if diaper quest is 0, say "[variable custom style]'[if interracial-scene is true]Err, ah... oh god... I'm about to get fucked up by four giant black cocks[otherwise if watersports-scene is true]Yeah, I'm going to be such a nasty slime slut for you all[otherwise if gangbang-scene is true]A greedy slut like me loves four big cocks at a time, baby[otherwise]Mmm, get out your cock and stroke it while you watch me get treated like I deserve, baby[end if]!'[roman type][line break]";
			otherwise say "[variable custom style]'Uh-oh... I think I might need to go [SlimeTargetP]...'[roman type][line break]";
		say severeHumiliateReflect;
	check for arousal change;
	if player-escaped is false:
		...
		if diaper quest is 0:
			repeat with MM running through unseen-strangers:
		...
		otherwise if M is unseen-stranger and diaper swapping >= 2:
			now the bladder of M is 1000;
			say "[BigNameDesc of M] takes out [his of M] [manly-penis] and prepares to [slimedrain] inside your diaper.";
			reset multiple choice questions;
			set numerical response 1 to "Completely refuse to help [him of M] degrade you like this.";
			set numerical response 2 to "Pull the waistband of your diaper forward for [NameDesc of M], to make it easier for [him of M] to [slimedrain] inside it.";
				...
				repeat with MM running through unseen-strangers:
					...
					otherwise:
						...
						otherwise if watersports-scene is true and a random number between 1 and 2 is 1:
							say "[BigNameDesc of MM] points [his of MM] [manly-penis] at your face and lets out a stream of [slime].";
							reset multiple choice questions;
							...
							if player-numerical-response is 1:
								decrease PP by a random number between 0 and 1;
								say "[BigNameDesc of M] chuckles as your face gets completely drenched in [his of M] [slime].";
								compute slime hitting face;
								GrossOut 3;
							otherwise if player-numerical-response is 2:
								if a random number between 0 and 1 is 1, decrease PP by a random number between 0 and 1;
								say "You hold your mouth open and let the [slime] splash against it like a water feature. [strongHumiliateReflect]";
		...
		if diaper quest is 0:
			...
			otherwise:
				repeat with MM running through unseen-strangers:
					...
					otherwise:
						let MMA be a random unseen-stranger penetrating asshole;
		...
		otherwise:
			...
			otherwise if baby-medium-diaper is clean:
				...
				set numerical response 2 to "Purposefully [slimeonself]";
				if diaper messing >= 4, set numerical response 3 to "Purposefully [slimedrain] on and mess yourself";
				compute multiple choice question;
				if player-numerical-response is 1:
					decrease PP by a random number between 1 and 2;
				otherwise if player-numerical-response is 2:
					say "You release your hold on your [SlimeContainer], and soil your diaper for the cameras.[line break][speech style of M]'Oopsie, looks like I spoke too soon!'[roman type][line break][BigNameDesc of M] laughs, as [he of M] strokes your warm, swelling padding.";
					AnnouncedExpel slime on baby-medium-diaper by the bladder of the player;
					now the bladder of the player is 0;
					say strongHumiliateReflect;
					if diaper messing >= 4, decrease PP by a random number between 0 and 1;
				otherwise if player-numerical-response is 2:
					say "You release your hold on your [SlimeContainer] and bowels, and mess your diaper for the cameras.[line break][speech style of M]'Oopsie, looks like I spoke too soon!'[roman type][line break][BigNameDesc of M] laughs, as [he of M] strokes your warm, swelling padding.";
					AnnouncedExpel slime on baby-medium-diaper by the bladder of the player;
					now the bladder of the player is 0;
			...
			if watersports-scene is true:
				if a2m fetish >= 2:
					...
					if MM is nothing: [just in case something's gone weird]
						now MM is M;
						now MM is penetrating asshole;
					say "[BigNameDesc of MM] positions just the tip of [his of MM] [if slimeshooter fetish is 1]slimeshooter[otherwise][DickDesc of shopkeeper][end if] inside your [asshole], and then a warm, heavy feeling begins to flood inside you.[line break][variable custom style][he of MM][']s [slime]ing in my ass![roman type][line break]Within a few short moments, the pressure has become too much, and you desperately need it to stop, and begin to exit rather than enter you![paragraph break][BigNameDesc of MM] suddenly pulls out, and instantly a rush of [slime] floods out of your [asshole] in a big gush.[line break][variable custom style]'Aaaaaah!'[roman type][paragraph break]What you didn't know, but find out immediately after, is that the group were holding a large wine glass in front of your [asshole] as you expelled your [slime] enema. And now a glass of [if slimeshooter fetish is 1]extra gross [slime][otherwise]ass-[slime][end if] is being brought towards your lips...[line break][speech style of MM]'Here, have a nice refreshing drink, straight from the tap...'[roman type][line break]";
					reset multiple choice questions;
					set numerical response 0 to "Turn away";
					...
					if player-numerical-response is 0:
						...
						AnnouncedSquirt slime on hair by 15;
					otherwise if player-numerical-response is 1:
						decrease PP by 1;
					...
					otherwise:
						...
						say obsceneHumiliateReflect;
				otherwise:
					say "The four [men of M] gather round your face... And what comes next is truly sordid. Four powerfut, hot streams of [slime] surge towards your face.";
					reset multiple choice questions;
					set numerical response 0 to "Turn away";
					set numerical response 1 to "Hold your face steady";
					set numerical response 2 to "Open your mouth";
					...
					if player-numerical-response is 0:
						decrease PP by a random number between 1 and 2;
						say "You do your best to turn away and prevent as much [slime] as possible from getting near your eyes, nose or lips. But it's hopeless - your hair and face and body still get drenched in the stuff.";
						AnnouncedSquirt slime on hair by 50;
					otherwise if player-numerical-response is 1:
						decrease PP by a random number between 0 and 1;
						say "You close your eyes and allow [if MN > 1]the [men of M][otherwise][NameDesc of M][end if] to desecrate your face with their [slime].";
						AnnouncedSquirt slime on face by 50;
						say severeHumiliateReflect;
					otherwise if player-numerical-response is 2:
						if a random number between 0 and 1 is 1, decrease PP by a random number between 0 and 1;
						say "You close your eyes, open your mouth, and allow [if MN > 1]the [men of M][otherwise][NameDesc of M][end if] to use your mouth as a [SlimeTargetU]. ";
						AnnouncedSquirt slime on face by 50;
						FaceFill slime by 2;
						say "Not only are you drenched in [slime], but you also have a [MouthfulDesc].";
						say obsceneHumiliateReflect;
					otherwise:
						say "You glug and chug mouthful after mouthful of [slime]. But it's coming so thick and fast, that for every mouthful you swallow, several more coat your face and body.";
						AnnouncedSquirt slime on face by 40;
						StomachSlimeUp 15;
						say obsceneHumiliateReflect;
						say "By the end of your sordid display, you feel extremely bloated, stomach full up to the brim with these guys['] [slime].";
				...
				otherwise:
					...
					otherwise:
						...
						if the semen-puddle of Predicament01 > 0:
							decrease the semen-puddle of Predicament01 by 1;
							FaceFill semen by 1;
						if the slime-puddle of Predicament01 > 0:
							decrease the slime-puddle of Predicament01 by 1;
							FaceFill slime by 1;
						if the slime-puddle of Predicament01 > 0:
							decrease the slime-puddle of Predicament01 by 1;
							FaceFill slime by 1;
		...
		if player-points of P >= rival-points of P:
			...
			repeat with BC running through worn clothing:
				fully clean BC;
			if player-points of P is rival-points of P, say "By the time you leave the shower, [trainee-name of ST] has gone.";
		otherwise:
...
tug-of-war-predicament is a team-predicament. tug-of-war-predicament has a number called player-soil. tug-of-war-predicament has a number called rival-soil.

Definition: tug-of-war-predicament is appropriate:
	if diaper quest is 1 and the rank of the player > 2, decide yes;
	decide no.

Definition: tug-of-war-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is knickers or (C is crotch covering and C is totally-exclusive), decide no;
	decide yes.

To execute (TWP - tug-of-war-predicament):
	let ST be team-predicament-partner;
	now ST is in Predicament01;
	now the stance of the player is 1;
	now the arousal of the player is 1000;
	update arousal;
	now refractoryperiod is 0;
	summon blue-velcro-diaper locked;
	let K be a random off-stage specific-key;
	if K is a thing:
		now K is covering blue-velcro-diaper;
		now K is in Park24;
		now blue-velcro-diaper is predicament-fixed;
	summon babydoll-top uncursed;
	blandify and reveal babydoll-skirt;
	now babydoll-skirt is in Predicament01;
	compute predicament bladder and bowel filling;
	now the player is in Predicament01;
	display stuff;
	say "[bold type]Suddenly you find yourself standing not far away from [NameDesc of ST], with both of you facing each other! [big he of ST] is wearing a babydoll top, and is locked in a thick diaper. You are wearing the same. You're tied to each other at the waist by a short rope with an electronic release mechanism! Wait... Between you, is a pit full of [if diaper messing < 7][slime]-soaked[otherwise]poop-filled used[end if] diapers! As you lock eyes, you both realise what the other might be about to do, and do the same. Grabbing the rope near your waist, you begin to pull, beginning a furious tug of war, where the loser will fall into the used diaper pit.[line break][first custom style]'WELCOME TO EXTRA CREDIT CLASS. [PredicamentRewardExplanation]TO EARN THIS TROPHY, YOU MUST SIMPLY... RETURN HOME. IT SEEMS YOU HAVE ALREADY GRASPED THE CORE CONCEPT OF THIS PREDICAMENT. WHEN ONE OF YOU FALLS INTO THE PIT, THE ROPE WILL BE RELEASED, AND YOU CAN BOTH LEAVE. HOWEVER THERE IS ONLY ONE SKIRT AVAILABLE FOR THE WALK HOME. THE WINNER OF THE TUG OF WAR WILL SURELY BE THE ONE WHO CAN FIRST CLAIM OWNERSHIP OF IT[if K is a thing]. THE KEY TO YOUR DIAPERS IS CURRENTLY IN THE POSSESSION OF AN ANGEL IN THE PARK[end if].'[roman type][line break]And so, just like that, you are locked into a fierce tug of war with [NameDesc of ST]! Who will relent first? And to add to the challenge, you can feel that you desperately need the [SlimeTarget]. Will the game be over before either of you has lost control of their [SlimeContainer][if diaper messing > 3] and bowels[end if]?!";
	let tug-status be 0;
	while tug-status > -4 and tug-status < 4:
		now temporaryYesNoBackground is Figure of tug of war predicament;
		if tug-status > 0:
			say "You are currently [if tug-status is 1]slightly[otherwise if tug-status is 2]close to[otherwise]very close to[end if] winning the tug-of-war.";
		otherwise if tug-status < 0:
			say "You are currently [if tug-status is -1]slightly[otherwise if tug-status is -2]close to[otherwise]very close to[end if] losing the tug-of-war.";
		otherwise:
			say "You are currently neither winning nor losing the tug-of-war.";
		if the player-soil of TWP is 0, say "You desperately need to pee[if diaper messing >= 4] and poop[end if].";
		if the player-soil of TWP is 1, say "You desperately need to poop.";
		[if the rival-soil of TWP is 1, say "[trainee-name of ST] has wet [himself of ST].";
		if the rival-soil of TWP is 2, say "[trainee-name of ST] has messed [himself of ST].";]
		let player-can-soil be false;
		let rival-can-soil be false;
		if the player-soil of TWP is 0 or (the player-soil of TWP is 1 and diaper messing >= 4), now player-can-soil is true;
		if the rival-soil of TWP is 0 or (the rival-soil of TWP is 1 and diaper messing >= 4), now rival-can-soil is true;
		reset multiple choice questions;
		set numerical response 1 to "pull hard[if player-can-soil is true] (50% chance of soiling self)[end if]";
		if player-can-soil is true:
			set numerical response 2 to "pull normally (10% chance of soiling self)";
			set numerical response 3 to "pull gently (will not soil self)";
		set numerical response 0 to "give up";
		compute multiple choice question;
		let player-tug be 4;
		let rival-tug be 3;
		if player-numerical-response is 0:
			say "You give up, and stop pulling the rope.";
			now tug-status is -4;
		otherwise:
			if rival-can-soil is true:
				let D be the dedication of ST;
				if D < a random number between 1 and 5, decrease rival-tug by 1;
				if tug-status > 0, increase rival-tug by a random number between 0 and 1;
				if tug-status < 0, decrease rival-tug by a random number between 0 and 1;
			otherwise:
				decrease rival-tug by a random number between 0 and 1;
			decrease player-tug by player-numerical-response;
			increase player-tug by a random number between -1 and 1;
			increase rival-tug by a random number between -1 and 1;
			if debuginfo > 0, say "[input-style]Final values: player pulls with strength [player-tug], [trainee-name of ST] pulls with strength [rival-tug].[roman type][line break]";
			let tug-diff be player-tug - rival-tug;
			if tug-diff > 0:
				say "You manage to pull yourself [if tug-diff is 1]slightly[otherwise][tug-diff] steps[end if] away from the pit, and [trainee-name of ST] closer to the pit!";
			otherwise if tug-diff < 0:
				say "You get pulled [if tug-diff is -1]slightly[otherwise][tug-diff * -1] steps[end if] towards the pit, and [trainee-name of ST] moves the same distance away from the pit!";
			otherwise:
				say "You both pull the rope with equal force, and neither of you move an inch!";
			increase tug-status by tug-diff;
		if tug-status > -4 and tug-status < 4:
			if the player-soil of TWP >= 2 and the player is upset about sitting in mess:
				say "You continue to wallow in your own mess.";
				SlowGrossOut 5;
			otherwise if the rival-soil of TWP >= 2:
				say "[one of][BigNameDesc of ST] grunts with disgust, clearly upset about being stuck in a huge messy diaper.[or][stopping]";
			let soiled-this-turn be false;
			if player-can-soil is true:
				if (player-numerical-response is 1 and a random number between 1 and 2 is 1) or (player-numerical-response is 2 and a random number between 1 and 10 is 1):
					if player-soil of TWP is 0:
						say "You let out a weak whine as you lose control of your [SlimeContainer], and hot [slime] soaks your diaper. You'll have to do the rest of this tug of war with warm, soggy nappy padding...";
						now the slime-soak of blue-velcro-diaper is the bladder of the player;
						now delayed urination is 0;
						now the bladder of the player is 0;
						progress temporary incontinence;
					otherwise:
						say "You shout with disbelief as you feel your sphincter open of its own accord, and warm mushy poop snake its way into your diaper.";
						MessSet blue-velcro-diaper to rectum;
						now rectum is 1;
						if the player is upset about sitting in mess, say "[variable custom style]No no no no no! GROSS!!![roman type][line break]";
						otherwise say "[variable custom style]Aww man, it's difficult to concentrate on more than one thing at a time...[roman type][line break]";
					increase the player-soil of TWP by 1;
					now soiled-this-turn is true;
			if rival-can-soil is true:
				if rival-tug > a random number between 0 and 3:
					if rival-soil of TWP is 0:
						say "[BigNameDesc of ST] turns bright red, and [his of ST] diaper starts to yellow. You realise that [he of ST] is wetting [himself of ST][if soiled-this-turn is true] at the same time[end if]!";
						if diaper messing < 4, HappinessDown ST;
					otherwise:
						say "[BigNameDesc of ST] groans with dismay as [his of ST] diaper balloons out behind [him of ST] with a huge quantity of poop[if soiled-this-turn is true and the player-soil of TWP >= 2] at exactly the same time as yours[end if]. [big he of ST] growls and turns bright red, seemingly angrier than before!";
						HappinessDown ST;
					increase the rival-soil of TWP by 1;
					now soiled-this-turn is true;
	if tug-status >= 4:
		say "With a shriek of terror, [NameDesc of ST] finds that there is no longer any ground beneath [him of ST]! [big he of ST] goes tumbling down into the pit, and at this exact moment, the rope around your waist is released, saving you from being pulled in on top of [him of ST].[line break][speech style of ST]'NOOOO-GGNMMMPH!'[roman type][line break][big his of ST] voice becomes muffled as [he of ST] is completely engulfed by the [if diaper messing >= 7]shit-filled[otherwise][slime]-soaked[end if] diapers.";
		HappinessDown ST;
	otherwise:
		say "Your foot tries to find purchase, but there is no longer any ground beneath it! You go tumbling down into the pit, and at this exact moment, the rope around [NameDesc of ST][']s waist is released, saving [him of ST] from being pulled in on top of you. You are suddenly engulfed in countless [if diaper messing >= 7]disgusting [end if]used diapers!";
		let GOV be 6; [gross out value]
		if diaper messing >= 7, now GOV is 11;
		SmellGrossOut GOV;
		compute single choice question "try to clamber out";
		say "It takes a painfully humiliatingly long time for you to manage to scramble through the [if diaper messing >= 7]shit-filled[otherwise][slime]-soaked[end if] diapers and make it to one edge of the pit. During this time, used diapers press in against you from every direction.";
		SmellGrossOut GOV;
		say "As you reach the edge of the pit, you witness [NameDesc of ST] claiming [NameDesc of babydoll-skirt] for [himself of ST], and putting it on, dooming you to the fate of walking home bottomless.[paragraph break]As you grasp the edge of the pit and prepare to pull yourself out, you watch [NameDesc of ST] leave to the north, taking the skirt with [him of ST].";
		only destroy babydoll-skirt;
		now ST is in Facility05;
		deinterest ST;
		let still-in-pit be 3;
		now temporaryYesNoBackground is Figure of diaper engulfed;
		while still-in-pit > 0:
			compute single choice question "try to clamber out";
			if the player is getting unlucky:
				say "As you try to pull yourself out, your grip slips, and you tumble back down into the diapers, which once again completely engulf you!";
				SmellGrossOut GOV;
				say GotUnluckyFlav;
				say "You frantically clamber back to the edge, until you're ready to pull yourself out again.";
				decrease still-in-pit by 1; [people might have used debug commands to set unlucky events to always happen]
			otherwise:
				now still-in-pit is 0;
				say "You heave yourself out of the pit and back to solid land.[line break][variable custom style]I can't believe that just happened...[roman type][line break]";
		if K is a thing and a random number between 1 and the aggro limit of ST > the favour of ST, now K is carried by ST. [rival takes the key]

An all time based rule (this is the diaper pit predicament rule):
	if current-predicament is tug-of-war-predicament:
		if the player is in Predicament01 and (babydoll-skirt is in Predicament01 or babydoll-skirt is held):
			say "[BigNameDesc of team-predicament-partner] is [one of][or]still [stopping]flailing around in the diaper pit[one of], unable to make much progress towards the edge, due to essentially being [']swimming['] in used diapers[or]. You decide against helping [him of team-predicament-partner] - you don't want to get into a wrestling match over the skirt[or][stopping].[line break][speech style of team-predicament-partner]'[one of]AAAH[or]EURGH[or]YEURGH[in random order]!'[roman type][line break]";
		otherwise if the player is in Predicament03 and there is a specific-key covering blue-velcro-diaper:
			say "[one of][bold type]You recall the robotic voice's words: [first custom style][']THE KEY TO YOUR DIAPERS IS CURRENTLY IN THE POSSESSION OF AN ANGEL IN THE PARK.['][roman type][line break]That must mean the statue of the angel towards the south![or][stopping]";
		otherwise if the player is in Park24:
			let K be a random specific-key covering blue-velcro-diaper;
			if K is a thing and K is carried by team-predicament-partner:
				say "[one of][bold type]You are dismayed to find that there is no key for your diaper here![line break][variable custom style]That bitch [trainee-name of team-predicament-partner] must have taken it![roman type][line break][or][stopping]";



breastfeeding-predicament is a predicament.
breastfeeding-predicament has a number called shock-strength. the shock-strength of breastfeeding-predicament is 10.
breastfeeding-predicament has a number called settled-phases.
Definition: breastfeeding-predicament is realistic-arms-requiring: decide yes.
To say defaultarmsflav of (P - breastfeeding-predicament):
	say "([bold type]and in this predicament, because of your starting situation, to make that happen you would have to manually change your priorities with 'adjust arms'[roman type])".
...
Definition: breastfeeding-predicament is always-perceiving:
	if fake-baby is held and fake-baby is not settled, decide yes; [when you are holding a screaming doll, people can't fail to notice you]
	if there is a sat-on breastfeeding-bench, decide yes; [when you are breastfeeding, people always notice you]
	decide no.

To execute (BFP - breastfeeding-predicament):
	if the number of worn nipple piercing is 0:
		summon nipple-rings uncursed;
		if piercing-fetish is 1, now nipple-rings is predicament-fixed;
	summon floral skirt uncursed;
	summon milk crop top uncursed;
	summon breastfeeding-bag uncursed;
	summon mask-gag locked;
	if the player is possessing a vagina:
		summon wired-vibe uncursed;
		dislodge wired-vibe;
		now wired-vibe is clit-taped;
	otherwise:
		summon remote-controlled-vibrator uncursed;
		dislodge remote-controlled-vibrator;
		now remote-controlled-vibrator is penetrating penis;
	summon leather-peep-toe-heels uncursed;
	summon fake-baby uncursed;
	now the defaultLeftTarget of arms is fake-baby;
	now the defaultRightTarget of arms is fake-baby;
	now entry 1 in the armUses of arms is fake-baby;
	now entry 2 in the armUses of arms is fake-baby;
	now the heel-height of leather-peep-toe-heels is 1;
	if diaper quest is 1, summon plain-largish-diaper uncursed;
	let LX be a list of numbers;
	let LP be the list of pinned park rooms;
	sort LP in random order;
	repeat with P running through LP:
		let E be the easting part of the grid position of P; [only one bench per x coordinate]
		unless E is listed in LX:
			add E to LX;
			let B be a random off-stage breastfeeding-bench;
			if B is a thing, now B is in P;
	now the player is in Predicament02;
	say "You find yourself in an alleyway somewhere outside. You are wearing skimpy white crop top that says [']MILK['] over each breast, and a long tight floral skirt. There is a [if diaper quest is 1]pacifier [end if]gag in your mouth, hidden behind a disposable face mask. It is connected by a discreet plastic tube to a pink diaper bag. [if diaper quest is 1]You can feel a large dry diaper sitting in between your legs. [end if]In your hand, you are holding... A baby?! Oh no, wait, it's just a doll. Still, it tricked you for a moment, partially because it looks quite realistic, and partially because it's rather heavy![paragraph break]A robotic voice speaks from a hidden speaker.[paragraph break][first custom style]'WELCOME TO EXTRA CREDIT CLASS. [PredicamentRewardExplanation]TO EARN THIS TROPHY, YOU MUST SIMPLY RETURN HOME. HOWEVER, WHENEVER YOU WALK EASTWARDS WITH AN [']UNSETTLED['] BABY, YOUR NEW NIPPLE PIERCINGS WILL ELECTROCUTE YOU. FURTHERMORE, FAILING TO HOLD DOWN THE TWO BUTTONS ON YOUR BABY - ONE AT THE SIDE OF THE NECK, AND ONE AT THE BASE OF THE SPINE, WILL ALSO CAUSE THE ELECTROCUTIONS.'[roman type][line break]As if to greet you with their presence, the nipple piercings that you hadn't even noticed yet release a bolt of electricity into your body.";
	PainUp 5;
	say "You quickly find the two buttons and push them down, one with each hand! You're now stuck holding this baby like that, and if you let go for even a moment, you'll get shocked!";
	say "[first custom style]'THE SHOCKS WILL GET MORE POWERFUL WITH EACH INFRACTION. TO SETTLE YOUR BABY, THE FOLLOWING OPTIONS ARE AVAILABLE:[line break]1: ROCK THE BABY - DURING THE FIRST 10 SECONDS OF AN UNSETTLED EPISODE, ROCKING THE BABY WHILE HUMMING HAS A CHANCE OF SETTLING THE BABY.[line break]2: BREASTFEED THE BABY - TO BREASTFEED THE BABY, YOU MUST SIT ON A BENCH AND PRESS THE BABY TO YOUR NIPPLE. THEN, YOU MUST SUCK ON THE STRAW AND DRINK THE DELICIOUS COCKTAIL IN YOUR BAG - A YUMMY MIXTURE OF [if diaper quest is 1 or lactation fetish is 1]HUMAN BREAST MILK[otherwise]FRESH SEMEN[end if] AND [if diaper quest is 1 and diaper messing >= 4]LAXATIVES[otherwise if watersports fetish is 1][caps slime][otherwise if alcohol fetish is 1]MEAD[otherwise]MUSCLE RELAXANT[end if]. WHEN YOU HAVE SUCKED UP SUFFICIENT DRINK, THE BABY WILL RETURN TO A SETTLED STATE.'[PredicamentRewardExplanationReaction][roman type][line break]";

To say PredicamentDescription of (P - breastfeeding-predicament):
	say "I'm currently in an Extra Credit training session where I need to get back to my home while getting shocked if I head eastwards when my fake baby is unsettled. To settle the baby, I must either try rocking it or sit on a brench and pretend to breastfeed it, whilst secretly drinking a disgusting drink myself...".

To set up bystanders for (P - breastfeeding-predicament):
	set up 20 bystanders;
	let LB be a list of people;
	add bro-bystander to LB;
	add bum-bystander to LB;
	repeat with M running through LB:
		now M is uninterested;
		now the worst-appearance of M is 0;
		now M is in Park16;
...
Figure of breastfeeding predicament hidden is the file "Special/Cutscene/cutscene-predicament-breastfeeding1.jpg".

A breastfeeding-bench is a kind of thing. There are 6 breastfeeding-bench. A breastfeeding-bench is not portable. The text-shortcut of a breastfeeding-bench is "bench". The printed name of a breastfeeding-bench is "park bench". Understand "park" as breastfeeding-bench. A breastfeeding-bench can be sat-on. A breastfeeding-bench can be just-sat-on.

Figure of park bench is the file "Env/Facility/parkbench1.png".

To decide which figure-name is the examine-image of (C - a breastfeeding-bench):
	decide on figure of park bench.

To decide which number is fake-baby-buttons-held:
	let buttons-held be 0;
	repeat with A running through the armUses of arms:
		if A is fake-baby, increase buttons-held by 1;
	if buttons-held > 2, decide on 2; [failsafe - should never happen]
	decide on buttons-held.

An all time based rule:
	if current-predicament is breastfeeding-predicament and the player is in a predicament room and the player is not in Predicament19 and the player is not in Predicament20:
		if fake-baby is not held or fake-baby-buttons-held < 2:
			[say "fake-baby-buttons-held is [fake-baby-buttons-held]. armUses of arms are [armUses of arms].";]
			say "You get punished for letting go of [if fake-baby-buttons-held is 1]one of the fake baby's buttons[otherwise]the fake baby[end if]!";
			compute breastfeeding-predicament-shock;
		if the unsettled-time of fake-baby > 0:
			increase the unsettled-time of fake-baby by time-seconds;
		let BB be a random sat-on breastfeeding-bench in the location of the player;
		if BB is a thing:
			if fake-baby is settled:
				cutshow Figure of breastfeeding predicament hidden for BB;
			otherwise if diaper quest is 1 and diaper messing >= 4:
				if BB is just-sat-on, mapcutshow Figure of breastfeeding predicament lax for BB;
				otherwise cutshow Figure of breastfeeding predicament lax for BB;
			otherwise if legacy watersports fetish is 1:
				if BB is just-sat-on, mapcutshow Figure of breastfeeding predicament piss for BB;
				otherwise cutshow Figure of breastfeeding predicament piss for BB;
			otherwise if alcohol fetish is 1:
				if BB is just-sat-on, mapcutshow Figure of breastfeeding predicament mead for BB;
				otherwise cutshow Figure of breastfeeding predicament mead for BB;
			otherwise:
				if BB is just-sat-on, mapcutshow Figure of breastfeeding predicament lax for BB;
				otherwise cutshow Figure of breastfeeding predicament lax for BB;
			now BB is not just-sat-on;
			repeat with M running through interested bystanders in the location of the player:
				compute continued breastfeeding perception of M.

This is the fake baby gets focused rule:
	if fake-baby is held and (fake-baby is not settled or there is a sat-on breastfeeding-bench):
		focus-consider fake-baby.
The fake baby gets focused rule is listed in the focus finding rules.

Check going when current-predicament is breastfeeding-predicament:
	if the player is prone and there is a sat-on breastfeeding-bench, say "You would need to stand up first." instead;
	if the player is prone and fake-baby is held, say "You can't crawl properly while holding the fake baby!" instead.

Report going:
	if current-predicament is breastfeeding-predicament and the player is in a predicament room:
		if fake-baby is not settled and the noun is east, compute breastfeeding-predicament-shock;
		if the player is upright,	check fake baby unsettling;

To compute breastfeeding-predicament-shock:
	say "Your nipple piercings unleash a powerful jolt of electricity!";
	PainUp shock-strength of breastfeeding-predicament;
	increase the shock-strength of breastfeeding-predicament by 5;
	say "[variable custom style][one of]That was MUCH stronger than the example shock at the start![or]Oh my god, that was way stronger than the last one![or]It's still getting even stronger! It hurts so much![stopping][roman type][line break]".

To check fake baby unsettling:
	if fake-baby is held and fake-baby is settled:
		let SP be the settled-phases of breastfeeding-predicament + 1;
		if (the player is in a park room or the player is in a road room) and a random number between 1 and SP is 1:
			now the unsettled-time of fake-baby is 1;
			say "[bold type]Suddenly, the fake baby starts emitting a very loud audio track of a baby crying![one of][or][variable custom style]Again?![stopping][roman type][line break]";
			let V be a random worn vibrator;
			if V is a thing, say "At the same time, [NameDesc of V] whirs into life![one of][line break][variable custom style]Wait what?! Whenever the baby is unsettled, this vibrator goes nuts?! Really?![roman type][line break][or][stopping]";
			compute cry attraction.

To compute cry attraction:
	repeat with M running through uninterested bystanders:
		if M is not in the location of the player:
			now neighbour finder is the location of M;
			let A be the best route from the location of M to the location of the player;
			if A is a direction:
				let P be the room A from (the location of M);
				if a random number between 1 and 5 > 3 and the number of barriers in P is 0 and the number of barriers in the location of M is 0, try M going A;
				if M is in the location of the player, say "[bold type][BigNameDesc of M] [bold type]must have been attracted here by the sound of the crying![roman type][line break]";
		otherwise:
			say "[BigNameDesc of M] is started by the baby, and looks around, now much more alert!";
		if M is in the location of the player:
			now M is interested;
			compute perception of M;
	display entire map. [Icons need to be moved]
...
To settle fake baby:
	if fake-baby is not settled:
		let V be a random worn vibrator;
		if V is a thing, say "[BigNameDesc of V] [bold type]shuts off.[roman type][line break][one of][variable custom style]I guess that means that the baby is [']settled['] again...[roman type][line break][or][stopping]";
		if the number of sat-on breastfeeding-bench is 0, say "The fake baby stops screaming. Thank goodness for that...";
		now the unsettled-time of fake-baby is 0.

FakeBabyRocking is an action applying to one thing.
Check FakeBabyRocking:
	if the noun is not fake-baby, say "How would you rock that?" instead;
	if there is a sat-on breastfeeding-bench, say "Now that you've quietened it by putting your nipple in its mouth, this seems like the wrong move." instead;
	if the player is prone, say "You'd need to be standing to attempt that." instead.
Carry out FakeBabyRocking:
	allocate 6 seconds;
	say "You urgently swing the baby side to side, and loudly hum a lullaby through your gag, trying to settle it within the first ten seconds!";
	if the player is getting lucky:
		settle fake baby;
		say GotLuckyFlav;
	otherwise:
		say "But it's still screaming! [if the unsettled-time of fake-baby > 10]Perhaps the ten second window has already closed[otherwise]Unlucky[end if]...".
Understand "rock [something]" as FakeBabyRocking.

Check entering breastfeeding-bench:
	if the player is prone, say "You would need to be standing up." instead;
	if fake-baby is not held, say "Why would you do that?" instead;
	allocate 5 seconds;
	now the noun is sat-on;
	now the noun is just-sat-on;
	now the stance of the player is 1;
	say "You sit down on the bench, expose one nipple, and then push it into the fake baby's mouth. It stops screaming![one of][line break][variable custom style]Thank fuck for that...[roman type][line break][or][stopping]" instead.

To construct normal buttons for (T - a breastfeeding-bench):
	if ButtonTableFull is 0:
		if T is sat-on and mask-gag is worn:
			choose a blank row in the Table of Buttons;
			now the ButtonImage entry is Figure of DrinkButton;
			now the ButtonCommand entry is "drink [text-shortcut of mask-gag]";
			now the ButtonColour entry is lightModeFullGreen;
		if the player is prone:
			choose a blank row in the Table of Buttons;
			now the ButtonImage entry is Figure of StandButton;
			now the ButtonCommand entry is "stand";
			now the ButtonColour entry is lightModeFullGreen;
		otherwise:
			choose a blank row in the Table of Buttons;
			now the ButtonImage entry is Figure of SitButton;
			now the ButtonCommand entry is "sit on [text-shortcut of T]";
			now the ButtonColour entry is lightModeFullGreen.

Check standing when there is an on-stage sat-on breastfeeding-bench:
	allocate 5 seconds;
	let BB be a random on-stage sat-on breastfeeding-bench;
	if fake-baby is held:
		say "You pull the fake baby off of your nipple and then stand up. ";
		if fake-baby is settled:
			say "You adjust your crop top to cover your exposed nipple as quickly and deftly as you can[if fake-baby is held and the defaultLeftTarget of arms is fake-baby] while holding onto the hidden buttons on the doll[end if].";
			let M be a random male interested camera-bystanders in the location of the player;
			if M is a thing:
				let T be the substituted form of "you [one of]gingerly[or]awkwardly[or]haphazardly[in random order] putting away your [one of]nipple[or]boob[or]breast[in random order] after finishing [']breastfeeding['] your fake baby";
				say "[BigNameDesc of M] ";
				compute rapid picture taking of M witnessing T;
		otherwise:
			say "It immediately begins screaming again![line break][variable custom style]Shit![roman type][line break]";
			compute cry attraction;
	otherwise:
		say "You stand up.";
	now BB is not sat-on;
	now the stance of the player is 0;
	do nothing instead.

Check taking off breastfeeding-bag:
	if mask-gag is worn, say "You can't do that without first removing the gag!" instead.

Check drinking breastfeeding-bag:
	unless there is a sat-on breastfeeding-bench, say "[variable custom style]I should sit on a bench and put the fake baby to my nipple first, or it won't count.[roman type][line break]" instead;
	allocate 3 seconds;
	let LQ be milk;
	if diaper quest is 0 and lactation fetish is 0, now LQ is semen;
	say "You suck a mouthful of [variable LQ] and [if diaper quest is 1 and diaper messing >= 4]laxatives[otherwise if watersports fetish is 1][slime][otherwise if alcohol fetish is 1]mead[otherwise]muscle relaxant[end if] through the straw[one of]. It tastes pretty much exactly as messed up as you expected...[or].[stopping]";
	if watersports fetish is 1 and (diaper quest is 0 or diaper messing < 4):
		FaceFill LQ by 1;
		FaceFill slime by 1;
		if the slime taste addiction of the player < 10 or the relevant taste addiction of LQ < 10, TasteGrossOut 5;
	otherwise:
		FaceFill LQ by 2;
		if LQ is not milk and the relevant taste addiction of LQ < 10, TasteGrossOut 5;
	if the player is getting unlucky:
		let V be a random worn vibrator;
		if V is a thing, say "[bold type]But [NameDesc of V] [bold type]doesn't shut off! [roman type][GotUnluckyFlav]";
	otherwise:
		settle fake baby;
	suggest swallowing instead.

Check drinking mask-gag:
	if breastfeeding-bag is worn, try drinking breastfeeding-bag instead.

To construct unique buttons for (T - breastfeeding-bag):
	if ButtonTableFull is 0:
		choose a blank row in the Table of Buttons;
		now the ButtonImage entry is Figure of DrinkButton;
		now the ButtonCommand entry is "drink [text-shortcut of T]";
		now the ButtonColour entry is lightModeFullGreen.
To construct unique buttons for (T - mask-gag):
	if breastfeeding-bag is worn and ButtonTableFull is 0:
		choose a blank row in the Table of Buttons;
		now the ButtonImage entry is Figure of DrinkButton;
		now the ButtonCommand entry is "drink [text-shortcut of T]";
		now the ButtonColour entry is lightModeFullGreen.

To compute special breastfeeding predicament drink:
	if mask-gag is worn:
		let TV be the total volume of face / 2;
		repeat with N running from 1 to TV:
			if diaper quest is 1 and diaper messing >= 4:
				increase suppository by 1;
				say "Your stomach gurgles as it processes the powerful laxative...";
			otherwise if watersports fetish is 0:
				if alcohol fetish is 1:
					if alcohol-level < 3, say "The strong alcohol hits you hard[one of]! Uh-oh, this could get dangerous quickly...[or].[stopping]";
					increase alcohol level;
				otherwise:
					say "You feel your muscles growing [one of]even [or][stopping]weaker...";
					increase temp_str_dam by 1;
					increase temp_dex_dam by 1.

To compute breastfeeding perception of (M - a bystander):
	if there is a sat-on breastfeeding-bench:
		if M is male:
			if M is camera-bystander, compute lewd perception reaction of M;
			otherwise say "[big he of M] gives your partially exposed nipple a good long stare before looking away.[one of][line break][variable custom style]Ugh, men.[roman type][line break][or][stopping]";
		otherwise:
			say "[big he of M] [one of]smiles at you sweetly[or]politely looks away[or]makes an enamoured face and puts a hand to [his of M] heart[at random].";
	otherwise if fake-baby is not settled:
		say "[big he of M] [one of]cringes[or]smiles awkwardly[or]gives you a pitying smile[or]pouts with pity[at random] as [he of M] watches you try to settle your [']baby['].".

To compute continued breastfeeding perception of (M - a bystander):
	if M is male:
		say "[BigNameDesc of M] [one of]is pretending[or]is still pretending[or]is trying[or]is doing [his of M] best[cycling] not to look at you, but [one of]keeps glancing[or]won't stop taking a quick peek[or]can't seem to stop subtly peeking[at random] at your partially exposed nipple. [slightHumiliateReflect]".

To compute breastfeeding perception of (M - bum-bystander):
	say "[big he of M] stares directly at you, grinning, showing no intention to be polite and look away.".
To compute continued breastfeeding perception of (M - bum-bystander):
	say "[BigNameDesc of M] [one of]has [his of M] hands down [his of M] pants and is gently toying with [his of M] [manly-penis][or]continues to touch [himself of M][stopping] as [he of M] stares at your partially exposed nipple. [strongHumiliateReflect]".

To compute breastfeeding perception of (M - bro-bystander):
	say "[big he of M] smirks as [he of M] watches you.".
To compute continued breastfeeding perception of (M - bro-bystander):
	say "[BigNameDesc of M] [one of]licks [his of M] lips[or]nods approvingly to [himself of M][stopping] as [he of M] stares at your partially exposed nipple. [moderateHumiliateReflect]".
...
Definition: team-three-leg-predicament is realistic-arms-requiring: decide yes.
To say defaultarmsflav of (P - team-three-leg-predicament):
	say "([bold type]and in this predicament, because of your starting situation, to make that happen you would have to manually change your priorities with 'adjust arms'[roman type])".
...
Check going when girlfriend-partner is in the location of the player:
	if current-predicament is team-three-leg-predicament and the noun is not north and the player is not in Predicament02 and the player is not in Predicament19:
		let R be a random number between 5 and 10;
		let V be the vindictiveness of team-predicament-partner;
		if debuginfo > 0, say "[input-style]Non-north movement check: d6+4 ([R]) + constant tugging pressure ([the rival-pressure of team-three-leg-predicament]) = ([R + the rival-pressure of team-three-leg-predicament]) | ([rival-plug-size of team-three-leg-predicament + V - 1].5) = ([V].5) rival vindictiveness + ([rival-plug-size of team-three-leg-predicament - 1]) rival vibrator size[roman type][line break]";
		if rival-plug-size of team-three-leg-predicament + V > R + the rival-pressure of team-three-leg-predicament:
			allocate 2 seconds;
			increase the rival-pressure of team-three-leg-predicament by 1;
			say "[BigNameDesc of girlfriend-partner] pulls back, clearly in too much discomfort, and desperately needing to move north to relieve some of the pressure in [his of girlfriend-partner] [if diaper quest is 1]pussy[otherwise]asshole[end if]. You could try again to keep doing your best to encourage [him of girlfriend-partner] to go where you want to go, or you could first move north to relieve some of [his of girlfriend-partner] pressure." instead;
		let L be the room the noun from the location of the player;
		let NM be the number of uninterested bystanders in L;
		if NM > 0:
			let D be the dedication of team-predicament-partner;
			if the partner-arousal of team-three-leg-predicament >= 25, decrease D by a random number between 1 and 3;
			if D + the rival-pressure of team-three-leg-predicament < NM:
				allocate 2 seconds;
				increase the rival-pressure of team-three-leg-predicament by 1;
				say "[BigNameDesc of girlfriend-partner] pulls back, clearly feeling much too shy to approach [if NM is 1][one of][or]yet [stopping]another neighbour who hasn't spotted you yet[otherwise]so many new strangers[end if]! You could try again to keep doing your best to encourage [him of girlfriend-partner] to go where you want to go, or you could first move north to relieve some of [his of girlfriend-partner] pressure." instead.

Carry out going when girlfriend-partner is in the location of the player:
	if current-predicament is team-three-leg-predicament:
		now the rival-pressure of team-three-leg-predicament is 0;
		let F be a random fuckhole penetrated by purple-vibrator;
		if the noun is north:
			if F is a thing:
				say "[BigNameDesc of purple-vibrator] inflates inside you!";
				increase the size of purple-vibrator by 2;
				RuinRoll F;
				say "You see [NameDesc of team-predicament-partner] sigh in relief as the vibrator inside [him of team-predicament-partner] significantly reduces in size.";
				decrease the rival-plug-size of team-three-leg-predicament by 2;
		otherwise if the noun is south:
			if F is a thing:
				say "[BigNameDesc of purple-vibrator] shrinks inside you!";
				decrease the size of purple-vibrator by 2;
				heal F times 1;
				say "You see [NameDesc of team-predicament-partner] wince with extreme discomfort as the vibrator inside [him of team-predicament-partner] significantly swells in size!";
				increase the rival-plug-size of team-three-leg-predicament by 2;

To say PredicamentDescription of (P - team-three-leg-predicament):
	say "I'm currently in an Extra Credit training session where I need to get back to my home with [girlfriend-partner] while avoiding as many bystanders from noticing me as possible. The further I move north, the larger my vibrator becomes.".

To set up bystanders for (P - team-three-leg-predicament):
	let LB be the list of bystanders;
	repeat with M running through LB:
		destroy M;
	sort LB in random order;
	repeat with M running through LB:
		...
		while valid-location-found is 0:
			now neighbour finder is a random outside predicament room;
			if a random number between 1 and 3 > 1 or neighbour finder is Predicament03:
				if a random number between 1 and 2 is 1, now neighbour finder is Park16;
				otherwise now neighbour finder is Park10;
				now valid-location-found is 1;
			now M is in neighbour finder;
			if valid-location-found is 0:
				let LA be the list of N-viable directions;
				repeat with D running through LA:
					now bystander-room-target is the room D from neighbour finder;
					if M is target-room-happy, now valid-location-found is 1; [This NPC will be able to find a valid direction to walk in]
...
To execute (L - team-three-leg-predicament):
	now the stance of the player is 0;
	now the player is in Predicament02;
	now the text-shortcut of girlfriend-partner is the trainee-name of team-predicament-partner;
	now girlfriend-partner is in Predicament02;
	if team-predicament-partner is female, now girlfriend-partner is female;
	otherwise now girlfriend-partner is male;
	if diaper quest is 1:
		summon plain-medium-diaper uncursed;
		compute cutscene bladder filling;
		now rectum is 25;
		summon blue-exercise-shorts uncursed;
		summon purple-vibrator vaginally;
	otherwise:
		summon purple-vibrator uncursed;
		summon tight-blue-exercise-shorts uncursed;
	now the size of purple-vibrator is 5;
	now the rival-plug-size of L is 5;
	summon running top uncursed;
	summon grey-sneakers uncursed;
	let NB be the substituted form of "[NameBimbo]";
	say "You find yourself standing in an alleyway with [team-predicament-partner], your left hand subtly attached to [his of team-predicament-partner] right by a pair of zip ties, and your left ankle is taped to [his of team-predicament-partner] right ankle, as if you're running a three-legged race. You are each wearing a tight blue running top and matching [if the diaper quest is 1]blue exercise shorts, bulked out by the familiar feeling of a thick diaper underneath. Which is probably necessary - you can feel that you are horribly bloated, absolutely full to the brim with an almost inhuman amount of poop. Most distressingly, you can feel a significantly sizeable vibrator in your [vagina][otherwise]super-tight blue exercise shorts that are giving you an extreme cameltoe. Most distressingly, you can feel a significantly sizeable vibrator in your [asshole][end if]![paragraph break]A robotic voice issues from a nearby loudspeaker.[line break][first custom style]'WELCOME TO THE EXTRA CREDIT ZONE. [PredicamentRewardExplanation]THE FURTHER YOU MOVE NORTH, [NB in upper case][']S [if diaper quest is 0]ANAL [end if]VIBRATOR WILL INFLATE MORE AND MORE, AND [trainee-name of team-predicament-partner in upper case][']S [if diaper quest is 0]ANAL [end if] VIBRATOR WILL SHRINK. CONVERSELY, THE FURTHER YOU MOVE SOUTH, THE MORE THAT [trainee-name of team-predicament-partner in upper case][']S [if diaper quest is 0]ANAL [end if] VIBRATOR WILL INFLATE, AND THE SMALLER [NB in upper case][']S [if diaper quest is 0]ANAL [end if] VIBRATOR WILL BECOME. [if diaper quest is 1][paragraph break]'PHOTOGRAPH RULES. WHENEVER OUR AUTOMATED SYSTEMS DETECT ANY PHOTOGRAPHS OR VIDEOS TAKEN OF YOU, YOUR VIBRATORS WILL EMIT SHARP ELECTRIC SHOCKS. [end if]FURTHERMORE, IF OUR SYSTEMS DETECT ANY PHOTOGRAPHS OR VIDEOS TAKEN OF YOU WITHOUT SMILES AND PEACE SIGNS HELD HIGH, YOUR VIBRATORS WILL DELIVER EXTREME 5000 VOLT ELECTRIC SHOCKS TO YOUR INSIDES FOR THE REMAINDER OF YOUR PREDICAMENT.'[roman type][paragraph break][BigNameDesc of team-predicament-partner] squeals with distress, and immediately holds up a peace sign, and so do you. It seems like [he of team-predicament-partner] is finding the size of these sex toys inside you too much to handle already! Moving further south might lead to [him of team-predicament-partner] freezing up on you...";
	try examining girlfriend-partner;
...
Check going when current-predicament is team-three-leg-predicament:
	if the player is prone and girlfriend-partner is in the location of the player, say "It's not practical to crawl while connected to [NameDesc of team-predicament-partner] at the ankle and wrist!" instead.

An all time based rule (this is the three legged race rule):
	if current-predicament is team-three-leg-predicament and the player is in a predicament room:
		increase the toy-charge of purple-vibrator by 10;
		increase the partner-arousal of team-three-leg-predicament by a random number between 0 and the rival-plug-size of team-three-leg-predicament;
		if the partner-arousal of team-three-leg-predicament >= 25 and the partner-arousal of team-three-leg-predicament < 50:
			if diaper quest is 0, say "[bold type]A big damp spot is appearing at the crotch of [NameDesc of team-predicament-partner][bold type][']s exercise shorts! [big he of team-predicament-partner] will be much less willing to appear in front of strangers now![roman type][line break]";
			otherwise say "[BigNameDesc of team-predicament-partner] grunts, groans and unleashes a torrent of mushy mess into [his of team-predicament-partner] diaper.[line break][variable custom style]'Oh my god, what the fuck, it won't stop coming!'[roman type][line break]For whatever cruel reason, [he of team-predicament-partner] is having the biggest messing episode of [his of team-predicament-partner] life. As the diaper expands, the exercise shorts can't hold it all together, causing several rips to appear, exposing [his of team-predicament-partner] brown stinky shame. [bold type][big he of team-predicament-partner] will be much less willing to appear in front of strangers now![roman type][line break]";
			now the partner-arousal of team-three-leg-predicament is 50;
			maybe-map-display examine-image of girlfriend-partner;
		if diaper quest is 0 and the player is horny:
			if tight-blue-exercise-shorts is dry:
				say "As you get hornier, a damp spot is forming at the front of your crotch![line break][variable custom style]Oh my god, how humiliating![roman type][line break]";
				increase the water-soak of tight-blue-exercise-shorts by 1;
				maybe-map-display examine-image of girlfriend-partner;
			if the water-soak of tight-blue-exercise-shorts < 5, increase the water-soak of tight-blue-exercise-shorts by 1;
		if plain-medium-diaper is worn and the mess of plain-medium-diaper >= 10:
			say "You can't believe just quite how much filth you've just packed your Pampers with! Your diaper is so impossible full that it is bursting at the seams... And then your exercise shorts DO burst at the seams!!![line break][variable custom style]No no no no no, now there's nothing concealing the shame of my filthy fucking diaper!!![roman type][line break]";
			silently transform plain-medium-diaper into plain-massive-diaper;
			now blue-exercise-shorts is crotch-ripped;
			maybe-map-display examine-image of girlfriend-partner.


Check taking off trousers when girlfriend-partner is in the location of the player:
	say "You can't remove those while your ankle is strapped to [NameDesc of girlfriend-partner]!" instead.
Check displacing trousers when girlfriend-partner is in the location of the player:
	say "They're too tight to displace when your hands are already quite busy!" instead.
...
Figure of lake predicament 1 is the file "Special/Cutscene/cutscene-lake-predicament1.jpg".

To set up bystanders for (P - team-lake-predicament):
	let LB be the list of bystanders;
	repeat with M running through LB:
		destroy M;
	sort LB in random order;
	repeat with M running through LB:
		...
		now the worst-appearance of M is 0;
		now neighbour finder is a random outside predicament room;
		now neighbour finder is Park11;
		let DR be a random N-viable direction;
		if a random number between 1 and 4 > 1:
			now neighbour finder is the room DR from Park11;
		now M is in neighbour finder;
		let valid-location-found be 0;
		now bystander-room-target is the room DR from neighbour finder;
		if M is target-room-happy, now valid-location-found is 1;
		while valid-location-found is 0:
			now neighbour finder is a random outside predicament room;
			now M is in neighbour finder;
			let LA be the list of N-viable directions;
...
Definition: team-lake-predicament is fixed-clothing-eligible:
	repeat with C running through worn predicament-reappearing clothing:
		if C is dress or C is knickers or C is bra, decide no;
		if C is penetrating a fuckhole, decide no;
	decide yes.

To execute (L - team-lake-predicament):
	set up bystanders for L;
	now the stance of the player is 0;
	now team-predicament-partner is in Park11;
	let K be a random off-stage specific-key;
	if K is nothing, now K is skeleton key;
	if diaper quest is 1:
		summon plain-massive-diaper uncursed;
		summon orange rubber diaper cover locked;
		now the water-soak of plain-massive-diaper is the soak-limit of plain-massive-diaper;
		now the perceived-water-soak of plain-massive-diaper is the soak-limit of plain-massive-diaper;
		now K is covering orange rubber diaper cover;
	otherwise:
		summon chastity-belt locked;
		now K is covering chastity-belt;
	now K is in Park24;
	blandify and reveal remote-controlled-vibrator;
	now remote-controlled-vibrator is worn by the player;
	if the player is possessing a vagina, now remote-controlled-vibrator is penetrating vagina;
	otherwise now remote-controlled-vibrator is penetrating penis;
	summon green bikini top uncursed;
	now the player is in Park11;
	say "You find yourself standing up to your waist in the water of a park lake with [team-predicament-partner]! You're wearing a relatively normal bikini top, but underneath the murky water, where people can't see clearly, you can feel that everything is far from normal. [if the diaper quest is 1]You're wearing the thickest, largest diaper ever, locked in place with a rubber diaper cover. And of course, because you're underwater, your padding is completely soaked to the brim[otherwise]A chastity belt is holding some sort of modern sex toy against your genitals[end if]![paragraph break]And then, just as you're getting your bearings, a robotic voice issues from the big inflatable beachball that [trainee-name of team-predicament-partner] is holding.[line break][first custom style]'WELCOME TO THE EXTRA CREDIT ZONE. [PredicamentRewardExplanation]WHILE THIS BEACHBALL IS CLOSER TO ONE OF YOU, THAT PARTICIPANT'S VIBARTOR WILL BE SET TO MAXIMUM POWER. BEING THE FIRST ONE TO LEAVE THE LAKE ALSO SETS YOUR VIBATOR TO MAXIMUM POWER. YOU CAN LEAVE AT ANY TIME, BUT YOU MAY WISH TO WAIT UNTIL THERE ARE LESS BYSTANDERS PRESENT! THE KEYS TO YOUR OUTFITS ARE HIDDEN AT THE FEET OF THE ANGEL STATUE.'[roman type][paragraph break][BigNameDesc of team-predicament-partner] squeals with distress, and immediately throws the beachball towards you! As soon as it's closer to you than [him of team-predicament-partner], your vibrator kicks into gear!";
	repeat with C running through worn clothing:
		now C is predicament-fixed;
	compute predicament map reveal;
	now refractoryperiod is 0;
	now the beachball-game of L is 1;
	let beachball-state be 10; [0: in player's hands; 1-9: thrown to the side with decreasing difficulty; 10: thrown straight but high; 11+ behind player; -1: in rival's hands, ready to throw, -2: in rival's hands; below -2: getting ball, increases by 1 per turn]
	let FTG be the buckle threshold of the player / 8;
	let F be 0; [player fatigue]
	let G be 0; [rival fatigue]
	let FTGT be 1;
	let rival-orgasm-timer be 5;
	let rival-tolerance be 2 + the dedication of team-predicament-partner + the vindictiveness of team-predicament-partner;
	while the beachball-game of L is 1:
		if beachball-state >= 0:
			if G > 0, decrease G by 1;
			say "[BigNameDesc of remote-controlled-vibrator] sends surges of pleasure into your [if the player is possessing a vagina][vagina][otherwise][player-penis][end if]!";
			stimulate vagina from remote-controlled-vibrator;
			check for arousal change;
			say "[variable custom style][one of]Fuck, this thing is strong!!![or]Holy shit...[or][one of]Haaah...[or]Oh my god...[or]Eeeeeee!!![at random][stopping][roman type][line break]";
		reset multiple choice questions;
		if refractoryperiod > 0:
			set numerical response 0 to "recover from your orgasm";
			now temporaryYesNoBackground is figure of lake predicament 1;
		otherwise:
			now temporaryYesNoBackground is figure of lake predicament 3;
			set numerical response 1 to "get out of the lake";
			set numerical response 2 to "wait[if the fatigue of the player > 0] and recover your breath[end if]";
			if beachball-state > 10:
				set numerical response 3 to "wade over to the ball";
			otherwise if beachball-state is 10:
				set numerical response 3 to "leap up and try to catch the beachball";
			otherwise if beachball-state > 0:
				set numerical response 3 to "stand still and try to catch the beachball ([beachball-state * 10]% chance of success)";
				set numerical response 4 to "throw yourself towards the beachball (success depends on fatigue; bigger penalty for missing)";
			otherwise if beachball-state is 0:
				say "[bold type][trainee-name of team-predicament-partner] looks [if G < 2]full of energy[otherwise if G < 4]tired[otherwise if G < 6]very tired[otherwise]exhausted[end if]...[roman type][line break]";
				set numerical response 3 to "throw the beachball above [trainee-name of team-predicament-partner] (reliably increase [his of team-predicament-partner] fatigue)";
				set numerical response 4 to "throw the beachball to the side of [trainee-name of team-predicament-partner] (difficulty to catch depends on your fatigue vs [hers of team-predicament-partner])";
			otherwise:
				now temporaryYesNoBackground is figure of lake predicament 2;
		compute multiple choice question;
		if player-numerical-response is 0:
			say "You breathe hard and try to stop your legs shaking.[line break][variable custom style]Holy shit...[roman type][line break]";
			now refractoryperiod is 0;
			if beachball-state > 0 and beachball-state <= 10, now beachball-state is 15; [ball was not caught]
		otherwise if player-numerical-response is 1:
			say "You give up, and wade out of the water as quickly as you can!";
			now the beachball-game of L is -1;
		otherwise if player-numerical-response is 2:
			say "You stay still[if the fatigue of the player > 0 and FTGT > 1], recovering energy even more rapidly thanks to your continued rest[otherwise if the fatigue of the player > 0], recovering some energy[end if].";
			decrease F by FTGT;
			increase FTGT by 1;
			if F < 0, now F is 0;
			if beachball-state > 0 and beachball-state <= 10:
				now beachball-state is 20;
				say "The beachball lands in the water somewhere behind you!";
		otherwise if player-numerical-response is not 0:
			now FTGT is 1;
			if beachball-state >= 20:
				say "You wade towards the ball, but it's so far away that you aren't there yet!";
				decrease beachball-state by 5;
			if beachball-state > 10:
				if the player is getting unlucky:
					say "You wade towards the ball, but it floats away from you! [GotUnluckyFlav]";
				otherwise:
					say "You wade over and pick up the ball.";
					now beachball-state is 0;
			otherwise if beachball-state is 10:
				say "You throw your arms up... ";
				if a random number between 1 and the buckle threshold of the player > the fatigue of the player:
					say "And catch the ball!";
					now beachball-state is 0;
				otherwise:
					say "But your legs are too weak and tired! The ball goes over your fingertips, and lands behind you.";
					now beachball-state is 11;
				say "[variable custom style]Aaah... This is exhausting...[roman type][line break]";
				increase F by 3;
			otherwise if beachball-state > 0:
				if player-numerical-response is 3: [stand still and catch]
					say "You hold your arm out... ";
					if a random number between 1 and 10 <= beachball-state:
						say "And catch the ball!";
						say "[variable custom style]Phew![roman type][line break]";
						now beachball-state is 0;
					otherwise:
						say "But the ball goes too wide!";
						say "[variable custom style]Shit![roman type][line break]";
						now beachball-state is 11;
				otherwise: [leap and catch]
					say "You try to dive for it... ";
					if a random number between 1 and the buckle threshold of the player > the fatigue of the player:
						say "And catch the ball!";
						now beachball-state is 0;
					otherwise:
			...
			otherwise:
				let D be the dedication of team-predicament-partner;
				if player-numerical-response is 3: [throw the beachball above]
					say "You throw the ball high... ";
					if D >= a random number between 0 and 4:
						say "But [trainee-name of team-predicament-partner] leaps up and catches it!";
						now beachball-state is -2;
					otherwise:
						say "And it sails over [trainee-name of team-predicament-partner][']s fingertips! It lands behind [him of team-predicament-partner], and [he of team-predicament-partner] has to turn around to go pick it up!";
						now beachball-state is -3;
					say BeachballRemoteClicksOff;
					increase G by 2;
				otherwise if player-numerical-response is 4: [throw the beachball to the side]
					say "You throw the ball to [trainee-name of team-predicament-partner][']s side. ";
					if D >= a random number between 1 and G:
						say "[trainee-name of team-predicament-partner] dives for it... ";
						if the player is getting lucky:
							say "But, [he of team-predicament-partner] completely misses it! [GotLuckyFlav]";
							now beachball-state is a random number between -3 and -4;
						otherwise:
							say "And [he of team-predicament-partner] manages to catch it!";
							now beachball-state is -2;
						increase G by 3;
					otherwise:
						say "[trainee-name of team-predicament-partner] reaches out for it... ";
						if a random number between 1 and F > a random number between 1 and G:
							say "And can't quite catch it! It lands behind [him of team-predicament-partner], and [he of 	team-predicament-partner] has to turn around to go pick it up!";
							now beachball-state is -3;
						otherwise:
							say "And can't quite catch it! It lands behind [him of team-predicament-partner], and [he of 	team-predicament-partner] has to turn around to go pick it up!";
							now beachball-state is -2;
					say BeachballRemoteClicksOff;
		[Rival's action]
		if beachball-state < -1:
			decrease rival-orgasm-timer by 1;
		if rival-orgasm-timer <= 0:
			say "[BigNameDesc of team-predicament-partner] shudders and wobbles as [he of team-predicament-partner] experiences a strong orgasm![line break][variable custom style][one of]Oh my gosh[or]That looks intense[cycling]...[roman type][line break]";
			now rival-orgasm-timer is 5;
			decrease rival-tolerance by 1;
		otherwise if beachball-state is -2:
			say "[trainee-name of team-predicament-partner] prepares to throw the ball...";
			now beachball-state is -1;
		otherwise if beachball-state is -1:
			if the vindictiveness of team-predicament-partner > a random number between 0 and 5:
				say "[big he of team-predicament-partner] launches it to your [one of]left[or]right[purely at random]!";
				let DD be (a random number between 1 and 5) + (a random number between 1 and 5);
				decrease DD by G;
				if DD >= 10:
					now beachball-state is 9;
				otherwise if DD < 1:
					now beachball-state is 1;
				otherwise:
					now beachball-state is DD;
				if DD < 4:
					say "[variable custom style]Shit, I don't think I can reach...[roman type][line break]";
				otherwise if DD < 7:
					say "[variable custom style]Can I reach it without diving?[roman type][line break]";
				otherwise:
					say "[variable custom style]I think I should be able to just catch that...?[roman type][line break]";
			otherwise:
				say "[big he of team-predicament-partner] throws the beachball above your head!";
				now beachball-state is 10;
		otherwise if beachball-state < -2:
			decrease rival-tolerance by 1;
			if rival-tolerance <= 0:
				say "[speech style of team-predicament-partner]'Argh, fuck this!'[roman type][line break][BigNameDesc of team-predicament-partner] appears to give up, and walks over to the edge of the lake, and heaves [himself of team-predicament-partner] out!";
				now the beachball-game of L is 0;
			increase beachball-state by 1;
			if beachball-state is -2, say "[big he of team-predicament-partner] gets the ball!";
			otherwise say "[big he of team-predicament-partner] is still wading towards the ball...";
		if FTG > 8, now FTG is 8;
		now the fatigue of the player is F * FTG;
		if the beachball-game of L > 0, compute extra turn;
	if the beachball-game of L is -1:
		HappinessUp team-predicament-partner;
		say "As a penalty for losing the [']game['], [NameDesc of remote-controlled-vibrator] is stuck on max power![line break][variable custom style]Fuck fuck fuck! I've got to get this thing out![roman type][line break][BigNameDesc of team-predicament-partner] sighs in relief, able to leave the lake and head for the angel statue (where your keys are hidden) without the constant torment of a strong vibrator.";
		update appearance level;
		repeat with M running through bystanders in the location of the player:
			compute perception of M;
	otherwise:
		repeat with M running through bystanders in the location of the player:
			say "[BigNameDesc of M] spots [NameDesc of team-predicament-partner] [one of]wearing [his of team-predicament-partner] [if diaper quest is 1]giant locked diaper[otherwise]vibrator belt[end if][or]too[stopping]![line break][speech style of M]'[one of]What the hell?!'[or]Holy...'[or]What the fuck?!'[or]Oh my god!'[cycling][roman type][line break]";
			HappinessDown team-predicament-partner;
		say "Consumed with shame, [NameDesc of team-predicament-partner] sprints torwards [his of team-predicament-partner] house!";
		say "As a reward for winning the [']game['], [NameDesc of remote-controlled-vibrator] is turned off![line break][variable custom style]Phew...[roman type][line break]You wade to the edge of the lake, preparing to get out when the way is clear...[line break][variable custom style]And before I go home, I need to go to the angel statue to get the key to this thing...[roman type][line break]";
		now lake-swimming is true;
		compute swimming in park-lake;
	now team-predicament-partner is in Facility05;
	deinterest team-predicament-partner.

To say BeachballRemoteClicksOff:
	say "[BigNameDesc of remote-controlled-vibrator] clicks off.[line break][variable custom style][one of]Thank god[or]Phew[or]That's better[or][stopping]...[roman type][line break]".




incontinence-awareness-predicament is a predicament. The printed name of incontinence-awareness-predicament is "microphone and speaker". Understand "microphone", "microphone and", "speaker" as incontinence-awareness-predicament. The text-shortcut of incontinence-awareness-predicament is "mas". incontinence-awareness-predicament is not portable. incontinence-awareness-predicament has a number called donations. incontinence-awareness-predicament has a number called speaker-strength.

Definition: incontinence-awareness-predicament is appropriate:
	if diaper messing < 4 or the player is not the donator or the player is rectum incontinent, decide no;
	if the rank of the player >= 2 and the rank of the player <= 3, decide yes;
	decide no.

Figure of incontinence awareness predicament is the file "Special/Cutscene/cutscene-incontinence-awareness-predicament1.jpg".
Figure of incontinence awareness predicament messy is the file "Special/Cutscene/cutscene-incontinence-awareness-predicament2.jpg".

To decide which figure-name is the examine-image of (P - incontinence-awareness-predicament):
	if there is a worn messed diaper, decide on Figure of incontinence awareness predicament messy;
	decide on Figure of incontinence awareness predicament.

To say ExamineDesc of (P - incontinence-awareness-predicament):
	say "A microphone connected to a speaker. You can use this to speak more audibly and help people understand that you're here on [']official business['].".
To say ShortDesc of (P - incontinence-awareness-predicament):
	say "microphone".
To say MediumDesc of (P - incontinence-awareness-predicament):
	say "microphone and speaker".

To construct normal buttons for (T - incontinence-awareness-predicament):
...
Check drinking incontinence-awareness-predicament:
	if the number of worn diaper is 0, say "You aren't wearing the correct uniform." instead;
	...
	set numerical response 5 to "Flirty (more humiliating, more likely to encourage donations from certain types)";
	set numerical response 6 to "Shamelessly (much more humiliating, more likely to encourage donations from everyone)";
	set numerical response 0 to "Cancel";
	now temporaryYesNoBackground is the examine-image of incontinence-awareness-predicament;
	compute multiple choice question;
	if player-numerical-response > 0:
		allocate 6 seconds;
		if player-numerical-response is 1:
			say "[variable custom style]'[one of]Hey everyone, your donations are welcome, but please remember, no pictures[or]No photographs or recording are permitted here, thank you for your understanding[cycling].'[roman type][line break][trivialHumiliateReflect]";
			repeat with M running through bystanders in the location of the player:
				let X be a random number between 0 and 2;
				decrease the bystanderInt1 of M by X;
				if the speaker-strength of incontinence-awareness-predicament < X, now the speaker-strength of incontinence-awareness-predicament is X;
				if M is camera-bystander:
					if the player is getting unlucky:
						if the bystanderInt3 of M >= 0, say "[BigNameDesc of M] seems to ignore you, and is still holding [his of M] camera! [GotUnluckyFlav]";
					otherwise:
						say "[BigNameDesc of M] puts [his of M] camera away.";
						now the bystanderInt3 of M is -1;
		otherwise if player-numerical-response is 2:
			say "[variable custom style]'[one of]We're here today to raise awareness and donations for our charity that supports young adults who suffer from high levels of incontinence[or][one of]The financial burden of 24/7 diaper dependence on a trainee or young professional is often insurmountable. You can provide a helping hand[or]Many diaper dependents find themselves isolated or even bullied. Our courses and camps help them find peace and acceptance[or]If you found yourself incontinent tomorrow, how would you cope? Wouldn't you need help? Think about how valuable that help would be[or]I'm not incontinent myself, but I'm passionate about supporting those that are[in random order][stopping].'[roman type][line break][slightHumiliateReflect]";
			repeat with M running through bystanders in the location of the player:
				let X be a random number between 0 and 3;
				decrease the bystanderInt1 of M by X;
				if the speaker-strength of incontinence-awareness-predicament < X, now the speaker-strength of incontinence-awareness-predicament is X;
		otherwise if player-numerical-response is 3:
			say "[variable custom style]'[one of]I'm wearing what some people have to wear every minute of every day of their whole lives. Imagine that. Your whole life in padding[or][one of]Donate now to show your support for incontinent young adults around the world who wear and use diapers like this every day[or]Look at what I'm wearing! Come on, this dedication to the bit must be worth a few bucks[or]If you found yourself incontinent tomorrow, how would you cope? Wouldn't you need help? Think about how valuable that help would be[in random order][stopping]!'[roman type][line break][moderateHumiliateReflect]";
			repeat with M running through bystanders in the location of the player:
				let X be a random number between 1 and 3;
				decrease the bystanderInt1 of M by X;
				now the speaker-strength of incontinence-awareness-predicament is X;
		otherwise if player-numerical-response is 4:
			say "[variable custom style]'[one of]Come one, come all, check out the weirdo wearing diapers in public, and find out what [he of the player][']s got to say[or]What to find out why I'm standing here wearing nothing but a diaper? Gather round, and come listen[in random order]!'[roman type][line break][moderateHumiliateReflect]";
			repeat with M running through bystanders in the location of the player:
				let X be a random number between 1 and 3;
				decrease the bystanderInt1 of M by X;
				if the speaker-strength of incontinence-awareness-predicament < X, now the speaker-strength of incontinence-awareness-predicament is X;
			repeat with M running through nearby bystanders:
				if M is not interested and a random number between 1 and 2 is 1:
					let L be the location of the player;
					repeat with D running through the Nviables of the location of M:
						if the room D from the location of M is L:
							try M going D;
							now M is moved;
		otherwise if player-numerical-response is 5:
			say "[variable custom style]'[one of]Can you imagine what it's like to get intimate with someone diapered like me? I mean, don't imagine yourself with me! Unless you want to...'[or]Some people find diapers sexy, you know. Or so I've heard. What do you think? Come on, show me some love!'[or]Would you let a diapered hottie like me give you a lapdance? That would be a rather unique sensation, wouldn't it?'[or]Our organisation helped countless diapered young people find friendship & love, and when I say love, these people certainly know how to make sweet sweet love, despite the obvious barriers...'[in random order][roman type][line break][strongHumiliateReflect]";
			repeat with M running through bystanders in the location of the player:
				let X be a random number between 1 and 3;
				decrease the bystanderInt1 of M by X;
				if M is male or M is construction-worker-bystander:
					decrease the bystanderInt1 of M by X;
					if the bystanderInt2 of M > 0, increase the bystanderInt2 of M by a random number between 1 and 3;
				if M is housewife-bystander:
					decrease the bystanderInt2 of M by a random number between 1 and 3;
				if the speaker-strength of incontinence-awareness-predicament < X + 1, now the speaker-strength of incontinence-awareness-predicament is X + 1;
		otherwise if player-numerical-response is 6:
			say "[variable custom style]'[one of]Don't make me work too hard for these donations, or I'm gonna poop myself, hahahaha. Just joking[or][one of]There's absolutely no shame in wearing diapers and making a stinky. Power to our pants pooping brothers and sisters[or]This thing is so thick that I'm waddling. Imagine what it must be like when it's full of shit and [slime][or]Imagine if I pooped myself in front of you, right now. Would you judge me? Photograph me? Some of you would, wouldn't you? Come on, the rest of you, let's battle this stigma together[or]You know, I could get used to this. Walking around with a portable poop collector under my butt... How super convenient would that be[in random order][stopping]!'[roman type][line break][one of][severeHumiliateReflect][or][strongHumiliateReflect][stopping]";
			repeat with M running through bystanders in the location of the player:
				let X be a random number between 2 and 9;
				decrease the bystanderInt1 of M by X;
				if the speaker-strength of incontinence-awareness-predicament < X, now the speaker-strength of incontinence-awareness-predicament is X;
	repeat with M running through bystanders in the location of the player:
		if a random number between 1 and 3 > 1, now M is moved;
	do nothing instead.

To set up bystanders for (P - incontinence-awareness-predicament):
	let LB be the list of bystanders;
	repeat with M running through LB:
		destroy M;
	sort LB in random order;
	repeat with M running through LB:
		...
		now the worst-appearance of M is 0;
		now neighbour finder is a random outside predicament room;
		now neighbour finder is Park02;
		let DR be a random N-viable direction;
		if a random number between 1 and 4 > 1:
			now neighbour finder is the room DR from Park02;
		now M is in neighbour finder;
		let valid-location-found be 0;
		now bystander-room-target is the room DR from neighbour finder;
		if M is target-room-happy and a random number between 1 and 5 > 2, now valid-location-found is 1;
		while valid-location-found is 0:
			now neighbour finder is a random outside predicament room;
			now M is in neighbour finder;
			let LA be the list of N-viable directions;
			repeat with D running through LA:
				...
				if M is target-room-happy, now valid-location-found is 1; [This NPC will be able to find a valid direction to walk in]
		now M is moved; [stops them moving instantly]
		set up awareness stats of M;

To set up awareness stats of (M - a bystander):
	now the bystanderInt1 of M is (a random number between 5 and 13) - (a random number between 1 and 6); [How much convincing do they need?]
	now the bystanderInt2 of M is a random number between 3 and 9; [How much is the maximum amount they'll donate?]
	now the bystanderInt3 of M is a random number between 1 and 3. [Turns until photo taken]
	...
	if current-predicament is incontinence-awareness-predicament:
		if the location of the player is Park02 and there is a worn diaper:
			...
			repeat with M running through interested bystanders in the location of the player:
				...
				decrease the bystanderInt3 of M by 1;
				if M is camera-bystander and the bystanderInt3 of M is -1:
					say "[BigNameDesc of M] ";
					compute careful picture taking of M;
				otherwise if the bystanderInt2 of M > 0 and the bystanderInt1 of M <= 0 and the speaker-strength of incontinence-awareness-predicament > 0:
					let N be a random number between 1 and the speaker-strength of incontinence-awareness-predicament;
					if N > the bystanderInt2 of M, now N is the bystanderInt2 of M;
					say "[BigNameDesc of M] touches [his of M] device to the card reader, donating $[N] to the charity.";
					increase the donations of incontinence-awareness-predicament by N;
					now the bystanderInt2 of M is 0; [won't donate again]
			now the speaker-strength of incontinence-awareness-predicament is 0;
		otherwise if the player is in a predicament room:
			now temporaryYesNoBackground is figure of small image.

To execute (L - incontinence-awareness-predicament):
	set up bystanders for L;
	now the stance of the player is 0;
	now the player is in Toilet01;
	now house-key is in Toilet02;
	compute predicament map reveal;
	summon plain-largish-diaper uncursed;
	...
	now incontinence-awareness T-Shirt is blessed;
	now incontinence-awareness T-Shirt is suppression;
	now incontinence-awareness T-Shirt is predicament-fixed;
	compute cutscene bowel filling;
	now suppository is 1;
	now L is in Park02;
	say "You find yourself standing at the exit to a public [SlimeTarget]s, wearing nothing but sneakers, a large plain diaper, and a shirt that says [']INCONTINENCE AWARENESS WEEK[']. Most troublingly, you can feel that you really, really need to poop.[paragraph break]And then, just as you're getting your bearings, a robotic voice talks to you from a hidden speaker.[line break][first custom style]'WELCOME TO THE EXTRA CREDIT ZONE. [PredicamentRewardExplanation]YOUR HOUSE KEY IS IN THE OTHER [caps SlimeTarget]S. PLEASE EARN AS MANY DONATIONS FOR YOUR NEW CHARITY AS POSSIBLE BEFORE ENTERING THE [caps SlimeTarget]S. YOU WILL BE PUNISHED, OR REWARDED, DEPENDING ON YOUR LEVEL OF DONATIONS REACHED WHEN YOU RE-ENTER THE [caps SlimeTarget]S. PLEASE LEAVE THE [caps SlimeTarget]S IMMEDIATELY, OR YOU WILL RECEIVE THE MAXIMUM PUNISHMENT AND NO REWARD.'[roman type][paragraph break]You squeal with panic.[line break][variable custom style]But I need to poo! They've given me some kind of suppository or laxative, haven't they!!![roman type][line break]";
	reset multiple choice questions;
	set numerical response 1 to "exit the men's [SlimeTarget]s";
	set numerical response 2 to "refuse to engage, and use the [SlimeTarget]";
	compute multiple choice question;
	if player-numerical-response is 1:
		say "Taking a deep breath, you open the door and step outside.";
		now the player is in Park02;
		display entire map and refresh icons;
	otherwise:
		say "You immediately rip off your diaper and sit on the [SlimeTarget].";
		compute toilet use;
		say "[first custom style]'PUNISHMENT HAS BEEN SET TO MAXIMUM.'[roman type][line break]Uh-oh. That sounds ominous. What's going to happen when you go to retrieve your housekey from the other [SlimeTarget]?!";
		now the donations of L is -999.

Report going when current-predicament is incontinence-awareness-predicament:
	if the donations of incontinence-awareness-predicament > -1000 and (the location of the player is Toilet01 or the location of the player is Toilet02):
		say "You hear a robotic voice coming from a speaker.[line break][first custom style]'AWARENESS CAMPAIGN COMPLETE. ASSESSING RESULT... ";
		if the donations of incontinence-awareness-predicament <= 0:
			say "TOTAL FAILURE. ADMINISTERING SEVERE PUNISHMENT. EXTRA CREDIT ZONE TROPHY HAS BEEN REVOKED. DEPLOYING TRANQUILIZER.'[roman type][line break]With a sudden soft sound, a dart is shot out of a hidden device, and jams into your side, injecting you with something!";
			let T be a random trophy in Predicament20;
			if T is trophy, destroy T;
			PainUp 10;
			now the fatigue of the player is the buckle threshold of the player;
			say "You feel one leg go numb, as you are drained of energy... And temporarily rendered fecally incontinent.";
			increase temporary-rectum-incontinence by 3;
		otherwise if the donations of incontinence-awareness-predicament < 10:
			say "$[donations of incontinence-awareness-predicament]. ADMINISTERING MODERATE PUNISHMENT. DEPLOYING TRANQUILIZER.'[roman type][line break]With a sudden soft sound, a dart is shot out of a hidden device, and jams into your side, injecting you with something!";
			PainUp 10;
			now the fatigue of the player is the buckle threshold of the player;
			say "You feel one leg go numb, as you are drained of energy... And temporarily rendered fecally incontinent.";
			increase temporary-rectum-incontinence by 3;
		otherwise if the donations of incontinence-awareness-predicament < 20:
			say "$[donations of incontinence-awareness-predicament]. ADMINISTERING MILD PUNISHMENT. DEPLOYING SEDATIVE.'[roman type][line break]With a sudden soft sound, a dart is shot out of a hidden device, and jams into your side, injecting you with something!";
			PainUp 10;
			now the fatigue of the player is the buckle threshold of the player;
			say "You feel one leg go numb, as you are drained of energy, and temporarily have your dexterity dampened.";
			increase temp_dex_dam by 6;
		otherwise if the donations of incontinence-awareness-predicament < 35:
			say "$[donations of incontinence-awareness-predicament]. ADMINISTERING MILD REWARD. DEPLOYING UNDERWEAR.'[roman type][line break]With a sudden soft sound, items drop from a hidden compartment above you!";
			let P be a random off-stage fetish appropriate plentiful briefs;
			if P is a thing:
				blandify and reveal P;
				now P is in the location of the player;
				now P is predicament-temporary;
		otherwise if the donations of incontinence-awareness-predicament < 50:
			say "$[donations of incontinence-awareness-predicament]. ADMINISTERING MODERATE REWARD. DEPLOYING SKIRT.'[roman type][line break]With a sudden soft sound, a skirt drops from a hidden compartment above you!";
			let P be a random off-stage fetish appropriate plentiful knee-length-or-longer not-butt-windowed skirt;
			if P is a thing:
				blandify and reveal P;
				now P is in the location of the player;
				now P is predicament-temporary;
		otherwise:
			say "$[donations of incontinence-awareness-predicament]. ADMINISTERING MAXIMUM REWARD. DEPLOYING CLOTHING, AND AN ADDITIONAL EXTRA CREDIT TROPHY HAS BEEN ADDED TO YOUR HOUSE.'[roman type][line break]With a sudden soft sound, items drop from a hidden compartment above you!";
			let TP be a random off-stage fetish appropriate trophy;
			if TP is trophy, now TP is in Predicament20;
			let P be a random off-stage fetish appropriate plentiful briefs;
			if P is a thing:
			...
			if P is a thing:
				...
				now P is in the location of the player;
				now P is predicament-temporary;
		now the donations of incontinence-awareness-predicament is -99999.


flag-wave-predicament is a predicament. flag-wave-predicament can be cutshown.
Definition: flag-wave-predicament is appropriate:
	if diaper quest is 0 and armband is ruby and the player is a september 2025 top donator, decide yes;
	decide no.
To execute (SSP - flag-wave-predicament):
	now the size of nintendolls flag plug is 1;
	summon nintendolls flag plug uncursed;
	now the player is in Predicament02;
	say "You find yourself in a dark alleyway near a park. You are completely naked except for a butt plug, attached to which is... a flagpole?!?![paragraph break]A robotic voice speaks to you from a hidden speaker.[line break][first custom style]'WELCOME TO EXTRA CREDIT CLASS. [PredicamentRewardExplanation]TO EARN THIS TROPHY, YOU MUST SIMPLY RETURN HOME. THE BUTT PLUG WILL SLOWLY INFLATE WHEN THE FLAGPOLE IS ANGLED DOWNWARDS, AND SLOWLY DEFLATE WHEN THE FLAGPOLE IS ANGLED UPWARDS. ATTEMPTING TO REMOVE THE PLUG WILL CAUSE IT TO IMMEDIATELY AND PAINFULLY EXPAND TO ITS MAXIMUM SIZE.'[PredicamentRewardExplanationReaction][line break][variable custom style]One thing is clear... While I'm on my knees, the flag will be flying high in the air, and [bold type]that will attract a lot of attention![roman type][line break]".

To decide which text is PredicamentActivity of (P - flag-wave-predicament):
	decide on "you [if nintendolls flag plug is worn and the player is prone]with a flag on a pole sticking proudly upwards out of your butthole, crawling through[otherwise if nintendolls flag plug is worn]with a flag on a pole sticking out of your butthole, hurrying through[otherwise][one of]sneaking through[or]rushing through[in random order][end if] [if the player is in a park room]a public park[otherwise]your neighbourhood[end if]".
To say PredicamentDescription of (P - flag-wave-predicament):
	say "I'm currently in an Extra Credit training session where I need to get home, but whenever I'm upright, the flag pole butt plug inflates, and whenever I'm crawling, it looks extra-humiliating waving up in the air, and attracts people from nearby.".

An all time based rule (this is the flagpole inflation rule):
	if current-predicament is flag-wave-predicament and the player is in a predicament room and nintendolls flag plug is worn and the player is not in Predicament20:
		if the player is upright:
			if the size of nintendolls flag plug < 10:
				say "The [nintendolls flag plug] inflates [if the size of nintendolls flag plug >= 3]even [end if]further, becoming a ";
				increase the size of nintendolls flag plug by 1;
				say "[nintendolls flag plug]!";
			otherwise:
				say "The [nintendolls flag plug] can't inflate any further!";
			focus-consider nintendolls flag plug;
		otherwise:
			if the size of nintendolls flag plug > 1:
				say "The [nintendolls flag plug] gradually deflates, becoming a ";
				decrease the size of nintendolls flag plug by 1;
				say "[nintendolls flag plug].";
			if flag-wave-predicament is cutshown:
				focus-consider nintendolls flag plug;
			otherwise:
				...
				if a random number between 1 and 4 is 1:
					if M is not in the location of the player:
						...
						if A is a direction:
							let P be the room A from (the location of M);
							if a random number between 1 and 5 > 3 and the number of barriers in P is 0 and the number of barriers in the location of M is 0, try M going A;
							if M is in the location of the player, say "[bold type][BigNameDesc of M] [bold type]must have been attracted here by the flag waving high in the air![roman type] [GotUnluckyFlav]";
					if M is in the location of the player:
						now M is interested;
						compute perception of M.

segway-predicament is a predicament. segway-predicament can be cutshown. segway-predicament has a number called segway-girth. segway-predicament has a direction called previousD. segway-predicament can be snapped-this-turn.
Definition: segway-predicament is appropriate:
	if diaper quest is 0 and the rank of the player < 4 and the player is a september 2025 top donator, decide yes;
	decide no.

The printed name of segway-predicament is "[TQlink of item described]segway dildo pole[TQxlink of item described][shortcut-desc]". Understand "dildo", "pole", "segway" as segway-predicament. The text-shortcut of segway-predicament is "pole".
To set up bystanders for (P - segway-predicament):
	set up 15 bystanders.
To say ExamineDesc of (V - segway-predicament):
	say "This Segway has a dildo on a pole mounted in the middle. The combination of high heels and pole means that you can't bend your knees at all, preventing you from jumping upwards and dismounting the dildo, or reaching down to unfasten your high heels. You're stuck!".
To decide which number is the girth of (P - segway-predicament):
	decide on the segway-girth of P.

To set up bystanders for (P - segway-predicament):
	set up 12 bystanders;
	set up 6 camera bystanders;
	repeat with M running through on-stage camera-bystanders:
		let R be a random number between 1 and 4;
		if R is 1, now M is in Park04;
		if R is 2, now M is in Park05;
		if R is 3, now M is in Park06;
		if R is 4, now M is in Park07;

To execute (SP - segway-predicament):
	now the segway-girth of SP is 2;
	if the player is possessing a vagina, now SP is penetrating vagina;
	otherwise now SP is penetrating asshole;
	summon huge-ballgag locked;
	let K be a random off-stage specific-key;
	if K is a thing, now K is covering huge-ballgag;
	now the player is in Predicament02;
	now SP is in the location of the player;
	now the previousD of SP is north;
	if the rank of the player >= 2 and the player is possessing a vagina:
		now the semen volume of belly is 20;
		if K is a thing:
			...
			let P be a random fertile pedestal in Holding Pen;
			now P is in Predicament20;
			now K is in P;
			now the paid of P is 3;
			now huge-ballgag is predicament-fixed;
	otherwise:
		if K is a thing, now K is in Predicament20;
	say "You find yourself in a dark alleyway near a park. You are completely naked, with a huge ballgag painfully stretching your jaw, a metal collar around your neck, and... On a Segway?! [ExamineDesc of SP][paragraph break]A robotic voice speaks to you from a hidden speaker.[line break][first custom style]'WELCOME TO EXTRA CREDIT CLASS, AND TO MR BONE'S WILD RIDE. [PredicamentRewardExplanation]TO EARN THIS TROPHY, YOU MUST SIMPLY RETURN HOME. THE SEGWAY WILL CONTINUE TO MOVE FOWARDS, AND IT IS UP TO YOU HOW YOU STEER IT. PLEASE BE AWARE THAT [bold type]THE DILDO WILL SIGNIFICANTLY INFLATE IN SIZE AND [if the rank of the player >= 3]DELIVER AN ELECTRIC SHOCK[otherwise]VIBRATE STRONGLY[end if] EACH TIME A CAMERA FLASH OR CAR HORN IS DETECTED[if the rank of the player >= 2 and the player is possessing a vagina]. [first custom style]FURTHERMORE, PLEASE BE AWARE THAT [bold type]YOUR RECTUM HAS BEEN FILLED WITH FRESH HUMAN SEMEN[first custom style], AND THE KEY IN YOUR HOUSE WILL ONLY BE RELEASED IF YOU CAN PROVIDE A SUFFICIENT QUANTITY OF THIS SEMEN INTO THE RECEPTIBLE. SO, PLEASE ATTEMPT TO KEEP SUFFICIENT AMOUNTS INSIDE DURING YOUR JOURNEY[end if][first custom style].'[PredicamentRewardExplanationReaction][line break][variable custom style][if the rank of the player >= 2]Holy shit, I really can feel huge amounts of cum inside me! This is insane! But I've GOT to try and hold it in... Or I won't be able to remove this gag that's keeping my jaw so painfully wide open[otherwise]This is insane... As if having photos taken of me naked and gagged would be bad enough[end if]![roman type][line break]";
	follow the segway time rule;

To decide which text is PredicamentActivity of (P - segway-predicament):
	decide on "you naked, collared and gagged, riding a segway with a dildo on a pole up your [if the player is possessing a vagina][vagina][otherwise][asshole].".
To say PredicamentDescription of (P - segway-predicament):
	say "I'm currently in an Extra Credit training session where I need to get home, but I'm stuck on this segway, which makes it difficult to turn backwards.".

This is the segway time rule:
	allocate 6 seconds;
	let LD be a list of directions;
	add the Nviables of the location of the player to LD;
	if debugmode > 0, say "directions possible should be [LD].";
	let OD be the opposite-direction of the previousD of segway-predicament;
	if up is listed in LD, remove up from LD;
	if down is listed in LD, remove down from LD;
	if OD is listed in LD, remove OD from LD;
	if the location of the player is Predicament03:
		if west is listed in LD, remove west from LD;
	reset multiple choice questions;
	repeat with D running through LD:
		let R be the room D from the location of the player;
		let NB be the number of bystanders in R;
		let UB be the number of uninterested bystanders in R;
		let CB be the number of camera-bystanders in R;
		set next numerical response to "[if D is the previousD of segway-predicament]continue going[otherwise]turn and travel[end if] [D] ([if NB is 0]currently empty of people[otherwise if NB is 1]1 person - [otherwise][NB] people - [end if][if UB is 1 and NB is 1]they have not yet seen you[otherwise if UB is 1]1 has not yet seen you[otherwise if UB > 1][UB] have not yet seen you[otherwise if NB is 1]they have already seen you[otherwise if NB > 1]all have already seen you[end if][if NB > 0], [end if][if CB is 0 and NB is 1]they do not have a camera[otherwise if CB is 1]1 has a camera[otherwise if CB > 1][CB] have cameras[otherwise if NB is 1]they do not have a camera[otherwise if NB > 1]none have a camera[end if])";
	if segway-predicament is not cutshown:
		now segway-predicament is cutshown;
		now temporaryYesNoBackground is examine-image of segway-predicament;
		set up predicament universe;
		compute predicament map reveal;
	otherwise:
		now temporaryYesNoBackground is Figure of no-image-yet;
	compute multiple choice question;
	now temporaryYesNoBackground is Figure of small image;
	if the player is in a park room and the player is getting unlucky:
		say "[bold type]The segway is struggling with the mud! [roman type]You make no significant progress this turn. [GotUnluckyFlav]";
	otherwise:
		let D be entry player-numerical-response in LD;
		let R be the room D from the location of the player;
		drag to R by segway-predicament;
		now the previousD of segway-predicament is D;
	if the player is in Predicament19:
		say "[bold type]The dildo on the segway slightly retracts, finally allowing you to dismount![line break][variable custom style]Phew![roman type][line break]";
		dislodge segway-predicament;
	otherwise:
		add the segway time rule to another-turn-rules;
		now another-turn is 1;
		if temporary-map-figure is not figure of no-image-yet, MapShowReset.

To say DragFlav of (M - segway-predicament) to (R - a room):
	say "[BigNameDesc of M] carries you to the [R]!".

To compute segway punishment:
	if current-predicament is segway-predicament:
		let F be a random fuckhole penetrated by segway-predicament;
		if F is a fuckhole:
			say "The dildo inside your [variable F] greatly inflates in size! ";
			increase the segway-girth of segway-predicament by 2;
			say "It is now [bold type][DongSize segway-girth of segway-predicament][roman type].";
			if the segway-girth of segway-predicament > 10, now the segway-girth of segway-predicament is 10;
			say "[variable custom style]Oh no, that means...[roman type][line break]";
			if the rank of the player >= 3:
				if debugmode > 0, say "Player rank is [the rank of the player].";
				say "BZZZT! You suffer a sudden electric shock deep inside your [variable F]!";
				if segway-predicament is snapped-this-turn, UnflinchingPainUp a random number between 15 and 30;
				otherwise PainUp a random number between 15 and 30;
			otherwise if segway-predicament is not snapped-this-turn:
				say "BZZZZZZZ... It also starts powerfully vibrating!";
				stimulate F from segway-predicament times 2;
			now segway-predicament is snapped-this-turn;
			ruin F times 1.
