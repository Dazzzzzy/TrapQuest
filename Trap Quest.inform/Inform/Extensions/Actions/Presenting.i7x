...
To decide whether (N - a monster) is a presenting-target for (B - a body part):
	if B is face:
		if N is willing to do oral, decide yes;
		if diaper quest is 0 and N is willing to do licking, decide yes;
		if diaper quest is 1 and N is willing to urinate, decide yes;
	if B is breasts and N is willing to do titfucks, decide yes;
	if B is vagina and N is willing to do vaginal, decide yes;
	if B is asshole and N is willing to do anal, decide yes;
	if B is penis and N is willing to charm snakes, decide yes;
	decide no.
...
Check presenting:
	...
	let M be nothing;
	[First Highest priority is anything unfriendly that will use that body part]
	repeat with N running through threatening interested unfriendly monsters in the location of the player:
		if M is nothing and N is a presenting-target for the noun, now M is N;
	[Second Highest priority is anything unfriendly, especially if intelligent]
	...
	[Third Highest priority is anything that will use that body part]
	if M is nothing:
		repeat with N running through threatening interested monsters in the location of the player:
			if M is nothing and N is a presenting-target for the noun, now M is N;
	[Fourth Highest priority is anything intelligent or raunchy (will bang)]
	...
	[Fifth Highest priority is anything interested at all]
	...
	[Sixth highest priority is anything friendly that will use that body part]
	if M is nothing:
		repeat with N running through threatening monsters in the location of the player:
			if M is nothing and N is a presenting-target for the noun, now M is N;
	[Seventh highest priority is anything intelligent or raunchy (will bang)]
	...
	[Eigth (and lowest) priority is anything at all]
...
Check direct-presenting something to:
	...
	if the noun is thighs, try direct-presenting face to the second noun instead;
	...
	if royal sceptre is worn and royal sceptre is blacked and the second noun is a friendly royal guard, compute princess guard tease of the second noun instead;
	unless the noun is actually presentable, do nothing instead;
	...
	if the player is prone:
		...
		if the noun is face:
			...
			otherwise if chosen numerical response matches the text "tongue":
				...
				now the second noun is getting-licked;
...
To say UnfriendlyAnilingusRequest of (M - a monster):
	if the player is a nympho:
		say "[if the player is feeling dominant and M is unintimidating][first custom style]'What are you waiting for, [stud-worm of M]? Make me degrade myself by tasting your rosebud. But bear in mind that I'll be back, and next time you'll be the one on [his of M] knees...'[otherwise if the grossness addiction of the player < 4][variable custom style]'I guess if you wanted to be as gross and cruel as possible, you'd make me lick your butthole, wouldn't you?'[otherwise if the grossness addiction of the player < 11][variable custom style]'Okay, you win. If you really want to treat me as a loser, I suggest you make me lick your nasty, sweaty arsehole...'[otherwise][second custom style]'[one of]I guess that makes me your personal taint worshipper now then, huh?'[or]Uh-oh, why do I get the feeling I'm about to be forced to taste your glorious butthole? *giggle*'[or]This pathetic butt-licker is ready for [his of the player] your delicious ass, [stud of M]!'[in random order][end if][roman type][line break]";
	otherwise:
...
To say TakeMyVirginity of (M - a monster):
	if M is unfriendly:
		if the anal sex addiction of the player < 3:
			say "[first custom style]'Only a weirdo would stick it in my butt. surely you'd rather take my virginity...'";
...
To say UnfriendlyAnalRequest of (M - a monster):
	if M is virginity taking and the vaginalvirgin of the player is 1 and the player is possessing a vagina:
		say "[variable custom style]'I guess you're [one of]planning[or]looking[at random] to take my virginity? But... [if the anal sex addiction of the player < 3]You could put it in my [asshole] instead!'[otherwise if the player is a nympho]You could abuse my slutty [asshole] instead!'[otherwise if the openness of asshole < 3]My [asshole] is much tighter!'[otherwise]Wouldn't you much rather fuck my [asshole] instead?'[end if]";
...
To say UnfriendlyPenetrationRequest of (M - a monster):
	if M is virginity taking and the analvirgin of the player is 1:
		say "[variable custom style]'My ass is still pure, [if the penis-obedience of penis < 3]so why not just use my [player-penis]?'[otherwise if the player is a nympho]which does need to change sometime soon, not when my [player-penis] is begging for an orgasm!'[otherwise if the size of penis < 3]so why not just use my [player-penis]? I know I'm not much, but...'[otherwise]wouldn't you much rather play with my [player-penis] instead?'[end if]";
	...
	otherwise:
		if the player is a nympho:
			say "[if the player is feeling dominant and M is unintimidating][first custom style]'I'll allow you to play around with my [player-penis] a little, but only as practice. You'll need it if you're going to be able to handle me when I finally get you on your knees...'[otherwise if the penis-obedience of penis < 4][first custom style]'My [player-penis] would be fun to mess around with, right?'[otherwise if the penis-obedience of penis < 6][line break][variable custom style]'[one of]I'm sure you can come up with lots of inventive ways to punish this nasty [player-penis]...'[or]Wanna let my [player-penis] take it from here? I'm sure you'll like it...'[in random order][otherwise][second custom style]'[one of]I bet you want me nice and hard, huh? What are you going to do to this nasty pervert's vulnerable and exposed [player-penis]?'[or]I bet it's my [player-penis] that you want, isn't it? Are we going to have some fun?'[in random order][end if]";
		otherwise:
...
To say TakeMyPenisVirginity of (M - a monster):
	if M is unfriendly:
		if the anal sex addiction of the player < 3:
			say "[first custom style]'Are you really going to stick it in my butt? I haven't even had normal sex yet...'";
