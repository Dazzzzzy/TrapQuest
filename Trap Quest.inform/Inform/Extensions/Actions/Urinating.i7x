...
toilet is a backdrop. The printed name of toilet is "[SlimeTarget]". Understand "potty", "throne", "bathroom", "slime target", "target" as toilet. The text-shortcut of toilet is "ttws". Figure of toilet is the file "Env/MultiFloor/toilet1.png". Figure of facility toilets is the file "Env/Facility/toilets1.jpg". Figure of facility toilets locked is the file "Env/Facility/toilets2.jpg".
urinal is a backdrop. The printed name of urinal is "[SlimeTargetU]". Understand "potty", "urinals", "bathroom", "slime target", "target" as urinal. The text-shortcut of urinal is "utws".



To say ExamineDesc of (T - toilet):
	say "A [SlimeTarget][if watersports mechanics is 1] with a deep bowl. You can [bold type]use[roman type] this to relieve yourself[end if].".
To say ExamineDesc of (T - urinal):
	if the hotel-urinal-scene of woman-player > 0 and the player is in Hotel31:
		say "[if there is a wrestler in Holding Pen][BigNameDesc of random wrestler in Holding Pen] and [NameDesc of woman-player] are here, bound in latex bodysuits as diapered human [SlimeTargetU]s[otherwise][BigNameDesc of woman-player] is here, bound in a latex bodysuit as a diapered human [SlimeTargetU][end if].";
	otherwise if T is grabbing the player or the urinal-rival of urinal-competition-punishment is guarding monster:
		say "Vibrators fixed in position at the front of the [SlimeTargetU]s torture the latex-clad people installed in place as human [SlimeTargetU]s.";
...
To decide which figure-name is the examine-image of (T - toilet):
	...
	if slimeshooter fetish is 1, decide on figure of toilet;
	if watersports fetish is 1:
		if the player is in Hotel38 and slimeshooter fetish is 0 and a2m fetish >= 2 and the human-toilet-scene of woman-player is not 1, decide on figure of human toilet;
...
To construct normal buttons for (T - toilet):
	if ButtonTableFull is 0 and watersports mechanics is 1 and (the total squirtable fill of belly > 0 or the player is not totally incontinent):
		choose a blank row in the Table of Buttons;
		now the ButtonImage entry is Figure of ToiletButton;
		now the ButtonCommand entry is the substituted form of "use [SlimeTarget]";
	...
	if ButtonTableFull is 0 and the human-toilet-scene of woman-player > 1 and there is a held unlock-key and the player is in Hotel38:
		...
		now the ButtonCommand entry is the substituted form of "unlock [SlimeTarget]";
		now the ButtonColour entry is lightModeFullGreen;
...
To construct normal buttons for (T - urinal):
	if ButtonTableFull is 0 and watersports mechanics is 1 and the player is not bladder incontinent:
		choose a blank row in the Table of Buttons;
		now the ButtonImage entry is Figure of ToiletButton;
...
Report going when the player is able to use the toilet past their diaper:
	...
	if (AC is clothing and the player is feeling full) or (PC is clothing and the player is bursting):
		...
		if legacy watersports mechanics is 1, say "[bold type]As you enter this room, the leg holes of your [ShortDesc of AC] [bold type][one of]seem to [or][stopping]temporarily loosen. [roman type][if AC is cursed][one of]You can't take them off because the waist is still tightly sealed, but you could probably displace the crotch in order to use the loo.[or][stopping][otherwise]You could probably pull it to the side and use the [SlimeTarget] without removing it if you liked.[end if]";
		say "[bold type]As you enter this room, the leg holes of your [ShortDesc of AC] [bold type][one of]seem to [or][stopping]temporarily loosen. [roman type][if AC is cursed][one of]You can't take them off because the waist is still tightly sealed, but you could probably displace the crotch in order to empty your slimepistol[or][stopping][otherwise]You could probably pull it to the side and empty your slimepistol without removing it if you liked.[end if]".
...
Check toileting:
	...
	if the location of the player is urinals:
		...
		if the player is bladder incontinent:
			if legacy watersports fetish is 1, say "You have no control over that - you are [one of]fully[or]hopelessly[or]utterly[or]totally[or]completely[at random] incontinent." instead;
			otherwise say "You have no control over that - your slime shooter does whatever it wants. It's [one of]fully[or]hopelessly[or]utterly[or]totally[or]completely[at random] incorrigible." instead;
	...
	otherwise if the location of the player is toilets:
		if the total squirtable fill of belly <= 0 and the player is totally incontinent:
			if legacy watersports fetish is 1, say "You can't use the [SlimeTarget] normally because you are [one of]fully[or]hopelessly[or]utterly[or]totally[or]completely[at random] incontinent." instead;
			otherwise
			say "You can't just shoot your slime shooter in the [SlimeTarget] because it's [one of]fully[or]hopelessly[or]utterly[or]totally[or]completely[at random] incorrigible. It does whatever it wants." instead;
		let P be a random thing penetrating asshole;
		if PC is clothing and (P is a thing or AC is a clothing):
			say "You can't [SlimeShoot] in the [SlimeTarget] because of [NameDesc of PC][run paragraph on]";
		...
		if PC is clothing or watersports mechanics is 0:
			if (the player is not full or diaper messing < 4) and the total squirtable fill of belly <= 0:
				say "You [if watersports mechanics is 1]can't [SlimeShoot] in the [SlimeTarget] because of your [ShortDesc of PC] and you [end if]don't need to expel anything from your [asshole]." instead;
			otherwise if watersports mechanics is 1 and P is not a thing and AC is not a clothing:
				if legacy watersports mechanics is 1, say "If you sit on the [SlimeTarget] and push, you might accidentally [slimedrain] in your [ShortDesc of PC] at the same time. Do you want to risk it?";
				say "If you sit on the [SlimeTarget] and push, you might accidentally [SlimeShoot] in your [ShortDesc of PC] at the same time. Do you want to risk it?";
				if the player is not consenting, say "Action cancelled." instead;
		if the player is toilet traumatised:
			if the number of intelligent awake friendly monsters in the location of the player is 0, say "After your traumatising experience with [NameDesc of toilet-monster], you're too scared to use the [SlimeTarget] all by yourself!" instead;
		otherwise if the player is proud:
			let M be a random reactive monster;
			if M is monster, say "[variable custom style]I can't [if PC is clothing]pull my [ShortDesc of PC] down and [end if]sit on the [SlimeTarget] and use it with the [ShortDesc of M] right there!!![roman type][line break]" instead.
...
Check urinating:
	...
	if the bladder of the player is 0 and the number of worn prostate massager plugs is 0:
		if delayed urination is 0, say "[if slimeshooter fetish is 1]Your slimeshooter isn't around right now.[otherwise]You don't feel the need.[end if]" instead;
		if debugmode is 1, say "resetting accidental [if legacy watersports fetish is 1]urination[otherwise]sliming[end if] flag.";
		...
		say "[one of]Player was forced to [slimedrain] when they had nothing in their [SlimeContainer], please report the bug to Aika![or][stopping]";
		do nothing instead;
	[if diaper lover is 0 and (the player is not bursting or the bladder of the player is 0):
		...
		do nothing instead;]
	if the player is bladder incontinent and delayed urination is 0:
		if legacy watersports fetish is 1, say "You are [one of]fully[or]hopelessly[or]utterly[or]totally[or]completely[at random] incontinent; you can't control this at all any more! Pushing just does nothing." instead;
		otherwise say "Your slimeshooter is [one of]fully[or]hopelessly[or]utterly[or]totally[or]completely[at random] incorrigible; you can't control it at all any more! Pulling the trigger just does nothing." instead;
	if the latex-transformation of the player > 4, say "[if delayed urination is 0]You can't [SlimeShoot], you're a sex doll![end if]" instead;
	...
	if delayed urination is 0 and (the player is able to use a toilet or the player is able to use a urinal):
		say "Did you want to use the [if the location of the player is urinals][SlimeTargetU][otherwise][SlimeTarget][end if]? ";
		...
		if toilet-refused is 1 or (the player is not able to use a toilet and the player is not able to use a urinal):
			...
			if there is pee covering actually unavoidable clothing or (there is a worn diaper and the diaper addiction of the player >= 3):
				...
				if P is not diaper or the diaper addiction of the player < 3:
					say "Do you really want to [SlimeShoot] inside your [printed name of P]? ";
			...
			otherwise if P is clothing:
				...
				if the player is able to displace:
					...
					otherwise:
						...
						if P is pants pee refusal inducing and debugmode is 0, do nothing instead;
				otherwise:
					say "You can't displace [NameDesc of P] right now, so [if legacy watersports fetish is 1]you'll have to pee[otherwise]your slimeshooter will definitely aim into[end if] into it. Are you sure that's what you want? ";
					now auto is 0;
	...
	if the player is prone and the location of the player is bathroom and delayed urination is 0 and urinal is not grabbing the player:
		say "Do you really want to try to [SlimeShoot] [if the player is pee protected]in your pants[otherwise]on the floor here[end if], while kneeling? ";
		if the player is bimbo consenting:
			...
			if (the delicateness of the player < 4 or (the player is modest and M is a person)):
				...
				otherwise say SlimeShootRefuseFlav to M;
		otherwise:
	...
	if a random lake monster is in the location of the player and the bimbo of the player < 14 and delayed urination is 0:
		say "Do you really want to try to [SlimeShoot] on the floor here? ";
		if the player is bimbo consenting:
			let M be a random reactive person;
			if (the delicateness of the player < 4 or (the player is modest and M is a person)):
				if debugmode > 0, say "If debugmode was disabled, the player would refuse to [SlimeShoot].";
				otherwise say SlimeShootRefuseFlav to M;
		otherwise:
...
Definition: a clothing (called P) is pants pee refusal inducing:
	let M be a random reactive person;
	if the slime-soak of P <= 0 and P is not diaper and (the delicateness of the player < 6 or (the player is not disgraced and M is a person)) and (diaper quest is 0 or M is a person):
		if debugmode > 0, say "If debugmode was disabled, the player would refuse to [SlimeShoot].";
		otherwise say SlimeShootRefuseFlav to M;
		decide yes;
	decide no.
...
To say SlimeShootRefuseFlav to (M - a person):
	say "[first custom style]I'm NOT [if legacy watersports fetish is 1]peeing[otherwise]letting my slimeshooter ruin[end if] my pants[if M is a person and the delicateness of the player >= 6] in front of [FuckerDesc of M][first custom style].[otherwise].[end if][roman type][line break]";
...
To compute toilet use:
	...
	if the location of the player is Facility10 and locked-toilets is true and facility-toilet-key is held, say "You unlock the [SlimeTarget] lid with the key.";
	if watersports mechanics is 1 and the player is not bladder incontinent and (the number of pee covering actually unavoidable clothing is 0 or the player is able to use the toilet past their diaper):
		if the bladder of the player > 0:
			...
			if diaper quest is 1 and the player is in Hotel38 and the human-toilet-scene of woman-player is 2:
				say "sit on the Punishment [big SlimeTargetP] above [NameDesc of woman-player][']s face and release your hold on your [SlimeContainer].[line break][speech style of woman-player]'Wait please, no, [one of]let's talk about this[or]not again[stopping]- FFFFFFFFBBBBBFFFFFTTTT!'[roman type][line break][BigNameDesc of woman-player][']s pleas are drowned out by the heavy flow of your [slime].[line break][speech style of woman-player]'You [bitch]!'[roman type][line break]";
				now too is "NOPE";
			otherwise:
				say "sit on the [if the player is in Dungeon11]training [SlimeTargetP] throne[otherwise]nearby [SlimeTarget][end if] [if the player is in Hotel38 and the human-toilet-scene of woman-player is 2]above [NameDesc of woman-player][']s ring-gagged face [end if]and release your hold on your [SlimeContainer].";
				now too is " too";
			now the bladder of the player is 0;
			now toiletJustUsed is true;
	let incontinent-and-no-enema be false;
	if the player is rectum incontinent and the total squirtable fill of belly <=  0, now incontinent-and-no-enema is true;
	...
	otherwise if initialBladder is 0:
		say "You squeeze your [SlimeContainer] but nothing comes out!";
	if the player is bursting and wetting-valued <= 0 and the bladder-incontinence of the player < 5 and the rectum-incontinence of the player < 5: [This happens if the player uses the toilet with undisplacable pee covering clothing, in order to empty their belly]
		say "As you push the contents of your belly out, you can't help but also release hold on your [SlimeContainer]!";
	...
	if the player is in Dungeon11:
		...
		if R is 1 and the player is not diapered:
			...
			say "As you finish your business, you suddenly feel a burst of energy and blinding light from the seat of the [SlimeTargetP]. Before you can react, you find a [ShortDesc of D] wrapped around your waist! Tugging at the waistband, you confirm your suspicion: it's cursed![line break][variable custom style][if the diaper addiction of the player < 11]But I just proved I can use the [otherwise]I got to go like a grown-up AND I got fresh protection! Thanks, [end if][SlimeTargetP]![roman type][line break]";
		otherwise if R is 2:
			say "As you finish your business, you suddenly feel a burst of energy rush from the seat of the [SlimeTargetP] to your head. ";
		...
		otherwise if (R is 3 or R is 4) and the diaper addiction of the player > 0:
			...
			say "You suddenly feel a rush of energy to your head, and your mind suddenly feels a little clearer. It seems the training [SlimeTargetP] has helped reduce your addiction to wearing and using diapers!";
		otherwise:
...
To compute urinal use:
	...
	otherwise say "You ";
	if the location of the player is urinals, say "[if the player is possessing a penis]walk up to[otherwise]squat in front of[end if] [if the player is in Dungeon41][NameDesc of shopkeeper][otherwise]the [SlimeTargetU][end if] and ";
	say "release your hold on your [slime].";
	if the bladder of the player > 0:
		if the hotel-urinal-scene of woman-player > 0 and the player is in Hotel31:
			let M be a random wrestler in Holding Pen;
			if M is monster:
				reset multiple choice questions; [ALWAYS REMEMBER THIS WHEN MAKING A MULTIPLE CHOICE QUESTION]
				set numerical response 1 to "[SlimeShoot] on [WomanName], letting [him of woman-player] drink it";
				set numerical response 2 to "[SlimeShoot] on [WomanName], NOT letting [him of woman-player] drink it";
				set numerical response 3 to "[SlimeShoot] on [NameDesc of M], letting [him of M] drink it";
				set numerical response 4 to "[SlimeShoot] on [NameDesc of M], NOT letting [him of M] drink it";
				...
				if player-numerical-response is 1 or player-numerical-response is 3:
					...
					say "Your [slime] fills [NameDesc of M][']s mouth, and [he of M] obediently gulps as much down as [he of M] can. Shortly afterwards you hear a hissing sound as [NameDesc of M] fills [his of M] diaper. At the same time, [he of M] squeals with a high pitched note as [his of M] vibrating wand brings [him of M] to another powerful orgasm.";
					if M is wrestler:
				...
				say "[if player-numerical-response is 2 or player-numerical-response is 4]Both human [SlimeTargetU]s groan as[otherwise]Some of[end if] the [slime] trickles down the latex bodysuit into the drain. And then, [one of]much to your surprise, [or][stopping]it's [NameDesc of M][']s wand vibrator that suddenly kicks into turbo mode, now vibrating even more loudly and powerfully. [one of][bold type]This must mean that each [SlimeTargetU]'s drain is somehow hooked up to the other human [SlimeTargetU]'s vibrator! [roman type][or][stopping][big he of M] screams in a mixture of panic and pleasure as [his of M] mind begins to melt from the excessive overstimulation.";
			...
			otherwise:
				...
				reset multiple choice questions; [ALWAYS REMEMBER THIS WHEN MAKING A MULTIPLE CHOICE QUESTION]
				set numerical response 1 to "[slimedrain] on [WomanName], letting [him of M] drink it";
				set numerical response 2 to "[slimedrain] on [WomanName], NOT letting [him of M] drink it";
				compute multiple choice question;
				if player-numerical-response is 1:
					...
					FavourUp M by 5;
				otherwise:
					say "[BigNameDesc of M] groans in frustration as the [slime] trickles down [his of M] latex bodysuit into the drain.";
					FavourDown woman-player;
		otherwise if diaper quest is 0 and the woman-status of woman-player is 98 and woman-player is in the location of the player:
			say "[speech style of woman-player]'[one of]What are you doing?! No no no wait-'[or]Not again?! Eeeek!'[stopping][roman type][line break][BigNameDesc of woman-player] shrieks with panic, before [his of woman-player] words are replaced with a strained gargle as your [slime] hits [his of woman-player] face and mouth.";
			DelicateDown 1;
			FavourDown woman-player;
			womanSluttify;
			say "[speech style of woman-player]'[NameBimbo], you bitch! I thought we were friends!'[roman type][line break][BigNameDesc of woman-player] cries angrily as [he of woman-player] spits out a mouthful of [slime].[line break][speech style of woman-player]'Gross gross gross!'[roman type][line break]";
		otherwise if the player is in Dungeon41:
			say "[BigNameDesc of shopkeeper] groans in frustration as the [slime] floods [his of shopkeeper] mouth, and [he of shopkeeper] is forced to swallow. But there's absolutely nothing [he of shopkeeper] can do about it but drink it down, and wait until it comes out [his of shopkeeper] own [SlimeContainer] a bit later and joins the rest of the [slime] soaking [his of shopkeeper] extra-thick padding. You can't help but feel a bit better about yourself in comparison to this pathetic human [SlimeTarget]!";
			Dignify 1000;
			DelicateDown 1;
		otherwise if the player is in Facility27:
			say "The human [SlimeTargetU] whimpers as the [slime] floods their mouth, and they are forced to swallow. But there's absolutely nothing they can do about it but drink it down, and wait until it comes out their own [SlimeContainer] a bit later and joins the rest of the [slime] soaking their extra-thick padding. You can't help but feel a bit better about yourself in comparison to this pathetic human [SlimeTarget]!";
			Dignify 1000;
			DelicateDown 1;
		otherwise if the player is in Facility35: [princess urinal]
			let M be ex-princess;
			say "[BigNameDesc of M] [if legacy content is 1]wails in despair as your [slime] flows towards [his of M] food.[speech style of M]'You fucking [cunt][otherwise]grunts in frustration as your [slime] flows towards [his of M] food.[speech style of M]'And there I thought maybe you were here to rescue me[end if]!'[roman type][line break]That's all [he of M] has time to say before [he of M] has to begin sucking up your [slime] and transferring it, mouthful by mouthful, into [his of M] [SlimeTarget][unless the player is broken]. You feel more dominant, and less ashamed of yourself[end if]!";
			DelicateDown 1;
			obsceneDignify;
...
This is the urination continues rule:
	if the player is in danger, say "You try to stop the flow but you can't! You continue to [slimedrain]. ";
	otherwise say "[one of]Time passes as you[or]You[cycling] continue to [slimedrain]. ";
	compute urination.
...
To start urination:
	...
	if pee-bottling is 1: [We are automatically urinating so we set delayed urination to 1 to make sure we skipped the check functions. But we don't want to tell the player that the urination was involuntary because that's not actually true.]
		...
		if debugmode is 1, say "resetting accidental [if slimeshooter fetish is 1]drain[otherwise]urination[end if] flag.";
	...
	if the player is not pee protected:
		if the player is flying:
			if legacy watersports fetish is 1, say "You [slimedrain] freely from the air onto the ground below.[line break][variable custom style]Well this is a new experience...[roman type][line break]";
			otherwise say "You freely empty your slime shooter onto the ground far below.[line break][variable custom style]No one can tell it was me...[roman type][line break]";
			if the bladder of the player > 6:
				SlimePuddleUp 6;
			otherwise:
				SlimePuddleUp the bladder of the player;
		otherwise if the player is able to use a body of water and delayed urination is 0:
			say "Get into the water (people won't be able to see you [slimedrain]ing but you'll get wet)? ";
			if the player is consenting:
				say "You walk past the edge, wading into the water until you can discreetly [slimedrain]. ";
				progress quest of careful-peeing-quest;
		...
		otherwise if player-currently-resting is 1 and there is a hotel bed in the location of the player:
			say "You [if delayed urination is 1]involuntarily [end if]release your hold on your [SlimeContainer]. Your [slime] soaks into the sheets and mattress.";
		...
		otherwise if gloryhole is grabbing the player and gloryhole is blowjob portal:
			say "[if the slime taste addiction of the player > 11 and delayed urination is 0]With an eager grin you[otherwise]You[end if] [if delayed urination is 1]involuntarily [end if]release your hold on your [SlimeContainer]. Your [slime] goes straight into your own mouth at full force!";
			FaceFill slime by 3;
		otherwise if gloryhole is grabbing the player and gloryhole is normal glory hole:
			say "You [if delayed urination is 1]involuntarily [end if]release your hold on your [SlimeContainer]. You are [one of][or]UN[stopping]surprised to find that your new anonymous oral servant doesn't pull away!";
		otherwise if diaper quest is 1 and the player is in Iron Maiden:
			say "Your [slime] soaks into the thick Iron Maiden padding pressed against your crotch.";
			DiaperAddictUp 1;
			say "The spell of your [slime] in the cramped room floods your nostrils.";
			if watersports fetish is 1, SlowSlimeTasteAddictUp 1;
			otherwise SlowSexAddictUp 1;
		otherwise:
			say "You [if delayed urination is 1]involuntarily [end if]release your hold on your [SlimeContainer]. Your [slime] [if the player is possessing a vagina and the player is dildo stuck]trickles around the dildo and down your thighs.[otherwise if playerRegion is Dungeon]flows to the ground, creating a puddle on the floor.[otherwise]flows to the ground.[end if]";
			say PeeReaction 1;
			if the bladder of the player > 6:
				SlimePuddleUp 6;
			otherwise:
				SlimePuddleUp the bladder of the player;
				punish puddle sliming;
	otherwise:
		...
		if the player is potentially able to use a toilet and delayed urination is 0 and the player is not immobile and the player is not in danger and the player is not flying:
			say "Would you like to sit on the [SlimeTarget] while [if legacy watersports mechanics is 1]you soil your [ShortDesc of K] (to avoid wetting the floor if you leak)?[otherwise]you fire your [ShortDesc of K] (to avoid wetting the floor if anything leaks)?[end if]";
			if the player is consenting:
				say "You sit on the [SlimeTarget]. ";
				now toilet-sitting is true;
			otherwise:
				say "You stay where you are. ";
		if delayed urination is 1, say "[if legacy watersports fetish is 1]You involuntarily release your hold on your [SlimeContainer]. [otherwise]You slimeshooter screeches as it pulls its own trigger. [end if]";
		otherwise say "You [if legacy watersports fetish is 1]release your hold on your [SlimeContainer]. [otherwise]pull the trigger on your slimeshooter. [end if]";
	...
	if diaper lover >= 1:
		...
		otherwise if delayed urination is 1 and the bladder-incontinence of the player < the max-bladder-incontinence of the player and the diaper addiction of the player > the bladder-incontinence of the player + 10 and there is a worn total protection diaper:
			say "[one of]You can't help but notice that losing control and [slime]ing your diaper feels weirdly nice and comforting. [or][stopping][bold type]You feel your [SlimeContainer] weakening[one of][or] further[stopping], in support of your growing addiction to and dependence for diapers.[roman type][line break]";
...
To compute pee protected urination:
	if WC-plug-panties is worn:
		say "Your plug panties [one of]seem to somehow absorb the [slime], and also vibrate powerfully in response! [or]absorb the [slime], vibrating powerfully as they do. [stopping]";
	...
	otherwise:
		...
		let oldSoak be 99;
		unless K is nothing, let oldSoak be the slime-soak of K;
		let N be 6;
			...
			repeat with X running from 1 to N:
				if K is nothing:
					if player-currently-resting is 1 and there is a hotel bed in the location of the player:
						if flav-said is 0, say "Your [slime] soaks into the sheets and mattress.";
						now a random hotel bed in the location of the player is soggy;
					otherwise if toilet-sitting is true:
						if flav-said is 0, say "Your [slime] drips through your clothing and into the [SlimeTarget].";
					otherwise:
						increase totalOverflow by 1;
						if flav-said is 0, say "Your [slime] drips through your clothing!";
					...
					trigger pee-wisp-trigger;
				otherwise:
					if flav-said is 0, say "Your [slime] flows into your [ShortDesc of K][unless K is fluid immune or K is diaper], soaking it[end if].";
					SlimeSoakUp K by 1;
					if X is 1:
					...
					if K is not sK and X < N: [We now need to soak past the original thing we were urinating into and this isn't the last unit of slime]
						say slimeoverflow of K;
					...
					otherwise if diaper quest is 1 and K is not sK: [exactly full]
						progress quest of priestess-service-quest;
				now flav-said is 1;
			if totalOverflow > 0, AnnouncedExpel slime on thighs by totalOverflow.
...
To compute urination:
	...
	if the player is not pee protected or the player is able to use a toilet:
		if the player is flying:
			say "You continue to [SlimeShoot] onto the ground below.";
			if the bladder of the player > 6:
				SlimePuddleUp 6;
			otherwise:
				SlimePuddleUp the bladder of the player;
		otherwise if the player is able to use a toilet:
			say "You continue to use the [SlimeTarget].";
		otherwise if the player is able to use a body of water:
			say "You continue to [SlimeShoot] into the water. ";
		otherwise if gloryhole is grabbing the player and gloryhole is blowjob portal:
			FaceFill slime by 3;
		otherwise if gloryhole is grabbing the player and gloryhole is normal glory hole:
			do nothing;
		otherwise if diaper quest is 1 and the player is in Iron Maiden:
			say "Your [slime] continues to soak into the thick Iron Maiden padding.";
		otherwise:
			say "Your [slime] continues to [if the player is possessing a vagina and the player is dildo stuck]trickle around the dildo and down your thighs.[otherwise if playerRegion is Dungeon]flow to the ground, creating a puddle on the floor.[otherwise]flows to the ground.[end if]";
			if the bladder of the player > 6:
				SlimePuddleUp 6;
			otherwise:
				SlimePuddleUp the bladder of the player;
	...
	otherwise:
		...
		if K is not diaper and diaper lover >= 1 and the bladder of the player <= 6:
			...
			if N is actually summonable and a random number between 1 and 6 - (unlucky * 4) is 1:
				...
				compute quest of N;
				say "[bold type]As you finish [SlimeDraining], a flimsy [ShortDesc of N] shimmers into existence around you![roman type][line break][variable custom style]Because I got [slime] on myself?[roman type][line break]";
...
To end urination:
	...
	if T is clothing and diaper lover >= 1 and the player is not in a predicament room and the class of the player is not berri:
		...
		if there is a worn diaper:
			if T is cursed and the slime-soak of a random worn diaper is 0 or the slime-soak of a random worn diaper is the soak-limit of a random worn diaper:
...
Understand "slime", "drain", "pee", "piss", "urinate", "wee" as urinating.

To say slimeoverflow of (K - a clothing):
	say "But your [ShortDesc of K] [if K is fluid immune]is waterproof, so the [slime] seeps through the leg-holes[otherwise]became so thoroughly sodden that the [slime] seeps through the fabric[end if].".

To say slimeoverflow of (K - a diaper):
	say "But your poor diaper is so completely full for your [slime] that it can't hold any more! You feel the diaper overflow through the leg holes.".

To punish puddle sliming:
	if playerRegion is Woods and the player is prone:
		let H be a random off-stage puppy ears;
		if H is actually summonable:
			summon H cursed;
			say "[bold type]Suddenly, a [H][bold type] appear on your head![if the bimbo of the player < 9][line break][variable custom style]Oh no... I this because I was [SlimeShoot]ing [slime] like a dog?[end if][roman type][line break]";
	check slime maidification.
...
To check (L - a liquid-object) maidification:
	unless the player is in a predicament room:
		if the class of the player is maid:
			...
			if (L is not slime or watersports fetish is 1) and bukkake fetish is 1 and a random number between 1 and 2 is 1:
			...
			otherwise if L is not slime or watersports fetish is 1:
				...
				if N > 0:
					...
					repeat with V running through carried vessels:
						...
						if L is slime, now the fill-colour of V is golden;
			...
			otherwise if the player is not bladder incontinent:
				if legacy watersports mechanics is 1, say "You feel a twinge from behind your [SlimeContainer], as if it is punishing you by making you gradually more incontinent...";
				otherwise say "You hear a pained whistle from your slime pistol as if it is punished...";
				SilentlyBladderIncontinenceUp 1;
...
To check pee leaking with reason (T - a text):
	...
	if diaper lover > 0 and the player is not incontinent:
		...
		if B > I:
			...
			if debuginfo > 0, say "[input-style][slime] leak check: [SlimeContainer] ([bladder of the player]) - continence rating ([I]) = [B] ---> RNG([bladder-difficulty] ~ [B]) = [R] | positive number[roman type][line break]";
			if R > 0:
...
To compute pee leaking with reason (T - a text):
	if the bladder of the player > 0:
		...
		if legacy watersports mechanics is 1:
			if T is "", say "[one of]You spontaneously leak a little pee[or]All of a sudden, your [SlimeContainer] spasms and you squirt out a little wee[or]Completely beyond your control, your [if the player is possessing a penis][player-penis][otherwise][SlimeContainer][end if] lets out a squirt of pee[in random order]. ";
			otherwise say "[T] [one of]you squirt out a little bit of pee[or]you leak a little squirt of pee[or]your [SlimeContainer] muscles tense and accidentally let out a little pee[in random order]. ";
		otherwise:
			if T is "", say "[one of]Your slime pistol spontaneously leaks a little slime[or]All of a sudden, your slime pistol spasms and squirts out a little slime[or]Completely beyond your control, your slime pistol lets out a squirt of slime[in random order]. ";
			otherwise say "[T] [one of]your slime pistol squirts out a little bit of slime[or]your slime pistol leaks a little squirt of slime[or]your slime pistol tenses and lets out a little slime[in random order]. ";
		say roman type;
		if K is clothing, AnnouncedExpel slime on K by 1;
		otherwise AnnouncedExpel slime on thighs by 1;
...
To check full wetting with reason (T - a text):
	if the player is bladder incontinent and (temporary-bladder-incontinence is 0 or the raw-bladder-incontinence of the player >= 8): [every single unit of pee immediately gets peed out unless it's temporary incontinence]
	...
	otherwise:
		...
		if B >= 0 or resting-wetter > 0:
			...
			if debuginfo > 1, say "[input-style]Automatic [slime]ing check: [if resting-wetter > 0 and B is resting-wetter]magic bed [slime]ing effect ([resting-wetter])[otherwise][SlimeContainer] ([bladder of the player]) - continence rating ([I]) = [B][end if] ---> RNG([bladder-difficulty] ~ [B]) + RNG([bladder-difficulty] ~ [B]) = [R] | positive number[roman type][line break]";
...
To say PeeReaction (N - 1):
	...
	if M is monster:
		...
		repeat with Z running through reactive monsters:
			...
			otherwise say GroundSlimeReaction of Z;
	otherwise:
		...
		if delayed urination is 0: [Purposeful urination]
			if M is monster:
				...
				repeat with Z running through reactive monsters:
					strongHumiliate;
					say GroundSlimeReaction of Z;
			otherwise if playerRegion is Woods:
				say "You sigh pleasantly, and now feel much more comfortable. ";
			otherwise:
				say "[variable custom style][if the humiliation of the player < HUMILIATION-MODEST - 1000 and playerRegion is Dungeon]How embarrassing! I need to find a [SlimeTarget] next time.[otherwise][one of]This isn't even the most humiliating thing I've been through today.[or]Ahh, sweet release.[stopping][end if][roman type][line break]";
...
To say GroundSlimeReaction of (M - a monster):
	if M is intelligent:
		say "[speech style of M]'[one of]Gross[or]Disgusting[or]Ugh[in random order]. [if diaper quest is 1][one of]Are you sure you don't need diapers?!'[or]This wouldn't happen if you were kept in diapers, you know.'[or]Do your parents know that you still [slime] on the floor?!'[in random order][otherwise][one of]Can't you find a [SlimeTarget] to do that?!'[or]Next time use a [SlimeTarget], you tramp!'[or]How shameless do you have to be to [slimedrain] on the floor in front of others?!'[in random order][end if][roman type][line break]";
		FavourDown M;
	...
	if diaper-reaction-said is true:
		...
		otherwise: [Diaper is fully visible, NPCs can see it]
			...
			if the player is not disgraced:
				...
				now diaper-reaction-said is false;
			otherwise if the player is not shameless:
				say "You [if delayed urination is 1]freeze in place and [end if]stare at [NameDesc of M] coyly, drawing [his of M] attention down to your rapidly dampening diaper.[if the diaper addiction of the player < 9][line break][first custom style][one of]What am I doing?![or]Why do I want [him of M] to notice?![in random order][otherwise][line break][variable custom style][one of]I'm so naughty![or]Come on, say something![or]That's right, down here! Now watch me [slime] on myself![in random order][end if][roman type][line break]";
			otherwise if the player is able to speak:
				say "You stare directly at [NameDesc of M].[line break][variable custom style]'[one of][if the intelligence of the player < 6][NameBimbo] is going tinkles!'[otherwise]Watch me [slime] on myself!'[end if][or]Please watch me as I [slime] in my diaper!'[or]Are you watching me wet myself?'[in random order][roman type][line break]".
...
To say ClothesPeeDeclaration of (M - a monster):
	...
	otherwise: [Accidental urination]
		if the humiliation of the player < HUMILIATION-SHAMELESS - 2000, say "[if the player is able to speak][line break][variable custom style]'Eek! Don't watch!'[roman type][line break][end if]You stay still like a deer in headlights as [NameDesc of M] watches you [slimeonself].";
		otherwise say "[BrokenPeeFlav during sex with M]".

To say ClothesPeeReaction of (M - a monster):
	if diaper quest is 1:
		if M is friendly:
			say "[BigNameDesc of M] frowns.[line break][speech style of M]'What are you doing?! It looks to me like you need diapers, [boy of the player]. I am very unimpressed.'[roman type][line break]";
			FavourDown M by 2;
			if M is unfriendly:
...
To say DiaperReaction of (M - a monster):
	...
	if D is not diaper and the player is not shameless and M is not concealment immune:
		say "You're confident that [NameDesc of M] has no idea [one of]what's happening underneath your clothes[or]that you're currently wetting yourself[or]that you're going to the [SlimeTarget] in your pants right in front of [him of M][at random].";
...
This is the compulsory urination rule:
	if delayed urination is 1 and busy is 0:
		...
		if the player is diapered and (the player is not bursting or the player is bladder incontinent): [The player can be bursting and incontinent at the same time thanks to the prostate plug]
			let D be a random worn diaper;
			if the bladder of the player > 0 and the slime-soak of D + the bladder of the player <= the soak-limit of D: [The player always notices properly if the diaper overflows.]
				let bladder-before be the bladder of the player;
				let slime-before be the slime-soak of D;
				StealthSlimeSoakUp D by the bladder of the player;
				now the bladder of the player is 0;
				...
				if there is an intelligent monster in the location of the player and (D is not currently visible or (D is diaper-stack and entry (number of entries in the list of stacked diapers) in the list of stacked diapers is dry diaper)), progress quest of stealth-diaper-quest;
		...
		if diaper-happened is 0:
			if delayed urination flav is not "":
				say "[bold type][delayed urination flav] you completely lose control of your [SlimeContainer].[roman type][line break]";
			otherwise if the player is bursting or diaper lover is 0:
				say "[bold type]You can't control your [SlimeContainer] any longer![roman type][line break]";
			otherwise:
				say "[bold type]A warm wet feeling makes you realise that [if legacy watersports mechanics is 0]your slimeshooter is leaking.[otherwise]you're currently peeing yourself.[end if][roman type][line break]";
		...
		if debugmode > 1, say "resetting accidental [if slimeshooter fetish is 1]drain[otherwise]urination[end if] flag.";
		now delayed urination flav is "";
		now delayed urination is 0.
The compulsory urination rule is listed in the compulsory action rules.

punishment-potty is a thing. punishment-potty is not portable. The printed name of punishment-potty is "[TQlink of item described]punishment [SlimeTargetP][shortcut-desc][TQxlink of item described][verb-desc of item described]". The text-shortcut of punishment-potty is "potty". Understand "punishment", "target", "potty" as punishment-potty.
...
To say ExamineDesc of (C - punishment-potty):
	say "A large hollow plastic [SlimeTargetP] frame that reads 'Punishment [big SlimeTargetP]' on the front. It is bolted to the floor, trapping your head inside.".
