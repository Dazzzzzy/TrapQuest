...
Check mopping:
	...
	if the milk-puddle of the location of the player + the semen-puddle of the location of the player + the slime-puddle of the location of the player < 1, say "But there's no mess to clean?" instead;
	...
	if debugmode is 0 and the noun is face and semen is not craved and the semen-puddle of the location of the player > 0 and the class of the player is not maid and the class of the player is not catgirl:
		if the semen taste addiction of the player < 5 and the player is not broken and the player is not extremely thirsty, say "[if the semen taste addiction of the player > 2][variable custom style]I'd have to be more thirsty than this before I'd consider doing that...[roman type][line break][otherwise]You can't bring yourself to do something so disgusting.[end if]" instead;
	...
	otherwise if debugmode is 0 and the noun is face and the slime-puddle of the location of the player > 0 and the class of the player is not maid and the class of the player is not catgirl:
		if the slime taste addiction of the player < 5 and the player is not broken and the player is not extremely thirsty, say "[if the slime taste addiction of the player > 2][variable custom style]I'd have to be thirstier than this before I'd consider doing that...[roman type][line break][otherwise]You can't bring yourself to do something so disgusting.[end if]" instead;
		if there is a dangerous monster in the location of the player and the humiliation of the player < (20 - the slime taste addiction of the player) * 1000, say "You wouldn't dare do that in front of the [random monster in the location of the player]. [if the slime taste addiction of the player > 14][line break][second custom style]no matter how much you want to...[roman type][line break][end if]" instead;
	if the noun is clothing and the noun is not pink-spraybottle:
		...
		if the semen-soak of the noun + the slime-soak of the noun + the milk-soak of the noun >= the soak-limit of the noun, say "The [printed name of noun] is too covered in bodily fluids to effectively remove any more from the ground." instead;
...
To compute mopdipping:
	if the largeness of breasts > 10 and total puddle > 0:
		...
		let U be the slime-puddle of the location of the player;
		...
		if (S is 0 and TBP is clothing) or (S > 0 and (TBP is not clothing or TBP is semen-soak-appropriate clothing)):
			...
			if U > 0 and TBP is clothing:
				UnannouncedSquirt slime on TBP by 1;
				decrease the slime-puddle of the location of the player by 1;
...
Carry out mopping face:
	allocate 6 seconds;
	let A be the slime-puddle of the location of the player;
	let B be the milk-puddle of the location of the player;
	let C be the semen-puddle of the location of the player;
	let D be 0;
	let E be 0;
	let F be 0;
	say "You begin [one of]to lick up[or]slurping up[or]licking up[at random] the [if C > 0 and C > B + A][semen] clinging to the floor[otherwise if A > 0 and A > C + B][slime] on the floor[otherwise if B > 0 and B > C + A]milk on the floor[otherwise]fluids on the floor[end if], [if the player is not shameless][one of]shuddering at the new low this game has forced you to.[or]once again finding yourself scraping the bottom of the barrel for humiliating acts.[stopping][otherwise][one of]shivering with excitement at your brand new low.[or]once again finding yourself at the peak of degrading acts.[stopping][end if]";
	now F is C;
	if F > 4, now F is 4;
	now E is B;
	...
	FaceFill slime by D;
	decrease the slime-puddle of the location of the player by D;
	let GO be 9;
	unless the class of the player matches the text "neko maid":
		UnflinchingNonAddictiveGrossOut GO with reason "" and sensation "taste";
		if the class of the player is not catgirl, compute potential addiction gain from grossness GO;
		let M be a random reactive monster;
		if M is monster:
			say "You can feel [NameDesc of M][']s eyes on you, silently judging.";
			if class of the player is catgirl:
				if the player is not broken, say HumiliateReflect ((SEVERE-HUMILIATION) - (40 * (the thirst of the player - 1)));
			otherwise:
				say HumiliateReflect ((OBSCENE-HUMILIATION) - (40 * (the thirst of the player + 2))); [This is probably one of the least dignified things you could do, but it's less humiliating the thirstier you are]
	suggest swallowing.
...
Carry out mopping clothing:[TODO: breasts largeness 10+ will touch the ground and become soaked.]
	...
	let SL be the soak-limit of the noun - (the semen-soak of the noun + the slime-soak of the noun + the milk-soak of the noun);
	if the noun is pink-spraybottle:
		increase E by 1;
		...
		if the class of the player is "cafe maid" and there is a carried waitress vessel:
			...
			if W is waitress vessel:
				if diaper quest is 1:
					if the slime-puddle of the location of the player > 0 and watersports fetish is 1, now the fill-colour of W is golden;
					otherwise now the fill-colour of W is white;
				otherwise:
					let FL be 0;
					if the slime-puddle of the location of the player > 0, increase FL by 1;
	...
	otherwise if E > SL:
		now E is SL;
	if the slime-puddle of the location of the player > 0 and E > 0:[Clean up is in order of thickness. slime is thinnest, followed by milk, followed by semen.]
		if the slime-puddle of the location of the player > E:
			increase D by E;
			decrease the slime-puddle of the location of the player by E;
			SlimeSoakUp the noun by E;
			now E is 0;
		otherwise:
			increase D by the slime-puddle of the location of the player;
			decrease E by the slime-puddle of the location of the player;
			SlimeSoakUp the noun by the slime-puddle of the location of the player;
			now the slime-puddle of the location of the player is 0;
	...
	FatigueUp D * 10;
	if the noun is not pink-spraybottle and the semen-soak of the noun + the slime-soak of the noun + the milk-soak of the noun >= the soak-limit of the noun, say "The [printed name of noun] is now fully saturated." instead;
...
Report Mopping:
	if the player is not in a predicament room:
		if the noun is face:
			if cat-ears is off-stage and cat-ears is actually summonable:
				...
				summon cat-ears cursed;
			otherwise if (cat-ears is worn or black maid headdress is worn) and maid-cat-ears is off-stage:
		...
		otherwise if there is a worn maid headdress or stripper-ears is worn or (the number of worn headgear is 0 and stripper maid headdress is off-stage):
			...
			if M is monster:
				...
				if H is stripper-ears or the player is getting very unlucky:
					...
					otherwise:
						summon stripper maid headdress cursed with quest;
			...
			otherwise if the noun is face and stripper-ears is not worn and maid-cat-ears is off-stage and (diaper quest is 0 and the player is a november 2025 top donator) or (diaper quest is 1 and the player is a september 2025 diaper donator):
				let H be a random worn headgear;
				if H is headgear:
					transform H into maid-cat-ears;
				otherwise:
					say "[bold type]You feel a headband with cat ears materialise on your head.[roman type][line break]";
					summon maid-cat-ears;
				say "[if the bimbo of the player > 12][second custom style]Tee-hee, I get it! Only a naughty pussycat would ever drink anything off the floor![otherwise][first custom style]I guess that's what I get for drinking off the floor like some kind of animal...[end if][roman type][line break]";
				let D be a random worn overdress;
				if D is not neko-maid-outfit and D is removable, transform D into neko-maid-outfit;
				if black-cat-tail-anal-beads is not worn and the number of embodied things penetrating asshole is 0, unclash class summon black-cat-tail-anal-beads;
		...
		if the noun is pink-spraybottle and pink-spraybottle is spray and total puddle <= 0 and the magic-fatigue of the player > 0 and the class of the player is maid:
...
Understand "clean slime with [something]", "clean piss with [something]", "clean urine with [something]" as mopping when the slime-puddle of the location of the player > 0.
