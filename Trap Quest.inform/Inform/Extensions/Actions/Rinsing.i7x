...
Check rinsing it in:
	...
	if the noun is body part:
		...
		otherwise:
			if the semen coating of the noun + the slime coating of the noun is 0, say "There's no need for that." instead;
	otherwise:
		if the noun is not clothing, say "You can't clean that in the water!" instead;
		if the semen-soak of the noun is 0 and the milk-soak of the noun is 0 and the slime-soak of the noun is 0 and the noun is not glued, say "That is completely clean already." instead;
	...
	if the player is at least partially immobile, say "You're a bit immobile at the moment!" instead.
...
To compute clothesRinsing of (C - a clothing) in (T - a thing):
	let S be the semen-soak of C + (the slime-soak of C * 2) - the milk-soak of C;
...
To compute bodyRinsing of (B - a body part) in (T - a thing):[TODO: change so it pollutes the pool again]
	...
	if B is hair:
		increase tracked-semen by the slime coating of hair / 2;
...
To say RinseFlav of (T - hair) in (P - an object):
	say "You lean over and let your hair fall into the cool and refreshing water. [if the semen coating of hair > 0]Soon all the [semen] has been washed away. [otherwise if the slime coating of hair > 0]Soon all the [slime] has been washed away. [end if]".
...
Understand "rinse [something] in [something]" as rinsing it in.
